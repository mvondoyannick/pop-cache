class Client
    ...
    begin
      ...
      if Fraud::Customer.passwordValidation(@password)[0]
        ...
        customer = Customer.new(
            ...
            phone: @phone,
            email: @email,
            password: ROTP::TOTP.new(ROTP::Base32.random, issuer: "PAYMEQUICK").now,
            type_id: 1,
            #cni: @cni,
            sexe: @sexe,
            ip: @ip,
          ...
          if customer.save
            ...
            # lets update two_fa for this customer
            # the one time password
            @totp = ROTP::TOTP.new("base32secret3232", issuer: "My Service")

            if customer.update(two_fa: @totp.now, perime_two_fa: 1.hours.from_now)

              #send SMS to customer with code
              Sms.nexah(customer.phone, "Votre mot de passe PAYMEQUICK est : #{@totp.now}")
              ...
              return @otp[1], I18n.t("SmsNotSend", locale: @locale)
  ...
  def self.signin(argc, locale = 'fr', message = nil) # prévoir l'ajout de l'@ IP
    ...
    if Customer.exists?(phone: @phone)
      @customer = Customer.find_by(phone: @phone, two_fa: 'authenticate')
      if @customer&.valid_password?(@password)
        # generate JWT for 7 days
        return true, @customer.as_json(only: [:name, :second_name, :authentication_token, :code, :pwd_changed]), CoreCustomer::Customers.expiration
      else
        return false, "Utilisateur ou mot de passe inconnu"
      end
    else
      return false, "Utilisateur inconnu"
    end
      ...
      if customer.blank?
        ...
        if antiFraud.blank?
          ...
          if @phone.eql?(antiFraud[:phone]) && @sim_phone.eql?(antiFraud[:sim_phone]) && @network_operator.eql?(antiFraud[:network_operator]) && @uuid.eql?(antiFraud[:uuid]) && @imei.eql?(antiFraud[:imei])
            ...
            result = lockCustomerAccount(phone, "Informations divergentes trouvées")
            if result[0]
            ...
            if customer_abonnement[0]
              ...
              if customer.await.amount.to_f > customer_abonnement[1].to_f
                ...
                if Time.now > awaits.end.to_time
                  ...
                  if account.blank?
                    puts "Impossible de trouver le compte de l'utilisateur #{customer.id}"
                    return false, "Compte inexistant"
                  else

                    puts "Suppression de l'intent de retrait ..."
                    awaits.destroy
          ...
          if client_account.amount.to_f >= Parametre::Parametre::agis_percentage(@amount)
            ...
            if client_account.update(amount: Parametre::Parametre::soldeTest(client_account.amount, @amount))
              ...
              if marchand_account.save
                ...
                PayJob.set(wait: 2.seconds).perform_later(@amount, marchand.phone, "merchant", client.complete_name, marchand_account.amount, @hash, marchand.id)
                ...
                if client_log.save
                  ...
                  if commission
                    ...
                    PayJob.set(wait: 2.seconds).perform_later(Parametre::Parametre::agis_percentage(@amount).to_f, client.phone, "payer", @hash, client.id)
