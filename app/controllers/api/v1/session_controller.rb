class Api::V1::SessionController < ApplicationController
    ...
    def signin
      ...
      locale = "fr" #request.headers["HTTP_LOCALE"]
        ...
        #query the user
        signin = Client::signin({phone: phone, password: password, device: device, ip: @ip})
        render json: signin
        ...
        if answer.blank?
          ...
          if lock[0] == true 
            ...
            if sms_code.save
              ...
              Sms.nexah(customer.phone, "SMS Validation mot de passe #{@sms_pwd}")
              render json: {
                status:   :found,
      ...
      if @token.present? && @previouPwd.present? && @newPwd.present?
        ...
        if Customer.exists?(authentication_token: @token)
          ...
          if @customer.valid_password?(@previouPwd)
            ...
            # ajout du control sur le mot de passe
            if Fraud::Customer.passwordValidation(@newPwd)[0]
              ...
              if @customer.update(password: @newPwd, authentication_token: nil)
                ...
                # update pwd_changed to true
                if @customer.update(pwd_changed: true)
                  #Notification SMS
                  Sms.nexah(@customer.phone, "#{@customer.complete_name} votre mot de passe a ete mis a jour, merci de vous reconnecter. PAYMEQUICK")

                  render json: {
                      status: true,
                      pwd_changed: @customer.pwd_changed,
              ...
              render json: {
                  ...
                  message: "Sécurité du mot de passe trop faible, merci de changer et de tester à nouveau"
              }
            end
          ...
          render json: {
              status: false,
              flag: :customer_not_found,
              message: "utilisateur ou mot de passe inexistant"
          }
        end
        ...
        render json: {
          status: false,
          flag:   "Des informations semblent etre manquantes",
      ...
      if Customer.exists?(phone: @phone)
        ...
        if @answer.blank?
          render json: {
              status: false,
              flag: false,
              message: "Aucune question pour cet utilisateur"
          },
                 status: :unauthorized
        else
          render json: {
    ...
    #lister les SOS d'un utilisateur
    def list_sos
      @token = request.headers["HTTP_X_API_POP_KEY"]
      if Customer.exists?(authentication_token: @token)
        @customer = Customer.find_by(authentication_token: @token)
    ...
    def details_sos
      ...
      if Customer.exists?(authentication_token: @token)
        @customer = Customer.find_by(authentication_token: @token)
        sos_detail = CorePayment::Sos.detail_sos(customer: @customer.id, sos: @code)
        render json: {
            status: sos_detail[0],
            message: sos_detail[1]
        }
      else
        render json: {
          status: false,
