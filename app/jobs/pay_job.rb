class PayJob < ApplicationJob
  ...
  # @params [String] hash
  # @params [Integer] id
  def perform(amount, phone, type, complete_name=nil, solde=nil, hash, id)
    @phone = phone
    @amount = amount
    @type = type
    ...
    when "payer"
      Sms.nexah(@phone, "Paiement de #{@amount} F CFA, Paiement effectué avec succès. Plus sur https://payquick-develop.herokuapp.com/webview/#{@hash}/#{@id}")
    else
