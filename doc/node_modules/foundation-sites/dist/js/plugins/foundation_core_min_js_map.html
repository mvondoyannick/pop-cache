...
<title>foundation.core.min.js.map - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/dist/js/plugins/foundation.core.min.js.map">
...
<em>webpack_require</em>(<em>webpack_require</em>.s = 0);n”,“// —
Foundation Core API —n// Initialize Foundation and add some utilities to
its public API for backward compatibility.n// Please note that every
utility do not have to be added to the core API.nimport $ from
&#39;jquery&#39;;nimport { Foundation } from
&#39;../../foundation.core&#39;;nimport { Plugin } from
&#39;../../foundation.core.plugin&#39;;nimport { rtl, GetYoDigits,
transitionend, RegExpEscape, onLoad } from
&#39;../../foundation.core.utils&#39;;nnFoundation.addToJquery($);nn//
Every plugin depends on plugin now, we can include that on the core for
then// script inclusion path.nFoundation.Plugin = Plugin;nn// These are now
separated out, but historically were a part of this module,n// and since
this is here for backwards compatibility we include them inn// this
entry.nFoundation.rtl = rtl;nFoundation.GetYoDigits =
GetYoDigits;nFoundation.transitionend =
transitionend;nFoundation.RegExpEscape = RegExpEscape;nFoundation.onLoad =
onLoad;nnwindow.Foundation = Foundation;nn// — Foundation Core exports —n//
Export "Plugin&quot; and all core utilities, since the `foundation.core`
entry plays the role ofn// all core source files.nnexport { Foundation
};nexport * from &#39;../../foundation.core.plugin&#39;;nexport * from
&#39;../../foundation.core.utils&#39;;n”,“&quot;use strict&quot;;nnimport $
from &#39;jquery&#39;;nimport { GetYoDigits } from
&#39;./foundation.core.utils&#39;;nimport { MediaQuery } from
&#39;./foundation.util.mediaQuery&#39;;nnvar FOUNDATION_VERSION =
&#39;6.5.3&#39;;nn// Global Foundation objectn// This is attached to the
window, or used as a module for AMD/Browserifynvar Foundation = {n 
version: FOUNDATION_VERSION,nn     n   * Stores initialized plugins.n     n
...
*/n  plugin: function(plugin, name) {n    // <a
href="../../../../../Object.html">Object</a> key to use when adding to
global Foundation objectn    // Examples: Foundation.Reveal,
Foundation.OffCanvasn    var className = (name || functionName(plugin));n  
// <a href="../../../../../Object.html">Object</a> key to use when storing
the plugin, also used to create the identifying data attribute for the
pluginn    // Examples: data-reveal, data-off-canvasn    var attrName  =
hyphenate(className);nn    // Add to the Foundation object and the plugins
list (for reflowing)n    <a href="attrName">this._plugins</a> = <a
href="className">this</a> = plugin;n  },n  /**n   * @functionn   *
Populates the _uuids array with pointers to each individual plugin
instance.n   * Adds the `zfPlugin` data-attribute to programmatically
created plugins to allow use of $(selector).foundation(method) calls.n   *
Also fires the initialization event for each plugin, consolidating
repetitive code.n   * @param {Object} plugin - an instance of a plugin,
usually `this` in context.n   * @param {String} name - the name of the
plugin, passed as a camelCased string.n   * @fires Plugin#initn   */n 
registerPlugin: function(plugin, name){n    var pluginName = name ?
hyphenate(name) : functionName(plugin.constructor).toLowerCase();n   
plugin.uuid = GetYoDigits(6, pluginName);nn   
if(!plugin.$element.attr(`data-${pluginName}`)){
plugin.$element.attr(`data-${pluginName}`, plugin.uuid); }n   
if(!plugin.$element.data(&#39;zfPlugin&#39;)){
plugin.$element.data(&#39;zfPlugin&#39;, plugin); }n          /**n         
* Fires when the plugin has initialized.n           * @event Plugin#initn  
*/n    plugin.$element.trigger(`init.zf.${pluginName}`);nn   
...
<a href="prop">plugin</a> = null;//clean up script to prep for garbage
collection.n    }n    return;n  },nn  /**n   * @functionn   * Causes one or
more active plugins to re-initialize, resetting event listeners,
recalculating positions, etc.n   * @param {String} plugins - optional
string of an individual plugin key, attained by calling
`$(element).data(&#39;pluginName&#39;)`, or string of a plugin class i.e.
`&#39;dropdown&#39;`n   * @default If no argument is passed, reflow all
currently active plugins.n   */n   reInit: function(plugins){n     var isJQ
= plugins instanceof $;n     try{n       if(isJQ){n        
plugins.each(function(){n          
$(this).data(&#39;zfPlugin&#39;)._init();n         });n       }else{n      
var type = typeof plugins,n         _this = this,n         fns = {n        
&#39;object&#39;: function(plgs){n             plgs.forEach(function(p){n  
p = hyphenate(p);n               $(&#39;[data-&#39;+ p
+&#39;]&#39;).foundation(&#39;_init&#39;);n             });n           },n 
&#39;string&#39;: function(){n             plugins = hyphenate(plugins);n  
$(&#39;[data-&#39;+ plugins +&#39;]&#39;).foundation(&#39;_init&#39;);n    
},n           &#39;undefined&#39;: function(){n             <a
href="'object'">this</a>(Object.keys(_this._plugins));n           }n       
};n         <a href="type">fns</a>(plugins);n       }n     }catch(err){n   
console.error(err);n     }finally{n       return plugins;n     }n   },nn 
/**n   * Initialize plugins on any elements within `elem` (and `elem`
itself) that aren&#39;t already initialized.n   * @param {Object} elem -
jQuery object containing the element to check inside. Also checks the
element itself, unless it&#39;s the `document` object.n   * @param
{String|Array} plugins - A list of plugins to initialize. Leave this out to
initialize everything.n   */n  reflow: function(elem, plugins) {nn    // If
plugins is undefined, just grab everythingn    if (typeof plugins ===
&#39;undefined&#39;) {n      plugins = Object.keys(this._plugins);n    }n  
// If plugins is a string, convert it to an array with one itemn    else if
(typeof plugins === &#39;string&#39;) {n      plugins = [plugins];n    }nn 
var _this = this;nn    // Iterate through each pluginn    $.each(plugins,
function(i, name) {n      // Get the current pluginn      var plugin = <a
href="name">_this._plugins</a>;nn      // Localize the search to all
elements inside elem, as well as elem itself, unless elem === documentn    
var $elem = $(<a href="data-'name'">elem).find(‘</a>&#39;).<a
href="data-'name'">addBack(’</a>&#39;);nn      // For each plugin found,
initialize itn      $elem.each(function() {n        var $el = $(this),n    
opts = {};n        // Don&#39;t double-dip on pluginsn        if
($el.data(&#39;zfPlugin&#39;)) {n          console.warn(&quot;Tried to
initialize "<code>name</code>&quot; on an element that already has a
Foundation plugin.&quot;);n          return;n        }nn       
if($el.attr(&#39;data-options&#39;)){n          var thing =
$el.attr(&#39;data-options&#39;).split(&#39;;&#39;).forEach(function(e,
i){n            var opt = e.split(&#39;:&#39;).map(function(el){ return
el.trim(); });n            <a href="0">if(opt</a>) <a
href="0">opts[opt</a>] = <a href="1">parseValue(opt</a>);n          });n   
}n        try{n          $el.data(&#39;zfPlugin&#39;, new plugin($(this),
opts));n        }catch(er){n          console.error(er);n        }finally{n
...
$(&#39;.no-js&#39;);nn      if($noJS.length){n       
$noJS.removeClass(&#39;no-js&#39;);n      }nn      if(type ===
&#39;undefined&#39;){//needs to initialize the Foundation object, or an
individual plugin.n        MediaQuery._init();n       
Foundation.reflow(this);n      }else if(type === &#39;string&#39;){//an
individual method to invoke on a plugin or group of pluginsn        var
args = Array.prototype.slice.call(arguments, 1);//collect all the
arguments, if necessaryn        var plugClass =
this.data(&#39;zfPlugin&#39;);//determine the class of pluginnn       
if(typeof plugClass !== &#39;undefined&#39; &amp;&amp; typeof <a
href="method">plugClass</a> !== &#39;undefined&#39;){//make sure both the
class and method existn          if(this.length === 1){//if there&#39;s
only one, call it directly.n              <a
href="method">plugClass</a>.apply(plugClass, args);n          }else{n      
this.each(function(i, el){//otherwise loop through the jQuery collection
and invoke the method on eachn              <a
href="method">plugClass</a>.apply($(el).data(&#39;zfPlugin&#39;), args);n  
});n          }n        }else{//error for no class or no methodn         
throw new ReferenceError(&quot;We&#39;re sorry, &#39;&quot; + method +
"&#39; is not an available method for " + (plugClass ?
functionName(plugClass) : &#39;this element&#39;) + &#39;.&#39;);n       
}n      }else{//error for invalid argument typen        throw new
TypeError(`We&#39;re sorry, ${type} is not a valid parameter. You must use
a string representing the method you wish to invoke.`);n      }n     
return this;n    };n    $.fn.foundation = foundation;n    return $;n 
...
{ return new Date().getTime(); };nn  var vendors = [&#39;webkit&#39;,
&#39;moz&#39;];n  for (var i = 0; i &lt; vendors.length &amp;&amp;
!window.requestAnimationFrame; ++i) {n      var vp = <a
href="i">vendors</a>;n      window.requestAnimationFrame = <a
href="vp+'RequestAnimationFrame'">window</a>;n     
window.cancelAnimationFrame = (<a
href="vp+'CancelAnimationFrame'">window</a>n                               
|| <a href="vp+'CancelRequestAnimationFrame'">window</a>);n  }n  if
(/iP(ad|hone|od).*OS 6/.test(window.navigator.userAgent)n    ||
!window.requestAnimationFrame || !window.cancelAnimationFrame) {n    var
lastTime = 0;n    window.requestAnimationFrame = function(callback) {n     
var now = Date.now();n        var nextTime = Math.max(lastTime + 16, now);n
return setTimeout(function() { callback(lastTime = nextTime); },n          
nextTime - now);n    };n    window.cancelAnimationFrame = clearTimeout;n 
}n  /**n   * Polyfill for performance.now, required by rAFn   */n 
if(!window.performance || !window.performance.now){n    window.performance
= {n      start: Date.now(),n      now: function(){ return Date.now() -
this.start; }n    };n  }n})();nif (!Function.prototype.bind) {n 
Function.prototype.bind = function(oThis) {n    if (typeof this !==
&#39;function&#39;) {n      // closest thing possible to the ECMAScript 5n 
// internal IsCallable functionn      throw new
TypeError(&#39;Function.prototype.bind - what is trying to be bound is not
callable&#39;);n    }nn    var aArgs   =
Array.prototype.slice.call(arguments, 1),n        fToBind = this,n       
fNOP    = function() {},n        fBound  = function() {n          return
fToBind.apply(this instanceof fNOPn                 ? thisn                
: oThis,n                
aArgs.concat(Array.prototype.slice.call(arguments)));n        };nn    if
(this.prototype) {n      // native functions don&#39;t have a prototypen   
fNOP.prototype = this.prototype;n    }n    fBound.prototype = new fNOP();nn
return fBound;n  };n}n// Polyfill to get the name of a function in
IE9nfunction functionName(fn) {n  if (typeof Function.prototype.name ===
&#39;undefined&#39;) {n    var funcNameRegex = /<a
href="^(">function\s(</a>{1,})\(/;n    var results =
(funcNameRegex).exec((fn).toString());n    return (results &amp;&amp;
results.length &gt; 1) ? <a href="1">results</a>.trim() : "&quot;;n  }n 
else if (typeof fn.prototype === &#39;undefined&#39;) {n    return
fn.constructor.name;n  }n  else {n    return
fn.prototype.constructor.name;n  }n}nfunction parseValue(str){n  if
(&#39;true&#39; === str) return true;n  else if (&#39;false&#39; === str)
return false;n  else if (!isNaN(str * 1)) return parseFloat(str);n  return
str;n}n// Convert PascalCase to kebab-casen// Thank you: <a
href="http://stackoverflow.com/a/8955580\nfunction">stackoverflow.com/a/8955580nfunction</a>
hyphenate(str) {n  return <a href="a-z">str.replace(/(</a>)([A-Z])/g,
&#39;$1-$2&#39;).toLowerCase();n}nnexport {Foundation};n”,“&#39;use
strict&#39;;nnimport $ from &#39;jquery&#39;;nimport { GetYoDigits } from
&#39;./foundation.core.utils&#39;;nn// Abstract class for providing
lifecycle hooks. Expect plugins to define AT LEASTn// {function} _setup
(replaces previous constructor),n// {function} _destroy (replaces previous
destroy)nclass Plugin {nn  constructor(element, options) {n   
this._setup(element, options);n    var pluginName = getPluginName(this);n  
this.uuid = GetYoDigits(6, pluginName);nn   
if(!this.$element.attr(`data-${pluginName}`)){
this.$element.attr(`data-${pluginName}`, this.uuid); }n   
if(!this.$element.data(&#39;zfPlugin&#39;)){
this.$element.data(&#39;zfPlugin&#39;, this); }n    /**n     * Fires when
the plugin has initialized.n     * @event Plugin#initn     */n   
this.$element.trigger(`init.zf.${pluginName}`);n  }nn  destroy() {n   
this._destroy();n    var pluginName = getPluginName(this);n   
this.$element.removeAttr(`data-${pluginName}`).removeData(&#39;zfPlugin&#39;)n
/**n         * Fires when the plugin has been destroyed.n         * @event
Plugin#destroyedn         */n       
.trigger(`destroyed.zf.${pluginName}`);n    for(var prop in this){n      <a
href="prop">this</a> = null;//clean up script to prep for garbage
collection.n    }n  }n}nn// Convert PascalCase to kebab-casen// Thank you:
<a
href="http://stackoverflow.com/a/8955580\nfunction">stackoverflow.com/a/8955580nfunction</a>
hyphenate(str) {n  return <a href="a-z">str.replace(/(</a>)([A-Z])/g,
&#39;$1-$2&#39;).toLowerCase();n}nnfunction getPluginName(obj) {n 
if(typeof(obj.constructor.name) !== &#39;undefined&#39;) {n    return
hyphenate(obj.constructor.name);n  } else {n    return
hyphenate(obj.className);n  }n}nnexport {Plugin};n”,“&quot;use
strict&quot;;nnimport $ from &#39;jquery&#39;;nn// Core Foundation
Utilities, utilized in a number of places.nn  /**n   * Returns a boolean
for RTL supportn   */nfunction rtl() {n  return
$(&#39;html&#39;).attr(&#39;dir&#39;) === &#39;rtl&#39;;n}nn/**n * returns
a random base-36 uid with namespacingn * @functionn * @param {Number}
length - number of random base-36 digits desired. Increase for more random
strings.n * @param {String} namespace - name of plugin to be incorporated
in uid, optional.n * @default {String} &#39;&#39; - if no plugin name is
provided, nothing is appended to the uid.n * @returns {String} - unique idn
*/nfunction GetYoDigits(length, namespace){n  length = length || 6;n 
return Math.round((Math.pow(36, length + 1) - Math.random() * Math.pow(36,
length))).toString(36).slice(1) + (namespace ? `-${namespace}` :
&#39;&#39;);n}nn/**n * Escape a string so it can be used as a regexp
patternn * @functionn * @see <a
href="https://stackoverflow.com/a/9310752/4317384\n">stackoverflow.com/a/9310752/4317384n</a>
*n * @param {String} str - string to escape.n * @returns {String} - escaped
stringn */nfunction RegExpEscape(str){n  return <a
href="\\">str.replace(/[-</a>{}()*+?.,\\^$|#\s]/g,
&#39;\\$&amp;&#39;);n}nnfunction transitionend($elem){n  var transitions =
{n    &#39;transition&#39;: &#39;transitionend&#39;,n   
&#39;WebkitTransition&#39;: &#39;webkitTransitionEnd&#39;,n   
&#39;MozTransition&#39;: &#39;transitionend&#39;,n   
&#39;OTransition&#39;: &#39;otransitionend&#39;n  };n  var elem =
document.createElement(&#39;div&#39;),n      end;nn  for (var t in
transitions){n    if (typeof <a href="t">elem.style</a> !==
&#39;undefined&#39;){n      end = <a href="t">transitions</a>;n    }n  }n 
if(end){n    return end;n  }else{n    end = setTimeout(function(){n     
$elem.triggerHandler(&#39;transitionend&#39;, [$elem]);n    }, 1);n   
return &#39;transitionend&#39;;n  }n}nn/**n * Return an event type to
listen for window load.n *n * If `$elem` is passed, an event will be
triggered on `$elem`. If window is already loaded, the event will still be
triggered.n * If `handler` is passed, attach it to the event on `$elem`.n *
Calling `onLoad` without handler allows you to get the event type that will
be triggered before attaching the handler by yourself.n * @functionn *n *
@param {Object} [] $elem - jQuery element on which the event will be
triggered if passed.n * @param {Function} [] handler - function to attach
to the event.n * @returns {String} - event type that should or will be
triggered.n */nfunction onLoad($elem, handler) {n  const didLoad =
document.readyState === &#39;complete&#39;;n  const eventType = (didLoad ?
&#39;_didLoad&#39; : &#39;load&#39;) + &#39;.zf.util.onLoad&#39;;n  const
cb = () =&gt; $elem.triggerHandler(eventType);nn  if ($elem) {n    if
(handler) $elem.one(eventType, handler);nn    if (didLoad)n     
setTimeout(cb);n    elsen      $(window).one(&#39;load&#39;, cb);n  }nn 
return eventType;n}nn/**n * Retuns an handler for the `mouseleave` that
ignore disappeared mouses.n *n * If the mouse "disappeared&quot; from the
document (like when going on a browser UI element, See <a
href="https://git.io/zf-11410),\n">git.io/zf-11410),n</a> * the event is
ignored.n * - If the `ignoreLeaveWindow` is `true`, the event is ignored
when the user actually left the windown *   (like by switching to an other
window with [<a href="Tab">Alt]+</a>).n * - If the `ignoreReappear` is
`true`, the event will be ignored when the mouse will reappear later on the
documentn *   outside of the element it left.n *n * @functionn *n * @param
{Function} [] handler - handler for the filtered `mouseleave` event to
watch.n * @param {Object} [] options - object of options:n * - {Boolean}
[false] ignoreLeaveWindow - also ignore when the user switched windows.n *
- {Boolean} [false] ignoreReappear - also ignore when the mouse reappeared
outside of the element it left.n * @returns {Function} - filtered handler
to use to listen on the `mouseleave` event.n */nfunction
ignoreMousedisappear(handler, { ignoreLeaveWindow = false, ignoreReappear =
false } = {}) {n  return function leaveEventHandler(eLeave, …rest) {n   
const callback = handler.bind(this, eLeave, …rest);nn    // The mouse left:
call the given callback if the mouse entered elsewheren    if
(eLeave.relatedTarget !== null) {n      return callback();n    }nn    //
Otherwise, check if the mouse actually left the window.n    // In firefox
if the user switched between windows, the window sill have the focus by the
timen    // the event is triggered. We have to debounce the event to test
this case.n    setTimeout(function leaveEventDebouncer() {n      if
(!ignoreLeaveWindow &amp;&amp; document.hasFocus &amp;&amp;
!document.hasFocus()) {n        return callback();n      }nn      //
Otherwise, wait for the mouse to reeapear outside of the element,n      if
(!ignoreReappear) {n        $(document).one(&#39;mouseenter&#39;, function
reenterEventHandler(eReenter) {n          if
...
&#39;jquery&#39;;nn// Default set of media queriesnconst defaultQueries =
{n  &#39;default&#39; : &#39;only screen&#39;,n  landscape : &#39;only
screen and (orientation: landscape)&#39;,n  portrait : &#39;only screen and
(orientation: portrait)&#39;,n  retina : &#39;only screen and
(-webkit-min-device-pixel-ratio: 2),&#39; +n    &#39;only screen and
(min–moz-device-pixel-ratio: 2),&#39; +n    &#39;only screen and
(-o-min-device-pixel-ratio: 2/1),&#39; +n    &#39;only screen and
(min-device-pixel-ratio: 2),&#39; +n    &#39;only screen and
(min-resolution: 192dpi),&#39; +n    &#39;only screen and (min-resolution:
2dppx)&#39;n  };nnn// matchMedia() polyfill - Test a CSS media type/query
in JS.n// Authors &amp; copyright© 2012: Scott Jehl, Paul Irish, Nicholas
Zakas, David Knight. MIT licensen/* eslint-disable */nwindow.matchMedia ||
(window.matchMedia = (function () {n  "use strict&quot;;nn  // For browsers
that support matchMedium api such as IE 9 and webkitn  var styleMedia =
(window.styleMedia || window.media);nn  // For those that don&#39;t support
matchMediumn  if (!styleMedia) {n    var style   =
document.createElement(&#39;style&#39;),n    script      = <a
href="0">document.getElementsByTagName(‘script’)</a>,n    info        =
...
&#39;window.getComputedStyle&#39; for all other browsersn    info =
(&#39;getComputedStyle&#39; in window) &amp;&amp;
window.getComputedStyle(style, null) || style.currentStyle;nn    styleMedia
= {n      matchMedium: function (media) {n        var text = &#39;@media
&#39; + media + &#39;{ matchmediajs-test { width: 1px; } }&#39;;nn       
// &#39;style.styleSheet&#39; is used by IE &lt;= 8 and
&#39;style.textContent&#39; for all other browsersn        if
(style.styleSheet) {n          style.styleSheet.cssText = text;n        }
else {n          style.textContent = text;n        }nn        // Test if
media query is true or falsen        return info.width === &#39;1px&#39;;n 
}n    };n  }nn  return function(media) {n    return {n      matches:
styleMedia.matchMedium(media || &#39;all&#39;),n      media: media ||
&#39;all&#39;n    };n  };n})());n/* eslint-enable */nnvar MediaQuery = {n 
queries: [],nn  current: &#39;&#39;,nn  /**n   * Initializes the media
query helper, by extracting the breakpoint list from the CSS and activating
the breakpoint watcher.n   * @functionn   * @privaten   */n  _init() {n   
var self = this;n    var $meta = $(&#39;meta.foundation-mq&#39;);n   
if(!$meta.length){n      $(&#39;&lt;meta
class=&quot;foundation-mq&quot;&gt;&#39;).appendTo(document.head);n    }nn 
var extractedStyles =
$(&#39;.foundation-mq&#39;).css(&#39;font-family&#39;);n    var
...
screen and (min-width: ${<a href="key">namedQueries</a>})`n        });n    
}n    }nn    this.current = this._getCurrentSize();nn    this._watcher();n 
},nn  /**n   * Checks if the screen is at least as wide as a breakpoint.n  
* @functionn   * @param {String} size - Name of the breakpoint to check.n  
* @returns {Boolean} `true` if the breakpoint matches, `false` if it&#39;s
smaller.n   */n  atLeast(size) {n    var query = this.get(size);nn    if
(query) {n      return window.matchMedia(query).matches;n    }nn    return
false;n  },nn  /**n   * Checks if the screen matches to a breakpoint.n   *
@functionn   * @param {String} size - Name of the breakpoint to check,
either &#39;small only&#39; or &#39;small&#39;. Omitting &#39;only&#39;
falls back to using atLeast() method.n   * @returns {Boolean} `true` if the
breakpoint matches, `false` if it does not.n   */n  is(size) {n    size =
size.trim().split(&#39; &#39;);n    if(size.length &gt; 1 &amp;&amp; <a
href="1">size</a> === &#39;only&#39;) {n      <a href="0">if(size</a> ===
this._getCurrentSize()) return true;n    } else {n      return <a
href="0">this.atLeast(size</a>);n    }n    return false;n  },nn  /**n   *
Gets the media query of a breakpoint.n   * @functionn   * @param {String}
size - Name of the breakpoint to get.n   * @returns {String|null} - The
media query of the breakpoint, or `null` if the breakpoint doesn&#39;t
exist.n   */n  get(size) {n    for (var i in this.queries) {n     
if(this.queries.hasOwnProperty(i)) {n        var query = <a
href="i">this.queries</a>;n        if (size === query.name) return
query.value;n      }n    }nn    return null;n  },nn  /**n   * Gets the
current breakpoint name by testing every breakpoint and returning the last
one to match (the biggest one).n   * @functionn   * @privaten   * @returns
{String} Name of the current breakpoint.n   */n  _getCurrentSize() {n   
var matched;nn    for (var i = 0; i &lt; this.queries.length; i++) {n     
var query = <a href="i">this.queries</a>;nn      if
(window.matchMedia(query.value).matches) {n        matched = query;n     
}n    }nn    if (typeof matched === &#39;object&#39;) {n      return
matched.name;n    } else {n      return matched;n    }n  },nn  /**n   *
Activates the breakpoint watcher, which fires an event on the window
whenever the breakpoint changes.n   * @functionn   * @privaten   */n 
_watcher() {n   
$(window).off(&#39;resize.zf.mediaquery&#39;).on(&#39;resize.zf.mediaquery&#39;,
() =&gt; {n      var newSize = this._getCurrentSize(), currentSize =
this.current;nn      if (newSize !== currentSize) {n        // Change the
current media queryn        this.current = newSize;nn        // Broadcast
the media query change on the windown       
$(window).trigger(&#39;changed.zf.mediaquery&#39;, [newSize,
currentSize]);n      }n    });n  }n};nnnn// Thank you: <a
href="https://github.com/sindresorhus/query-string\nfunction">github.com/sindresorhus/query-stringnfunction</a>
parseStyleToObject(str) {n  var styleObject = {};nn  if (typeof str !==
&#39;string&#39;) {n    return styleObject;n  }nn  str =
str.trim().slice(1, -1); // browsers re-quote string style valuesnn  if
(!str) {n    return styleObject;n  }nn  styleObject =
str.split(&#39;&amp;&#39;).reduce(function(ret, param) {n    var parts =
param.replace(/+/g, &#39; &#39;).split(&#39;=&#39;);n    var key = <a
href="0">parts</a>;n    var val = <a href="1">parts</a>;n    key =
decodeURIComponent(key);nn    // missing `=` should be `null`:n    // <a
href="http://w3.org/TR/2012/WD-url-20120524/#collect-url-parameters\n">w3.org/TR/2012/WD-url-20120524/#collect-url-parametersn</a>
val = typeof val === &#39;undefined&#39; ? null :
decodeURIComponent(val);nn    if (!ret.hasOwnProperty(key)) {n      <a
href="key">ret</a> = val;n    } else if (<a
href="key">Array.isArray(ret</a>)) {n      <a
href="key">ret</a>.push(val);n    } else {n      <a href="key">ret</a> =
[<a href="key">ret</a>, val];n    }n    return ret;n  }, {});nn  return
styleObject;n}nnexport {MediaQuery};n”,“module.exports =
