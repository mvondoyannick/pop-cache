...
<title>foundation.util.keyboard.min.js.map - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/dist/js/plugins/foundation.util.keyboard.min.js.map">
...
href="\"foundation.util.keyboard\"">object’)nttexports</a> =
...
from &#39;./foundation.core&#39;;nimport { Keyboard } from
&#39;../../foundation.util.keyboard&#39;;nnFoundation.Keyboard =
Keyboard;nnexport { Foundation, Keyboard };n”,“                            
n *                                         *n * This util was created by
Marius Olbertz *n * Please thank Marius on GitHub /owlbertz *n * or the web
<a href="http://www.mariusolbertz.de">www.mariusolbertz.de</a>/ *n *       
*n                                            nn&#39;use
strict&#39;;nnimport $ from &#39;jquery&#39;;nimport { rtl as Rtl } from
&#39;./foundation.core.utils&#39;;nnconst keyCodes = {n  9: &#39;TAB&#39;,n
13: &#39;ENTER&#39;,n  27: &#39;ESCAPE&#39;,n  32: &#39;SPACE&#39;,n  35:
&#39;END&#39;,n  36: &#39;HOME&#39;,n  37: &#39;ARROW_LEFT&#39;,n  38:
&#39;ARROW_UP&#39;,n  39: &#39;ARROW_RIGHT&#39;,n  40:
&#39;ARROW_DOWN&#39;n}nnvar commands = {}nn// Functions pulled out to be
referenceable from internalsnfunction findFocusable($element) {n 
if(!$element) {return false; }n  return $<a
href="href">element.find(‘a</a>, <a href="href">area</a>, <a
href="disabled">input:not(</a>), <a href="disabled">select:not(</a>), <a
href="disabled">textarea:not(</a>), <a href="disabled">button:not(</a>),
iframe, object, embed, *[tabindex],
*[contenteditable]&#39;).filter(function() {n    if
(!$(this).is(&#39;:visible&#39;) || $(this).attr(&#39;tabindex&#39;) &lt;
0) { return false; } //only have visible elements and those that have a
tabindex greater or equal 0n    return true;n  });n}nnfunction
parseKey(event) {n  var key = keyCodes[event.which || event.keyCode] ||
String.fromCharCode(event.which).toUpperCase();nn  // Remove un-printable
characters, e.g. for `fromCharCode` calls for CTRL only eventsn  key =
key.replace(/\W+/, &#39;&#39;);nn  if (event.shiftKey) key =
`SHIFT_${key}`;n  if (event.ctrlKey) key = `CTRL_${key}`;n  if
(event.altKey) key = `ALT_${key}`;nn  // Remove trailing underscore, in
case only modifiers were used (e.g. only `CTRL_ALT`)n  key =
key.replace(/_$/, &#39;&#39;);nn  return key;n}nnvar Keyboard = {n  keys:
getKeyCodes(keyCodes),nn     n   * Parses the (keyboard) event and returns
a String that represents its keyn   * Can be used like
Foundation.parseKey(event) === Foundation.keys.SPACEn   * @param {Event}
event - the event generated by the event handlern   * @return String key -
String that represents the key pressedn     n  parseKey: parseKey,nn  /**n 
* Handles the given (keyboard) eventn   * @param {Event} event - the event
generated by the event handlern   * @param {String} component - Foundation
component&#39;s name, e.g. Slider or Revealn   * @param {Objects} functions
- collection of functions that are to be executedn   */n  handleKey(event,
component, functions) {n    var commandList = <a
href="component">commands</a>,n      keyCode = this.parseKey(event),n     
cmds,n      command,n      fn;nn    if (!commandList) return
console.warn(&#39;Component not defined!&#39;);nn    if (typeof
commandList.ltr === &#39;undefined&#39;) { // this component does not
differentiate between ltr and rtln        cmds = commandList; // use plain
listn    } else { // merge ltr and rtl: if document is rtl, rtl overwrites
ltr and vice versan        if (Rtl()) cmds = $.extend({}, commandList.ltr,
commandList.rtl);nn        else cmds = $.extend({}, commandList.rtl,
commandList.ltr);n    }n    command = <a href="keyCode">cmds</a>;nn    fn =
<a href="command">functions</a>;n    if (fn &amp;&amp; typeof fn ===
&#39;function&#39;) { // execute function  if existsn      var returnValue
= fn.apply();n      if (functions.handled || typeof functions.handled ===
&#39;function&#39;) { // execute function when event was handledn         
functions.handled(returnValue);n      }n    } else {n      if
(functions.unhandled || typeof functions.unhandled === &#39;function&#39;)
{ // execute function when event was not handledn         
functions.unhandled();n      }n    }n  },nn  /**n   * Finds all focusable
elements within the given `$element`n   * @param {jQuery} $element - jQuery
object to search withinn   * @return {jQuery} $focusable - all focusable
elements within `$element`n   */nn  findFocusable: findFocusable,nn  /**n  
* Returns the component name namen   * @param {Object} component -
Foundation component, e.g. Slider or Revealn   * @return String
componentNamen   */nn  register(componentName, cmds) {n    <a
href="componentName">commands</a> = cmds;n  },nnn  // TODO9438: These
references to Keyboard need to not require global. Will &#39;this&#39; work
in this context?n  //n  /**n   * Traps the focus in the given element.n   *
@param  {jQuery} $element  jQuery object to trap the foucs into.n   */n 
trapFocus($element) {n    var $focusable = findFocusable($element),n       
$firstFocusable = $focusable.eq(0),n        $lastFocusable =
$focusable.eq(-1);nn    $element.on(&#39;keydown.zf.trapfocus&#39;,
function(event) {n      if (event.target === $<a href="0">lastFocusable</a>
&amp;&amp; parseKey(event) === &#39;TAB&#39;) {n       
event.preventDefault();n        $firstFocusable.focus();n      }n      else
if (event.target === $<a href="0">firstFocusable</a> &amp;&amp;
parseKey(event) === &#39;SHIFT_TAB&#39;) {n        event.preventDefault();n
$lastFocusable.focus();n      }n    });n  },n  /**n   * Releases the
trapped focus from the given element.n   * @param  {jQuery} $element 
jQuery object to release the focus for.n   */n  releaseFocus($element) {n  
$element.off(&#39;keydown.zf.trapfocus&#39;);n  }n}nn/*n * Constants for
easier comparing.n * Can be used like Foundation.parseKey(event) ===
Foundation.keys.SPACEn */nfunction getKeyCodes(kcs) {n  var k = {};n  for
(var kc in kcs) <a href="kc">k[kcs</a>] = <a href="kc">kcs</a>;n  return
k;n}nnexport {Keyboard};n”,“module.exports =
