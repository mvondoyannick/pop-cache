...
<title>foundation.util.motion.min.js.map - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/dist/js/plugins/foundation.util.motion.min.js.map">
...
href="\"foundation.util.motion\"">object’)nttexports</a> =
...
from &#39;./foundation.core&#39;;nimport { Motion, Move } from
&#39;../../foundation.util.motion&#39;;nnFoundation.Motion =
Motion;nFoundation.Move = Move;nnexport { Foundation, Motion, Move
};n”,“&#39;use strict&#39;;nnimport $ from &#39;jquery&#39;;nimport {
transitionend } from &#39;./foundation.core.utils&#39;;nn   n * Motion
module.n * @module foundation.motionn   nnconst initClasses   =
[&#39;mui-enter&#39;, &#39;mui-leave&#39;];nconst activeClasses =
[&#39;mui-enter-active&#39;, &#39;mui-leave-active&#39;];nnconst Motion =
{n  animateIn: function(element, animation, cb) {n    animate(true,
element, animation, cb);n  },nn  animateOut: function(element, animation,
cb) {n    animate(false, element, animation, cb);n  }n}nnfunction
Move(duration, elem, fn){n  var anim, prog, start = null;n  //
console.log(&#39;called&#39;);nn  if (duration === 0) {n   
fn.apply(elem);n    elem.trigger(&#39;finished.zf.animate&#39;,
[elem]).triggerHandler(&#39;finished.zf.animate&#39;, [elem]);n    return;n
}nn  function move(ts){n    if(!start) start = ts;n    //
console.log(start, ts);n    prog = ts - start;n    fn.apply(elem);nn   
if(prog &lt; duration){ anim = window.requestAnimationFrame(move, elem); }n
else{n      window.cancelAnimationFrame(anim);n     
elem.trigger(&#39;finished.zf.animate&#39;,
[elem]).triggerHandler(&#39;finished.zf.animate&#39;, [elem]);n    }n  }n 
anim = window.requestAnimationFrame(move);n}nn   n * Animates an element in
or out using a CSS transition class.n * @functionn * @privaten * @param
{Boolean} isIn - Defines if the animation is in or out.n * @param {Object}
element - jQuery or HTML object to animate.n * @param {String} animation -
CSS class to use.n * @param {Function} cb - Callback to run when animation
is finished.n   nfunction animate(isIn, element, animation, cb) {n  element
= $(element).eq(0);nn  if (!element.length) return;nn  var initClass = isIn
? <a href="0">initClasses</a> : <a href="1">initClasses</a>;n  var
activeClass = isIn ? <a href="0">activeClasses</a> : <a
href="1">activeClasses</a>;nn  // Set up the animationn  reset();nn 
elementn    .addClass(animation)n    .css(&#39;transition&#39;,
&#39;none&#39;);nn  requestAnimationFrame(() =&gt; {n   
element.addClass(initClass);n    if (isIn) element.show();n  });nn  //
Start the animationn  requestAnimationFrame(() =&gt; {n    <a
href="0">element</a>.offsetWidth;n    elementn     
.css(&#39;transition&#39;, &#39;&#39;)n      .addClass(activeClass);n 
});nn  // Clean up the animation when it finishesn 
element.one(transitionend(element), finish);nn  // Hides the element (for
out animations), resets the element, and runs a callbackn  function
finish() {n    if (!isIn) element.hide();n    reset();n    if (cb)
cb.apply(element);n  }nn  // Resets transitions and removes motion-specific
classesn  function reset() {n    <a
href="0">element</a>.style.transitionDuration = 0;n   
element.removeClass(`${initClass} ${activeClass} ${animation}`);n 
}n}nnexport { Move, Motion };nn”,“module.exports =
