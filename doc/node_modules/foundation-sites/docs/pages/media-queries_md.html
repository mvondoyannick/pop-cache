...
<title>media-queries - RDoc Documentation</title>
...
<div class="nav-section">
  ...
  <ul class="link-list" role="directory">
    ...
    <li><a href="#label-Default+Media+Queries">Default Media Queries</a>
    <li><a href="#label-Upgrading+from+Foundation+5">Upgrading from Foundation 5</a>
    <li><a href="#label-Changing+the+Breakpoints">Changing the Breakpoints</a>
    <li><a href="#label-Sass">Sass</a>
    <li><a href="#label-The+Breakpoint+Mixin">The Breakpoint Mixin</a>
    <li><a href="#label-Breakpoint+Function">Breakpoint Function</a>
    <li><a href="#label-JavaScript">JavaScript</a>
    <li><a href="#label-Working+with+Media+Queries">Working with Media Queries</a>
    <li><a href="#label-Watching+for+Breakpoint+Changes">Watching for Breakpoint Changes</a>
  </ul>
</div>
...
<main role="main" aria-label="Page node_modules/foundation-sites/docs/pages/media-queries.md">

<p>— title: Media Queries description: CSS media queries allow us to adjust
the display and orientation of content at different screen sizes. video:
gqqi2cqlST8 sass: scss/util/_breakpoint.scss js:
js/foundation.util.mediaQuery.js tags:</p>

<h2 id="label-+-+breakpoints"> - breakpoints<span><a href="#label-+-+breakpoints">&para;</a> <a href="#top">&uarr;</a></span></h2>
...
<h2 id="label-Default+Media+Queries">Default Media Queries<span><a href="#label-Default+Media+Queries">&para;</a> <a href="#top">&uarr;</a></span></h2>

<p>Foundation for Sites has three core breakpoints:</p>
<ul><li>
<p><strong>Small:</strong> any screen.</p>
</li><li>
<p><strong>Medium:</strong> any screen 640 pixels or wider.</p>
</li><li>
<p><strong>Large:</strong> any screen 1024 pixels or wider.</p>
</li></ul>

<p>Many components can be modified at different screen sizes using special
<em>breakpoint classes</em>. The grid is the most obvious example. In the
code below, the left-hand column is six columns wide on small screens,
hence <code>.small-6</code>. On medium-sized screens, the class
<code>.medium-4</code> overrides the small style, changing the column to be
four wide.</p>
...
<p>If you&#39;re using the CSS version of Foundation, use these media queries
to imitate the three core breakpoints:</p>
...
/* Large only */
@media screen and (min-width: 64em) and (max-width: 74.9375em) {}</pre>
<hr>

<h2 id="label-Upgrading+from+Foundation+5">Upgrading from Foundation 5<span><a href="#label-Upgrading+from+Foundation+5">&para;</a> <a href="#top">&uarr;</a></span></h2>

<p>In Foundation 5, breakpoints were accessed using a series of Sass variables
named <code>$small-up</code>, <code>$small-only</code>,
<code>$medium-only</code>, and so on. In Foundation 6, this method of
writing media queries has been replaced with a dedicated <a
href="#the-breakpoint-mixin">breakpoint mixin</a>, described below.
<strong>The legacy variables will be removed in Foundation 6.3.</strong></p>

<p>To upgrade your existing media queries, replace rulesets like this:</p>

<pre>@media #{$medium-only} {
}</pre>

<p>With this:</p>

<pre>@include breakpoint(medium only) {
}</pre>
<hr>

<h2 id="label-Changing+the+Breakpoints">Changing the Breakpoints<span><a href="#label-Changing+the+Breakpoints">&para;</a> <a href="#top">&uarr;</a></span></h2>

<p>If you&#39;re using the Sass version of Foundation, the default breakpoints
can be changed. The names of the breakpoints, and their widths, are stored
in a <code>$breakpoints</code> variable in the settings file.</p>
...
<p>Changing the widths of any of the breakpoints is as easy as changing the
pixel values in this map. Note that here there are two extra breakpoints:
<code>xlarge</code> and <code>xxlarge</code>. We don&#39;t use these for
any components, and also don&#39;t output any CSS classes that use them by
default.</p>

<p>You can change that by modifying the <code>$breakpoint-classes</code>
variable in your settings file. This is a list of breakpoint names. Adding
or removing names from the list will change the CSS class output. It looks
like this by default:</p>

<pre>$breakpoint-classes: (small medium large);</pre>

<p>For example, to get <code>.xlarge</code> classes in your CSS, for use in
the grid, Menu, and more, just add it to the end of the list:</p>

<pre>$breakpoint-classes: (small medium large xlarge);</pre>
<hr>

<h2 id="label-Sass">Sass<span><a href="#label-Sass">&para;</a> <a href="#top">&uarr;</a></span></h2>

<h3 id="label-The+Breakpoint+Mixin">The Breakpoint Mixin<span><a href="#label-The+Breakpoint+Mixin">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>Our <code>breakpoint()</code> mixin makes it easy to write media queries.
You can use the named breakpoints, or a custom pixel, rem, or em value.</p>

<p>To use the mixin, call it with <code>@include</code>, and then include the
CSS content you want inside the curly braces.</p>
...
<p>The behavior of the media query can be changed by adding the keyword
<code>down</code> or <code>only</code> after the breakpoint value,
...
<p>It&#39;s also possible to pass in custom values. You can enter a pixel,
rem, or em value—all values are converted to em at the end.</p>
...
<p>Lastly, there are three special media queries that are not width-based:
<code>portrait</code>, <code>landscape</code>, and <code>retina</code>.
Using these keywords with the <code>breakpoint()</code> mixin will output a
media query for device orientation or pixel density, rather than screen
width.</p>
...
<h3 id="label-Breakpoint+Function">Breakpoint Function<span><a href="#label-Breakpoint+Function">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>The functionality of the <code>breakpoint()</code> mixin comes from an
internal function, also called <code>breakpoint()</code>. If you want to
write your own media queries, you can use the <code>breakpoint()</code>
function to access the logic of the mixin directly.</p>

<pre>@media screen and #{breakpoint(medium)} {
  // Medium and up styles
}</pre>

<p>This can be used to combine multiple media queries together.</p>

<pre>@media screen and #{breakpoint(medium)} and #{breakpoint(xlarge down)} {
  // Medium to extra large styles
}</pre>
<hr>

<h2 id="label-JavaScript">JavaScript<span><a href="#label-JavaScript">&para;</a> <a href="#top">&uarr;</a></span></h2>

<h3 id="label-Working+with+Media+Queries">Working with Media Queries<span><a href="#label-Working+with+Media+Queries">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>The Foundation JavaScript includes a set of helper functions for working
with media queries. They&#39;re all on the
<code>Foundation.MediaQuery</code> object.</p>
...
<p>Get the name of the current breakpoint with
<code>MediaQuery.current</code>.</p>

<pre>Foundation.MediaQuery.current // =&gt; &#39;small&#39;, &#39;medium&#39;, etc.</pre>

<p>To see if the screen is currently a certain breakpoint or larger, use
<code>MediaQuery.atLeast</code>.</p>

<pre>if (Foundation.MediaQuery.atLeast(&#39;medium&#39;)) {
  // True if medium or large
  // False if small
}</pre>

<p>To see if the screen is currently a certain breakpoint, use
<code>MediaQuery.is</code>.</p>

<pre>if (Foundation.MediaQuery.is(&#39;small only&#39;)) {
  // True if small
  // False if medium or large
}</pre>

<p>To get the media query of a breakpoint, use <code>MediaQuery.get</code>.</p>

<pre>Foundation.MediaQuery.get(&#39;medium&#39;) // =&gt; only screen and (min-width: 640px)</pre>
<hr>

<h3 id="label-Watching+for+Breakpoint+Changes">Watching for Breakpoint Changes<span><a href="#label-Watching+for+Breakpoint+Changes">&para;</a> <a href="#top">&uarr;</a></span></h3>

<p>The media query helper broadcasts an event on the window every time the
breakpoint changes. We use this internally with plugins like Interchange to
detect a shift in breakpoint. You can also subscribe to the event yourself.</p>

<pre>$(window).on(&#39;changed.zf.mediaquery&#39;, function(event, newSize, oldSize) {
