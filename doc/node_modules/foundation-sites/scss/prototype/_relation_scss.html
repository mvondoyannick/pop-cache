...
<title>_relation.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/prototype/_relation.scss">

<p>/// Select all children from the first to `$num`. /// @param {Number}
$num[] First `n` numbers of total children  @mixin first($num) {</p>
...
<p>/// Select the first exact child @mixin first-child {</p>
...
<p>/// Select all children from the last to `$num`. /// @param {Number} $num[]
Last `n` numbers of total children  @mixin last($num) {</p>

<pre>&amp;:nth-last-child(-n + #{$num}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select the last exact child @mixin last-child {</p>
...
<p>/// Select children every `$num`. /// @param {Number} $num[] Every `n`
number of all children @mixin every($num) {</p>
...
<p>/// Select only the first and last child. @mixin first-last {</p>
...
<p>/// Select all children after the first to `$num`. /// @param {Number}
$num[] After First `n` numbers of total children  @mixin after-first($num)
{</p>

<pre>&amp;:nth-child(n + #{$num + 1}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select all children before `$num` from the last. /// @param {Number}
$num[] From Last `n` numbers of total children  @mixin from-last($num) {</p>

<pre>&amp;:nth-last-child(#{$num}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select the `$num` child from the first and the `$num` child from the
last. /// @param {Number} $num[] `n` number called from first and last 
@mixin from-first-last($num) {</p>
...
<p>/// Select all children but `$num`. /// @param {Number} $num[] `n` number
that should be excluded from all other children  @mixin all-but($num) {</p>

<pre>&amp;:not(:nth-child(#{$num})) {
  @content;
}</pre>

<p>}</p>

<p>/// Select all children between the `$num` first and the `$num` last. ///
@param {Number} $num[] `n` number excluded from first and last from all
other children @mixin all-but-first-last($num) {</p>

<pre>&amp;:nth-child(n + #{$num}):nth-last-child(n + #{$num}) {
  @content;
}</pre>

<p>}</p>

<p>/// Will only select the child if it&#39;s unique. That means that if there
are at least 2 children, the style will not be applied. @mixin unique {</p>
...
<p>/// Will only select children if they are not unique. That means that if
there are at least 2 children, the style will be applied. @mixin
not-unique() {</p>
...
<p>/// Select all children between `$first` and `$last`. /// @param {Number}
$first[] First `nth` number /// @param {Number} $last[] Last `nth` number
@mixin between($first, $last) {</p>

<pre>&amp;:nth-child(n + #{$first}):nth-child(-n + #{$last}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select all even children. @mixin even {</p>
...
<p>/// Select all even children between `$first` and `$last`. /// @param
{Number} $first[] First `nth` number /// @param {Number} $last[] Last `nth`
number @mixin even-between($first, $last) {</p>

<pre>&amp;:nth-child(even):nth-child(n + #{$first}):nth-child(-n + #{$last}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select all odd children. @mixin odd {</p>
...
<p>/// Select all odd children between `$first` and `$last`. /// @param
{Number} $first[] First `nth` number /// @param {Number} $last[] Last `nth`
number @mixin odd-between($first, $last) {</p>

<pre>&amp;:nth-child(odd):nth-child(n + #{$first}):nth-child(-n + #{$last}) {
  @content;
}</pre>

<p>}</p>

<p>/// Select all `$num` children between `$first` and `$last`. /// @param
{Number} $num[] Every `n` number between `$first` and `$last`. /// @param
{Number} $first[] First `n` number /// @param {Number} $last[] Last `n`
number @mixin number-between($num, $first, $last) {</p>

<pre>&amp;:nth-child(#{$num}n):nth-child(n + #{$first}):nth-child(-n + #{$last}) {
  @content;
}</pre>
