...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/prototype/_position.scss">

<p>// Foundation for Sites by ZURB // foundation.zurb.com // Licensed under
MIT Open Source</p>

<p>//// /// @group prototype-position ////</p>

<p>/// Responsive breakpoints for position helpers /// @type Boolean
$prototype-position-breakpoints: $global-prototype-breakpoints !default;</p>

<p>/// Map containing all the `position` classes /// @type Map
...
<p>/// z-index for fixed positioning /// @type Number
$prototype-position-z-index: 975 !default;</p>

<p>/// Position classes, by default coming through a map
`$prototype-position`, whereas all the offset values are multiplied by
`$global-position` which by default is equal to `1rem`. /// @param {String}
$position [] Position classes, Either `static`, `relative`, `absolute` or
`fixed` /// @param {Number} $top [null] - Top offset /// @param {Number}
$right [null] - Right offset /// @param {Number} $bottom [null] - Bottom
offset /// @param {Number} $left [null] - Left offset @mixin position(</p>
...
<pre>position: $position !important;
@if $top != null {
...
<p>/// Position Fixed on top corners /// @param {Number} $z-index
[$prototype-position-z-index] z-index for `position-fixed-top` @mixin
position-fixed-top(</p>

<pre>$z-index: $prototype-position-z-index</pre>

<p>) {</p>

<pre>@include position(fixed, 0, 0, null, 0);
z-index: $z-index;</pre>

<p>}</p>

<p>/// Position Fixed on bottom corners /// @param {Number} $z-index
[$prototype-position-z-index] z-index for `position-fixed-bottom` @mixin
position-fixed-bottom(</p>

<pre>$z-index: $prototype-position-z-index</pre>

<p>) {</p>

<pre>@include position(fixed, null, 0, 0, 0);
z-index: $z-index;</pre>

<p>}</p>

<p>@mixin foundation-prototype-position {</p>

<pre>// Position: Static, Relative, Fixed, Absolute
