...
<title>_off-canvas.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/components/_off-canvas.scss">

<p>// Foundation for Sites by ZURB // foundation.zurb.com // Licensed under
MIT Open Source</p>

<p>//// /// @group off-canvas ////</p>

<p>/// Width map of a left/right off-canvas panel. /// @type Map
$offcanvas-sizes: (</p>
...
<p>/// Height map of a top/bottom off-canvas panel. /// @type Map
$offcanvas-vertical-sizes: (</p>

<pre>small: 250px,</pre>

<p>) !default;</p>

<p>/// Background color of an off-canvas panel. /// @type Color
$offcanvas-background: $light-gray !default;</p>

<p>/// Box shadow for the off-canvas overlap panel. /// @type Shadow
$offcanvas-shadow: 0 0 10px rgba($black, 0.7) !default;</p>

<p>/// Inner box shadow size for the off-canvas push panel. /// @type Number
$offcanvas-inner-shadow-size: 20px !default;</p>

<p>/// Inner box shadow color for the off-canvas push panel. /// @type Color
$offcanvas-inner-shadow-color: rgba($black, 0.25) !default;</p>

<p>/// Z-index of an off-canvas content overlay. /// @type Number
$offcanvas-overlay-zindex: 11 !default;</p>

<p>/// Z-index of an off-canvas panel with the `push` transition. /// @type
Number $offcanvas-push-zindex: 12 !default;</p>

<p>/// Z-index of an off-canvas panel with the `overlap` transition. /// @type
Number $offcanvas-overlap-zindex: 13 !default;</p>

<p>/// Z-index of an off-canvas panel using the `reveal-for-*` classes or
mixin. /// @type Number $offcanvas-reveal-zindex: 12 !default;</p>

<p>/// Length of the animation on an off-canvas panel. /// @type Number
$offcanvas-transition-length: 0.5s !default;</p>

<p>/// Timing function of the animation on an off-canvas panel. /// @type
Keyword $offcanvas-transition-timing: ease !default;</p>

<p>/// If `true`, a revealed off-canvas will be fixed-position, and scroll
with the screen. /// @type Bool $offcanvas-fixed-reveal: true !default;</p>

<p>/// Background color for the overlay that appears when an off-canvas panel
is open. /// @type Color $offcanvas-exit-background: rgba($white, 0.25)
!default;</p>

<p>/// CSS class used for the main content area. The off-canvas mixins use
this to target the page content. $maincontent-class:
&#39;off-canvas-content&#39; !default;</p>

<p>/// Adds baseline styles for off-canvas. This CSS is required to make the
other pieces work. @mixin off-canvas-basics {</p>

<pre>/// Transform deprecated size settings into map &amp; show warning
...
<p>// Adds basic styles for an off-canvas wrapper. @mixin off-canvas-wrapper()
{</p>
...
<p>/// Adds basic styles for an off-canvas panel. @mixin off-canvas-base(</p>

<pre>$background: $offcanvas-background,
...
<p>/// Adds styles to position an off-canvas panel to the
left/right/top/bottom. @mixin off-canvas-position(</p>

<pre>$position: left,
$orientation: horizontal,
$sizes: if($orientation == horizontal, $offcanvas-sizes, $offcanvas-vertical-sizes)</pre>

<p>) {</p>

<pre>@if $position == left {
  top: 0;
  left: 0;
  height: 100%;
  overflow-y: auto;
...
<p>/// Sets the styles for the content container. @mixin off-canvas-content()
{</p>
...
<p>/// Adds styles that reveal an off-canvas panel. @mixin off-canvas-reveal(
$position: left, $zindex: $offcanvas-reveal-zindex, $content:
$maincontent-class, $breakpoint: small ) {</p>
...
<p>/// Overrides the off-canvas styles @mixin in-canvas() {</p>
...
<p>@mixin foundation-off-canvas {</p>

<pre>@include off-canvas-basics;
