...
<title>_button-group.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/components/_button-group.scss">

<p>// Foundation for Sites by ZURB // foundation.zurb.com // Licensed under
MIT Open Source</p>

<p>//// /// @group button-group ////</p>

<p>/// Margin for button groups. /// @type Number $buttongroup-margin: 1rem
!default;</p>

<p>/// Margin between buttons in a button group. /// @type Number
$buttongroup-spacing: 1px !default;</p>

<p>/// Selector for the buttons inside a button group. /// @type String
$buttongroup-child-selector: &#39;.button&#39; !default;</p>

<p>/// Maximum number of buttons that can be in an even-width button group.
(Only needed when $global-flexbox: false;) /// @type Number
$buttongroup-expand-max: 6 !default;</p>

<p>/// Determines if $button-radius is applied to each button or the button
group as a whole. Use $global-radius in _settings.scss to change radius.
/// @type Boolean $buttongroup-radius-on-each: true !default;</p>

<p>/// Add styles for a button group container. /// @param {String}
$child-selector [$buttongroup-child-selector] - Selector for the buttons
inside a button group. /// @param {Number} $spacing [$buttongroup-spacing]
- Spacing between buttons in a button group. @mixin button-group(</p>

<pre>$child-selector: $buttongroup-child-selector,
$spacing: $buttongroup-spacing</pre>
...
<p>/// Creates a full-width button group, making each button equal width. ///
@param {String} $selector [$buttongroup-child-selector] - Selector for the
buttons inside a button group. /// @param {Number} $spacing
[$buttongroup-spacing] - Spacing between buttons in a button group. @mixin
button-group-expand(</p>

<pre>$selector: $buttongroup-child-selector,
$spacing: $buttongroup-spacing,
$count: null</pre>

<p>) {</p>

<pre>@if not $global-flexbox {
  margin-#{$global-right}: -$spacing;
...
<p>/// Stacks the buttons in a button group. /// @param {String} $selector
[$buttongroup-child-selector] - Selector for the buttons inside the button
group. @mixin button-group-stack(</p>

<pre>$selector: $buttongroup-child-selector</pre>
...
<p>/// Un-stacks the buttons in a button group. /// @param {String} $selector
[$buttongroup-child-selector] - Selector for the buttons inside the button
group. @mixin button-group-unstack(</p>
...
<p>@mixin foundation-button-group {</p>
