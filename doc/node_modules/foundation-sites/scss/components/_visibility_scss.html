...
<title>_visibility.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/components/_visibility.scss">

<p>// Foundation for Sites by ZURB // foundation.zurb.com // Licensed under
MIT Open Source</p>

<p>/// Hide an element by default, only displaying it above a certain screen
size. /// @param {Keyword} $size - Breakpoint to use. **Must be a
breakpoint defined in `$breakpoints`.** @mixin show-for($size) {</p>

<pre>$size: map-get($breakpoints, $size);
...
<p>/// Hide an element by default, only displaying it within a certain
breakpoint. /// @param {Keyword} $size - Breakpoint to use. **Must be a
breakpoint defined in `$breakpoints`.** @mixin show-for-only($size) {</p>

<pre>$lower-bound-size: map-get($breakpoints, $size);
...
<p>/// Show an element by default, and hide it above a certain screen size.
/// @param {Keyword} $size - Breakpoint to use. **Must be a breakpoint
defined in `$breakpoints`.** @mixin hide-for($size) {</p>

<pre>@include breakpoint($size) {
  display: none !important;
}</pre>
...
<p>/// Show an element by default, and hide it above a certain screen size.
/// @param {Keyword} $size - Breakpoint to use. **Must be a breakpoint
defined in `$breakpoints`.** @mixin hide-for-only($size) {</p>

<pre>@include breakpoint($size only) {
  display: none !important;
}</pre>

<p>}</p>

<p>@mixin foundation-visibility-classes {</p>

<pre>// Basic hiding classes
.hide {
  display: none !important;
}
