...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/components/_reveal.scss">
...
<p>/// Default background color of a modal. /// @type Color
$reveal-background: $white !default;</p>

<p>/// Default width of a modal, with no class applied. /// @type Number
$reveal-width: 600px !default;</p>

<p>/// Default maximum width of a modal. /// @type Number $reveal-max-width:
$global-width !default;</p>

<p>/// Default padding inside a modal. /// @type Number $reveal-padding:
$global-padding !default;</p>

<p>/// Default border around a modal. /// @type Number $reveal-border: 1px
solid $medium-gray !default;</p>

<p>/// Default radius for modal. /// @type Number $reveal-radius:
$global-radius !default;</p>

<p>/// z-index for modals. The overlay uses this value, while the modal itself
uses this value plus one. /// @type Number $reveal-zindex: 1005 !default;</p>

<p>/// Background color of modal overlays. /// @type Color
$reveal-overlay-background: rgba($black, 0.45) !default;</p>

<p>// Placeholder selector for medium-and-up modals // Prevents duplicate CSS
when defining multiple Reveal sizes // This should be in the same
breakpoint then `@mixin reveal-modal-width` @include breakpoint(medium) {</p>
...
<p>/// Adds styles for a modal overlay. /// @param {Color} $background
[$reveal-overlay-background] - Background color of the overlay. @mixin
reveal-overlay($background: $reveal-overlay-background) {</p>
...
<p>/// Adds base styles for a modal. @mixin reveal-modal-base {</p>
...
<p>/// Adjusts the width of a modal. /// @param {Number} $width - Width of the
modal. Generally a percentage. /// @param {Number} $max-width
[$reveal-max-width] - Maximum width of the modal. @mixin
reveal-modal-width(</p>

<pre>$width: $reveal-width,
$max-width: $reveal-max-width</pre>

<p>) {</p>

<pre>// Extends must be made outside of breakpoints for compatibility with newer Sass versions (libsass v3.5)
...
<p>/// Creates a full-screen modal, which stretches the full width and height
of the window. @mixin reveal-modal-fullscreen {</p>
...
<p>@mixin foundation-reveal {</p>

<pre>/// Disables the scroll when Reveal is shown to prevent the background from shifting
html.is-reveal-open {
  position: fixed;
  width: 100%;
  overflow-y: hidden;
