...
<title>_size.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_size.scss">
...
<p>/// Set the width of a grid column. /// /// @param {Number|List} $width
[$grid-column-count] - Width to make the column. You can pass in any value
accepted by the `grid-column()` function, such as `6`, `50%`, or `1 of 2`.
@mixin grid-column-size(</p>

<pre>$columns: $grid-column-count</pre>

<p>) {</p>

<pre>width: grid-column($columns);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-size()`. /// @alias grid-column-size @mixin
grid-col-size(</p>
...
<pre>@include grid-column-size($columns);</pre>
