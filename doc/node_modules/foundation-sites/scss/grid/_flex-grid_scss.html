...
<title>_flex-grid.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_flex-grid.scss">
...
<p>/// Creates a container for a flex grid row. /// /// @param {Keyword|List}
$behavior [null] ///   Modifications to the default grid styles. `nest`
...
combines both behaviors. /// @param {Keyword|Number} $size
[$grid-row-width] Maximum size of the row. Set to `expand` to make the row
taking the full width. /// @param {Number} $columns [null] - Number of
columns to use for this row. If set to `null` (the default), the global
column count will be used. /// @param {Boolean} $base [true] - Set to
`false` to prevent basic styles from being output. Useful if you&#39;re
calling this mixin on the same element twice, as it prevents duplicate CSS
output. /// @param {Boolean} $wrap [true] - Set to `false` to have row
wrapping behavior set to nowrap /// @param {Number|Map} $gutters
[$grid-column-gutter] - Gutter map or single value to use when inverting
margins, in case the row is nested. Responsive gutter settings by default.
@mixin flex-grid-row(</p>
...
$base: true,
$wrap: true,
$gutters: $grid-column-gutter</pre>
...
<p>/// Calculates the `flex` property for a flex grid column. It accepts all
of the same values as the basic `grid-column()` function, along with two
extras: ///   - `expand` (the default) will make the column expand to fill
space. ///   - `shrink` will make the column contract, so it only takes up
the horizontal space it needs. /// /// @param {Mixed} $columns [expand] -
Width of the column. @function flex-grid-column($columns: expand) {</p>

<pre>$flex: 1 1 0px; // sass-lint:disable-line zero-unit
...
<p>/// Creates a column for a flex grid. By default, the column will stretch
to the full width of its container, but this can be overridden with sizing
classes, or by using the `unstack` class on the parent flex row. /// ///
@param {Mixed} $columns [expand] - Width of the column. Refer to the
`flex-grid-column()` function to see possible values. /// @param
{Number|Map} $gutters [$grid-column-gutter] - Map or single value for
gutters width. See the `grid-column-gutter` mixin. @mixin flex-grid-column(</p>
...
<p>/// Creates a block grid for a flex grid row. /// /// @param {Number} $n -
Number of columns to display on each row. /// @param {String} $selector -
Selector to use to target columns within the row. @mixin flex-grid-layout(</p>
...
<p>/// Changes the width flex grid column. /// @param {Mixed} $columns
[expand] - Width of the column. Refer to the `flex-grid-column()` function
to see possible values. @mixin flex-grid-size($columns: null) {</p>

<pre>$columns: $columns or expand;
...
<p>@mixin foundation-flex-grid {</p>
