...
<title>_column.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_column.scss">
...
<p>/// Calculates the width of a column based on a number of factors. /// ///
@param {Number|List} $columns ///   Width of the column. Accepts multiple
values: ///   - A percentage value will make the column that exact size.
///   - A single digit will make the column span that number of columns
wide, taking into account the column count of the parent row. ///   - A
list of the format “x of y” (without quotes) will make a column that is
<strong>x</strong> columns wide, assuming <strong>y</strong> total columns
for the parent. /// /// @returns {Number} A calculated percentage value.
@function grid-column($columns) {</p>
...
<p>/// Creates a grid column. /// /// @param {Mixed} $columns
[$grid-column-count] - Width of the column. Refer to the `grid-column()`
function to see possible values. /// @param {Mixed} $gutters
[$grid-column-gutter] - Spacing between columns. Refer to the
`grid-column-gutter()` function to see possible values. @mixin grid-column(</p>

<pre>$columns: $grid-column-count,
$gutters: $grid-column-gutter</pre>

<p>) {</p>

<pre>@include grid-column-size($columns);
float: $global-left;
...
// Position
@include grid-col-pos(auto);</pre>

<p>}</p>

<p>/// Creates a grid column row. This is the equivalent of adding `.row` and
`.column` to the same element. /// /// @param {Mixed} $gutters
[$grid-column-gutter] - Width of the gutters on either side of the column
row. Refer to the `grid-column-gutter()` function to see possible values.
@mixin grid-column-row(</p>

<pre>$gutters: $grid-column-gutter</pre>

<p>) {</p>

<pre>@include grid-row;
...
<p>/// Shorthand for `grid-column()`. /// @alias grid-column @function
grid-col(</p>

<pre>$columns: $grid-column-count</pre>

<p>) {</p>

<pre>@return grid-column($columns);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column()`. /// @alias grid-column @mixin grid-col(</p>
...
<pre>@include grid-column($columns, $gutters);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-row()`. /// @alias grid-column-row @mixin
grid-col-row(</p>
...
<pre>@include grid-column-row($gutters);</pre>
