...
<title>_position.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_position.scss">
...
<p>/// Reposition a column. /// /// @param {Number|Keyword} $position - It can
be: ///   * A number: The column will move equal to the width of the column
count ///     specified. A positive number will push the column to the
right, while ///     a negative number will pull it to the left. ///   *
`center`: Column will be centered ///   * `auto`: Column will be pushed to
the left (or to the right for the last column). @mixin
grid-column-position($position) {</p>

<pre>// Auto positioning
@if $position == auto {
...
<p>/// Reset a position definition. @mixin grid-column-unposition {</p>

<pre>@include grid-column-position(auto);
position: static;
margin-right: 0;
margin-left: 0;</pre>
...
<p>/// Offsets a column to the right by `$n` columns. /// @param {Number|List}
$n - Width to offset by. You can pass in any value accepted by the
`grid-column()` mixin, such as `6`, `50%`, or `1 of 2`. @mixin
grid-column-offset($n) {</p>

<pre>margin-#{$global-left}: grid-column($n);</pre>

<p>}</p>

<p>/// Disable the default behavior of the last column in a row aligning to
the opposite edge. @mixin grid-column-end {</p>

<pre>// This extra specificity is required for the property to be applied
...
<p>/// Shorthand for `grid-column-position()`. /// @alias grid-column-position
@mixin grid-col-pos($position) {</p>

<pre>@include grid-column-position($position);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-unposition()`. /// @alias
grid-column-unposition @mixin grid-col-unpos {</p>

<pre>@include grid-column-unposition;</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-offset()`. /// @alias grid-column-offset
@mixin grid-col-off($n) {</p>

<pre>@include grid-column-offset($n);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-end()`. /// @alias grid-column-end @mixin
grid-col-end {</p>

<pre>@include grid-column-end;</pre>
