...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_layout.scss">
...
@param {String} $selector [&#39;.column&#39;] - Selector(s) to use for
child elements. /// @param {Number|List} $gutter ///   The gutter to apply
to child elements. Accepts multiple values: ///   - $grid-column-gutter
will use the values in the $grid-column-gutter map, including breakpoint
sizes. ///   - A fixed numeric value will apply this gutter to all
breakpoints. @mixin grid-layout(</p>
...
<p>/// Adds extra CSS to block grid children so the last items in the row
center automatically. Apply this to the columns, not the row. /// ///
@param {Number} $n - Number of items that appear in each row. @mixin
grid-layout-center-last($n) {</p>
