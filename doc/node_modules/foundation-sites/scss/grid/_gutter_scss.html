...
<title>_gutter.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/grid/_gutter.scss">
...
<p>/// Set the gutters on a column /// @param {Number|Keyword} $gutter [auto]
///   Spacing between columns, accepts multiple values: ///   - A single
value will make the gutter that exact size. ///   - A breakpoint name will
make the gutter the corresponding size in the $gutters map. ///   - “auto”
will make the gutter responsive, using the $gutters map values. /// @param
{Number|Map} $gutters [$grid-column-gutter] - Gutter map or single value to
use. Responsive gutter settings by default. @mixin grid-column-gutter(</p>
...
<pre>@include -zf-breakpoint-value($gutter, $gutters) {
...
<p>/// Collapse the gutters on a column by removing the padding.
*<strong>Note:</strong>* only use this mixin within a breakpoint. To
collapse a column&#39;s gutters on all screen sizes, use the `$gutter`
parameter of the `grid-column()` mixin instead. @mixin grid-column-collapse
{</p>

<pre>@include grid-column-gutter(0);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-gutter()`. /// @alias grid-column-gutter
@mixin grid-col-gutter(</p>
...
<pre>@include grid-column-gutter($gutter, $gutters);</pre>

<p>}</p>

<p>/// Shorthand for `grid-column-collapse()`. /// @alias grid-column-collapse
@mixin grid-col-collapse {</p>

<pre>@include grid-column-collapse;</pre>

<p>}</p>

<p>/// Sets bottom margin on grid columns to match gutters /// @param
{Number|Keyword} $margin [auto] ///   The bottom margin on grid columns,
accepts multiple values: ///   - A single value will make the margin that
exact size. ///   - A breakpoint name will make the margin the
corresponding size in the $margins map. ///   - “auto” will make the margin
responsive, using the $margins map values. /// @param {Number|Map} $margins
[$grid-column-gutter] - Map or single value to use. Responsive gutter
settings by default. @mixin grid-column-margin (</p>

<pre>$margin: auto,
$margins: $grid-column-gutter</pre>

<p>) {</p>

<pre>@include -zf-breakpoint-value($margin, $margins) {
