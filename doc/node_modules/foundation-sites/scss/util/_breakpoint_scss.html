...
<title>_breakpoint.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/util/_breakpoint.scss">
...
<p>/// A list of named breakpoints. You can use these with the `breakpoint()`
mixin to quickly create media queries. /// @type Map $breakpoints: (</p>
...
<p>/// The largest named breakpoint in which to include print as a media type
/// @type Keyword $print-breakpoint: large !default;</p>

<p>$-zf-zero-breakpoint: small !default;</p>

<p>$-zf-breakpoints-keys: map-to-list($breakpoints, &#39;keys&#39;);</p>

<p>@if nth(map-values($breakpoints), 1) != 0 {</p>

<pre>@error &#39;The first key in the $breakpoints map must have a value of &quot;0&quot;.&#39;;</pre>

<p>} @else {</p>

<pre>$-zf-zero-breakpoint: nth(map-keys($breakpoints), 1);</pre>

<p>}</p>

<p>/// All of the names in this list will be output as classes in your CSS,
like `.small-12`, `.medium-6`, and so on. Each value in this list must also
be in the `$breakpoints` map. /// @type List $breakpoint-classes: (small
medium large) !default;</p>

<p>/// Generates a media query string matching the input value. Refer to the
documentation for the `breakpoint()` mixin to see what the possible inputs
are. /// /// @param {Keyword|Number} $val [small] - Breakpoint name, or px,
rem, or em value to process. @function breakpoint($val:
$-zf-zero-breakpoint) {</p>
...
<p>/// Wraps a media query around the content you put inside the mixin. This
mixin accepts a number of values: ///  - If a string is passed, the mixin
will look for it in the `$breakpoints` map, and use a media query there.
///  - If a pixel value is passed, it will be converted to an em value
using `$global-font-size` as the base. ///  - If a rem value is passed, the
unit will be changed to em. ///  - If an em value is passed, the value will
be used as-is. /// /// @param {Keyword|Number} $value - Breakpoint name, or
px, rem, or em value to process. /// /// @output If the breakpoint is “0px
and larger”, outputs the content as-is. Otherwise, outputs the content
wrapped in a media query. @mixin breakpoint($value) {</p>

<pre>$str: breakpoint($value);
...
$-zf-size: $old-zf-size !global;</pre>

<p>}</p>

<p>/// Converts the breakpoints map to a URL-encoded string, like this:
`key1=value1&amp;key2=value2`. The value is then dropped into the CSS for a
special `&lt;meta&gt;` tag, which is read by the Foundation JavaScript.
This is how we transfer values from Sass to JavaScript, so they can be
defined in one place. /// @access private /// /// @param {Map} $map - Map
to convert. /// /// @returns {String} A string containing the map&#39;s
contents. @function -zf-bp-serialize($map) {</p>
...
<p>/// Find the next key in a map. /// @access private /// /// @param {Map}
$map - Map to traverse. /// @param {Mixed} $key - Key to use as a starting
point. /// /// @returns {Mixed} The value for the key after `$key`, if
`$key` was found. If `$key` was not found, or `$key` was the last value in
the map, returns `null`. @function -zf-map-next($map, $key) {</p>

<pre>// Store the keys of the map as a list
$values: map-keys($map);
...
<p>/// Return a list of our named breakpoints less than $key. Useful for
dealing with /// responsive gutters for the grid. /// @access private ///
/// @param {String} $key - Key to use as last breakpoint. /// /// @returns
{Array} The list of breakpoints up to and. If $key is auto, returns
breakpoints above the zero @function -zf-breakpoints-less-than($key) {</p>
...
returns breakpoints above the zero @function
-zf-closest-named-breakpoint($breakpoint) {</p>

<pre>$last: $-zf-zero-breakpoint;
$found: false;
...
<p>/// Get a value for a breakpoint from a responsive config map or single
value. /// - If the config is a single value, return it regardless of
`$value`. /// - If the config is a map and has the key `$value`, the exact
breakpoint value is returned. /// - If the config is a map and does
<strong>not</strong> have the breakpoint, the value matching the next
lowest breakpoint in the config map is returned. /// @access private ///
/// @param {Number|Map} $map - Responsive config map or single value. ///
@param {Keyword} $value - Breakpoint name to use. /// /// @return {Mixed}
The corresponding breakpoint value. @function -zf-get-bp-val($map, $value)
{</p>

<pre>// If the given map is a single value, return it
...
<p>$small-up: &#39;&#39;; $small-only: &#39;&#39;;</p>

<p>@if map-has-key($breakpoints, small) {</p>

<pre>$small-up: screen;
$small-only: unquote(&#39;screen and #{breakpoint(small only)}&#39;);</pre>

<p>}</p>

<p>$medium-up: &#39;&#39;; $medium-only: &#39;&#39;;</p>

<p>@if map-has-key($breakpoints, medium) {</p>

<pre>$medium-up: unquote(&#39;screen and #{breakpoint(medium)}&#39;);
$medium-only: unquote(&#39;screen and #{breakpoint(medium only)}&#39;);</pre>

<p>}</p>

<p>$large-up: &#39;&#39;; $large-only: &#39;&#39;;</p>

<p>@if map-has-key($breakpoints, large) {</p>

<pre>$large-up: unquote(&#39;screen and #{breakpoint(large)}&#39;);
$large-only: unquote(&#39;screen and #{breakpoint(large only)}&#39;);</pre>

<p>}</p>

<p>$xlarge-up: &#39;&#39;; $xlarge-only: &#39;&#39;;</p>

<p>@if map-has-key($breakpoints, xlarge) {</p>

<pre>$xlarge-up: unquote(&#39;screen and #{breakpoint(xlarge)}&#39;);
$xlarge-only: unquote(&#39;screen and #{breakpoint(xlarge only)}&#39;);</pre>

<p>}</p>

<p>$xxlarge-up: &#39;&#39;;</p>

<p>@if map-has-key($breakpoints, xxlarge) {</p>

<pre>$xxlarge-up: unquote(&#39;screen and #{breakpoint(xxlarge)}&#39;);</pre>
