...
<title>_color.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/util/_color.scss">
...
<p>$contrast-warnings: true !default;</p>

<p>//// /// @group functions ////</p>

<p>/// Checks the luminance of `$color`. /// /// @param {Color} $color - Color
to check the luminance of. /// /// @returns {Number} The luminance of
`$color`. @function color-luminance($color) {</p>

<pre>// Adapted from: https://github.com/LeaVerou/contrast-ratio/blob/gh-pages/color.js
...
@return 0.2126 * nth($rgba2, 1) + 0.7152 * nth($rgba2, 2) + 0.0722 * nth($rgba2, 3);</pre>

<p>}</p>

<p>/// Checks the contrast ratio of two colors. /// /// @param {Color} $color1
- First color to compare. /// @param {Color} $color2 - Second color to
compare. /// /// @returns {Number} The contrast ratio of the compared
colors. @function color-contrast($color1, $color2) {</p>
...
<p>/// Checks the luminance of `$base`, and returns the color from `$colors`
(list of colors) that has the most contrast. /// /// @param {Color} $base -
Color to check luminance. /// @param {List} $colors [($white, $black)] -
Colors to compare. /// @param {Number} $tolerance
[$global-color-pick-contrast-tolerance] - Contrast tolerance. /// ///
@returns {Color} the color from `$colors` (list of colors) that has the
most contrast. @function color-pick-contrast($base, $colors: ($white,
$black), $tolerance: $global-color-pick-contrast-tolerance) {</p>

<pre>$contrast: color-contrast($base, nth($colors, 1));
$best: nth($colors, 1);
...
<p>/// Scales a color to be darker if it&#39;s light, or lighter if it&#39;s
dark. Use this function to tint a color appropriate to its lightness. ///
/// @param {Color} $color - Color to scale. /// @param {Percentage} $scale
[5%] - Amount to scale up or down. /// @param {Percentage} $threshold [40%]
- Threshold of lightness to check against. /// /// @returns {Color} A
scaled color. @function smart-scale($color, $scale: 5%, $threshold: 40%) {</p>

<pre>@if lightness($color) &gt; $threshold {
  $scale: -$scale;
}
@return scale-color($color, $lightness: $scale);</pre>

<p>}</p>

<p>/// Get color from foundation-palette /// /// @param {key} color key from
foundation-palette /// /// @returns {Color} color from foundation-palette
@function get-color($key) {</p>

<pre>@if map-has-key($foundation-palette, $key) {
...
<p>/// Transfers the colors in the `$foundation-palette` map into variables,
such as `$primary-color` and `$secondary-color`. Call this mixin below the
Global section of your settings file to properly migrate your codebase.
@mixin add-foundation-colors() {</p>

<pre>@if map-has-key($foundation-palette, primary) {
