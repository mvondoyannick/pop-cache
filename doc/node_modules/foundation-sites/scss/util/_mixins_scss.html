...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/util/_mixins.scss">
...
<p>/// Creates an inner box-shadow for only one side /// /// @param {Keyword}
$side - Side the shadow is supposed to appear. Can be `top`, `left`,
`right` or `bottom`. /// @param {Number} $size - Width for the target side.
/// @param {Color} $color - Color of the shadow. @mixin inner-side-shadow(</p>

<pre>$side: bottom,
$size: 20px,
$color: rgba($black, 0.25)</pre>

<p>) {</p>

<pre>$helper: round($size * 0.65);
...
<p>/// Creates a CSS triangle, which can be used for dropdown arrows, dropdown
pips, and more. Use this mixin inside a `&amp;::before` or `&amp;::after`
selector, to attach the triangle to an existing element. /// /// @param
{Number} $triangle-size - Width of the triangle. /// @param {Color}
$triangle-color - Color of the triangle. /// @param {Keyword}
$triangle-direction - Direction the triangle points. Can be `up`, `right`,
`down`, or `left`. @mixin css-triangle(</p>

<pre>$triangle-size,
$triangle-color,
$triangle-direction</pre>
...
<p>/// Creates a menu icon with a set width, height, number of bars, and
colors. The mixin uses the height of the icon and the weight of the bars to
determine spacing. &lt;div class=“docs-example-burger”&gt;&lt;/div&gt; ///
/// @param {Color} $color [$black] - Color to use for the icon. /// @param
{Color} $color-hover [$dark-gray] - Color to use when the icon is hovered
over. /// @param {Number} $width [20px] - Width of the icon. /// @param
{Number} $height [16px] - Height of the icon. /// @param {Number} $weight
[2px] - Height of individual bars in the icon. /// @param {Number} $bars
[3] - Number of bars in the icon. @mixin hamburger(</p>
...
<p>/// Adds a downward-facing triangle as a background image to an element.
The image is formatted as an SVG, making it easy to change the color.
Because Internet Explorer doesn&#39;t support encoded SVGs as background
images, a PNG fallback is also included. /// There are two PNG fallbacks: a
black triangle and a white triangle. The one used depends on the lightness
of the input color. /// /// @param {Color} $color [$black] - Color to use
for the triangle. @mixin background-triangle($color: $black) {</p>

<pre>$rgb: &#39;rgb%28#{round(red($color))}, #{round(green($color))}, #{round(blue($color))}%29&#39;;
...
<p>/// Applies the micro clearfix hack popularized by Nicolas Gallagher.
Include this mixin on a container if its children are all floated, to give
the container a proper height. /// The clearfix is augmented with specific
styles to prevent borders in flexbox environments /// @link <a
href="http://nicolasgallagher.com/micro-clearfix-hack">nicolasgallagher.com/micro-clearfix-hack</a>/
Micro Clearfix Hack /// @link <a
...
<p>/// Adds CSS for a “quantity query” selector that automatically sizes
elements based on how many there are inside a container. /// @link <a
href="http://alistapart.com/article/quantity-queries-for-css">alistapart.com/article/quantity-queries-for-css</a>
Quantity Queries for CSS /// /// @param {Number} $max - Maximum number of
items to detect. The higher this number is, the more CSS that&#39;s
required to cover each number of items. /// @param {Keyword} $elem [li] -
Tag to use for sibling selectors. @mixin auto-width($max, $elem: li) {</p>

<pre>@for $i from 2 through $max {
...
<p>/// Removes the focus ring around an element when a mouse input is
detected. @mixin disable-mouse-outline {</p>

<pre>[data-whatinput=&#39;mouse&#39;] &amp; {
  outline: 0;
}</pre>

<p>}</p>

<p>/// Makes an element visually hidden, but still accessible to keyboards and
assistive devices. /// @link <a
...
Hiding Content for Accessibility /// @link <a
href="http://hugogiraudel.com/2016/10/13/css-hide-and-seek">hugogiraudel.com/2016/10/13/css-hide-and-seek</a>/
@mixin element-invisible {</p>
...
<p>/// Reverses the CSS output created by the `element-invisible()` mixin.
@mixin element-invisible-off {</p>
...
<p>/// Vertically centers the element inside of its first non-static parent,
/// @link <a
href="http://www.sitepoint.com/centering-with-sass">www.sitepoint.com/centering-with-sass</a>/
Centering With Sass @mixin vertical-center {</p>
...
<p>/// Horizontally centers the element inside of its first non-static parent,
/// @link <a
href="http://www.sitepoint.com/centering-with-sass">www.sitepoint.com/centering-with-sass</a>/
Centering With Sass @mixin horizontal-center {</p>
...
<p>/// Absolutely centers the element inside of its first non-static parent,
/// @link <a
href="http://www.sitepoint.com/centering-with-sass">www.sitepoint.com/centering-with-sass</a>/
Centering With Sass @mixin absolute-center {</p>
...
<p>/// Iterates through breakpoints defined in `$breakpoint-classes` and
prints the CSS inside the mixin at each breakpoint&#39;s media query. Use
this with the grid, or any other component that has responsive classes. ///
/// @param {Boolean} $small [true] - If `false`, the mixin will skip the
`small` breakpoint. Use this with components that don&#39;t prefix classes
with `small-`, only `medium-` and up. /// @param {Boolean}
...
media query generation as in the margin grid. @mixin
-zf-each-breakpoint($small: true, $auto-insert-breakpoints: true) {</p>

<pre>$list: $breakpoint-classes;
...
<p>/// Generate the `@content` passed to the mixin with a value
`$-zf-bp-value` related to a breakpoint, depending on the `$name`
parameter: /// - For a single value, `$-zf-bp-value` is this value. /// -
For a breakpoint name, `$-zf-bp-value` is the corresponding breakpoint
value in `$map`. /// - For “auto”, `$-zf-bp-value` is the corresponding
breakpoint value in `$map` and is passed to `@content`, which is made
responsive for each breakpoint of `$map`. /// @param {Number|Array|Keyword}
$name [auto] - Single value, breakpoint name, or list of breakpoint names
to use. “auto” by default. /// @param {Number|Map} $map - Map of
breakpoints and values or single value to use. @mixin -zf-breakpoint-value(</p>
...
<pre>@if $name == auto and type-of($map) == &#39;map&#39; {
  // &quot;auto&quot;
  @each $k, $v in $map {
