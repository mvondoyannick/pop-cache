...
<title>_value.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/foundation-sites/scss/util/_value.scss">
...
<p>/// Determine if a value is not falsey, in CSS terms. Falsey values are
`null`, `none`, `0` with any unit, or an empty list. /// /// @param {Mixed}
$val - Value to check. /// /// @returns {Boolean} `true` if `$val` is not
falsey. @function has-value($val) {</p>

<pre>@if $val == null or $val == none {
  @return false;
}
...
<p>/// Determine a top/right/bottom/right value on a padding, margin, etc.
property, no matter how many values were passed in. Use this function if
you need to know the specific side of a value, but don&#39;t know if the
value is using a shorthand format. /// /// @param {List|Number} $val -
...
`bottom`, or `left`. /// /// @returns {Number} A single value based on
`$val` and `$side`. @function get-side($val, $side) {</p>

<pre>$length: length($val);
...
<p>/// Given border $val, find a specific element of the border, which is
$elem. The possible values for $elem are width, style, and color. /// ///
@param {List} $val - Border value to find a value in. /// @param {Keyword}
$elem - Border component to extract. /// /// @returns {Mixed} If the value
exists, returns the value. If the value is not in the border definition,
the function will return a 0px width, solid style, or black border.
@function get-border-value($val, $elem) {</p>

<pre>// Find the width, style, or color and return it
@each $v in $val {
  $type: type-of($v);
...
@return map-get($defaults, $elem);</pre>

<p>}</p>

<p>/// Finds a value in a nested map. /// @link <a
href="https://css-tricks.com/snippets/sass/deep-getset-maps">css-tricks.com/snippets/sass/deep-getset-maps</a>/
Deep Get/Set in Maps /// /// @param {Map} $map - Map to pull a value from.
/// @param {String} $keys… - Keys to use when looking for a value. ///
@returns {Mixed} The value found in the map. @function map-deep-get($map,
$keys…) {</p>

<pre>@each $key in $keys {
  $map: map-get($map, $key);
}
@return $map;</pre>
...
<p>/// Casts a map into a list. /// @link <a
href="http://hugogiraudel.com/2014/04/28/casting-map-into-list">hugogiraudel.com/2014/04/28/casting-map-into-list</a>/
/// /// @param {Map} $map - Map to pull a value from. /// /// @returns
{List} Depending on the flag, returns either $keys or $values or both.
@function map-to-list($map, $keep: &#39;both&#39;) {</p>

<pre>$keep: if(index(&#39;keys&#39; &#39;values&#39;, $keep), $keep, &#39;both&#39;);
...
@return if(type-of($map) != &#39;list&#39;, ($value,), $map);</pre>

<p>}</p>

<p>/// Safely return a value from a map. /// /// @param {Map} $map - Map to
retrieve a value from. /// @param {String} $key - Name of the map key. ///
/// @returns {List} Found value. @function map-safe-get($map, $key) {</p>

<pre>@if (type-of($map) == &#39;map&#39; or (type-of($map) == &#39;list&#39; and length($map) == 0)) {
