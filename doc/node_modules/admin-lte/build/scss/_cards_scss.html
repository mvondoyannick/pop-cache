...
<title>_cards.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/build/scss/_cards.scss">
...
<p>.card, .overlay-wrapper {</p>

<pre>// Box overlay for LOADING STATE effect
&gt; .overlay,
&gt; .loading-img {
  position: absolute;
  top: 0;
...
<p>// Add clearfix to header, body and footer .card-header, .card-body,
.card-footer {</p>

<pre>@include clearfix();</pre>

<p>}</p>

<p>// Box header .card-header {</p>
...
<pre>font-size: $card-title-font-size;
font-weight: $card-title-font-weight;
margin: 0;</pre>

<p>}</p>

<p>// Box Tools Buttons .btn-tool {</p>
...
<p>// Box Body .card-body {</p>

<pre>// @include border-radius-sides(0, 0, $border-radius, $border-radius);
.no-header &amp; {
  // @include border-top-radius($border-radius);
}
...
<p>// Widget: TODO LIST .todo-list {</p>
...
<p>// Input in box .card-input {</p>

<pre>max-width: 200px;</pre>

<p>}</p>

<p>@each $name, $color in $theme-colors {</p>
