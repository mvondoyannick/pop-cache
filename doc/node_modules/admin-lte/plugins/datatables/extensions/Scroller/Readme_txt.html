...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/datatables/extensions/Scroller/Readme.txt">

<p># Scroller</p>

<p>Scroller is a virtual rendering plug-in for DataTables which allows large
datasets to be drawn on screen every quickly. What the virtual rendering
means is that only the visible portion of the table (and a bit to either
side to make the scrolling smooth) is drawn, while the scrolling container
gives the visual impression that the whole table is visible. This is done
by making use of the pagination abilities of DataTables and moving the
table around in the scrolling container DataTables adds to the page. The
scrolling container is forced to the height it would be for the full table
display using an extra element.</p>

<p>Key features include:</p>
<ul><li>
<p>Speed! The aim of Scroller for DataTables is to make rendering large data
sets fast</p>
</li><li>
<p>Full compatibility with DataTables&#39; deferred rendering for maximum
speed</p>
</li><li>
<p>Integration with state saving in DataTables (scrolling position is saved)</p>
</li><li>
<p>Support for scrolling with millions of rows</p>
</li><li>
<p>Easy to use</p>
</li></ul>
...
<p>To use Scroller, first download DataTables ( <a
href="http://datatables.net/download">datatables.net/download</a> ) and
place the unzipped Scroller package into a `extensions` directory in the
DataTables package. This will allow the pages in the examples to operate
correctly. To see the examples running, open the `examples` directory in
your web-browser.</p>

<p># Basic usage</p>

<p>Scroller is initialised by simply including the letter `dt-string S` in the
`dt-init dom` for the table you want to have this feature enabled on. Note
that the `dt-string S` must come after the `dt-string t` parameter in
`dom`. For example:</p>

<p>“`js $(document).ready( function () {</p>

<pre>$(&#39;#example&#39;).DataTable( {
        dom: &#39;lfrtipS&#39;
} );</pre>

<p>} ); “`</p>

<p>Note that rows in the table must all be the same height. Information in a
cell which expands on to multiple lines will cause some odd behaviour in
the scrolling. Additionally, the table&#39;s `cellspacing` parameter must
be set to 0, again to ensure the information display is correct.</p>

<p># Documentation / support</p>
<ul><li>
<p>Documentation: <a
href="http://datatables.net/extensions/scroller">datatables.net/extensions/scroller</a>/</p>
</li><li>
<p>DataTables support forums: <a
href="http://datatables.net/forums">datatables.net/forums</a></p>
</li></ul>

<p># GitHub</p>

<p>If you fancy getting involved with the development of Scroller and help
make it better, please refer to its GitHub repo: <a
href="https://github.com/DataTables/Scroller">github.com/DataTables/Scroller</a></p>
</main>
