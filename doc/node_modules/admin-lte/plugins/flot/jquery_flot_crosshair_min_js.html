...
<title>jquery.flot.crosshair.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.crosshair.min.js">

<p>(function($){var options={crosshair:{mode:null,color:“rgba(170, 0, 0,
...
offset=plot.offset();crosshair.x=Math.max(0,Math.min(e.pageX-offset.left,plot.width()));crosshair.y=Math.max(0,Math.min(e.pageY-offset.top,plot.height()));plot.triggerRedrawOverlay()}plot.hooks.bindEvents.push(function(plot,eventHolder){if(!plot.getOptions().crosshair.mode)return;eventHolder.mouseout(onMouseOut);eventHolder.mousemove(onMouseMove)});plot.hooks.drawOverlay.push(function(plot,ctx){var
...
drawY=Math.round(crosshair.y)+adj;ctx.moveTo(0,drawY);ctx.lineTo(plot.width(),drawY)}ctx.stroke()}ctx.restore()});plot.hooks.shutdown.push(function(plot,eventHolder){eventHolder.unbind(“mouseout”,onMouseOut);eventHolder.unbind(“mousemove”,onMouseMove)})}$.plot.plugins.push({init:init,options:options,name:“crosshair”,version:“1.0”})})(jQuery);</p>
</main>
