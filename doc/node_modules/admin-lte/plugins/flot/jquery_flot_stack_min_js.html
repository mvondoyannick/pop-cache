...
<title>jquery.flot.stack.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.stack.min.js">

<p>(function($){var options={series:{stack:null}};function init(plot){function
findMatchingSeries(s,allseries){var res=null;for(var
i=0;i&lt;allseries.length;++i){<a href="i">if(s==allseries</a>)<a
href="i">break;if(allseries</a>.<a
href="i">stack==s.stack)res=allseries</a>}return res}function
stackData(plot,s,datapoints){if(s.stack==null||s.stack===false)return;var
other=findMatchingSeries(s,plot.getData());if(!other)return;var <a
href="2">ps=datapoints.pointsize,points=datapoints.points,otherps=other.datapoints.pointsize,otherpoints=other.datapoints.points,newpoints=[],px,py,intery,qx,qy,bottom,withlines=s.lines.show,horizontal=s.bars.horizontal,withbottom=ps>2&&(horizontal?datapoints.format</a>.<a
href="2">x:datapoints.format</a>.y),withsteps=withlines&amp;&amp;s.lines.steps,fromgap=true,keyOffset=horizontal?1:0,accumulateOffset=horizontal?0:1,i=0,j=0,l,m;while(true){<a
href="i">if(i>=points.length)break;l=newpoints.length;if(points</a>==null){<a
href="i+m">for(m=0;m<ps;++m)newpoints.push(points</a>);i+=ps}else
if(j&gt;=otherpoints.length){if(!withlines){<a
href="i+m">for(m=0;m<ps;++m)newpoints.push(points</a>)}i+=ps}else <a
href="j">if(otherpoints</a>==null){for(m=0;m&lt;ps;++m)newpoints.push(null);fromgap=true;j+=otherps}else{<a
href="i+keyOffset">px=points</a>;<a
href="i+accumulateOffset">py=points</a>;<a
href="j+keyOffset">qx=otherpoints</a>;<a
href="j+accumulateOffset">qy=otherpoints</a>;bottom=0;if(px==qx){<a
href="i+m">for(m=0;m<ps;++m)newpoints.push(points</a>);<a
href="l+accumulateOffset">newpoints</a>+=qy;bottom=qy;i+=ps;j+=otherps}else
if(px&gt;qx){<a href="i-ps">if(withlines&&i>0&&points</a>!=null){<a
href="i-ps+accumulateOffset">intery=py+(points</a>-<a
href="i-ps+keyOffset">py)*(qx-px)/(points</a>-<a
href="i+m">px);newpoints.push(qx);newpoints.push(intery+qy);for(m=2;m<ps;++m)newpoints.push(points</a>);bottom=qy}j+=otherps}else{if(fromgap&amp;&amp;withlines){i+=ps;continue}<a
href="i+m">for(m=0;m<ps;++m)newpoints.push(points</a>);<a
href="j-otherps">if(withlines&&j>0&&otherpoints</a>!=<a
href="j-otherps+accumulateOffset">null)bottom=qy+(otherpoints</a>-<a
href="j-otherps+keyOffset">qy)*(px-qx)/(otherpoints</a>-<a
href="l+accumulateOffset">qx);newpoints</a>+=bottom;i+=ps}<a
href="l+2">fromgap=false;if(l!=newpoints.length&&withbottom)newpoints</a>+=bottom}<a
href="l">if(withsteps&&l!=newpoints.length&&l>0&&newpoints</a>!=<a
href="l">null&&newpoints</a>!=<a href="l-ps">newpoints</a>&amp;&amp;<a
href="l+1">newpoints</a>!=<a href="l-ps+1">newpoints</a>){<a
href="l+ps+m">for(m=0;m<ps;++m)newpoints</a>=<a href="l+m">newpoints</a>;<a
href="l+1">newpoints</a>=<a
href="l-ps+1">newpoints</a>}}datapoints.points=newpoints}plot.hooks.processDatapoints.push(stackData)}$.plot.plugins.push({init:init,options:options,name:“stack”,version:“1.2”})})(jQuery);</p>
</main>
