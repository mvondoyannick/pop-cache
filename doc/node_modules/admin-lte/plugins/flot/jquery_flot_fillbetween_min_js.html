...
<title>jquery.flot.fillbetween.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.fillbetween.min.js">

<p>(function($){var options={series:{fillBetween:null}};function
init(plot){function findBottomSeries(s,allseries){var
i;for(i=0;i&lt;allseries.length;++i){<a
href="i">if(allseries</a>.id===s.fillBetween){return <a
href="i">allseries</a>}}if(typeof
s.fillBetween===“number”){if(s.fillBetween&lt;0||s.fillBetween&gt;=allseries.length){return
null}return <a href="s.fillBetween">allseries</a>}return null}function
computeFillBottoms(plot,s,datapoints){if(s.fillBetween==null){return}var
other=findBottomSeries(s,plot.getData());if(!other){return}var <a
href="2">ps=datapoints.pointsize,points=datapoints.points,otherps=other.datapoints.pointsize,otherpoints=other.datapoints.points,newpoints=[],px,py,intery,qx,qy,bottom,withlines=s.lines.show,withbottom=ps>2&&datapoints.format</a>.y,withsteps=withlines&amp;&amp;s.lines.steps,fromgap=true,i=0,j=0,l,m;while(true){if(i&gt;=points.length){break}<a
href="i">l=newpoints.length;if(points</a>==null){for(m=0;m&lt;ps;++m){<a
href="i+m">newpoints.push(points</a>)}i+=ps}else
if(j&gt;=otherpoints.length){if(!withlines){for(m=0;m&lt;ps;++m){<a
href="i+m">newpoints.push(points</a>)}}i+=ps}else <a
href="j">if(otherpoints</a>==null){for(m=0;m&lt;ps;++m){newpoints.push(null)}fromgap=true;j+=otherps}else{<a
href="i">px=points</a>;<a href="i+1">py=points</a>;<a
href="j">qx=otherpoints</a>;<a
href="j+1">qy=otherpoints</a>;bottom=0;if(px===qx){for(m=0;m&lt;ps;++m){<a
href="i+m">newpoints.push(points</a>)}bottom=qy;i+=ps;j+=otherps}else
if(px&gt;qx){<a href="i-ps">if(withlines&&i>0&&points</a>!=null){<a
href="i-ps+1">intery=py+(points</a>-<a
href="i-ps">py)*(qx-px)/(points</a>-px);newpoints.push(qx);newpoints.push(intery);for(m=2;m&lt;ps;++m){<a
href="i+m">newpoints.push(points</a>)}bottom=qy}j+=otherps}else{if(fromgap&amp;&amp;withlines){i+=ps;continue}for(m=0;m&lt;ps;++m){<a
href="i+m">newpoints.push(points</a>)}<a
href="j-otherps">if(withlines&&j>0&&otherpoints</a>!=null){<a
href="j-otherps+1">bottom=qy+(otherpoints</a>-<a
href="j-otherps">qy)*(px-qx)/(otherpoints</a>-qx)}i+=ps}fromgap=false;if(l!==newpoints.length&amp;&amp;withbottom){<a
href="l+2">newpoints</a>=bottom}}<a
...
href="l-ps+1">newpoints</a>){for(m=0;m&lt;ps;++m){<a
href="l+ps+m">newpoints</a>=<a href="l+m">newpoints</a>}<a
href="l+1">newpoints</a>=<a
href="l-ps+1">newpoints</a>}}datapoints.points=newpoints}plot.hooks.processDatapoints.push(computeFillBottoms)}$.plot.plugins.push({init:init,options:options,name:“fillbetween”,version:“1.0”})})(jQuery);</p>
</main>
