...
<title>jquery.flot.errorbars.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.errorbars.min.js">

<p>(function($){var
options={series:{points:{errorbars:null,xerr:{err:“x”,show:null,asymmetric:null,upperCap:null,lowerCap:null,color:null,radius:null},yerr:{err:“y”,show:null,asymmetric:null,upperCap:null,lowerCap:null,color:null,radius:null}}}};function
processRawData(plot,series,data,datapoints){if(!series.points.errorbars)return;var
<a
href="{x:true,number:true,required:true},{y:true,number:true,required:true}">format=</a>;var
errors=series.points.errorbars;if(errors==“x”||errors==“xy”){if(series.points.xerr.asymmetric){format.push({x:true,number:true,required:true});format.push({x:true,number:true,required:true})}else
format.push({x:true,number:true,required:true})}if(errors==“y”||errors==“xy”){if(series.points.yerr.asymmetric){format.push({y:true,number:true,required:true});format.push({y:true,number:true,required:true})}else
format.push({y:true,number:true,required:true})}datapoints.format=format}function
parseErrors(series,i){var points=series.datapoints.points;var
exl=null,exu=null,eyl=null,eyu=null;var
xerr=series.points.xerr,yerr=series.points.yerr;var
eb=series.points.errorbars;if(eb==“x”||eb==“xy”){if(xerr.asymmetric){<a
href="i+2">exl=points</a>;<a
href="i+3">exu=points</a>;if(eb==“xy”)if(yerr.asymmetric){<a
href="i+4">eyl=points</a>;<a href="i+5">eyu=points</a>}else <a
href="i+4">eyl=points</a>}else{<a
href="i+2">exl=points</a>;if(eb==“xy”)if(yerr.asymmetric){<a
href="i+3">eyl=points</a>;<a href="i+4">eyu=points</a>}else <a
href="i+3">eyl=points</a>}}else if(eb==“y”)if(yerr.asymmetric){<a
href="i+2">eyl=points</a>;<a href="i+3">eyu=points</a>}else <a
href="i+2">eyl=points</a>;if(exu==null)exu=exl;if(eyu==null)eyu=eyl;var <a
href="exl,exu,eyl,eyu">errRanges=</a>;if(!xerr.show){<a
href="0">errRanges</a>=<a
href="1">null;errRanges</a>=null}if(!yerr.show){<a
href="2">errRanges</a>=<a href="3">null;errRanges</a>=null}return
errRanges}function drawSeriesErrors(plot,ctx,s){var <a
href="s.xaxis,s.yaxis">points=s.datapoints.points,ps=s.datapoints.pointsize,ax=</a>,<a
href="s.points.xerr,s.points.yerr">radius=s.points.radius,err=</a>;var <a
href="0">invertX=false;if(ax</a>.<a href="0">p2c(ax</a>.<a
href="0">max)<ax</a>.<a href="0">p2c(ax</a>.min)){invertX=true;var <a
href="0">tmp=err</a>.<a href="0">lowerCap;err</a>.<a
href="0">lowerCap=err</a>.<a href="0">upperCap;err</a>.upperCap=tmp}var <a
href="1">invertY=false;if(ax</a>.<a href="1">p2c(ax</a>.<a
href="1">min)<ax</a>.<a href="1">p2c(ax</a>.max)){invertY=true;var <a
href="1">tmp=err</a>.<a href="1">lowerCap;err</a>.<a
href="1">lowerCap=err</a>.<a href="1">upperCap;err</a>.upperCap=tmp}for(var
i=0;i&lt;s.datapoints.points.length;i+=ps){var
errRanges=parseErrors(s,i);for(var e=0;e&lt;err.length;e++){var <a
href="e">minmax=[ax</a>.<a href="e">min,ax</a>.<a
href="e*err.length">max];if(errRanges</a>){var <a href="i">x=points</a>,<a
href="i+1">y=points</a>;var <a href="x,y">upper=</a>[<a
href="e*err.length+1">e]+errRanges</a>,<a href="x,y">lower=</a>[<a
href="e*err.length">e]-errRanges</a>;<a href="e">if(err</a>.<a
href="1">err==“x”)if(y>ax</a>.<a href="1">max||y<ax</a>.<a
href="0">min||upper<ax</a>.<a href="0">min||lower>ax</a>.<a
href="e">max)continue;if(err</a>.<a href="0">err==“y”)if(x>ax</a>.<a
href="0">max||x<ax</a>.<a href="1">min||upper<ax</a>.<a
href="1">min||lower>ax</a>.max)continue;var <a
href="1">drawUpper=true,drawLower=true;if(upper>minmax</a>){<a
href="1">drawUpper=false;upper=minmax</a>}<a
href="0">if(lower<minmax</a>){<a
href="0">drawLower=false;lower=minmax</a>}<a href="e">if(err</a>.<a
href="e">err==“x”&&invertX||err</a>.err==“y”&amp;&amp;invertY){var <a
href="0">tmp=lower;lower=upper;upper=tmp;tmp=drawLower;drawLower=drawUpper;drawUpper=tmp;tmp=minmax</a>;<a
href="0">minmax</a>=<a href="1">minmax</a>;<a href="1">minmax</a>=tmp}<a
href="0">x=ax</a>.<a href="1">p2c(x),y=ax</a>.<a
href="e">p2c(y),upper=ax</a>.<a href="e">p2c(upper);lower=ax</a>.<a
href="0">p2c(lower);minmax</a>=<a href="e">ax</a>.<a
href="0">p2c(minmax</a>);<a href="1">minmax</a>=<a href="e">ax</a>.<a
href="1">p2c(minmax</a>);var <a href="e">lw=err</a>.<a
href="e">lineWidth?err</a>.lineWidth:s.points.lineWidth,sw=s.points.shadowSize!=null?s.points.shadowSize:s.shadowSize;if(lw&gt;0&amp;&amp;sw&gt;0){var
<a
href="e">w=sw/2;ctx.lineWidth=w;ctx.strokeStyle=“rgba(0,0,0,0.1)”;drawError(ctx,err</a>,<a
href="e">x,y,upper,lower,drawUpper,drawLower,radius,w+w/2,minmax);ctx.strokeStyle=“rgba(0,0,0,0.2)”;drawError(ctx,err</a>,x,y,upper,lower,drawUpper,drawLower,radius,w/2,minmax)}<a
href="e">ctx.strokeStyle=err</a>.<a href="e">color?err</a>.<a
href="e">color:s.color;ctx.lineWidth=lw;drawError(ctx,err</a>,x,y,upper,lower,drawUpper,drawLower,radius,0,minmax)}}}}function
drawError(ctx,err,x,y,upper,lower,drawUpper,drawLower,radius,offset,minmax){y+=offset;upper+=offset;lower+=offset;if(err.err==“x”){<a
href="upper,y">if(upper>x+radius)drawPath(ctx,[</a>,[<a
href="0">Math.max(x+radius,minmax</a>),y]]);else <a
href="1">drawUpper=false;if(lower<x-radius)drawPath(ctx,[[Math.min(x-radius,minmax</a>),<a
href="lower,y">y],</a>]);else drawLower=false}else{<a
href="x,upper">if(upper<y-radius)drawPath(ctx,[</a>,[<a
href="0">x,Math.min(y-radius,minmax</a>)]]);else <a
href="1">drawUpper=false;if(lower>y+radius)drawPath(ctx,[[x,Math.max(y+radius,minmax</a>)],[x,lower]]);else
drawLower=false}radius=err.radius!=null?err.radius:radius;if(drawUpper){if(err.upperCap==“-”){<a
href="upper,y-radius">if(err.err==“x”)drawPath(ctx,[</a>,[upper,y+radius]]);else
<a href="x-radius,upper">drawPath(ctx,[</a>,[x+radius,upper]])}else
if($.isFunction(err.upperCap)){if(err.err==“x”)err.upperCap(ctx,upper,y,radius);else
err.upperCap(ctx,x,upper,radius)}}if(drawLower){if(err.lowerCap==“-”){<a
href="lower,y-radius">if(err.err==“x”)drawPath(ctx,[</a>,[lower,y+radius]]);else
<a href="x-radius,lower">drawPath(ctx,[</a>,[x+radius,lower]])}else
if($.isFunction(err.lowerCap)){if(err.err==“x”)err.lowerCap(ctx,lower,y,radius);else
err.lowerCap(ctx,x,lower,radius)}}}function drawPath(ctx,pts){<a
href="0">ctx.beginPath();ctx.moveTo(pts</a>[<a
href="0">0],pts</a>[1]);for(var <a
href="p">p=1;p<pts.length;p++)ctx.lineTo(pts</a>[<a
href="p">0],pts</a>[1]);ctx.stroke()}function draw(plot,ctx){var
plotOffset=plot.getPlotOffset();ctx.save();ctx.translate(plotOffset.left,plotOffset.top);$.each(plot.getData(),function(i,s){if(s.points.errorbars&amp;&amp;(s.points.xerr.show||s.points.yerr.show))drawSeriesErrors(plot,ctx,s)});ctx.restore()}function
init(plot){plot.hooks.processRawData.push(processRawData);plot.hooks.draw.push(draw)}$.plot.plugins.push({init:init,options:options,name:“errorbars”,version:“1.0”})})(jQuery);</p>
</main>
