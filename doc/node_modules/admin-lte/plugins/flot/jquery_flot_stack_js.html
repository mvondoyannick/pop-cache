...
<title>jquery.flot.stack.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.stack.js">

<pre>Flot plugin for stacking data sets rather than overlyaing them.</pre>

<p>Copyright © 2007-2013 IOLA and Ole Laursen. Licensed under the MIT license.</p>

<p>The plugin assumes the data is sorted on x (or y if stacking horizontally).
For line charts, it is assumed that if a line has an undefined gap (from a
null point), then the line above it should have the same gap - insert zeros
instead of “null” if you want another behaviour. This also holds for the
start and end of the chart. Note that stacking a mix of positive and
negative values in most instances doesn&#39;t make sense (so it looks
weird).</p>

<p>Two or more series are stacked when their “stack” attribute is set to the
same key (which can be any number or string or just “true”). To specify the
default stack, you can set the stack option like this:</p>

<pre>series: {
        stack: null/false, true, or a key (number/string)
}</pre>

<p>You can also specify it for a single series, like this:</p>
...
<p>The stacking order is determined by the order of the data series in the
array (later series end up on top of the previous).</p>

<p>Internally, the plugin modifies the datapoints in each series, adding an
offset to the y value. For line series, extra data points are inserted
through interpolation. If there&#39;s a second y value, it&#39;s also
adjusted (e.g for bar charts or filled areas).</p>
