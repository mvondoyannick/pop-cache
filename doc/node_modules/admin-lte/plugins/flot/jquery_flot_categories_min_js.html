...
<title>jquery.flot.categories.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.categories.min.js">
...
<a href="format.length-1">format</a>.<a
href="format.length-1">y;format</a>.x=true}}datapoints.format=format}for(var
m=0;m&lt;format.length;++m){<a href="m">if(format</a>.<a
href="m">x&&xCategories)format</a>.<a
href="m">number=false;if(format</a>.<a
href="m">y&&yCategories)format</a>.number=false}}function
getNextIndex(categories){var index=-1;for(var v in <a
href="v">categories)if(categories</a>&gt;<a
href="v">index)index=categories</a>;return index+1}function
categoriesTickGenerator(axis){var res=[];for(var label in
axis.categories){var <a href="label">v=axis.categories</a>;<a
href="v,label">if(v>=axis.min&&v<=axis.max)res.push(</a>)}res.sort(function(a,b){return
<a href="0">a</a>-<a href="0">b</a>});return res}function
setupCategoriesForAxis(series,axis,datapoints){<a
href="axis">if(series</a>.<a
href="axis">options.mode!=“categories”)return;if(!series</a>.categories){var
c={},<a
href="axis">o=series</a>.options.categories||{};if($.isArray(o)){for(var <a
href="i">i=0;i<o.length;++i)c[o</a>]=i}else{for(var v in <a
href="v">o)c</a>=<a href="v">o</a>}<a
href="axis">series</a>.categories=c}<a href="axis">if(!series</a>.<a
href="axis">options.ticks)series</a>.<a
href="axis">options.ticks=categoriesTickGenerator;transformPointsOnAxis(datapoints,axis,series</a>.categories)}function
...
i=0;i&lt;points.length;i+=ps){<a
href="i">if(points</a>==null)continue;for(var m=0;m&lt;ps;++m){var <a
href="i+m">val=points</a>;<a
href="m">if(val==null||!format</a>[formatColumn])continue;if(!(val in
categories)){<a href="val">categories</a>=index;++index}<a
href="i+m">points</a>=<a href="val">categories</a>}}}function
...
init(plot){plot.hooks.processRawData.push(processRawData);plot.hooks.processDatapoints.push(processDatapoints)}$.plot.plugins.push({init:init,options:options,name:“categories”,version:“1.0”})})(jQuery);</p>
</main>
