...
<title>jquery.flot.selection.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.selection.min.js">

<p>(function($){function init(plot){var
selection={first:{x:-1,y:-1},second:{x:-1,y:-1},show:false,active:false};var
savedhandlers={};var mouseUpHandler=null;function
onMouseMove(e){if(selection.active){<a
href="getSelection()">updateSelection(e);plot.getPlaceholder().trigger(“plotselecting”,</a>)}}function
...
onMouseUp(e){mouseUpHandler=null;if(document.onselectstart!==undefined)document.onselectstart=savedhandlers.onselectstart;if(document.ondrag!==undefined)document.ondrag=savedhandlers.ondrag;selection.active=false;updateSelection(e);if(selectionIsSane())triggerSelectedEvent();else{<a
href="null">plot.getPlaceholder().trigger(“plotunselected”,[]);plot.getPlaceholder().trigger(“plotselecting”,</a>)}return
...
<a href="axis.direction">p1=axis.c2p(c1</a>),<a
href="axis.direction">p2=axis.c2p(c2</a>);<a
href="name">r</a>={from:Math.min(p1,p2),to:Math.max(p1,p2)}}});return
r}function triggerSelectedEvent(){var <a
href="r">r=getSelection();plot.getPlaceholder().trigger(“plotselected”,</a>);<a
href="{x1:r.xaxis.from,y1:r.yaxis.from,x2:r.xaxis.to,y2:r.yaxis.to}">if(r.xaxis&&r.yaxis)plot.getPlaceholder().trigger(“selected”,</a>)}function
clamp(min,value,max){return
value&lt;min?min:value&gt;max?max:value}function setSelectionPos(pos,e){var
o=plot.getOptions();var offset=plot.getPlaceholder().offset();var
plotOffset=plot.getPlotOffset();pos.x=clamp(0,e.pageX-offset.left-plotOffset.left,plot.width());pos.y=clamp(0,e.pageY-offset.top-plotOffset.top,plot.height());if(o.selection.mode==“y”)pos.x=pos==selection.first?0:plot.width();if(o.selection.mode==“x”)pos.y=pos==selection.first?0:plot.height()}function
...
extractRange(ranges,coord){var axis,from,to,key,axes=plot.getAxes();for(var
k in axes){<a href="k">axis=axes</a>;if(axis.direction==coord){<a
href="key">key=coord+axis.n+“axis”;if(!ranges</a>&amp;&amp;<a
href="key">axis.n==1)key=coord+“axis”;if(ranges</a>){<a
href="key">from=ranges</a>.<a href="key">from;to=ranges</a>.to;break}}}<a
href="key">if(!ranges</a>){<a
href="0">axis=coord==“x”?plot.getXAxes()</a>:<a
href="0">plot.getYAxes()</a>;<a href="coord+"1"">from=ranges</a>;<a
href="coord+"2"">to=ranges</a>}if(from!=null&amp;&amp;to!=null&amp;&amp;from&gt;to){var
...
Math.abs(selection.second.x-selection.first.x)&gt;=minSize&amp;&amp;Math.abs(selection.second.y-selection.first.y)&gt;=minSize}plot.clearSelection=clearSelection;plot.setSelection=setSelection;plot.getSelection=getSelection;plot.hooks.bindEvents.push(function(plot,eventHolder){var
...
x=Math.min(selection.first.x,selection.second.x)+.5,y=Math.min(selection.first.y,selection.second.y)+.5,w=Math.abs(selection.second.x-selection.first.x)-1,h=Math.abs(selection.second.y-selection.first.y)-1;ctx.fillRect(x,y,w,h);ctx.strokeRect(x,y,w,h);ctx.restore()}});plot.hooks.shutdown.push(function(plot,eventHolder){eventHolder.unbind(“mousemove”,onMouseMove);eventHolder.unbind(“mousedown”,onMouseDown);if(mouseUpHandler)$(document).unbind(“mouseup”,mouseUpHandler)})}$.plot.plugins.push({init:init,options:{selection:{mode:null,color:“#e8cfac”,shape:“round”,minSize:5}},name:“selection”,version:“1.1”})})(jQuery);</p>
</main>
