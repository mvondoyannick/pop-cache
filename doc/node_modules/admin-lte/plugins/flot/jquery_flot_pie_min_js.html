...
<title>jquery.flot.pie.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.pie.min.js">

<p>(function($){var REDRAW_ATTEMPTS=10;var REDRAW_SHRINK=.95;function
init(plot){var
...
options=plot.getOptions();if(options.series.pie.show){if(options.grid.hoverable){eventHolder.unbind(“mousemove”).mousemove(onMouseMove)}if(options.grid.clickable){eventHolder.unbind(“click”).click(onClick)}}});plot.hooks.processDatapoints.push(function(plot,series,data,datapoints){var
...
i=0;i&lt;data.length;++i){var <a
href="i">value=data</a>.data;if($.isArray(value)&amp;&amp;value.length==1){<a
href="0">value=value</a>}if($.isArray(value)){<a
href="1">if(!isNaN(parseFloat(value</a>))&amp;&amp;<a
href="1">isFinite(value</a>)){<a href="1">value</a>=+<a
href="1">value</a>}else{<a href="1">value</a>=0}}else
if(!isNaN(parseFloat(value))&amp;&amp;isFinite(value)){<a
href="1,+value">value=</a>}else{<a href="1,0">value=</a>}<a
href="i">data</a>.<a href="value">data=</a>}for(var
i=0;i&lt;data.length;++i){<a href="i">total+=data</a>.<a
href="0">data</a>[1]}for(var i=0;i&lt;data.length;++i){var <a
href="i">value=data</a>.<a
href="0">data</a>[1];if(value/total&lt;=options.series.pie.combine.threshold){combined+=value;numCombined++;if(!color){<a
href="i">color=data</a>.color}}}for(var i=0;i&lt;data.length;++i){var <a
href="i">value=data</a>.<a
href="0">data</a>[1];if(numCombined&lt;2||value/total&gt;options.series.pie.combine.threshold){newdata.push({<a
href="1,value">data:[</a>],<a href="i">color:data</a>.<a
href="i">color,label:data</a>.label,angle:value*Math.PI*2/total,percent:value/(total/100)})}}if(numCombined&gt;1){newdata.push({<a
href="1,combined">data:[</a>],color:color,label:options.series.pie.combine.label,angle:combined*Math.PI*2/total,percent:combined/(total/100)})}return
newdata}function draw(plot,newCtx){if(!target){return}var
canvasWidth=plot.getPlaceholder().width(),canvasHeight=plot.getPlaceholder().height(),legendWidth=target.children().filter(“.legend”).children().width()||0;ctx=newCtx;processed=false;maxRadius=Math.min(canvasWidth,canvasHeight/options.series.pie.tilt)/2;centerTop=canvasHeight/2+options.series.pie.offset.top;centerLeft=canvasWidth/2;if(options.series.pie.offset.left==“auto”){if(options.legend.position.match(“w”)){centerLeft+=legendWidth/2}else{centerLeft-=legendWidth/2}if(centerLeft&lt;maxRadius){centerLeft=maxRadius}else
...
currentAngle=startAngle;for(var i=0;i&lt;slices.length;++i){<a
href="i">slices</a>.<a
href="i">startAngle=currentAngle;drawSlice(slices</a>.<a
href="i">angle,slices</a>.color,true)}ctx.restore();if(options.series.pie.stroke.width&gt;0){ctx.save();ctx.lineWidth=options.series.pie.stroke.width;currentAngle=startAngle;for(var
i=0;i&lt;slices.length;++i){<a
href="i">drawSlice(slices</a>.angle,options.series.pie.stroke.color,false)}ctx.restore()}drawDonutHole(ctx);ctx.restore();if(options.series.pie.label.show){return
...
i=0;i&lt;slices.length;++i){<a
href="i">if(slices</a>.percent&gt;=options.series.pie.label.threshold*100){<a
href="i">if(!drawLabel(slices</a>,currentAngle,i)){return false}}<a
href="i">currentAngle+=slices</a>.angle}return true;function
drawLabel(slice,startAngle,index){<a
href="0">if(slice.data</a>[1]==0){return true}var
...
html=“&lt;span class=&#39;pieLabel&#39;
id=&#39;pieLabel”<code>index</code>“&#39;
style=&#39;position:absolute;top:”<code>y</code>“px;left:”<code>x</code>“px;&#39;&gt;”<code>text</code>“&lt;/span&gt;”;target.append(html);var
label=target.children(“#pieLabel”+index);var
labelTop=y-label.height()/2;var
labelLeft=x-label.width()/2;label.css(“top”,labelTop);label.css(“left”,labelLeft);if(0-labelTop&gt;0||0-labelLeft&gt;0||canvasHeight-(labelTop+label.height())&lt;0||canvasWidth-(labelLeft+label.width())&lt;0){return
...
pos=“top:”<code>labelTop</code>“px;left:”<code>labelLeft</code>“px;”;$(“&lt;div
class=&#39;pieLabelBackground&#39;
style=&#39;position:absolute;width:”+label.width()+“px;height:”+label.height()+“px;”<code>pos</code>“background-color:”<code>c</code>“;&#39;&gt;&lt;/div&gt;”).css(“opacity”,options.series.pie.label.background.opacity).insertBefore(label)}return
true}}}}function
...
isPointInPoly(poly,pt){for(var <a
href="i">c=false,i=-1,l=poly.length,j=l-1;++i<l;j=i)(poly</a>[<a
href="1">1]<=pt</a>&amp;&amp;<a href="1">pt</a>&lt;<a href="j">poly</a>[<a
href="j">1]||poly</a>[<a href="1">1]<=pt</a>&amp;&amp;<a
href="1">pt</a>&lt;<a href="i">poly</a>[<a href="0">1])&&pt</a>&lt;(<a
href="j">poly</a>[<a href="i">0]-poly</a>[<a href="1">0])*(pt</a>-<a
href="i">poly</a>[<a href="j">1])/(poly</a>[<a href="i">1]-poly</a>[<a
href="i">1])+poly</a>[0]&amp;&amp;(c=!c);return c}function
findNearbySlice(mouseX,mouseY){var
...
i=0;i&lt;slices.length;++i){var <a
href="i">s=slices</a>;if(s.pie.show){ctx.save();ctx.beginPath();ctx.moveTo(0,0);ctx.arc(0,0,radius,s.startAngle,s.startAngle+s.angle/2,false);ctx.arc(0,0,radius,s.startAngle+s.angle/2,s.startAngle+s.angle,false);ctx.closePath();x=mouseX-centerLeft;y=mouseY-centerTop;if(ctx.isPointInPath){if(ctx.isPointInPath(mouseX-centerLeft,mouseY-centerTop)){ctx.restore();return{<a
href="s.percent,s.data">datapoint:</a>,dataIndex:0,series:s,seriesIndex:i}}}else{var
<a
href="0,0">p1X=radius*Math.cos(s.startAngle),p1Y=radius*Math.sin(s.startAngle),p2X=radius*Math.cos(s.startAngle+s.angle/4),p2Y=radius*Math.sin(s.startAngle+s.angle/4),p3X=radius*Math.cos(s.startAngle+s.angle/2),p3Y=radius*Math.sin(s.startAngle+s.angle/2),p4X=radius*Math.cos(s.startAngle+s.angle/1.5),p4Y=radius*Math.sin(s.startAngle+s.angle/1.5),p5X=radius*Math.cos(s.startAngle+s.angle),p5Y=radius*Math.sin(s.startAngle+s.angle),arrPoly=[</a>,[<a
href="p2X,p2Y">p1X,p1Y],</a>,[<a href="p4X,p4Y">p3X,p3Y],</a>,[<a
href="x,y">p5X,p5Y]],arrPoint=</a>;if(isPointInPoly(arrPoly,arrPoint)){ctx.restore();return{<a
href="s.percent,s.data">datapoint:</a>,dataIndex:0,series:s,seriesIndex:i}}}ctx.restore()}}return
...
i=0;i&lt;highlights.length;++i){var <a
href="i">h=highlights</a>;if(h.auto==eventname&amp;&amp;!(item&amp;&amp;h.series==item.series)){unhighlight(h.series)}}}if(item){highlight(item.series,eventname)}var
pos={pageX:e.pageX,pageY:e.pageY};<a
href="pos,item">target.trigger(eventname,</a>)}function
highlight(s,auto){var
i=indexOfHighlight(s);if(i==-1){highlights.push({series:s,auto:auto});plot.triggerRedrawOverlay()}else
if(!auto){<a href="i">highlights</a>.auto=false}}function
...
indexOfHighlight(s){for(var i=0;i&lt;highlights.length;++i){var <a
href="i">h=highlights</a>;if(h.series==s)return i}return-1}function
...
i=0;i&lt;highlights.length;++i){<a
href="i">drawHighlight(highlights</a>.series)}drawDonutHole(octx);octx.restore();function
drawHighlight(series){if(series.angle&lt;=0||isNaN(series.angle)){return}octx.fillStyle=“rgba(255,
255, 255,
”<code>options.series.pie.highlight.opacity</code>“)”;octx.beginPath();if(Math.abs(series.angle-Math.PI*2)&gt;1e-9){octx.moveTo(0,0)}octx.arc(0,0,radius,series.startAngle,series.startAngle+series.angle/2,false);octx.arc(0,0,radius,series.startAngle+series.angle/2,series.startAngle+series.angle,false);octx.closePath();octx.fill()}}}var
...
style=&#39;font-size:x-small;text-align:center;padding:2px;color:”<code>slice.color</code>“;&#39;&gt;”<code>label</code>“&lt;br/&gt;”+Math.round(slice.percent)+“%&lt;/div&gt;”},radius:1,background:{color:null,opacity:0},threshold:0},combine:{threshold:-1,color:null,label:“Other”},highlight:{opacity:.5}}}};$.plot.plugins.push({init:init,options:options,name:“pie”,version:“1.1”})})(jQuery);</p>
</main>
