...
<title>jquery.flot.canvas.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.canvas.js">

<pre>Flot plugin for drawing all elements of a plot on the canvas.</pre>

<p>Copyright © 2007-2013 IOLA and Ole Laursen. Licensed under the MIT license.</p>

<p>Flot normally produces certain elements, like axis labels and the legend,
using HTML elements. This permits greater interactivity and customization,
and often looks better, due to cross-browser canvas text inconsistencies
and limitations.</p>

<p>It can also be desirable to render the plot entirely in canvas,
particularly if the goal is to save it as an image, or if Flot is being
used in a context where the HTML DOM does not exist, as is the case within
Node.js. This plugin switches out Flot&#39;s standard drawing operations
for canvas-only replacements.</p>

<p>Currently the plugin supports only axis labels, but it will eventually
allow every element of the plot to be rendered directly to canvas.</p>

<p>The plugin supports these options:</p>

<p>{</p>

<pre>canvas: boolean</pre>

<p>}</p>

<p>The “canvas” option controls whether full canvas drawing is enabled, making
it possible to toggle on and off. This is useful when a plot uses HTML text
in the browser, but needs to redraw with canvas text when exporting as an
image.</p>
