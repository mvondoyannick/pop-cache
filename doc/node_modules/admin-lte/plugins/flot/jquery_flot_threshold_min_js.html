...
<title>jquery.flot.threshold.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.threshold.min.js">

<p>(function($){var options={series:{threshold:null}};function
init(plot){function thresholdData(plot,s,datapoints,below,color){var
ps=datapoints.pointsize,i,x,y,p,prevp,thresholded=$.extend({},s);thresholded.datapoints={<a
href="">points:[],pointsize:ps,format:datapoints.format};thresholded.label=null;thresholded.color=color;thresholded.threshold=null;thresholded.originSeries=s;thresholded.data=</a>;var
origpoints=datapoints.points,addCrossingPoints=s.lines.show;var
threspoints=[];var newpoints=[];var
m;for(i=0;i&lt;origpoints.length;i+=ps){<a href="i">x=origpoints</a>;<a
href="i+1">y=origpoints</a>;prevp=p;if(y&lt;below)p=threspoints;else <a
href="i-ps">p=newpoints;if(addCrossingPoints&&prevp!=p&&x!=null&&i>0&&origpoints</a>!=null){var
<a href="i-ps">interx=x+(below-y)*(x-origpoints</a>)/(<a
href="i-ps+1">y-origpoints</a>);<a
href="i+m">prevp.push(interx);prevp.push(below);for(m=2;m<ps;++m)prevp.push(origpoints</a>);<a
href="i+m">p.push(null);p.push(null);for(m=2;m<ps;++m)p.push(origpoints</a>);<a
href="i+m">p.push(interx);p.push(below);for(m=2;m<ps;++m)p.push(origpoints</a>)}<a
href="i+m">p.push(x);p.push(y);for(m=2;m<ps;++m)p.push(origpoints</a>)}datapoints.points=newpoints;thresholded.datapoints.points=threspoints;if(thresholded.datapoints.points.length&gt;0){var
...
a.below-b.below});$(s.threshold).each(function(i,th){thresholdData(plot,s,datapoints,th.below,th.color)})}else{thresholdData(plot,s,datapoints,s.threshold.below,s.threshold.color)}}plot.hooks.processDatapoints.push(processThresholds)}$.plot.plugins.push({init:init,options:options,name:“threshold”,version:“1.2”})})(jQuery);</p>
</main>
