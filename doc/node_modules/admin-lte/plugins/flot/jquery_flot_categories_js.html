...
<title>jquery.flot.categories.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.categories.js">

<pre>Flot plugin for plotting textual data or categories.</pre>

<p>Copyright © 2007-2013 IOLA and Ole Laursen. Licensed under the MIT license.</p>

<p>Consider a dataset like [[“February”, 34], [“March”, 20], …]. This plugin
allows you to plot such a dataset directly.</p>

<p>To enable it, you must specify mode: “categories” on the axis with the
textual labels, e.g.</p>

<pre>$.plot(&quot;#placeholder&quot;, data, { xaxis: { mode: &quot;categories&quot; } });</pre>

<p>By default, the labels are ordered as they are met in the data series. If
you need a different ordering, you can specify “categories” on the axis
options and list the categories there:</p>
...
<p>If you need to customize the distances between the categories, you can
specify “categories” as an object mapping labels to values</p>
...
<p>If you don&#39;t specify all categories, the remaining categories will be
numbered from the max value plus 1 (with a spacing of 1 between each).</p>

<p>Internally, the plugin works by transforming the input data through an
auto- generated mapping where the first category becomes 0, the second 1,
etc. Hence, a point like [“February”, 34] becomes [0, 34] internally in
Flot (this is visible in hover and click events that return numbers rather
than the category labels). The plugin also overrides the tick generator to
spit out the categories as ticks instead of the values.</p>

<p>If you need to map a value back to its label, the mapping is always
accessible as “categories” on the axis object, e.g.
plot.getAxes().xaxis.categories.</p>
