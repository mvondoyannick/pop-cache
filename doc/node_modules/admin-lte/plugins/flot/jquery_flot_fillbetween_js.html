...
<title>jquery.flot.fillbetween.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.fillbetween.js">

<pre>Flot plugin for computing bottoms for filled line and bar charts.</pre>

<p>Copyright © 2007-2013 IOLA and Ole Laursen. Licensed under the MIT license.</p>

<p>The case: you&#39;ve got two series that you want to fill the area between.
In Flot terms, you need to use one as the fill bottom of the other. You can
specify the bottom of each data point as the third coordinate manually, or
you can use this plugin to compute it for you.</p>

<p>In order to name the other series, you need to give it an id, like this:</p>
...
$.plot($(&quot;#placeholder&quot;), dataset, { lines: { show: true, fill: true }});</pre>

<p>As a convenience, if the id given is a number that doesn&#39;t appear as an
id in the series, it is interpreted as the index in the array instead (so
fillBetween: 0 can also mean the first series).</p>

<p>Internally, the plugin modifies the datapoints in each series. For line
series, extra data points might be inserted through interpolation. Note
that at points where the bottom line is not defined (due to a null point or
start/end of line), the current line will show a gap too. The algorithm
comes from the jquery.flot.stack.js plugin, possibly some code could be
shared.</p>
