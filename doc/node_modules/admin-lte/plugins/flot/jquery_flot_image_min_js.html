...
<title>jquery.flot.image.min.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.image.min.js">

<p>(function($){var
options={series:{images:{show:false,alpha:1,anchor:“corner”}}};$.plot.image={};$.plot.image.loadDataImages=function(series,options,callback){var
<a href="">urls=[],points=</a>;var
defaultShow=options.series.images.show;$.each(series,function(i,s){if(!(defaultShow||s.images.show))return;if(s.data)s=s.data;$.each(s,function(i,p){if(typeof
<a href="0">p</a>==“string”){<a
href="0">urls.push(p</a>);points.push(p)}})});$.plot.image.load(urls,function(loadedImages){$.each(points,function(i,p){var
<a href="0">url=p</a>;<a href="url">if(loadedImages</a>)<a
href="0">p</a>=<a
href="url">loadedImages</a>});callback()})};$.plot.image.load=function(urls,callback){var
missing=urls.length,loaded={};if(missing==0)callback({});$.each(urls,function(i,url){var
handler=function(){–<a
href="url">missing;loaded</a>=this;if(missing==0)callback(loaded)};$(“&lt;img
/&gt;”).load(handler).error(handler).attr(“src”,url)})};function
drawSeries(plot,ctx,series){var
plotOffset=plot.getPlotOffset();if(!series.images||!series.images.show)return;var
points=series.datapoints.points,ps=series.datapoints.pointsize;for(var
i=0;i&lt;points.length;i+=ps){var <a href="i">img=points</a>,<a
href="i+1">x1=points</a>,<a href="i+2">y1=points</a>,<a
href="i+3">x2=points</a>,<a
href="i+4">y2=points</a>,xaxis=series.xaxis,yaxis=series.yaxis,tmp;if(!img||img.width&lt;=0||img.height&lt;=0)continue;if(x1&gt;x2){tmp=x2;x2=x1;x1=tmp}if(y1&gt;y2){tmp=y2;y2=y1;y1=tmp}if(series.images.anchor==“center”){tmp=.5*(x2-x1)/(img.width-1);x1-=tmp;x2+=tmp;tmp=.5*(y2-y1)/(img.height-1);y1-=tmp;y2+=tmp}if(x1==x2||y1==y2||x1&gt;=xaxis.max||x2&lt;=xaxis.min||y1&gt;=yaxis.max||y2&lt;=yaxis.min)continue;var
sx1=0,sy1=0,sx2=img.width,sy2=img.height;if(x1&lt;xaxis.min){sx1+=(sx2-sx1)*(xaxis.min-x1)/(x2-x1);x1=xaxis.min}if(x2&gt;xaxis.max){sx2+=(sx2-sx1)*(xaxis.max-x2)/(x2-x1);x2=xaxis.max}if(y1&lt;yaxis.min){sy2+=(sy1-sy2)*(yaxis.min-y1)/(y2-y1);y1=yaxis.min}if(y2&gt;yaxis.max){sy1+=(sy1-sy2)*(yaxis.max-y2)/(y2-y1);y2=yaxis.max}x1=xaxis.p2c(x1);x2=xaxis.p2c(x2);y1=yaxis.p2c(y1);y2=yaxis.p2c(y2);if(x1&gt;x2){tmp=x2;x2=x1;x1=tmp}if(y1&gt;y2){tmp=y2;y2=y1;y1=tmp}tmp=ctx.globalAlpha;ctx.globalAlpha*=series.images.alpha;ctx.drawImage(img,sx1,sy1,sx2-sx1,sy2-sy1,x1+plotOffset.left,y1+plotOffset.top,x2-x1,y2-y1);ctx.globalAlpha=tmp}}function
processRawData(plot,series,data,datapoints){<a
href="{required:true},{x:true,number:true,required:true},{y:true,number:true,required:true},{x:true,number:true,required:true},{y:true,number:true,required:true}">if(!series.images.show)return;datapoints.format=</a>}function
init(plot){plot.hooks.processRawData.push(processRawData);plot.hooks.drawSeries.push(drawSeries)}$.plot.plugins.push({init:init,options:options,name:“image”,version:“1.1”})})(jQuery);</p>
</main>
