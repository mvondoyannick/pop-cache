...
<title>jquery.flot.image.js - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/flot/jquery.flot.image.js">

<pre>Flot plugin for plotting images.</pre>

<p>Copyright © 2007-2013 IOLA and Ole Laursen. Licensed under the MIT license.</p>

<p>The data syntax is [ [ image, x1, y1, x2, y2 ], … ] where (x1, y1) and (x2,
y2) are where you intend the two opposite corners of the image to end up in
the plot. Image must be a fully loaded Javascript image (you can make one
with new Image()). If the image is not complete, it&#39;s skipped when
plotting.</p>

<p>There are two helpers included for retrieving images. The easiest work the
way that you put in URLs instead of images in the data, like this:</p>

<pre class="ruby">[ <span class="ruby-string">&quot;myimage.png&quot;</span>, <span class="ruby-value">0</span>, <span class="ruby-value">0</span>, <span class="ruby-value">10</span>, <span class="ruby-value">10</span> ]
</pre>

<p>Then call $.plot.image.loadData( data, options, callback ) where data and
options are the same as you pass in to $.plot. This loads the images,
replaces the URLs in the data with the corresponding images and calls
“callback” when all images are loaded (or failed loading). In the callback,
you can then call $.plot with the data set. See the included example.</p>

<p>A more low-level helper, $.plot.image.load(urls, callback) is also
included. Given a list of URLs, it calls callback with an object mapping
from URL to Image object when all images are loaded or have failed loading.</p>
...
<p>They can be specified for a specific series:</p>

<pre>$.plot( $(&quot;#placeholder&quot;), [{
        data: [ ... ],
        images: { ... }
])</pre>

<p>Note that because the data format is different from usual data points, you
can&#39;t use images with anything else in a specific data series.</p>

<p>Setting “anchor” to “center” causes the pixels in the image to be anchored
at the corner pixel centers inside of at the pixel corners, effectively
letting half a pixel stick out to each side in the plot.</p>

<p>A possible future direction could be support for tiling for large images
(like Google Maps).</p>
