...
<title>bootstrap.bundle.js.map - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/admin-lte/plugins/bootstrap/js/bootstrap.bundle.js.map">

<p><a
href="data-target=\"${target}\"">“version”:3,“file”:“bootstrap.bundle.js”,“sources”:[“../../rollupPluginBabelHelpers”,“../../js/src/util.js”,“../../js/src/alert.js”,“../../js/src/button.js”,“../../js/src/carousel.js”,“../../js/src/collapse.js”,“../../node_modules/popper.js/dist/esm/popper.js”,“../../js/src/dropdown.js”,“../../js/src/modal.js”,“../../js/src/tooltip.js”,“../../js/src/popover.js”,“../../js/src/scrollspy.js”,“../../js/src/tab.js”,“../../js/src/index.js”],“sourcesContent”:[“export
{ _createClass as createClass, _defineProperty as defineProperty,
_objectSpread as objectSpread, _inheritsLoose as inheritsLoose };nnfunction
_defineProperties(target, props) {n  for (var i = 0; i < props.length; i++)
...
--------------------------------------------------------------------------\n
\n\nconst Util = (($) => {n     n   * ————————————————————————n   * Private
TransitionEnd Helpersn   * ————————————————————————n     nn  const
TRANSITION_END = ‘transitionend’n  const MAX_UID = 1000000n  const
MILLISECONDS_MULTIPLIER = 1000nn  // Shoutout AngusCroll
(https://goo.gl/pxwQGp)n  function toType(obj) {n    return
{}.toString.call(obj).match(/\s([a-z]+)/i)[1].toLowerCase()n  }nn  function
getSpecialTransitionEndEvent() {n    return {n      bindType:
TRANSITION_END,n      delegateType: TRANSITION_END,n      handle(event) {n 
if ($(event.target).is(this)) {n          return
event.handleObj.handler.apply(this, arguments) // eslint-disable-line
prefer-rest-paramsn        }n        return undefined //
eslint-disable-line no-undefinedn      }n    }n  }nn  function
transitionEndEmulator(duration) {n    let called = falsenn   
$(this).one(Util.TRANSITION_END, () => {n      called = truen    })nn   
setTimeout(() => {n      if (!called) {n       
Util.triggerTransitionEnd(this)n      }n    }, duration)nn    return thisn 
}nn  function setTransitionEndSupport() {n    $.fn.emulateTransitionEnd =
transitionEndEmulatorn    $.event.special[Util.TRANSITION_END] =
getSpecialTransitionEndEvent()n  }nn  /**n   * ————————————————————————–n  
* Public Util Apin   * ————————————————————————–n   */nn  const Util = {nn 
TRANSITION_END: ‘bsTransitionEnd’,nn    getUID(prefix) {n      do {n       
// eslint-disable-next-line no-bitwisen        prefix += ~~(Math.random() *
MAX_UID) // "~~" acts like a faster Math.floor() heren      } while
(document.getElementById(prefix))n      return prefixn    },nn   
getSelectorFromElement(element) {n      let selector =
element.getAttribute(‘data-target’)n      if (!selector || selector ===
‘#’) {n        selector = element.getAttribute(‘href’) || ”n      }nn     
try {n        const $selector = $(document).find(selector)n        return
$selector.length > 0 ? selector : nulln      } catch (err) {n        return
nulln      }n    },nn    getTransitionDurationFromElement(element) {n     
if (!element) {n        return 0n      }nn      // Get transition-duration
of the elementn      let transitionDuration =
$(element).css(‘transition-duration’)n      const floatTransitionDuration =
parseFloat(transitionDuration)nn      // Return 0 if element or transition
duration is not foundn      if (!floatTransitionDuration) {n        return
0n      }nn      // If multiple durations are defined, take the firstn     
transitionDuration = transitionDuration.split(‘,’)[0]nn      return
parseFloat(transitionDuration) * MILLISECONDS_MULTIPLIERn    },nn   
reflow(element) {n      return element.offsetHeightn    },nn   
triggerTransitionEnd(element) {n      $(element).trigger(TRANSITION_END)n  
},nn    // TODO: Remove in v5n    supportsTransitionEnd() {n      return
Boolean(TRANSITION_END)n    },nn    isElement(obj) {n      return (obj[0]
|| obj).nodeTypen    },nn    typeCheckConfig(componentName, config,
configTypes) {n      for (const property in configTypes) {n        if
(Object.prototype.hasOwnProperty.call(configTypes, property)) {n         
const expectedTypes = configTypes[property]n          const value         =
config[property]n          const valueType     = value &&
Util.isElement(value)n            ? ‘element’ : toType(value)nn          if
(!new RegExp(expectedTypes).test(valueType)) {n            throw new
Error(n              `${componentName.toUpperCase()}: ` +n             
`Option "${property}" provided type "${valueType}" ` +n              `but
expected type "${expectedTypes}".`)n          }n        }n      }n    }n 
}nn  setTransitionEndSupport()nn  return Utiln})($)nnexport default
Utiln”,“import $ from ‘jquery’nimport Util from ‘./util’nn/**n *
————————————————————————–n * Bootstrap (v4.1.0): alert.jsn * Licensed under
MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Alert = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME                = ‘alert’n  const VERSION             =
‘4.1.0’n  const DATA_KEY            = ‘bs.alert’n  const EVENT_KEY         
= `.${DATA_KEY}`n  const DATA_API_KEY        = ‘.data-api’n  const
JQUERY_NO_CONFLICT  = $.fn[NAME]nn  const Selector = {n    DISMISS :
‘[data-dismiss="alert"]’n  }nn  const Event = {n    CLOSE          :
`close${EVENT_KEY}`,n    CLOSED         : `closed${EVENT_KEY}`,n   
CLICK_DATA_API : `click${EVENT_KEY}${DATA_API_KEY}`n  }nn  const ClassName
= {n    ALERT : ‘alert’,n    FADE  : ‘fade’,n    SHOW  : ‘show’n  }nn  /**n
* ————————————————————————n   * Class Definitionn   *
————————————————————————n   */nn  class Alert {n    constructor(element) {n
this._element = elementn    }nn    // Gettersnn    static get VERSION() {n 
return VERSIONn    }nn    // Publicnn    close(element) {n      element =
element || this._elementnn      const rootElement =
this._getRootElement(element)n      const customEvent =
this._triggerCloseEvent(rootElement)nn      if
(customEvent.isDefaultPrevented()) {n        returnn      }nn     
this._removeElement(rootElement)n    }nn    dispose() {n     
$.removeData(this._element, DATA_KEY)n      this._element = nulln    }nn   
// Privatenn    _getRootElement(element) {n      const selector =
Util.getSelectorFromElement(element)n      let parent     = falsenn      if
(selector) {n        parent = $(selector)[0]n      }nn      if (!parent) {n
parent = $(element).closest(`.${ClassName.ALERT}`)[0]n      }nn      return
parentn    }nn    _triggerCloseEvent(element) {n      const closeEvent =
$.Event(Event.CLOSE)nn      $(element).trigger(closeEvent)n      return
closeEventn    }nn    _removeElement(element) {n     
$(element).removeClass(ClassName.SHOW)nn      if
(!$(element).hasClass(ClassName.FADE)) {n       
this._destroyElement(element)n        returnn      }nn      const
transitionDuration = Util.getTransitionDurationFromElement(element)nn     
$(element)n        .one(Util.TRANSITION_END, (event) =>
this._destroyElement(element, event))n       
.emulateTransitionEnd(transitionDuration)n    }nn   
_destroyElement(element) {n      $(element)n        .detach()n       
.trigger(Event.CLOSED)n        .remove()n    }nn    // Staticnn    static
_jQueryInterface(config) {n      return this.each(function () {n       
const $element = $(this)n        let data       = $element.data(DATA_KEY)nn
if (!data) {n          data = new Alert(this)n         
$element.data(DATA_KEY, data)n        }nn        if (config === ‘close’) {n
data[config](this)n        }n      })n    }nn    static
_handleDismiss(alertInstance) {n      return function (event) {n        if
(event) {n          event.preventDefault()n        }nn       
alertInstance.close(this)n      }n    }n  }nn  /**n   *
————————————————————————n   * Data Api implementationn   *
————————————————————————n   */nn  $(document).on(n   
Event.CLICK_DATA_API,n    Selector.DISMISS,n    Alert._handleDismiss(new
Alert())n  )nn  /**n   * ————————————————————————n   * jQueryn   *
————————————————————————n   */nn  $.fn[NAME]             =
Alert._jQueryInterfacen  $.fn[NAME].Constructor = Alertn 
$.fn[NAME].noConflict  = function () {n    $.fn[NAME] = JQUERY_NO_CONFLICTn
return Alert._jQueryInterfacen  }nn  return Alertn})($)nnexport default
Alertn”,“import $ from ‘jquery’nn/**n * ————————————————————————–n *
Bootstrap (v4.1.0): button.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Button = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME                = ‘button’n  const VERSION             =
‘4.1.0’n  const DATA_KEY            = ‘bs.button’n  const EVENT_KEY        
= `.${DATA_KEY}`n  const DATA_API_KEY        = ‘.data-api’n  const
JQUERY_NO_CONFLICT  = $.fn[NAME]nn  const ClassName = {n    ACTIVE :
‘active’,n    BUTTON : ‘btn’,n    FOCUS  : ‘focus’n  }nn  const Selector =
{n    DATA_TOGGLE_CARROT : ‘[data-toggle^="button"]’,n    DATA_TOGGLE      
: ‘[data-toggle="buttons"]’,n    INPUT              : ‘input’,n    ACTIVE  
: ‘.active’,n    BUTTON             : ‘.btn’n  }nn  const Event = {n   
CLICK_DATA_API      : `click${EVENT_KEY}${DATA_API_KEY}`,n   
FOCUS_BLUR_DATA_API : `focus${EVENT_KEY}${DATA_API_KEY} ` +n               
`blur${EVENT_KEY}${DATA_API_KEY}`n  }nn  /**n   * ————————————————————————n
* Class Definitionn   * ————————————————————————n   */nn  class Button {n  
constructor(element) {n      this._element = elementn    }nn    //
Gettersnn    static get VERSION() {n      return VERSIONn    }nn    //
Publicnn    toggle() {n      let triggerChangeEvent = truen      let
addAriaPressed = truen      const rootElement = $(this._element).closest(n 
Selector.DATA_TOGGLEn      )[0]nn      if (rootElement) {n        const
input = $(this._element).find(Selector.INPUT)[0]nn        if (input) {n    
if (input.type === ‘radio’) {n            if (input.checked &&n            
$(this._element).hasClass(ClassName.ACTIVE)) {n             
triggerChangeEvent = falsen            } else {n              const
activeElement = $(rootElement).find(Selector.ACTIVE)[0]nn              if
(activeElement) {n               
$(activeElement).removeClass(ClassName.ACTIVE)n              }n           
}n          }nn          if (triggerChangeEvent) {n            if
(input.hasAttribute(‘disabled’) ||n             
rootElement.hasAttribute(‘disabled’) ||n             
input.classList.contains(‘disabled’) ||n             
rootElement.classList.contains(‘disabled’)) {n              returnn        
}n            input.checked = !$(this._element).hasClass(ClassName.ACTIVE)n
$(input).trigger(‘change’)n          }nn          input.focus()n         
addAriaPressed = falsen        }n      }nn      if (addAriaPressed) {n     
this._element.setAttribute(‘aria-pressed’,n         
!$(this._element).hasClass(ClassName.ACTIVE))n      }nn      if
(triggerChangeEvent) {n       
$(this._element).toggleClass(ClassName.ACTIVE)n      }n    }nn    dispose()
{n      $.removeData(this._element, DATA_KEY)n      this._element = nulln  
}nn    // Staticnn    static _jQueryInterface(config) {n      return
this.each(function () {n        let data = $(this).data(DATA_KEY)nn       
if (!data) {n          data = new Button(this)n         
$(this).data(DATA_KEY, data)n        }nn        if (config === ‘toggle’) {n
data[config]()n        }n      })n    }n  }nn  /**n   *
————————————————————————n   * Data Api implementationn   *
————————————————————————n   */nn  $(document)n    .on(Event.CLICK_DATA_API,
Selector.DATA_TOGGLE_CARROT, (event) => {n      event.preventDefault()nn   
let button = event.targetnn      if (!$(button).hasClass(ClassName.BUTTON))
{n        button = $(button).closest(Selector.BUTTON)n      }nn     
Button._jQueryInterface.call($(button), ‘toggle’)n    })n   
.on(Event.FOCUS_BLUR_DATA_API, Selector.DATA_TOGGLE_CARROT, (event) => {n  
const button = $(event.target).closest(Selector.BUTTON)[0]n     
$(button).toggleClass(ClassName.FOCUS, /^focus(in)?$/.test(event.type))n   
})nn  /**n   * ————————————————————————n   * jQueryn   *
————————————————————————n   */nn  $.fn[NAME] = Button._jQueryInterfacen 
$.fn[NAME].Constructor = Buttonn  $.fn[NAME].noConflict = function () {n   
$.fn[NAME] = JQUERY_NO_CONFLICTn    return Button._jQueryInterfacen  }nn 
return Buttonn})($)nnexport default Buttonn”,“import $ from ‘jquery’nimport
Util from ‘./util’nn/**n * ————————————————————————–n * Bootstrap (v4.1.0):
carousel.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Carousel = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME                   = ‘carousel’n  const VERSION            
= ‘4.1.0’n  const DATA_KEY               = ‘bs.carousel’n  const EVENT_KEY 
= `.${DATA_KEY}`n  const DATA_API_KEY           = ‘.data-api’n  const
JQUERY_NO_CONFLICT     = $.fn[NAME]n  const ARROW_LEFT_KEYCODE     = 37 //
KeyboardEvent.which value for left arrow keyn  const ARROW_RIGHT_KEYCODE   
= 39 // KeyboardEvent.which value for right arrow keyn  const
TOUCHEVENT_COMPAT_WAIT = 500 // Time for mouse compat events to fire after
touchnn  const Default = {n    interval : 5000,n    keyboard : true,n   
slide    : false,n    pause    : ‘hover’,n    wrap     : truen  }nn  const
DefaultType = {n    interval : ‘(number|boolean)’,n    keyboard :
‘boolean’,n    slide    : ‘(boolean|string)’,n    pause    :
‘(string|boolean)’,n    wrap     : ‘boolean’n  }nn  const Direction = {n   
NEXT     : ‘next’,n    PREV     : ‘prev’,n    LEFT     : ‘left’,n    RIGHT 
: ‘right’n  }nn  const Event = {n    SLIDE          : `slide${EVENT_KEY}`,n
SLID           : `slid${EVENT_KEY}`,n    KEYDOWN        :
`keydown${EVENT_KEY}`,n    MOUSEENTER     : `mouseenter${EVENT_KEY}`,n   
MOUSELEAVE     : `mouseleave${EVENT_KEY}`,n    TOUCHEND       :
`touchend${EVENT_KEY}`,n    LOAD_DATA_API  :
`load${EVENT_KEY}${DATA_API_KEY}`,n    CLICK_DATA_API :
`click${EVENT_KEY}${DATA_API_KEY}`n  }nn  const ClassName = {n    CAROUSEL
: ‘carousel’,n    ACTIVE   : ‘active’,n    SLIDE    : ‘slide’,n    RIGHT   
: ‘carousel-item-right’,n    LEFT     : ‘carousel-item-left’,n    NEXT    
: ‘carousel-item-next’,n    PREV     : ‘carousel-item-prev’,n    ITEM     :
‘carousel-item’n  }nn  const Selector = {n    ACTIVE      : ‘.active’,n   
ACTIVE_ITEM : ‘.active.carousel-item’,n    ITEM        : ‘.carousel-item’,n
NEXT_PREV   : ‘.carousel-item-next, .carousel-item-prev’,n    INDICATORS  :
‘.carousel-indicators’,n    DATA_SLIDE  : ‘[data-slide], [data-slide-to]’,n
DATA_RIDE   : ‘[data-ride="carousel"]’n  }nn  /**n   *
————————————————————————n   * Class Definitionn   *
————————————————————————n   */nn  class Carousel {n    constructor(element,
config) {n      this._items              = nulln      this._interval       
= nulln      this._activeElement      = nullnn      this._isPaused         
= falsen      this._isSliding          = falsenn      this.touchTimeout    
= nullnn      this._config             = this._getConfig(config)n     
this._element            = $(element)[0]n      this._indicatorsElement  =
$(this._element).find(Selector.INDICATORS)[0]nn     
this._addEventListeners()n    }nn    // Gettersnn    static get VERSION()
{n      return VERSIONn    }nn    static get Default() {n      return
Defaultn    }nn    // Publicnn    next() {n      if (!this._isSliding) {n  
this._slide(Direction.NEXT)n      }n    }nn    nextWhenVisible() {n      //
Don’t call next when the page isn’t visiblen      // or the carousel or its
parent isn’t visiblen      if (!document.hidden &&n       
($(this._element).is(‘:visible’) && $(this._element).css(‘visibility’) !==
‘hidden’)) {n        this.next()n      }n    }nn    prev() {n      if
(!this._isSliding) {n        this._slide(Direction.PREV)n      }n    }nn   
pause(event) {n      if (!event) {n        this._isPaused = truen      }nn 
if ($(this._element).find(Selector.NEXT_PREV)[0]) {n       
Util.triggerTransitionEnd(this._element)n        this.cycle(true)n      }nn
clearInterval(this._interval)n      this._interval = nulln    }nn   
cycle(event) {n      if (!event) {n        this._isPaused = falsen      }nn
if (this._interval) {n        clearInterval(this._interval)n       
this._interval = nulln      }nn      if (this._config.interval &&
!this._isPaused) {n        this._interval = setInterval(n         
(document.visibilityState ? this.nextWhenVisible : this.next).bind(this),n 
this._config.intervaln        )n      }n    }nn    to(index) {n     
this._activeElement = $(this._element).find(Selector.ACTIVE_ITEM)[0]nn     
const activeIndex = this._getItemIndex(this._activeElement)nn      if
(index > this._items.length - 1 || index < 0) {\n        return\n     
}\n\n      if (this._isSliding) {\n        $(this._element).one(Event.SLID,
() => this.to(index))n        returnn      }nn      if (activeIndex ===
index) {n        this.pause()n        this.cycle()n        returnn      }nn
const direction = index > activeIndexn        ? Direction.NEXTn        :
Direction.PREVnn      this._slide(direction, this._items[index])n    }nn   
dispose() {n      $(this._element).off(EVENT_KEY)n     
$.removeData(this._element, DATA_KEY)nn      this._items             =
nulln      this._config            = nulln      this._element           =
nulln      this._interval          = nulln      this._isPaused          =
nulln      this._isSliding         = nulln      this._activeElement     =
nulln      this._indicatorsElement = nulln    }nn    // Privatenn   
_getConfig(config) {n      config = {n        …Default,n        …confign   
}n      Util.typeCheckConfig(NAME, config, DefaultType)n      return
confign    }nn    _addEventListeners() {n      if (this._config.keyboard)
{n        $(this._element)n          .on(Event.KEYDOWN, (event) =>
this._keydown(event))n      }nn      if (this._config.pause === ‘hover’) {n
$(this._element)n          .on(Event.MOUSEENTER, (event) =>
this.pause(event))n          .on(Event.MOUSELEAVE, (event) =>
this.cycle(event))n        if (‘ontouchstart’ in document.documentElement)
{n          // If it’s a touch-enabled device, mouseenter/leave are fired
asn          // part of the mouse compatibility events on first tap - the
carouseln          // would stop cycling until user tapped out of it;n     
// here, we listen for touchend, explicitly pause the carouseln          //
(as if it’s the second time we tap on it, mouseenter compat eventn         
// is NOT fired) and after a timeout (to allow for mouse compatibilityn    
// events to fire) we explicitly restart cyclingn         
$(this._element).on(Event.TOUCHEND, () => {n            this.pause()n      
if (this.touchTimeout) {n              clearTimeout(this.touchTimeout)n    
}n            this.touchTimeout = setTimeout((event) => this.cycle(event),
TOUCHEVENT_COMPAT_WAIT + this._config.interval)n          })n        }n    
}n    }nn    _keydown(event) {n      if
(/input|textarea/i.test(event.target.tagName)) {n        returnn      }nn  
switch (event.which) {n        case ARROW_LEFT_KEYCODE:n         
event.preventDefault()n          this.prev()n          breakn        case
ARROW_RIGHT_KEYCODE:n          event.preventDefault()n         
this.next()n          breakn        default:n      }n    }nn   
_getItemIndex(element) {n      this._items =
$.makeArray($(element).parent().find(Selector.ITEM))n      return
this._items.indexOf(element)n    }nn    _getItemByDirection(direction,
activeElement) {n      const isNextDirection = direction ===
Direction.NEXTn      const isPrevDirection = direction === Direction.PREVn 
const activeIndex     = this._getItemIndex(activeElement)n      const
lastItemIndex   = this._items.length - 1n      const isGoingToWrap   =
isPrevDirection && activeIndex === 0 ||n                             
isNextDirection && activeIndex === lastItemIndexnn      if (isGoingToWrap
&& !this._config.wrap) {n        return activeElementn      }nn      const
delta     = direction === Direction.PREV ? -1 : 1n      const itemIndex =
(activeIndex + delta) % this._items.lengthnn      return itemIndex === -1n 
? this._items[this._items.length - 1] : this._items[itemIndex]n    }nn   
_triggerSlideEvent(relatedTarget, eventDirectionName) {n      const
targetIndex = this._getItemIndex(relatedTarget)n      const fromIndex =
this._getItemIndex($(this._element).find(Selector.ACTIVE_ITEM)[0])n     
const slideEvent = $.Event(Event.SLIDE, {n        relatedTarget,n       
direction: eventDirectionName,n        from: fromIndex,n        to:
targetIndexn      })nn      $(this._element).trigger(slideEvent)nn     
return slideEventn    }nn    _setActiveIndicatorElement(element) {n      if
(this._indicatorsElement) {n        $(this._indicatorsElement)n         
.find(Selector.ACTIVE)n          .removeClass(ClassName.ACTIVE)nn       
const nextIndicator = this._indicatorsElement.children[n         
this._getItemIndex(element)n        ]nn        if (nextIndicator) {n       
$(nextIndicator).addClass(ClassName.ACTIVE)n        }n      }n    }nn   
_slide(direction, element) {n      const activeElement =
$(this._element).find(Selector.ACTIVE_ITEM)[0]n      const
activeElementIndex = this._getItemIndex(activeElement)n      const
nextElement   = element || activeElement &&n       
this._getItemByDirection(direction, activeElement)n      const
nextElementIndex = this._getItemIndex(nextElement)n      const isCycling =
Boolean(this._interval)nn      let directionalClassNamen      let
orderClassNamen      let eventDirectionNamenn      if (direction ===
Direction.NEXT) {n        directionalClassName = ClassName.LEFTn       
orderClassName = ClassName.NEXTn        eventDirectionName =
Direction.LEFTn      } else {n        directionalClassName =
ClassName.RIGHTn        orderClassName = ClassName.PREVn       
eventDirectionName = Direction.RIGHTn      }nn      if (nextElement &&
$(nextElement).hasClass(ClassName.ACTIVE)) {n        this._isSliding =
falsen        returnn      }nn      const slideEvent =
this._triggerSlideEvent(nextElement, eventDirectionName)n      if
(slideEvent.isDefaultPrevented()) {n        returnn      }nn      if
(!activeElement || !nextElement) {n        // Some weirdness is happening,
so we bailn        returnn      }nn      this._isSliding = truenn      if
(isCycling) {n        this.pause()n      }nn     
this._setActiveIndicatorElement(nextElement)nn      const slidEvent =
$.Event(Event.SLID, {n        relatedTarget: nextElement,n       
direction: eventDirectionName,n        from: activeElementIndex,n       
to: nextElementIndexn      })nn      if
($(this._element).hasClass(ClassName.SLIDE)) {n       
$(nextElement).addClass(orderClassName)nn        Util.reflow(nextElement)nn
$(activeElement).addClass(directionalClassName)n       
$(nextElement).addClass(directionalClassName)nn        const
transitionDuration = Util.getTransitionDurationFromElement(activeElement)nn
$(activeElement)n          .one(Util.TRANSITION_END, () => {n           
$(nextElement)n              .removeClass(`${directionalClassName}
${orderClassName}`)n              .addClass(ClassName.ACTIVE)nn           
$(activeElement).removeClass(`${ClassName.ACTIVE} ${orderClassName}
${directionalClassName}`)nn            this._isSliding = falsenn           
setTimeout(() => $(this._element).trigger(slidEvent), 0)n          })n     
.emulateTransitionEnd(transitionDuration)n      } else {n       
$(activeElement).removeClass(ClassName.ACTIVE)n       
$(nextElement).addClass(ClassName.ACTIVE)nn        this._isSliding = falsen
$(this._element).trigger(slidEvent)n      }nn      if (isCycling) {n       
this.cycle()n      }n    }nn    // Staticnn    static
_jQueryInterface(config) {n      return this.each(function () {n        let
data = $(this).data(DATA_KEY)n        let _config = {n          …Default,n 
…$(this).data()n        }nn        if (typeof config === ‘object’) {n      
_config = {n            …_config,n            …confign          }n       
}nn        const action = typeof config === ‘string’ ? config :
_config.slidenn        if (!data) {n          data = new Carousel(this,
_config)n          $(this).data(DATA_KEY, data)n        }nn        if
(typeof config === ‘number’) {n          data.to(config)n        } else if
(typeof action === ‘string’) {n          if (typeof data[action] ===
‘undefined’) {n            throw new TypeError(`No method named
"${action}"`)n          }n          data[action]()n        } else if
(_config.interval) {n          data.pause()n          data.cycle()n       
}n      })n    }nn    static _dataApiClickHandler(event) {n      const
selector = Util.getSelectorFromElement(this)nn      if (!selector) {n      
returnn      }nn      const target = $(selector)[0]nn      if (!target ||
!$(target).hasClass(ClassName.CAROUSEL)) {n        returnn      }nn     
const config = {n        …$(target).data(),n        …$(this).data()n     
}n      const slideIndex = this.getAttribute(‘data-slide-to’)nn      if
(slideIndex) {n        config.interval = falsen      }nn     
Carousel._jQueryInterface.call($(target), config)nn      if (slideIndex) {n
$(target).data(DATA_KEY).to(slideIndex)n      }nn     
event.preventDefault()n    }n  }nn  /**n   * ————————————————————————n   *
Data Api implementationn   * ————————————————————————n   */nn  $(document)n
.on(Event.CLICK_DATA_API, Selector.DATA_SLIDE,
Carousel._dataApiClickHandler)nn  $(window).on(Event.LOAD_DATA_API, () =>
{n    $(Selector.DATA_RIDE).each(function () {n      const $carousel =
$(this)n      Carousel._jQueryInterface.call($carousel, $carousel.data())n 
})n  })nn  /**n   * ————————————————————————n   * jQueryn   *
————————————————————————n   */nn  $.fn[NAME] = Carousel._jQueryInterfacen 
$.fn[NAME].Constructor = Carouseln  $.fn[NAME].noConflict = function () {n 
$.fn[NAME] = JQUERY_NO_CONFLICTn    return Carousel._jQueryInterfacen  }nn 
return Carouseln})($)nnexport default Carouseln”,“import $ from
‘jquery’nimport Util from ‘./util’nn/**n * ————————————————————————–n *
Bootstrap (v4.1.0): collapse.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Collapse = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME                = ‘collapse’n  const VERSION             =
‘4.1.0’n  const DATA_KEY            = ‘bs.collapse’n  const EVENT_KEY      
= `.${DATA_KEY}`n  const DATA_API_KEY        = ‘.data-api’n  const
JQUERY_NO_CONFLICT  = $.fn[NAME]nn  const Default = {n    toggle : true,n  
parent : ”n  }nn  const DefaultType = {n    toggle : ‘boolean’,n    parent
: ‘(string|element)’n  }nn  const Event = {n    SHOW           :
`show${EVENT_KEY}`,n    SHOWN          : `shown${EVENT_KEY}`,n    HIDE     
: `hide${EVENT_KEY}`,n    HIDDEN         : `hidden${EVENT_KEY}`,n   
CLICK_DATA_API : `click${EVENT_KEY}${DATA_API_KEY}`n  }nn  const ClassName
= {n    SHOW       : ‘show’,n    COLLAPSE   : ‘collapse’,n    COLLAPSING :
‘collapsing’,n    COLLAPSED  : ‘collapsed’n  }nn  const Dimension = {n   
WIDTH  : ‘width’,n    HEIGHT : ‘height’n  }nn  const Selector = {n   
ACTIVES     : ‘.show, .collapsing’,n    DATA_TOGGLE :
‘[data-toggle="collapse"]’n  }nn  /**n   * ————————————————————————n   *
Class Definitionn   * ————————————————————————n   */nn  class Collapse {n  
constructor(element, config) {n      this._isTransitioning = falsen     
this._element         = elementn      this._config          =
this._getConfig(config)n      this._triggerArray    = $.makeArray($(n      
`[data-toggle="collapse"][href="#${element.id}"],` +n       
`[data-toggle="collapse"][data-target="#${element.id}"]`n      ))n     
const tabToggles = $(Selector.DATA_TOGGLE)n      for (let i = 0; i <
tabToggles.length; i++) {\n        const elem = tabToggles[i]\n       
const selector = Util.getSelectorFromElement(elem)\n        if (selector
!== null && $(selector).filter(element).length > 0) {n         
this._selector = selectorn          this._triggerArray.push(elem)n       
}n      }nn      this._parent = this._config.parent ? this._getParent() :
nullnn      if (!this._config.parent) {n       
this._addAriaAndCollapsedClass(this._element, this._triggerArray)n      }nn
if (this._config.toggle) {n        this.toggle()n      }n    }nn    //
Gettersnn    static get VERSION() {n      return VERSIONn    }nn    static
get Default() {n      return Defaultn    }nn    // Publicnn    toggle() {n 
if ($(this._element).hasClass(ClassName.SHOW)) {n        this.hide()n     
} else {n        this.show()n      }n    }nn    show() {n      if
(this._isTransitioning ||n       
$(this._element).hasClass(ClassName.SHOW)) {n        returnn      }nn     
let activesn      let activesDatann      if (this._parent) {n       
actives = $.makeArray(n          $(this._parent)n           
.find(Selector.ACTIVES)n           
.filter(`[data-parent="${this._config.parent}"]`)n        )n        if
(actives.length === 0) {n          actives = nulln        }n      }nn     
if (actives) {n        activesData =
$(actives).not(this._selector).data(DATA_KEY)n        if (activesData &&
activesData._isTransitioning) {n          returnn        }n      }nn     
const startEvent = $.Event(Event.SHOW)n     
$(this._element).trigger(startEvent)n      if
(startEvent.isDefaultPrevented()) {n        returnn      }nn      if
(actives) {n       
Collapse._jQueryInterface.call($(actives).not(this._selector), ‘hide’)n    
if (!activesData) {n          $(actives).data(DATA_KEY, null)n        }n   
}nn      const dimension = this._getDimension()nn      $(this._element)n   
.removeClass(ClassName.COLLAPSE)n        .addClass(ClassName.COLLAPSING)nn 
this._element.style[dimension] = 0nn      if (this._triggerArray.length >
0) {n        $(this._triggerArray)n         
.removeClass(ClassName.COLLAPSED)n          .attr(‘aria-expanded’, true)n  
}nn      this.setTransitioning(true)nn      const complete = () => {n      
$(this._element)n          .removeClass(ClassName.COLLAPSING)n         
.addClass(ClassName.COLLAPSE)n          .addClass(ClassName.SHOW)nn       
this._element.style[dimension] = ”nn        this.setTransitioning(false)nn 
$(this._element).trigger(Event.SHOWN)n      }nn      const
capitalizedDimension = dimension[0].toUpperCase() + dimension.slice(1)n    
const scrollSize = `scroll${capitalizedDimension}`n      const
transitionDuration = Util.getTransitionDurationFromElement(this._element)nn
$(this._element)n        .one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)nn     
this._element.style[dimension] = `${this._element[scrollSize]}px`n    }nn  
hide() {n      if (this._isTransitioning ||n       
!$(this._element).hasClass(ClassName.SHOW)) {n        returnn      }nn     
const startEvent = $.Event(Event.HIDE)n     
$(this._element).trigger(startEvent)n      if
(startEvent.isDefaultPrevented()) {n        returnn      }nn      const
dimension = this._getDimension()nn      this._element.style[dimension] =
`${this._element.getBoundingClientRect()[dimension]}px`nn     
Util.reflow(this._element)nn      $(this._element)n       
.addClass(ClassName.COLLAPSING)n        .removeClass(ClassName.COLLAPSE)n  
.removeClass(ClassName.SHOW)nn      if (this._triggerArray.length > 0) {n  
for (let i = 0; i < this._triggerArray.length; i++) {\n          const
trigger = this._triggerArray[i]\n          const selector =
Util.getSelectorFromElement(trigger)\n          if (selector !== null) {\n 
const $elem = $(selector)\n            if (!$elem.hasClass(ClassName.SHOW))
{\n              $(trigger).addClass(ClassName.COLLAPSED)\n               
.attr('aria-expanded', false)\n            }\n          }\n        }\n     
}\n\n      this.setTransitioning(true)\n\n      const complete = () => {n  
this.setTransitioning(false)n        $(this._element)n         
.removeClass(ClassName.COLLAPSING)n          .addClass(ClassName.COLLAPSE)n
.trigger(Event.HIDDEN)n      }nn      this._element.style[dimension] = ”n  
const transitionDuration =
Util.getTransitionDurationFromElement(this._element)nn     
$(this._element)n        .one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)n    }nn   
setTransitioning(isTransitioning) {n      this._isTransitioning =
isTransitioningn    }nn    dispose() {n      $.removeData(this._element,
DATA_KEY)nn      this._config          = nulln      this._parent          =
nulln      this._element         = nulln      this._triggerArray    = nulln
this._isTransitioning = nulln    }nn    // Privatenn    _getConfig(config)
{n      config = {n        …Default,n        …confign      }n     
config.toggle = Boolean(config.toggle) // Coerce string valuesn     
Util.typeCheckConfig(NAME, config, DefaultType)n      return confign    }nn
_getDimension() {n      const hasWidth =
$(this._element).hasClass(Dimension.WIDTH)n      return hasWidth ?
Dimension.WIDTH : Dimension.HEIGHTn    }nn    _getParent() {n      let
parent = nulln      if (Util.isElement(this._config.parent)) {n       
parent = this._config.parentnn        // It’s a jQuery objectn        if
(typeof this._config.parent.jquery !== ‘undefined’) {n          parent =
this._config.parent[0]n        }n      } else {n        parent =
$(this._config.parent)[0]n      }nn      const selector =n       
`[data-toggle="collapse"][data-parent="${this._config.parent}"]`nn     
$(parent).find(selector).each((i, element) => {n       
this._addAriaAndCollapsedClass(n         
Collapse._getTargetFromElement(element),n          [element]n        )n    
})nn      return parentn    }nn    _addAriaAndCollapsedClass(element,
triggerArray) {n      if (element) {n        const isOpen =
$(element).hasClass(ClassName.SHOW)nn        if (triggerArray.length > 0)
{n          $(triggerArray)n            .toggleClass(ClassName.COLLAPSED,
!isOpen)n            .attr(‘aria-expanded’, isOpen)n        }n      }n   
}nn    // Staticnn    static _getTargetFromElement(element) {n      const
selector = Util.getSelectorFromElement(element)n      return selector ?
$(selector)[0] : nulln    }nn    static _jQueryInterface(config) {n     
return this.each(function () {n        const $this   = $(this)n        let
data      = $this.data(DATA_KEY)n        const _config = {n         
…Default,n          …$this.data(),n          …typeof config === ‘object’ &&
confign        }nn        if (!data && _config.toggle &&
/show|hide/.test(config)) {n          _config.toggle = falsen        }nn   
if (!data) {n          data = new Collapse(this, _config)n         
$this.data(DATA_KEY, data)n        }nn        if (typeof config ===
‘string’) {n          if (typeof data[config] === ‘undefined’) {n          
throw new TypeError(`No method named "${config}"`)n          }n         
data[config]()n        }n      })n    }n  }nn  /**n   *
————————————————————————n   * Data Api implementationn   *
————————————————————————n   */nn  $(document).on(Event.CLICK_DATA_API,
Selector.DATA_TOGGLE, function (event) {n    // preventDefault only for <a>
elements (which change the URL) not inside the collapsible elementn    if
(event.currentTarget.tagName === ‘A’) {n      event.preventDefault()n   
}nn    const $trigger = $(this)n    const selector =
Util.getSelectorFromElement(this)n    $(selector).each(function () {n     
const $target = $(this)n      const data    = $target.data(DATA_KEY)n     
const config  = data ? ‘toggle’ : $trigger.data()n     
Collapse._jQueryInterface.call($target, config)n    })n  })nn  /**n   *
————————————————————————n   * jQueryn   * ————————————————————————n   */nn 
$.fn[NAME] = Collapse._jQueryInterfacen  $.fn[NAME].Constructor = Collapsen
$.fn[NAME].noConflict = function () {n    $.fn[NAME] = JQUERY_NO_CONFLICTn 
return Collapse._jQueryInterfacen  }nn  return Collapsen})($)nnexport
default Collapsen”,“/**!n * @fileOverview Kickass library to create and
place poppers near their reference elements.n * @version 1.14.1n *
@licensen * Copyright © 2016 Federico Zivolo and contributorsn *n *
Permission is hereby granted, free of charge, to any person obtaining a
copyn * of this software and associated documentation files (the
"Software"), to dealn * in the Software without restriction, including
without limitation the rightsn * to use, copy, modify, merge, publish,
distribute, sublicense, and/or selln * copies of the Software, and to
permit persons to whom the Software isn * furnished to do so, subject to
the following conditions:n *n * The above copyright notice and this
permission notice shall be included in alln * copies or substantial
portions of the Software.n *n * THE SOFTWARE IS PROVIDED "AS IS", WITHOUT
WARRANTY OF ANY KIND, EXPRESS ORn * IMPLIED, INCLUDING BUT NOT LIMITED TO
THE WARRANTIES OF MERCHANTABILITY,n * FITNESS FOR A PARTICULAR PURPOSE AND
NONINFRINGEMENT. IN NO EVENT SHALL THEn * AUTHORS OR COPYRIGHT HOLDERS BE
LIABLE FOR ANY CLAIM, DAMAGES OR OTHERn * LIABILITY, WHETHER IN AN ACTION
OF CONTRACT, TORT OR OTHERWISE, ARISING FROM,n * OUT OF OR IN CONNECTION
WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THEn * SOFTWARE.n */nvar
isBrowser = typeof window !== ‘undefined’ && typeof document !==
‘undefined’;nvar longerTimeoutBrowsers = [‘Edge’, ‘Trident’,
‘Firefox’];nvar timeoutDuration = 0;nfor (var i = 0; i <
longerTimeoutBrowsers.length; i += 1) {\n  if (isBrowser &&
navigator.userAgent.indexOf(longerTimeoutBrowsers[i]) >= 0) {n   
timeoutDuration = 1;n    break;n  }n}nnfunction microtaskDebounce(fn) {n 
var called = false;n  return function () {n    if (called) {n      return;n
}n    called = true;n    window.Promise.resolve().then(function () {n     
called = false;n      fn();n    });n  };n}nnfunction taskDebounce(fn) {n 
var scheduled = false;n  return function () {n    if (!scheduled) {n     
scheduled = true;n      setTimeout(function () {n        scheduled =
false;n        fn();n      }, timeoutDuration);n    }n  };n}nnvar
supportsMicroTasks = isBrowser && window.Promise;nn/*n Create a debounced
version of a method, that’s asynchronously deferredn* but called in the
minimum time possible.n*n* @methodn* @memberof Popper.Utilsn* @argument
{Function} fnn* @returns {Function}n*/nvar debounce = supportsMicroTasks ?
microtaskDebounce : taskDebounce;nn/**n * Check if the given variable is a
functionn * @methodn * @memberof Popper.Utilsn * @argument {Any}
functionToCheck - variable to checkn * @returns {Boolean} answer to: is a
function?n */nfunction isFunction(functionToCheck) {n  var getType = {};n 
return functionToCheck && getType.toString.call(functionToCheck) ===
‘[object Function]’;n}nn/**n * Get CSS computed property of the given
elementn * @methodn * @memberof Popper.Utilsn * @argument {Eement} elementn
* @argument {String} propertyn */nfunction
getStyleComputedProperty(element, property) {n  if (element.nodeType !== 1)
{n    return [];n  }n  // NOTE: 1 DOM access heren  var css =
getComputedStyle(element, null);n  return property ? css[property] :
css;n}nn/**n * Returns the parentNode or the host of the elementn *
@methodn * @memberof Popper.Utilsn * @argument {Element} elementn *
@returns {Element} parentn */nfunction getParentNode(element) {n  if
(element.nodeName === ‘HTML’) {n    return element;n  }n  return
element.parentNode || element.host;n}nn/**n * Returns the scrolling parent
of the given elementn * @methodn * @memberof Popper.Utilsn * @argument
{Element} elementn * @returns {Element} scroll parentn */nfunction
getScrollParent(element) {n  // Return body, `getScroll` will take care to
get the correct `scrollTop` from itn  if (!element) {n    return
document.body;n  }nn  switch (element.nodeName) {n    case ‘HTML’:n    case
‘BODY’:n      return element.ownerDocument.body;n    case ‘#document’:n    
return element.body;n  }nn  // Firefox want us to check `-x` and `-y`
variations as wellnn  var _getStyleComputedProp =
getStyleComputedProperty(element),n      overflow =
_getStyleComputedProp.overflow,n      overflowX =
_getStyleComputedProp.overflowX,n      overflowY =
_getStyleComputedProp.overflowY;nn  if
(/(auto|scroll|overlay)/.test(overflow + overflowY + overflowX)) {n   
return element;n  }nn  return
getScrollParent(getParentNode(element));n}nn/**n * Tells if you are running
Internet Explorern * @methodn * @memberof Popper.Utilsn * @argument
{number} version to checkn * @returns {Boolean} isIEn */nvar cache =
{};nnvar isIE = function () {n  var version = arguments.length > 0 &&
arguments[0] !== undefined ? arguments[0] : ‘all’;nn  version =
version.toString();n  if (cache.hasOwnProperty(version)) {n    return
cache[version];n  }n  switch (version) {n    case ‘11’:n     
cache[version] = navigator.userAgent.indexOf(‘Trident’) !== -1;n     
break;n    case ‘10’:n      cache[version] =
navigator.appVersion.indexOf(‘MSIE 10’) !== -1;n      break;n    case
‘all’:n      cache[version] = navigator.userAgent.indexOf(‘Trident’) !== -1
|| navigator.userAgent.indexOf(‘MSIE’) !== -1;n      break;n  }nn  //Set
IEn  cache.all = cache.all || Object.keys(cache).some(function (key) {n   
return cache[key];n  });n  return cache[version];n};nn/**n * Returns the
offset parent of the given elementn * @methodn * @memberof Popper.Utilsn *
@argument {Element} elementn * @returns {Element} offset parentn
*/nfunction getOffsetParent(element) {n  if (!element) {n    return
document.documentElement;n  }nn  var noOffsetParent = isIE(10) ?
document.body : null;nn  // NOTE: 1 DOM access heren  var offsetParent =
element.offsetParent;n  // Skip hidden elements which don’t have an
offsetParentn  while (offsetParent === noOffsetParent &&
element.nextElementSibling) {n    offsetParent = (element =
element.nextElementSibling).offsetParent;n  }nn  var nodeName =
offsetParent && offsetParent.nodeName;nn  if (!nodeName || nodeName ===
‘BODY’ || nodeName === ‘HTML’) {n    return element ?
element.ownerDocument.documentElement : document.documentElement;n  }nn  //
.offsetParent will return the closest TD or TABLE in casen  // no
offsetParent is present, I hate this job…n  if ([‘TD’,
‘TABLE’].indexOf(offsetParent.nodeName) !== -1 &&
getStyleComputedProperty(offsetParent, ‘position’) === ‘static’) {n   
return getOffsetParent(offsetParent);n  }nn  return
offsetParent;n}nnfunction isOffsetContainer(element) {n  var nodeName =
element.nodeName;nn  if (nodeName === ‘BODY’) {n    return false;n  }n 
return nodeName === ‘HTML’ || getOffsetParent(element.firstElementChild)
=== element;n}nn/**n * Finds the root node (document, shadowDOM root) of
the given elementn * @methodn * @memberof Popper.Utilsn * @argument
{Element} noden * @returns {Element} root noden */nfunction getRoot(node)
{n  if (node.parentNode !== null) {n    return getRoot(node.parentNode);n 
}nn  return node;n}nn/**n * Finds the offset parent common to the two
provided nodesn * @methodn * @memberof Popper.Utilsn * @argument {Element}
element1n * @argument {Element} element2n * @returns {Element} common
offset parentn */nfunction findCommonOffsetParent(element1, element2) {n 
// This check is needed to avoid errors in case one of the elements isn’t
defined for any reasonn  if (!element1 || !element1.nodeType || !element2
|| !element2.nodeType) {n    return document.documentElement;n  }nn  //
Here we make sure to give as "start" the element that comes first in the
DOMn  var order = element1.compareDocumentPosition(element2) &
Node.DOCUMENT_POSITION_FOLLOWING;n  var start = order ? element1 :
element2;n  var end = order ? element2 : element1;nn  // Get common
ancestor containern  var range = document.createRange();n 
range.setStart(start, 0);n  range.setEnd(end, 0);n  var
commonAncestorContainer = range.commonAncestorContainer;nn  // Both nodes
are inside #documentnn  if (element1 !== commonAncestorContainer &&
element2 !== commonAncestorContainer || start.contains(end)) {n    if
(isOffsetContainer(commonAncestorContainer)) {n      return
commonAncestorContainer;n    }nn    return
getOffsetParent(commonAncestorContainer);n  }nn  // one of the nodes is
inside shadowDOM, find which onen  var element1root = getRoot(element1);n 
if (element1root.host) {n    return
findCommonOffsetParent(element1root.host, element2);n  } else {n    return
findCommonOffsetParent(element1, getRoot(element2).host);n  }n}nn/**n *
Gets the scroll value of the given element in the given side (top and
left)n * @methodn * @memberof Popper.Utilsn * @argument {Element} elementn
* @argument {String} side `top` or `left`n * @returns {number} amount of
scrolled pixelsn */nfunction getScroll(element) {n  var side =
arguments.length > 1 && arguments[1] !== undefined ? arguments[1] :
‘top’;nn  var upperSide = side === ‘top’ ? ‘scrollTop’ : ‘scrollLeft’;n 
var nodeName = element.nodeName;nn  if (nodeName === ‘BODY’ || nodeName ===
‘HTML’) {n    var html = element.ownerDocument.documentElement;n    var
scrollingElement = element.ownerDocument.scrollingElement || html;n   
return scrollingElement[upperSide];n  }nn  return
element[upperSide];n}nn/*n * Sum or subtract the element scroll values
(left and top) from a given rect objectn * @methodn * @memberof
Popper.Utilsn * @param {Object} rect - Rect object you want to changen *
@param {HTMLElement} element - The element from the function reads the
scroll valuesn * @param {Boolean} subtract - set to true if you want to
subtract the scroll valuesn * @return {Object} rect - The modifier rect
objectn */nfunction includeScroll(rect, element) {n  var subtract =
arguments.length > 2 && arguments[2] !== undefined ? arguments[2] :
false;nn  var scrollTop = getScroll(element, ‘top’);n  var scrollLeft =
getScroll(element, ‘left’);n  var modifier = subtract ? -1 : 1;n  rect.top
+= scrollTop * modifier;n  rect.bottom += scrollTop * modifier;n  rect.left
+= scrollLeft * modifier;n  rect.right += scrollLeft * modifier;n  return
rect;n}nn/*n * Helper to detect borders of a given elementn * @methodn *
@memberof Popper.Utilsn * @param {CSSStyleDeclaration} stylesn * Result of
`getStyleComputedProperty` on the given elementn * @param {String} axis -
`x` or `y`n * @return {number} borders - The borders size of the given
axisn */nnfunction getBordersSize(styles, axis) {n  var sideA = axis ===
‘x’ ? ‘Left’ : ‘Top’;n  var sideB = sideA === ‘Left’ ? ‘Right’ :
‘Bottom’;nn  return parseFloat(styles[‘border’ + sideA + ‘Width’], 10) +
parseFloat(styles[‘border’ + sideB + ‘Width’], 10);n}nnfunction
getSize(axis, body, html, computedStyle) {n  return Math.max(body[‘offset’
+ axis], body[‘scroll’ + axis], html[‘client’ + axis], html[‘offset’ +
axis], html[‘scroll’ + axis], isIE(10) ? html[‘offset’ + axis] +
computedStyle[‘margin’ + (axis === ‘Height’ ? ‘Top’ : ‘Left’)] +
computedStyle[‘margin’ + (axis === ‘Height’ ? ‘Bottom’ : ‘Right’)] :
0);n}nnfunction getWindowSizes() {n  var body = document.body;n  var html =
document.documentElement;n  var computedStyle = isIE(10) &&
getComputedStyle(html);nn  return {n    height: getSize(‘Height’, body,
html, computedStyle),n    width: getSize(‘Width’, body, html,
computedStyle)n  };n}nnvar classCallCheck = function (instance,
Constructor) {n  if (!(instance instanceof Constructor)) {n    throw new
TypeError("Cannot call a class as a function");n  }n};nnvar createClass =
function () {n  function defineProperties(target, props) {n    for (var i =
0; i < props.length; i++) {\n      var descriptor = props[i];\n     
...
offsets.height\n  });\n}\n\n/**\n * Get bounding client rect of given
element\n * @method\n * @memberof Popper.Utils\n * @param {HTMLElement}
element\n * @return {Object} client rect\n */\nfunction
getBoundingClientRect(element) {\n  var rect = {};\n\n  // IE10 10 FIX:
Please, don't ask, the element isn't\n  // considered in DOM in some
circumstances...\n  // This isn't reproducible in IE10 compatibility mode
of IE11\n  try {\n    if (isIE(10)) {\n      rect =
...
scrollbar size from sizes\n  var sizes = element.nodeName === 'HTML' ?
getWindowSizes() : {};\n  var width = sizes.width || element.clientWidth ||
result.right - result.left;\n  var height = sizes.height ||
element.clientHeight || result.bottom - result.top;\n\n  var horizScrollbar
= element.offsetWidth - width;\n  var vertScrollbar = element.offsetHeight
- height;\n\n  // if an hypothetical scrollbar is detected, we must be sure
it's not a `border`\n  // we make this check conditional for performance
reasons\n  if (horizScrollbar || vertScrollbar) {\n    var styles =
getStyleComputedProperty(element);\n    horizScrollbar -=
getBordersSize(styles, 'x');\n    vertScrollbar -= getBordersSize(styles,
'y');\n\n    result.width -= horizScrollbar;\n    result.height -=
vertScrollbar;\n  }\n\n  return getClientRect(result);\n}\n\nfunction
getOffsetRectRelativeToArbitraryNode(children, parent) {\n  var
fixedPosition = arguments.length > 2 && arguments[2] !== undefined ?
arguments[2] : false;nn  var isIE10 = isIE(10);n  var isHTML =
parent.nodeName === ‘HTML’;n  var childrenRect =
getBoundingClientRect(children);n  var parentRect =
getBoundingClientRect(parent);n  var scrollParent =
getScrollParent(children);nn  var styles =
getStyleComputedProperty(parent);n  var borderTopWidth =
parseFloat(styles.borderTopWidth, 10);n  var borderLeftWidth =
parseFloat(styles.borderLeftWidth, 10);nn  // In cases where the parent is
fixed, we must ignore negative scroll in offset calcn  if (fixedPosition &&
parent.nodeName === ‘HTML’) {n    parentRect.top = Math.max(parentRect.top,
0);n    parentRect.left = Math.max(parentRect.left, 0);n  }n  var offsets =
getClientRect({n    top: childrenRect.top - parentRect.top -
borderTopWidth,n    left: childrenRect.left - parentRect.left -
borderLeftWidth,n    width: childrenRect.width,n    height:
childrenRect.heightn  });n  offsets.marginTop = 0;n  offsets.marginLeft =
0;nn  // Subtract margins of documentElement in case it’s being used as
parentn  // we do this only on HTML because it’s the only element that
behavesn  // differently when margins are applied to it. The margins are
included inn  // the box of the documentElement, in the other cases not.n 
if (!isIE10 && isHTML) {n    var marginTop = parseFloat(styles.marginTop,
10);n    var marginLeft = parseFloat(styles.marginLeft, 10);nn   
offsets.top -= borderTopWidth - marginTop;n    offsets.bottom -=
borderTopWidth - marginTop;n    offsets.left -= borderLeftWidth -
marginLeft;n    offsets.right -= borderLeftWidth - marginLeft;nn    //
Attach marginTop and marginLeft because in some circumstances we may need
themn    offsets.marginTop = marginTop;n    offsets.marginLeft =
marginLeft;n  }nn  if (isIE10 && !fixedPosition ?
parent.contains(scrollParent) : parent === scrollParent &&
scrollParent.nodeName !== ‘BODY’) {n    offsets = includeScroll(offsets,
parent);n  }nn  return offsets;n}nnfunction
getViewportOffsetRectRelativeToArtbitraryNode(element) {n  var
excludeScroll = arguments.length > 1 && arguments[1] !== undefined ?
arguments[1] : false;nn  var html = element.ownerDocument.documentElement;n
var relativeOffset = getOffsetRectRelativeToArbitraryNode(element, html);n 
var width = Math.max(html.clientWidth, window.innerWidth || 0);n  var
height = Math.max(html.clientHeight, window.innerHeight || 0);nn  var
scrollTop = !excludeScroll ? getScroll(html) : 0;n  var scrollLeft =
!excludeScroll ? getScroll(html, ‘left’) : 0;nn  var offset = {n    top:
scrollTop - relativeOffset.top + relativeOffset.marginTop,n    left:
scrollLeft - relativeOffset.left + relativeOffset.marginLeft,n    width:
width,n    height: heightn  };nn  return getClientRect(offset);n}nn/**n *
Check if the given element is fixed or is inside a fixed parentn * @methodn
* @memberof Popper.Utilsn * @argument {Element} elementn * @argument
{Element} customContainern * @returns {Boolean} answer to "isFixed?"n
*/nfunction isFixed(element) {n  var nodeName = element.nodeName;n  if
(nodeName === ‘BODY’ || nodeName === ‘HTML’) {n    return false;n  }n  if
(getStyleComputedProperty(element, ‘position’) === ‘fixed’) {n    return
true;n  }n  return isFixed(getParentNode(element));n}nn/**n * Finds the
first parent of an element that has a transformed property definedn *
@methodn * @memberof Popper.Utilsn * @argument {Element} elementn *
@returns {Element} first transformed parent or documentElementn
*/nnfunction getFixedPositionOffsetParent(element) {n  // This check is
needed to avoid errors in case one of the elements isn’t defined for any
reasonn  if (!element || !element.parentElement || isIE()) {n    return
document.documentElement;n  }n  var el = element.parentElement;n  while (el
&& getStyleComputedProperty(el, ‘transform’) === ‘none’) {n    el =
el.parentElement;n  }n  return el || document.documentElement;n}nn/**n *
Computed the boundaries limits and return themn * @methodn * @memberof
Popper.Utilsn * @param {HTMLElement} poppern * @param {HTMLElement}
referencen * @param {number} paddingn * @param {HTMLElement}
boundariesElement - Element used to define the boundariesn * @param
{Boolean} fixedPosition - Is in fixed position moden * @returns {Object}
Coordinates of the boundariesn */nfunction getBoundaries(popper, reference,
padding, boundariesElement) {n  var fixedPosition = arguments.length > 4 &&
arguments[4] !== undefined ? arguments[4] : false;nn  // NOTE: 1 DOM access
herenn  var boundaries = { top: 0, left: 0 };n  var offsetParent =
fixedPosition ? getFixedPositionOffsetParent(popper) :
findCommonOffsetParent(popper, reference);nn  // Handle viewport casen  if
(boundariesElement === ‘viewport’) {n    boundaries =
getViewportOffsetRectRelativeToArtbitraryNode(offsetParent,
fixedPosition);n  } else {n    // Handle other cases based on DOM element
used as boundariesn    var boundariesNode = void 0;n    if
(boundariesElement === ‘scrollParent’) {n      boundariesNode =
getScrollParent(getParentNode(reference));n      if
(boundariesNode.nodeName === ‘BODY’) {n        boundariesNode =
popper.ownerDocument.documentElement;n      }n    } else if
(boundariesElement === ‘window’) {n      boundariesNode =
popper.ownerDocument.documentElement;n    } else {n      boundariesNode =
boundariesElement;n    }nn    var offsets =
getOffsetRectRelativeToArbitraryNode(boundariesNode, offsetParent,
fixedPosition);nn    // In case of HTML, we need a different computationn  
if (boundariesNode.nodeName === ‘HTML’ && !isFixed(offsetParent)) {n     
var _getWindowSizes = getWindowSizes(),n          height =
_getWindowSizes.height,n          width = _getWindowSizes.width;nn     
boundaries.top += offsets.top - offsets.marginTop;n      boundaries.bottom
= height + offsets.top;n      boundaries.left += offsets.left -
offsets.marginLeft;n      boundaries.right = width + offsets.left;n    }
else {n      // for all the other DOM elements, this one is goodn     
boundaries = offsets;n    }n  }nn  // Add paddingsn  boundaries.left +=
padding;n  boundaries.top += padding;n  boundaries.right -= padding;n 
boundaries.bottom -= padding;nn  return boundaries;n}nnfunction
getArea(_ref) {n  var width = _ref.width,n      height = _ref.height;nn 
return width * height;n}nn/**n * Utility used to transform the `auto`
placement to the placement with moren * available space.n * @methodn *
@memberof Popper.Utilsn * @argument {Object} data - The data object
generated by update methodn * @argument {Object} options - Modifiers
configuration and optionsn * @returns {Object} The data object, properly
modifiedn */nfunction computeAutoPlacement(placement, refRect, popper,
reference, boundariesElement) {n  var padding = arguments.length > 5 &&
arguments[5] !== undefined ? arguments[5] : 0;nn  if
(placement.indexOf(‘auto’) === -1) {n    return placement;n  }nn  var
boundaries = getBoundaries(popper, reference, padding,
boundariesElement);nn  var rects = {n    top: {n      width:
boundaries.width,n      height: refRect.top - boundaries.topn    },n   
right: {n      width: boundaries.right - refRect.right,n      height:
boundaries.heightn    },n    bottom: {n      width: boundaries.width,n     
height: boundaries.bottom - refRect.bottomn    },n    left: {n      width:
refRect.left - boundaries.left,n      height: boundaries.heightn    }n 
};nn  var sortedAreas = Object.keys(rects).map(function (key) {n    return
_extends({n      key: keyn    }, rects[key], {n      area:
getArea(rects[key])n    });n  }).sort(function (a, b) {n    return b.area -
a.area;n  });nn  var filteredAreas = sortedAreas.filter(function (_ref2) {n
var width = _ref2.width,n        height = _ref2.height;n    return width >=
popper.clientWidth && height >= popper.clientHeight;n  });nn  var
computedPlacement = filteredAreas.length > 0 ? filteredAreas[0].key :
sortedAreas[0].key;nn  var variation = placement.split(‘-’)[1];nn  return
computedPlacement + (variation ? ‘-’ + variation : ”);n}nn/**n * Get
offsets to the reference elementn * @methodn * @memberof Popper.Utilsn *
@param {Object} staten * @param {Element} popper - the popper elementn *
@param {Element} reference - the reference element (the popper will be
relative to this)n * @param {Element} fixedPosition - is in fixed position
moden * @returns {Object} An object containing the offsets which will be
applied to the poppern */nfunction getReferenceOffsets(state, popper,
reference) {n  var fixedPosition = arguments.length > 3 && arguments[3] !==
undefined ? arguments[3] : null;nn  var commonOffsetParent = fixedPosition
? getFixedPositionOffsetParent(popper) : findCommonOffsetParent(popper,
reference);n  return getOffsetRectRelativeToArbitraryNode(reference,
commonOffsetParent, fixedPosition);n}nn/**n * Get the outer sizes of the
given element (offset size + margins)n * @methodn * @memberof Popper.Utilsn
* @argument {Element} elementn * @returns {Object} object containing width
and height propertiesn */nfunction getOuterSizes(element) {n  var styles =
getComputedStyle(element);n  var x = parseFloat(styles.marginTop) +
parseFloat(styles.marginBottom);n  var y = parseFloat(styles.marginLeft) +
parseFloat(styles.marginRight);n  var result = {n    width:
element.offsetWidth + y,n    height: element.offsetHeight + xn  };n  return
result;n}nn/**n * Get the opposite placement of the given onen * @methodn *
@memberof Popper.Utilsn * @argument {String} placementn * @returns {String}
flipped placementn */nfunction getOppositePlacement(placement) {n  var hash
= { left: ‘right’, right: ‘left’, bottom: ‘top’, top: ‘bottom’ };n  return
placement.replace(/left|right|bottom|top/g, function (matched) {n    return
hash[matched];n  });n}nn/**n * Get offsets to the poppern * @methodn *
@memberof Popper.Utilsn * @param {Object} position - CSS position the
Popper will get appliedn * @param {HTMLElement} popper - the popper
elementn * @param {Object} referenceOffsets - the reference offsets (the
popper will be relative to this)n * @param {String} placement - one of the
valid placement optionsn * @returns {Object} popperOffsets - An object
containing the offsets which will be applied to the poppern */nfunction
getPopperOffsets(popper, referenceOffsets, placement) {n  placement =
placement.split(‘-’)[0];nn  // Get popper node sizesn  var popperRect =
getOuterSizes(popper);nn  // Add position, width and height to our offsets
objectn  var popperOffsets = {n    width: popperRect.width,n    height:
popperRect.heightn  };nn  // depending by the popper placement we have to
compute its offsets slightly differentlyn  var isHoriz = [‘right’,
‘left’].indexOf(placement) !== -1;n  var mainSide = isHoriz ? ‘top’ :
‘left’;n  var secondarySide = isHoriz ? ‘left’ : ‘top’;n  var measurement =
isHoriz ? ‘height’ : ‘width’;n  var secondaryMeasurement = !isHoriz ?
‘height’ : ‘width’;nn  popperOffsets[mainSide] = referenceOffsets[mainSide]
+ referenceOffsets[measurement] / 2 - popperRect[measurement] / 2;n  if
(placement === secondarySide) {n    popperOffsets[secondarySide] =
referenceOffsets[secondarySide] - popperRect[secondaryMeasurement];n  }
else {n    popperOffsets[secondarySide] =
referenceOffsets[getOppositePlacement(secondarySide)];n  }nn  return
popperOffsets;n}nn/**n * Mimics the `find` method of Arrayn * @methodn *
@memberof Popper.Utilsn * @argument {Array} arrn * @argument propn *
@argument valuen * @returns index or -1n */nfunction find(arr, check) {n 
// use native find if supportedn  if (Array.prototype.find) {n    return
arr.find(check);n  }nn  // use `filter` to obtain the same behavior of
`find`n  return arr.filter(check)[0];n}nn/**n * Return the index of the
matching objectn * @methodn * @memberof Popper.Utilsn * @argument {Array}
arrn * @argument propn * @argument valuen * @returns index or -1n
*/nfunction findIndex(arr, prop, value) {n  // use native findIndex if
supportedn  if (Array.prototype.findIndex) {n    return
arr.findIndex(function (cur) {n      return cur[prop] === value;n    });n 
}nn  // use `find` + `indexOf` if `findIndex` isn’t supportedn  var match =
find(arr, function (obj) {n    return obj[prop] === value;n  });n  return
arr.indexOf(match);n}nn/**n * Loop trough the list of modifiers and run
them in order,n * each of them will then edit the data object.n * @methodn
* @memberof Popper.Utilsn * @param {dataObject} datan * @param {Array}
modifiersn * @param {String} ends - Optional modifier name used as stoppern
* @returns {dataObject}n */nfunction runModifiers(modifiers, data, ends) {n
var modifiersToRun = ends === undefined ? modifiers : modifiers.slice(0,
findIndex(modifiers, ‘name’, ends));nn  modifiersToRun.forEach(function
(modifier) {n    if (modifier[‘function’]) {n      // eslint-disable-line
dot-notationn      console.warn(‘`modifier.function` is deprecated, use
`modifier.fn`!’);n    }n    var fn = modifier[‘function’] || modifier.fn;
// eslint-disable-line dot-notationn    if (modifier.enabled &&
isFunction(fn)) {n      // Add properties to offsets to make them a
complete clientRect objectn      // we do this before each modifier to make
sure the previous one doesn’tn      // mess with these valuesn     
data.offsets.popper = getClientRect(data.offsets.popper);n     
data.offsets.reference = getClientRect(data.offsets.reference);nn      data
= fn(data, modifier);n    }n  });nn  return data;n}nn/**n * Updates the
position of the popper, computing the new offsets and applyingn * the new
style.<br />n * Prefer `scheduleUpdate` over `update` because of
performance reasons.n * @methodn * @memberof Poppern */nfunction update()
{n  // if popper is destroyed, don’t perform any further updaten  if
(this.state.isDestroyed) {n    return;n  }nn  var data = {n    instance:
this,n    styles: {},n    arrowStyles: {},n    attributes: {},n    flipped:
false,n    offsets: {}n  };nn  // compute reference element offsetsn 
data.offsets.reference = getReferenceOffsets(this.state, this.popper,
this.reference, this.options.positionFixed);nn  // compute auto placement,
store placement inside the data object,n  // modifiers will be able to edit
`placement` if neededn  // and refer to originalPlacement to know the
original valuen  data.placement =
computeAutoPlacement(this.options.placement, data.offsets.reference,
this.popper, this.reference, this.options.modifiers.flip.boundariesElement,
this.options.modifiers.flip.padding);nn  // store the computed placement
inside `originalPlacement`n  data.originalPlacement = data.placement;nn 
data.positionFixed = this.options.positionFixed;nn  // compute the popper
offsetsn  data.offsets.popper = getPopperOffsets(this.popper,
data.offsets.reference, data.placement);n  data.offsets.popper.position =
this.options.positionFixed ? ‘fixed’ : ‘absolute’;nn  // run the modifiersn
data = runModifiers(this.modifiers, data);nn  // the first `update` will
call `onCreate` callbackn  // the other ones will call `onUpdate` callbackn
if (!this.state.isCreated) {n    this.state.isCreated = true;n   
this.options.onCreate(data);n  } else {n    this.options.onUpdate(data);n 
}n}nn/**n * Helper used to know if the given modifier is enabled.n *
@methodn * @memberof Popper.Utilsn * @returns {Boolean}n */nfunction
isModifierEnabled(modifiers, modifierName) {n  return
modifiers.some(function (_ref) {n    var name = _ref.name,n        enabled
= _ref.enabled;n    return enabled && name === modifierName;n  });n}nn/**n
* Get the prefixed supported property namen * @methodn * @memberof
Popper.Utilsn * @argument {String} property (camelCase)n * @returns
{String} prefixed property (camelCase or PascalCase, depending on the
vendor prefix)n */nfunction getSupportedPropertyName(property) {n  var
prefixes = [false, ‘ms’, ‘Webkit’, ‘Moz’, ‘O’];n  var upperProp =
property.charAt(0).toUpperCase() + property.slice(1);nn  for (var i = 0; i
< prefixes.length; i++) {\n    var prefix = prefixes[i];\n    var toCheck =
prefix ? '' + prefix + upperProp : property;\n    if (typeof
...
this.disableEventListeners();\n\n  // remove the popper if user explicity
asked for the deletion on destroy\n  // do not use `remove` because IE11
...
recalculate popper position\n * when they are triggered. It also won't
trigger onUpdate callback anymore,\n * unless you call `update` method
manually.\n * @method\n * @memberof Popper\n */\nfunction
...
{Object} styles\n * Object with a list of properties and values which will
...
attributes to the given popper\n * @method\n * @memberof Popper.Utils\n *
...
data.attributes - List of attribute properties - values to apply to popper
element\n * @argument {Object} options - Modifiers configuration and
options\n * @returns {Object} The same data object\n */\nfunction
applyStyle(data) {\n  // any property present in `data.styles` will be
applied to the popper,\n  // in this way we can make the 3rd party
modifiers add custom styles to it\n  // Be aware, modifiers could override
the properties defined in the previous\n  // lines of this modifier!\n 
setStyles(data.instance.popper, data.styles);\n\n  // any property present
in `data.attributes` will be applied to the popper,\n  // they will be set
as HTML attributes of the element\n  setAttributes(data.instance.popper,
data.attributes);\n\n  // if arrowElement is defined and arrowStyles has
some properties\n  if (data.arrowElement &&
Object.keys(data.arrowStyles).length) {\n    setStyles(data.arrowElement,
data.arrowStyles);\n  }\n\n  return data;\n}\n\n/**\n * Set the x-placement
attribute before everything else because it could be used\n * to add
margins to the popper margins needs to be calculated to get the\n * correct
popper offsets.\n * @method\n * @memberof Popper.modifiers\n * @param
{HTMLElement} reference - The reference element used to position the
popper\n * @param {HTMLElement} popper - The HTML element used as popper\n
* @param {Object} options - Popper.js options\n */\nfunction
applyStyleOnLoad(reference, popper, options, modifierOptions, state) {\n 
// compute reference element offsets\n  var referenceOffsets =
getReferenceOffsets(state, popper, reference, options.positionFixed);\n\n 
// compute auto placement, store placement inside the data object,\n  //
modifiers will be able to edit `placement` if needed\n  // and refer to
originalPlacement to know the original value\n  var placement =
computeAutoPlacement(options.placement, referenceOffsets, popper,
reference, options.modifiers.flip.boundariesElement,
options.modifiers.flip.padding);\n\n  popper.setAttribute('x-placement',
placement);\n\n  // Apply `position` to popper before anything else
because\n  // without the position applied we can't guarantee correct
computations\n  setStyles(popper, { position: options.positionFixed ?
'fixed' : 'absolute' });\n\n  return options;\n}\n\n/**\n * @function\n *
@memberof Modifiers\n * @argument {Object} data - The data object generated
...
support in Popper.js v2\n\n  var legacyGpuAccelerationOption =
find(data.instance.modifiers, function (modifier) {\n    return
modifier.name === 'applyStyle';\n  }).gpuAcceleration;\n  if
(legacyGpuAccelerationOption !== undefined) {\n    console.warn('WARNING:
`gpuAcceleration` option moved to `computeStyle` modifier and will not be
supported in future versions of Popper.js!');\n  }\n  var gpuAcceleration =
legacyGpuAccelerationOption !== undefined ? legacyGpuAccelerationOption :
options.gpuAcceleration;\n\n  var offsetParent =
getOffsetParent(data.instance.popper);\n  var offsetParentRect =
getBoundingClientRect(offsetParent);\n\n  // Styles\n  var styles = {\n   
position: popper.position\n  };\n\n  // floor sides to avoid blurry text\n 
var offsets = {\n    left: Math.floor(popper.left),\n    top:
Math.floor(popper.top),\n    bottom: Math.floor(popper.bottom),\n    right:
Math.floor(popper.right)\n  };\n\n  var sideA = x === 'bottom' ? 'top' :
'bottom';\n  var sideB = y === 'right' ? 'left' : 'right';\n\n  // if
gpuAcceleration is set to `true` and transform is supported,\n  //  we use
`translate3d` to apply the position to the popper we\n  // automatically
use the supported prefixed version if needed\n  var prefixedProperty =
getSupportedPropertyName('transform');\n\n  // now, let's make a step back
and look at this code closely (wtf?)\n  // If the content of the popper
grows once it's been positioned, it\n  // may happen that the popper gets
misplaced because of the new content\n  // overflowing its reference
element\n  // To avoid this problem, we provide two options (x and y),
which allow\n  // the consumer to define the offset origin.\n  // If we
position a popper on top of a reference element, we can set\n  // `x` to
`top` to make the popper grow towards its top instead of\n  // its
bottom.\n  var left = void 0,\n      top = void 0;\n  if (sideA ===
'bottom') {\n    top = -offsetParentRect.height + offsets.bottom;\n  } else
{\n    top = offsets.top;\n  }\n  if (sideB === 'right') {\n    left =
-offsetParentRect.width + offsets.right;\n  } else {\n    left =
offsets.left;\n  }\n  if (gpuAcceleration && prefixedProperty) {\n   
styles[prefixedProperty] = 'translate3d(' + left + 'px, ' + top + 'px,
0)';\n    styles[sideA] = 0;\n    styles[sideB] = 0;\n    styles.willChange
= 'transform';\n  } else {\n    // othwerise, we use the standard `top`,
`left`, `bottom` and `right` properties\n    var invertTop = sideA ===
'bottom' ? -1 : 1;\n    var invertLeft = sideB === 'right' ? -1 : 1;\n   
styles[sideA] = top * invertTop;\n    styles[sideB] = left * invertLeft;\n 
styles.willChange = sideA + ', ' + sideB;\n  }\n\n  // Attributes\n  var
attributes = {\n    'x-placement': data.placement\n  };\n\n  // Update
`data` attributes, styles and arrowStyles\n  data.attributes = _extends({},
attributes, data.attributes);\n  data.styles = _extends({}, styles,
data.styles);\n  data.arrowStyles = _extends({}, data.offsets.arrow,
data.arrowStyles);\n\n  return data;\n}\n\n/**\n * Helper used to know if
the given modifier depends from another one.<br />n * It checks if the
needed modifier is listed and enabled.n * @methodn * @memberof
Popper.Utilsn * @param {Array} modifiers - list of modifiersn * @param
{String} requestingName - name of requesting modifiern * @param {String}
requestedName - name of requested modifiern * @returns {Boolean}n
*/nfunction isModifierRequired(modifiers, requestingName, requestedName) {n
var requesting = find(modifiers, function (_ref) {n    var name =
_ref.name;n    return name === requestingName;n  });nn  var isRequired =
!!requesting && modifiers.some(function (modifier) {n    return
modifier.name === requestedName && modifier.enabled && modifier.order <
requesting.order;\n  });\n\n  if (!isRequired) {\n    var _requesting = '`'
...
(!isModifierRequired(data.instance.modifiers, 'arrow', 'keepTogether')) {\n
return data;\n  }\n\n  var arrowElement = options.element;\n\n  // if
arrowElement is a string, suppose it's a CSS selector\n  if (typeof
arrowElement === 'string') {\n    arrowElement =
data.instance.popper.querySelector(arrowElement);\n\n    // if arrowElement
is not found, don't run the modifier\n    if (!arrowElement) {\n     
return data;\n    }\n  } else {\n    // if the arrowElement isn't a query
selector we must check that the\n    // provided DOM node is child of its
popper node\n    if (!data.instance.popper.contains(arrowElement)) {\n     
console.warn('WARNING: `arrow.element` must be child of its popper
element!');\n      return data;\n    }\n  }\n\n  var placement =
data.placement.split('-')[0];\n  var _data$offsets = data.offsets,\n     
popper = _data$offsets.popper,\n      reference =
_data$offsets.reference;\n\n  var isVertical = ['left',
'right'].indexOf(placement) !== -1;\n\n  var len = isVertical ? 'height' :
'width';\n  var sideCapitalized = isVertical ? 'Top' : 'Left';\n  var side
= sideCapitalized.toLowerCase();\n  var altSide = isVertical ? 'left' :
'top';\n  var opSide = isVertical ? 'bottom' : 'right';\n  var
arrowElementSize = getOuterSizes(arrowElement)[len];\n\n  //\n  // extends
keepTogether behavior making sure the popper and its\n  // reference have
enough pixels in conjuction\n  //\n\n  // top/left side\n  if
(reference[opSide] - arrowElementSize < popper[side]) {\n   
data.offsets.popper[side] -= popper[side] - (reference[opSide] -
arrowElementSize);\n  }\n  // bottom/right side\n  if (reference[side] +
arrowElementSize > popper[opSide]) {n    data.offsets.popper[side] +=
reference[side] + arrowElementSize - popper[opSide];n  }n 
data.offsets.popper = getClientRect(data.offsets.popper);nn  // compute
center of the poppern  var center = reference[side] + reference[len] / 2 -
arrowElementSize / 2;nn  // Compute the sideValue using the updated popper
offsetsn  // take popper margin in account because we don’t have this info
availablen  var css = getStyleComputedProperty(data.instance.popper);n  var
popperMarginSide = parseFloat(css[‘margin’ + sideCapitalized], 10);n  var
popperBorderSide = parseFloat(css[‘border’ + sideCapitalized + ‘Width’],
10);n  var sideValue = center - data.offsets.popper[side] -
popperMarginSide - popperBorderSide;nn  // prevent arrowElement from being
placed not contiguously to its poppern  sideValue =
Math.max(Math.min(popper[len] - arrowElementSize, sideValue), 0);nn 
data.arrowElement = arrowElement;n  data.offsets.arrow =
(_data$offsets$arrow = {}, defineProperty(_data$offsets$arrow, side,
Math.round(sideValue)), defineProperty(_data$offsets$arrow, altSide, ”),
_data$offsets$arrow);nn  return data;n}nn/**n * Get the opposite placement
variation of the given onen * @methodn * @memberof Popper.Utilsn *
@argument {String} placement variationn * @returns {String} flipped
placement variationn */nfunction getOppositeVariation(variation) {n  if
(variation === ‘end’) {n    return ‘start’;n  } else if (variation ===
‘start’) {n    return ‘end’;n  }n  return variation;n}nn/**n * List of
accepted placements to use as values of the `placement` option.<br />n *
Valid placements are:n * - `auto`n * - `top`n * - `right`n * - `bottom`n *
- `left`n *n * Each placement can have a variation from this list:n * -
`-start`n * - `-end`n *n * Variations are interpreted easily if you think
of them as the left to rightn * written languages. Horizontally (`top` and
`bottom`), `start` is left and `end`n * is right.<br />n * Vertically
(`left` and `right`), `start` is top and `end` is bottom.n *n * Some valid
examples are:n * - `top-end` (on top of reference, right aligned)n * -
`right-start` (on right of reference, top aligned)n * - `bottom` (on
bottom, centered)n * - `auto-right` (on the side with more space available,
alignment depends by placement)n *n * @staticn * @type {Array}n * @enum
{String}n * @readonlyn * @method placementsn * @memberof Poppern */nvar
placements = [‘auto-start’, ‘auto’, ‘auto-end’, ‘top-start’, ‘top’,
‘top-end’, ‘right-start’, ‘right’, ‘right-end’, ‘bottom-end’, ‘bottom’,
‘bottom-start’, ‘left-end’, ‘left’, ‘left-start’];nn// Get rid of `auto`
`auto-start` and `auto-end`nvar validPlacements =
placements.slice(3);nn/**n * Given an initial placement, returns all the
subsequent placementsn * clockwise (or counter-clockwise).n *n * @methodn *
@memberof Popper.Utilsn * @argument {String} placement - A valid placement
(it accepts variations)n * @argument {Boolean} counter - Set to true to
walk the placements counterclockwisen * @returns {Array} placements
including their variationsn */nfunction clockwise(placement) {n  var
counter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1]
: false;nn  var index = validPlacements.indexOf(placement);n  var arr =
validPlacements.slice(index + 1).concat(validPlacements.slice(0, index));n 
return counter ? arr.reverse() : arr;n}nnvar BEHAVIORS = {n  FLIP: ‘flip’,n
CLOCKWISE: ‘clockwise’,n  COUNTERCLOCKWISE: ‘counterclockwise’n};nn/**n *
@functionn * @memberof Modifiersn * @argument {Object} data - The data
object generated by update methodn * @argument {Object} options - Modifiers
configuration and optionsn * @returns {Object} The data object, properly
modifiedn */nfunction flip(data, options) {n  // if `inner` modifier is
enabled, we can’t use the `flip` modifiern  if
(isModifierEnabled(data.instance.modifiers, ‘inner’)) {n    return data;n 
}nn  if (data.flipped && data.placement === data.originalPlacement) {n   
// seems like flip is trying to loop, probably there’s not enough space on
any of the flippable sidesn    return data;n  }nn  var boundaries =
getBoundaries(data.instance.popper, data.instance.reference,
options.padding, options.boundariesElement, data.positionFixed);nn  var
placement = data.placement.split(‘-’)[0];n  var placementOpposite =
getOppositePlacement(placement);n  var variation =
data.placement.split(‘-’)[1] || ”;nn  var flipOrder = [];nn  switch
(options.behavior) {n    case BEHAVIORS.FLIP:n      flipOrder = [placement,
placementOpposite];n      break;n    case BEHAVIORS.CLOCKWISE:n     
flipOrder = clockwise(placement);n      break;n    case
BEHAVIORS.COUNTERCLOCKWISE:n      flipOrder = clockwise(placement, true);n 
break;n    default:n      flipOrder = options.behavior;n  }nn 
flipOrder.forEach(function (step, index) {n    if (placement !== step ||
flipOrder.length === index + 1) {n      return data;n    }nn    placement =
data.placement.split(‘-’)[0];n    placementOpposite =
getOppositePlacement(placement);nn    var popperOffsets =
data.offsets.popper;n    var refOffsets = data.offsets.reference;nn    //
using floor because the reference offsets may contain decimals we are not
going to consider heren    var floor = Math.floor;n    var overlapsRef =
...
floor(popperOffsets.bottom) > floor(refOffsets.top) || placement ===
‘bottom’ && floor(popperOffsets.top) < floor(refOffsets.bottom);\n\n    var
overflowsLeft = floor(popperOffsets.left) < floor(boundaries.left);\n   
var overflowsRight = floor(popperOffsets.right) > floor(boundaries.right);n
var overflowsTop = floor(popperOffsets.top) < floor(boundaries.top);\n   
var overflowsBottom = floor(popperOffsets.bottom) >
floor(boundaries.bottom);nn    var overflowsBoundaries = placement ===
‘left’ && overflowsLeft || placement === ‘right’ && overflowsRight ||
placement === ‘top’ && overflowsTop || placement === ‘bottom’ &&
overflowsBottom;nn    // flip the variation if requiredn    var isVertical
= [‘top’, ‘bottom’].indexOf(placement) !== -1;n    var flippedVariation =
!!options.flipVariations && (isVertical && variation === ‘start’ &&
overflowsLeft || isVertical && variation === ‘end’ && overflowsRight ||
!isVertical && variation === ‘start’ && overflowsTop || !isVertical &&
variation === ‘end’ && overflowsBottom);nn    if (overlapsRef ||
overflowsBoundaries || flippedVariation) {n      // this boolean to detect
any flip loopn      data.flipped = true;nn      if (overlapsRef ||
overflowsBoundaries) {n        placement = flipOrder[index + 1];n      }nn 
if (flippedVariation) {n        variation =
getOppositeVariation(variation);n      }nn      data.placement = placement
+ (variation ? ‘-’ + variation : ”);nn      // this object contains
`position`, we want to preserve it along withn      // any additional
property we may add in the futuren      data.offsets.popper = _extends({},
data.offsets.popper, getPopperOffsets(data.instance.popper,
data.offsets.reference, data.placement));nn      data =
runModifiers(data.instance.modifiers, data, ‘flip’);n    }n  });n  return
data;n}nn/**n * @functionn * @memberof Modifiersn * @argument {Object} data
- The data object generated by update methodn * @argument {Object} options
- Modifiers configuration and optionsn * @returns {Object} The data object,
properly modifiedn */nfunction keepTogether(data) {n  var _data$offsets =
data.offsets,n      popper = _data$offsets.popper,n      reference =
_data$offsets.reference;nn  var placement = data.placement.split(‘-’)[0];n 
var floor = Math.floor;n  var isVertical = [‘top’,
‘bottom’].indexOf(placement) !== -1;n  var side = isVertical ? ‘right’ :
‘bottom’;n  var opSide = isVertical ? ‘left’ : ‘top’;n  var measurement =
isVertical ? ‘width’ : ‘height’;nn  if (popper[side] <
floor(reference[opSide])) {\n    data.offsets.popper[opSide] =
floor(reference[opSide]) - popper[measurement];\n  }\n  if (popper[opSide]
> floor(reference[side])) {n    data.offsets.popper[opSide] =
floor(reference[side]);n  }nn  return data;n}nn/**n * Converts a string
containing value + unit into a px value numbern * @functionn * @memberof
{modifiers~offset}n * @privaten * @argument {String} str - Value + unit
stringn * @argument {String} measurement - `height` or `width`n * @argument
{Object} popperOffsetsn * @argument {Object} referenceOffsetsn * @returns
{Number|String}n * Value in pixels, or original string if no values were
extractedn */nfunction toValue(str, measurement, popperOffsets,
referenceOffsets) {n  // separate value from unitn  var split =
str.match(/((?:\-|+)?\d*\.?\d*)(.*)/);n  var value = +split[1];n  var unit
= split[2];nn  // If it’s not a number it’s an operator, I guessn  if
(!value) {n    return str;n  }nn  if (unit.indexOf(‘%’) === 0) {n    var
element = void 0;n    switch (unit) {n      case ‘%p’:n        element =
popperOffsets;n        break;n      case ‘%’:n      case ‘%r’:n     
default:n        element = referenceOffsets;n    }nn    var rect =
getClientRect(element);n    return rect[measurement] / 100 * value;n  }
else if (unit === ‘vh’ || unit === ‘vw’) {n    // if is a vh or vw, we
calculate the size based on the viewportn    var size = void 0;n    if
(unit === ‘vh’) {n      size =
Math.max(document.documentElement.clientHeight, window.innerHeight || 0);n 
} else {n      size = Math.max(document.documentElement.clientWidth,
window.innerWidth || 0);n    }n    return size / 100 * value;n  } else {n  
// if is an explicit pixel unit, we get rid of the unit and keep the valuen
// if is an implicit unit, it’s px, and we return just the valuen    return
value;n  }n}nn/**n * Parse an `offset` string to extrapolate `x` and `y`
numeric offsets.n * @functionn * @memberof {modifiers~offset}n * @privaten
* @argument {String} offsetn * @argument {Object} popperOffsetsn *
@argument {Object} referenceOffsetsn * @argument {String} basePlacementn *
@returns {Array} a two cells array with x and y offsets in numbersn
*/nfunction parseOffset(offset, popperOffsets, referenceOffsets,
basePlacement) {n  var offsets = [0, 0];nn  // Use height if placement is
left or right and index is 0 otherwise use widthn  // in this way the first
offset will use an axis and the second onen  // will use the other onen 
var useHeight = [‘right’, ‘left’].indexOf(basePlacement) !== -1;nn  //
Split the offset string to obtain a list of values and operandsn  // The
regex addresses values with the plus or minus sign in front (+10, -20,
etc)n  var fragments = offset.split(/(+|\-)/).map(function (frag) {n   
return frag.trim();n  });nn  // Detect if the offset string contains a pair
of values or a single onen  // they could be separated by comma or spacen 
var divider = fragments.indexOf(find(fragments, function (frag) {n   
return frag.search(/,|\s/) !== -1;n  }));nn  if (fragments[divider] &&
fragments[divider].indexOf(‘,’) === -1) {n    console.warn(‘Offsets
separated by white space(s) are deprecated, use a comma (,) instead.’);n 
}nn  // If divider is found, we divide the list of values and operands to
dividen  // them by ofset X and Y.n  var splitRegex = /\s*,\s*|\s+/;n  var
ops = divider !== -1 ? [fragments.slice(0,
divider).concat([fragments[divider].split(splitRegex)[0]]),
[fragments[divider].split(splitRegex)[1]].concat(fragments.slice(divider +
1))] : [fragments];nn  // Convert the values with units to absolute pixels
to allow our computationsn  ops = ops.map(function (op, index) {n    //
Most of the units rely on the orientation of the poppern    var measurement
= (index === 1 ? !useHeight : useHeight) ? ‘height’ : ‘width’;n    var
mergeWithPrevious = false;n    return opn    // This aggregates any `+` or
`-` sign that aren’t considered operatorsn    // e.g.: 10 + +5 => [10, +,
+5]n    .reduce(function (a, b) {n      if (a[a.length - 1] === ” && [‘+’,
‘-’].indexOf(b) !== -1) {n        a[a.length - 1] = b;n       
mergeWithPrevious = true;n        return a;n      } else if
(mergeWithPrevious) {n        a[a.length - 1] += b;n       
mergeWithPrevious = false;n        return a;n      } else {n        return
a.concat(b);n      }n    }, [])n    // Here we convert the string values
into number values (in px)n    .map(function (str) {n      return
toValue(str, measurement, popperOffsets, referenceOffsets);n    });n  });nn
// Loop trough the offsets arrays and execute the operationsn 
ops.forEach(function (op, index) {n    op.forEach(function (frag, index2)
{n      if (isNumeric(frag)) {n        offsets[index] += frag * (op[index2
- 1] === ‘-’ ? -1 : 1);n      }n    });n  });n  return offsets;n}nn/**n *
@functionn * @memberof Modifiersn * @argument {Object} data - The data
object generated by update methodn * @argument {Object} options - Modifiers
configuration and optionsn * @argument {Number|String} options.offset=0n *
The offset value as described in the modifier descriptionn * @returns
{Object} The data object, properly modifiedn */nfunction offset(data, _ref)
{n  var offset = _ref.offset;n  var placement = data.placement,n     
_data$offsets = data.offsets,n      popper = _data$offsets.popper,n     
reference = _data$offsets.reference;nn  var basePlacement =
placement.split(‘-’)[0];nn  var offsets = void 0;n  if (isNumeric(+offset))
{n    offsets = [+offset, 0];n  } else {n    offsets = parseOffset(offset,
popper, reference, basePlacement);n  }nn  if (basePlacement === ‘left’) {n 
popper.top += offsets[0];n    popper.left -= offsets[1];n  } else if
(basePlacement === ‘right’) {n    popper.top += offsets[0];n    popper.left
+= offsets[1];n  } else if (basePlacement === ‘top’) {n    popper.left +=
offsets[0];n    popper.top -= offsets[1];n  } else if (basePlacement ===
‘bottom’) {n    popper.left += offsets[0];n    popper.top += offsets[1];n 
}nn  data.popper = popper;n  return data;n}nn/**n * @functionn * @memberof
Modifiersn * @argument {Object} data - The data object generated by
`update` methodn * @argument {Object} options - Modifiers configuration and
optionsn * @returns {Object} The data object, properly modifiedn
*/nfunction preventOverflow(data, options) {n  var boundariesElement =
options.boundariesElement || getOffsetParent(data.instance.popper);nn  //
If offsetParent is the reference element, we really want ton  // go one
step up and use the next offsetParent as reference ton  // avoid to make
this modifier completely useless and look like brokenn  if
(data.instance.reference === boundariesElement) {n    boundariesElement =
getOffsetParent(boundariesElement);n  }nn  var boundaries =
getBoundaries(data.instance.popper, data.instance.reference,
options.padding, boundariesElement, data.positionFixed);n 
options.boundaries = boundaries;nn  var order = options.priority;n  var
popper = data.offsets.popper;nn  var check = {n    primary: function
primary(placement) {n      var value = popper[placement];n      if
(popper[placement] < boundaries[placement] && !options.escapeWithReference)
{\n        value = Math.max(popper[placement], boundaries[placement]);\n   
}\n      return defineProperty({}, placement, value);\n    },\n   
secondary: function secondary(placement) {\n      var mainSide = placement
=== 'right' ? 'left' : 'top';\n      var value = popper[mainSide];\n     
if (popper[placement] > boundaries[placement] &&
!options.escapeWithReference) {n        value = Math.min(popper[mainSide],
boundaries[placement] - (placement === ‘right’ ? popper.width :
popper.height));n      }n      return defineProperty({}, mainSide, value);n
}n  };nn  order.forEach(function (placement) {n    var side = [‘left’,
‘top’].indexOf(placement) !== -1 ? ‘primary’ : ‘secondary’;n    popper =
_extends({}, popper, check[side](placement));n  });nn  data.offsets.popper
= popper;nn  return data;n}nn/**n * @functionn * @memberof Modifiersn *
@argument {Object} data - The data object generated by `update` methodn *
@argument {Object} options - Modifiers configuration and optionsn *
@returns {Object} The data object, properly modifiedn */nfunction
shift(data) {n  var placement = data.placement;n  var basePlacement =
placement.split(‘-’)[0];n  var shiftvariation = placement.split(‘-’)[1];nn 
// if shift shiftvariation is specified, run the modifiern  if
(shiftvariation) {n    var _data$offsets = data.offsets,n        reference
= _data$offsets.reference,n        popper = _data$offsets.popper;nn    var
isVertical = [‘bottom’, ‘top’].indexOf(basePlacement) !== -1;n    var side
= isVertical ? ‘left’ : ‘top’;n    var measurement = isVertical ? ‘width’ :
‘height’;nn    var shiftOffsets = {n      start: defineProperty({}, side,
reference[side]),n      end: defineProperty({}, side, reference[side] +
reference[measurement] - popper[measurement])n    };nn   
data.offsets.popper = _extends({}, popper, shiftOffsets[shiftvariation]);n 
}nn  return data;n}nn/**n * @functionn * @memberof Modifiersn * @argument
{Object} data - The data object generated by update methodn * @argument
{Object} options - Modifiers configuration and optionsn * @returns {Object}
The data object, properly modifiedn */nfunction hide(data) {n  if
(!isModifierRequired(data.instance.modifiers, ‘hide’, ‘preventOverflow’))
{n    return data;n  }nn  var refRect = data.offsets.reference;n  var bound
= find(data.instance.modifiers, function (modifier) {n    return
modifier.name === ‘preventOverflow’;n  }).boundaries;nn  if (refRect.bottom
< bound.top || refRect.left > bound.right || refRect.top > bound.bottom ||
refRect.right < bound.left) {\n    // Avoid unnecessary DOM access if
visibility hasn't changed\n    if (data.hide === true) {\n      return
data;\n    }\n\n    data.hide = true;\n   
data.attributes['x-out-of-boundaries'] = '';\n  } else {\n    // Avoid
unnecessary DOM access if visibility hasn't changed\n    if (data.hide ===
false) {\n      return data;\n    }\n\n    data.hide = false;\n   
...
each modifier can have a function of this type assigned\n * to its `fn`
property.<br />n * These functions will be called on each update, this
means that you mustn * make sure they are performant enough to avoid
performance bottlenecks.n *n * @function ModifierFnn * @argument
{dataObject} data - The data object generated by `update` methodn *
@argument {Object} options - Modifiers configuration and optionsn *
@returns {dataObject} The data object, properly modifiedn */nn/**n *
Modifiers are plugins used to alter the behavior of your poppers.<br />n *
Popper.js uses a set of 9 modifiers to provide all the basic
functionalitiesn * needed by the library.n *n * Usually you don’t want to
override the `order`, `fn` and `onLoad` props.n * All the other properties
are configurations that could be tweaked.n * @namespace modifiersn */nvar
modifiers = {n  /**n   * Modifier used to shift the popper on the start or
end of its referencen   * element.<br />n   * It will read the variation of
the `placement` property.<br />n   * It can be one either `-end` or
`-start`.n   * @memberof modifiersn   * @innern   */n  shift: {n    /**
@prop {number} order=100 - Index used to define the order of execution */n 
order: 100,n    /** @prop {Boolean} enabled=true - Whether the modifier is
enabled or not */n    enabled: true,n    /** @prop {ModifierFn} */n    fn:
shiftn  },nn  /**n   * The `offset` modifier can shift your popper on both
its axis.n   *n   * It accepts the following units:n   * - `px` or
unitless, interpreted as pixelsn   * - `%` or `%r`, percentage relative to
the length of the reference elementn   * - `%p`, percentage relative to the
length of the popper elementn   * - `vw`, CSS viewport width unitn   * -
`vh`, CSS viewport height unitn   *n   * For length is intended the main
axis relative to the placement of the popper.<br />n   * This means that if
the placement is `top` or `bottom`, the length will be then   * `width`. In
case of `left` or `right`, it will be the height.n   *n   * You can provide
a single value (as `Number` or `String`), or a pair of valuesn   * as
`String` divided by a comma or one (or more) white spaces.<br />n   * The
latter is a deprecated method because it leads to confusion and will ben  
* removed in v2.<br />n   * Additionally, it accepts additions and
subtractions between different units.n   * Note that multiplications and
divisions aren’t supported.n   *n   * Valid examples are:n   * “`n   * 10n 
* ‘10%’n   * ‘10, 10’n   * ‘10%, 10’n   * ‘10 + 10%’n   * ‘10 - 5vh + 3%’n 
* ‘-10px + 5vh, 5px - 6%’n   * “`n   * > *NB*: If you desire to apply
offsets to your poppers in a way that may make them overlapn   * > with
their reference element, unfortunately, you will have to disable the `flip`
modifier.n   * > More on this [reading this
issue](https://github.com/FezVrasta/popper.js/issues/373)n   *n   *
@memberof modifiersn   * @innern   */n  offset: {n    /** @prop {number}
order=200 - Index used to define the order of execution */n    order: 200,n
/** @prop {Boolean} enabled=true - Whether the modifier is enabled or not
*/n    enabled: true,n    /** @prop {ModifierFn} */n    fn: offset,n    /**
@prop {Number|String} offset=0n     * The offset value as described in the
modifier descriptionn     */n    offset: 0n  },nn  /**n   * Modifier used
to prevent the popper from being positioned outside the boundary.n   *n   *
An scenario exists where the reference itself is not within the
boundaries.<br />n   * We can say it has "escaped the boundaries" — or just
"escaped".<br />n   * In this case we need to decide whether the popper
should either:n   *n   * - detach from the reference and remain "trapped"
in the boundaries, orn   * - if it should ignore the boundary and "escape
with its reference"n   *n   * When `escapeWithReference` is set to`true`
and reference is completelyn   * outside its boundaries, the popper will
overflow (or completely leave)n   * the boundaries in order to remain
attached to the edge of the reference.n   *n   * @memberof modifiersn   *
@innern   */n  preventOverflow: {n    /** @prop {number} order=300 - Index
used to define the order of execution */n    order: 300,n    /** @prop
{Boolean} enabled=true - Whether the modifier is enabled or not */n   
enabled: true,n    /** @prop {ModifierFn} */n    fn: preventOverflow,n   
/**n     * @prop {Array} [priority=[‘left’,‘right’,‘top’,‘bottom’]]n     *
Popper will try to prevent overflow following these priorities by default,n
* then, it could overflow on the left and on top of the
`boundariesElement`n     */n    priority: [‘left’, ‘right’, ‘top’,
‘bottom’],n    /**n     * @prop {number} padding=5n     * Amount of pixel
used to define a minimum distance between the boundariesn     * and the
popper this makes sure the popper has always a little paddingn     *
between the edges of its containern     */n    padding: 5,n    /**n     *
@prop {String|HTMLElement} boundariesElement=‘scrollParent’n     *
Boundaries used by the modifier, can be `scrollParent`, `window`,n     *
`viewport` or any DOM element.n     */n    boundariesElement:
‘scrollParent’n  },nn  /**n   * Modifier used to make sure the reference
and its popper stay near eachothersn   * without leaving any gap between
the two. Expecially useful when the arrow isn   * enabled and you want to
assure it to point to its reference element.n   * It cares only about the
first axis, you can still have poppers with marginn   * between the popper
and its reference element.n   * @memberof modifiersn   * @innern   */n 
keepTogether: {n    /** @prop {number} order=400 - Index used to define the
order of execution */n    order: 400,n    /** @prop {Boolean} enabled=true
- Whether the modifier is enabled or not */n    enabled: true,n    /**
@prop {ModifierFn} */n    fn: keepTogethern  },nn  /**n   * This modifier
is used to move the `arrowElement` of the popper to maken   * sure it is
positioned between the reference element and its popper element.n   * It
will read the outer size of the `arrowElement` node to detect how manyn   *
pixels of conjuction are needed.n   *n   * It has no effect if no
`arrowElement` is provided.n   * @memberof modifiersn   * @innern   */n 
arrow: {n    /** @prop {number} order=500 - Index used to define the order
of execution */n    order: 500,n    /** @prop {Boolean} enabled=true -
Whether the modifier is enabled or not */n    enabled: true,n    /** @prop
{ModifierFn} */n    fn: arrow,n    /** @prop {String|HTMLElement}
element=‘[x-arrow]’ - Selector or node used as arrow */n    element:
‘[x-arrow]’n  },nn  /**n   * Modifier used to flip the popper’s placement
when it starts to overlap itsn   * reference element.n   *n   * Requires
the `preventOverflow` modifier before it in order to work.n   *n   *
*NOTE:* this modifier will interrupt the current update cycle and willn   *
restart it if it detects the need to flip the placement.n   * @memberof
modifiersn   * @innern   */n  flip: {n    /** @prop {number} order=600 -
Index used to define the order of execution */n    order: 600,n    /**
@prop {Boolean} enabled=true - Whether the modifier is enabled or not */n  
enabled: true,n    /** @prop {ModifierFn} */n    fn: flip,n    /**n     *
@prop {String|Array} behavior=‘flip’n     * The behavior used to change the
popper’s placement. It can be one ofn     * `flip`, `clockwise`,
`counterclockwise` or an array with a list of validn     * placements (with
optional variations).n     */n    behavior: ‘flip’,n    /**n     * @prop
{number} padding=5n     * The popper will flip if it hits the edges of the
`boundariesElement`n     */n    padding: 5,n    /**n     * @prop
{String|HTMLElement} boundariesElement=‘viewport’n     * The element which
will define the boundaries of the popper position,n     * the popper will
never be placed outside of the defined boundariesn     * (except if
keepTogether is enabled)n     */n    boundariesElement: ‘viewport’n  },nn 
/**n   * Modifier used to make the popper flow toward the inner of the
reference element.n   * By default, when this modifier is disabled, the
popper will be placed outsiden   * the reference element.n   * @memberof
modifiersn   * @innern   */n  inner: {n    /** @prop {number} order=700 -
Index used to define the order of execution */n    order: 700,n    /**
@prop {Boolean} enabled=false - Whether the modifier is enabled or not */n 
enabled: false,n    /** @prop {ModifierFn} */n    fn: innern  },nn  /**n  
* Modifier used to hide the popper when its reference element is outside of
then   * popper boundaries. It will set a `x-out-of-boundaries` attribute
which cann   * be used to hide with a CSS selector the popper when its
reference isn   * out of boundaries.n   *n   * Requires the
`preventOverflow` modifier before it in order to work.n   * @memberof
modifiersn   * @innern   */n  hide: {n    /** @prop {number} order=800 -
Index used to define the order of execution */n    order: 800,n    /**
@prop {Boolean} enabled=true - Whether the modifier is enabled or not */n  
enabled: true,n    /** @prop {ModifierFn} */n    fn: hiden  },nn  /**n   *
Computes the style that will be applied to the popper element to getsn   *
properly positioned.n   *n   * Note that this modifier will not touch the
DOM, it just prepares the stylesn   * so that `applyStyle` modifier can
apply it. This separation is usefuln   * in case you need to replace
`applyStyle` with a custom implementation.n   *n   * This modifier has
`850` as `order` value to maintain backward compatibilityn   * with
previous versions of Popper.js. Expect the modifiers ordering methodn   *
to change in future major versions of the library.n   *n   * @memberof
modifiersn   * @innern   */n  computeStyle: {n    /** @prop {number}
order=850 - Index used to define the order of execution */n    order: 850,n
/** @prop {Boolean} enabled=true - Whether the modifier is enabled or not
*/n    enabled: true,n    /** @prop {ModifierFn} */n    fn: computeStyle,n 
/**n     * @prop {Boolean} gpuAcceleration=truen     * If true, it uses the
CSS 3d transformation to position the popper.n     * Otherwise, it will use
the `top` and `left` properties.n     */n    gpuAcceleration: true,n   
/**n     * @prop {string} [x=‘bottom’]n     * Where to anchor the X axis
(`bottom` or `top`). AKA X offset origin.n     * Change this if your popper
should grow in a direction different from `bottom`n     */n    x:
‘bottom’,n    /**n     * @prop {string} [x=‘left’]n     * Where to anchor
the Y axis (`left` or `right`). AKA Y offset origin.n     * Change this if
your popper should grow in a direction different from `right`n     */n   
y: ‘right’n  },nn  /**n   * Applies the computed styles to the popper
element.n   *n   * All the DOM manipulations are limited to this modifier.
This is useful in casen   * you want to integrate Popper.js inside a
framework or view library and youn   * want to delegate all the DOM
manipulations to it.n   *n   * Note that if you disable this modifier, you
must make sure the popper elementn   * has its position set to `absolute`
before Popper.js can do its work!n   *n   * Just disable this modifier and
define you own to achieve the desired effect.n   *n   * @memberof
modifiersn   * @innern   */n  applyStyle: {n    /** @prop {number}
order=900 - Index used to define the order of execution */n    order: 900,n
/** @prop {Boolean} enabled=true - Whether the modifier is enabled or not
*/n    enabled: true,n    /** @prop {ModifierFn} */n    fn: applyStyle,n   
/** @prop {Function} */n    onLoad: applyStyleOnLoad,n    /**n     *
@deprecated since version 1.10.0, the property moved to `computeStyle`
modifiern     * @prop {Boolean} gpuAcceleration=truen     * If true, it
uses the CSS 3d transformation to position the popper.n     * Otherwise, it
will use the `top` and `left` properties.n     */n    gpuAcceleration:
undefinedn  }n};nn/**n * The `dataObject` is an object containing all the
informations used by Popper.jsn * this object get passed to modifiers and
to the `onCreate` and `onUpdate` callbacks.n * @name dataObjectn *
@property {Object} data.instance The Popper.js instancen * @property
{String} data.placement Placement applied to poppern * @property {String}
data.originalPlacement Placement originally defined on initn * @property
{Boolean} data.flipped True if popper has been flipped by flip modifiern *
@property {Boolean} data.hide True if the reference element is out of
boundaries, useful to know when to hide the popper.n * @property
{HTMLElement} data.arrowElement Node used as arrow by arrow modifiern *
@property {Object} data.styles Any CSS property defined here will be
applied to the popper, it expects the JavaScript nomenclature (eg.
`marginBottom`)n * @property {Object} data.arrowStyles Any CSS property
defined here will be applied to the popper arrow, it expects the JavaScript
nomenclature (eg. `marginBottom`)n * @property {Object} data.boundaries
Offsets of the popper boundariesn * @property {Object} data.offsets The
measurements of popper, reference and arrow elements.n * @property {Object}
data.offsets.popper `top`, `left`, `width`, `height` valuesn * @property
{Object} data.offsets.reference `top`, `left`, `width`, `height` valuesn *
@property {Object} data.offsets.arrow] `top` and `left` offsets, only one
of them will be different from 0n */nn/**n * Default options provided to
Popper.js constructor.<br />n * These can be overriden using the `options`
argument of Popper.js.<br />n * To override an option, simply pass as 3rd
argument an object with the samen * structure of this object, example:n *
“`n * new Popper(ref, pop, {n *   modifiers: {n *     preventOverflow: {
enabled: false }n *   }n * })n * “`n * @type {Object}n * @staticn *
@memberof Poppern */nvar Defaults = {n  /**n   * Popper’s placementn   *
@prop {Popper.placements} placement=‘bottom’n   */n  placement: ‘bottom’,nn
/**n   * Set this to true if you want popper to position it self in ‘fixed’
moden   * @prop {Boolean} positionFixed=falsen   */n  positionFixed:
false,nn  /**n   * Whether events (resize, scroll) are initially enabledn  
* @prop {Boolean} eventsEnabled=truen   */n  eventsEnabled: true,nn  /**n  
* Set to true if you want to automatically remove the popper whenn   * you
call the `destroy` method.n   * @prop {Boolean} removeOnDestroy=falsen  
*/n  removeOnDestroy: false,nn  /**n   * Callback called when the popper is
created.<br />n   * By default, is set to no-op.<br />n   * Access
Popper.js instance with `data.instance`.n   * @prop {onCreate}n   */n 
onCreate: function onCreate() {},nn  /**n   * Callback called when the
popper is updated, this callback is not calledn   * on the
initialization/creation of the popper, but only on subsequentn   *
updates.<br />n   * By default, is set to no-op.<br />n   * Access
Popper.js instance with `data.instance`.n   * @prop {onUpdate}n   */n 
onUpdate: function onUpdate() {},nn  /**n   * List of modifiers used to
modify the offsets before they are applied to the popper.n   * They provide
most of the functionalities of Popper.jsn   * @prop {modifiers}n   */n 
modifiers: modifiersn};nn/**n * @callback onCreaten * @param {dataObject}
datan */nn/**n * @callback onUpdaten * @param {dataObject} datan */nn//
Utilsn// Methodsnvar Popper = function () {n  /**n   * Create a new
Popper.js instancen   * @class Poppern   * @param
{HTMLElement|referenceObject} reference - The reference element used to
position the poppern   * @param {HTMLElement} popper - The HTML element
used as popper.n   * @param {Object} options - Your custom options to
override the ones defined in [Defaults](#defaults)n   * @return {Object}
instance - The generated Popper.js instancen   */n  function
Popper(reference, popper) {n    var _this = this;nn    var options =
arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};n   
classCallCheck(this, Popper);nn    this.scheduleUpdate = function () {n    
return requestAnimationFrame(_this.update);n    };nn    // make update()
debounced, so that it only runs at most once-per-tickn    this.update =
debounce(this.update.bind(this));nn    // with {} we create a new object
with the options inside itn    this.options = _extends({}, Popper.Defaults,
options);nn    // init staten    this.state = {n      isDestroyed: false,n 
isCreated: false,n      scrollParents: []n    };nn    // get reference and
popper elements (allow jQuery wrappers)n    this.reference = reference &&
reference.jquery ? reference[0] : reference;n    this.popper = popper &&
popper.jquery ? popper[0] : popper;nn    // Deep merge modifiers optionsn  
this.options.modifiers = {};n    Object.keys(_extends({},
Popper.Defaults.modifiers, options.modifiers)).forEach(function (name) {n  
_this.options.modifiers[name] = _extends({},
Popper.Defaults.modifiers[name] || {}, options.modifiers ?
options.modifiers[name] : {});n    });nn    // Refactoring modifiers’ list
(Object => Array)n    this.modifiers =
Object.keys(this.options.modifiers).map(function (name) {n      return
_extends({n        name: namen      }, _this.options.modifiers[name]);n   
})n    // sort the modifiers by ordern    .sort(function (a, b) {n     
return a.order - b.order;n    });nn    // modifiers have the ability to
execute arbitrary code when Popper.js get initedn    // such code is
executed in the same order of its modifiern    // they could add new
properties to their options configurationn    // BE AWARE: don’t add
options to `options.modifiers.name` but to `modifierOptions`!n   
this.modifiers.forEach(function (modifierOptions) {n      if
(modifierOptions.enabled && isFunction(modifierOptions.onLoad)) {n       
modifierOptions.onLoad(_this.reference, _this.popper, _this.options,
modifierOptions, _this.state);n      }n    });nn    // fire the first
update to position the popper in the right placen    this.update();nn   
var eventsEnabled = this.options.eventsEnabled;n    if (eventsEnabled) {n  
// setup event listeners, they will take care of update the position in
specific situationsn      this.enableEventListeners();n    }nn   
this.state.eventsEnabled = eventsEnabled;n  }nn  // We can’t use class
properties because they don’t get listed in then  // class prototype and
break stuff like Sinon stubsnnn  createClass(Popper, [{n    key: ‘update’,n
value: function update$$1() {n      return update.call(this);n    }n  }, {n
key: ‘destroy’,n    value: function destroy$$1() {n      return
destroy.call(this);n    }n  }, {n    key: ‘enableEventListeners’,n   
value: function enableEventListeners$$1() {n      return
enableEventListeners.call(this);n    }n  }, {n    key:
‘disableEventListeners’,n    value: function disableEventListeners$$1() {n 
return disableEventListeners.call(this);n    }nn    /**n     * Schedule an
update, it will run on the next UI update availablen     * @method
scheduleUpdaten     * @memberof Poppern     */nnn    /**n     * Collection
of utilities useful when writing custom modifiers.n     * Starting from
version 1.7, this method is available only if youn     * include
`popper-utils.js` before `popper.js`.n     *n     * *DEPRECATION*: This way
to access PopperUtils is deprecatedn     * and will be removed in v2! Use
the PopperUtils module directly instead.n     * Due to the high instability
of the methods contained in Utils, we can’tn     * guarantee them to follow
semver. Use them at your own risk!n     * @staticn     * @privaten     *
@type {Object}n     * @deprecated since version 1.8n     * @member Utilsn  
* @memberof Poppern     */nn  }]);n  return Popper;n}();nn/**n * The
`referenceObject` is an object that provides an interface compatible with
Popper.jsn * and lets you use it as replacement of a real DOM node.<br />n
* You can use this method to position a popper relatively to a set of
coordinatesn * in case you don’t have a DOM node to use as reference.n *n *
“`n * new Popper(referenceObject, popperNode);n * “`n *n * NB: This feature
isn’t supported in Internet Explorer 10n * @name referenceObjectn *
@property {Function} data.getBoundingClientRectn * A function that returns
a set of coordinates compatible with the native `getBoundingClientRect`
method.n * @property {number} data.clientWidthn * An ES6 getter that will
return the width of the virtual reference element.n * @property {number}
data.clientHeightn * An ES6 getter that will return the height of the
virtual reference element.n */nnnPopper.Utils = (typeof window !==
‘undefined’ ? window : global).PopperUtils;nPopper.placements =
placements;nPopper.Defaults = Defaults;nnexport default Popper;n//#
sourceMappingURL=popper.js.mapn”,“import $ from ‘jquery’nimport Popper from
‘popper.js’nimport Util from ‘./util’nn/**n * ————————————————————————–n *
Bootstrap (v4.1.0): dropdown.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Dropdown = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME                     = ‘dropdown’n  const VERSION          
= ‘4.1.0’n  const DATA_KEY                 = ‘bs.dropdown’n  const
EVENT_KEY                = `.${DATA_KEY}`n  const DATA_API_KEY            
= ‘.data-api’n  const JQUERY_NO_CONFLICT       = $.fn[NAME]n  const
ESCAPE_KEYCODE           = 27 // KeyboardEvent.which value for Escape (Esc)
keyn  const SPACE_KEYCODE            = 32 // KeyboardEvent.which value for
space keyn  const TAB_KEYCODE              = 9 // KeyboardEvent.which value
for tab keyn  const ARROW_UP_KEYCODE         = 38 // KeyboardEvent.which
value for up arrow keyn  const ARROW_DOWN_KEYCODE       = 40 //
KeyboardEvent.which value for down arrow keyn  const
RIGHT_MOUSE_BUTTON_WHICH = 3 // MouseEvent.which value for the right button
(assuming a right-handed mouse)n  const REGEXP_KEYDOWN           = new
RegExp(`${ARROW_UP_KEYCODE}|${ARROW_DOWN_KEYCODE}|${ESCAPE_KEYCODE}`)nn 
const Event = {n    HIDE             : `hide${EVENT_KEY}`,n    HIDDEN      
: `hidden${EVENT_KEY}`,n    SHOW             : `show${EVENT_KEY}`,n   
SHOWN            : `shown${EVENT_KEY}`,n    CLICK            :
`click${EVENT_KEY}`,n    CLICK_DATA_API   :
`click${EVENT_KEY}${DATA_API_KEY}`,n    KEYDOWN_DATA_API :
`keydown${EVENT_KEY}${DATA_API_KEY}`,n    KEYUP_DATA_API   :
`keyup${EVENT_KEY}${DATA_API_KEY}`n  }nn  const ClassName = {n    DISABLED 
: ‘disabled’,n    SHOW      : ‘show’,n    DROPUP    : ‘dropup’,n   
DROPRIGHT : ‘dropright’,n    DROPLEFT  : ‘dropleft’,n    MENURIGHT :
‘dropdown-menu-right’,n    MENULEFT  : ‘dropdown-menu-left’,n   
POSITION_STATIC : ‘position-static’n  }nn  const Selector = {n   
DATA_TOGGLE   : ‘[data-toggle="dropdown"]’,n    FORM_CHILD    : ‘.dropdown
form’,n    MENU          : ‘.dropdown-menu’,n    NAVBAR_NAV    :
‘.navbar-nav’,n    VISIBLE_ITEMS : ‘.dropdown-menu
.dropdown-item:not(.disabled):not(:disabled)’n  }nn  const AttachmentMap =
{n    TOP       : ‘top-start’,n    TOPEND    : ‘top-end’,n    BOTTOM    :
‘bottom-start’,n    BOTTOMEND : ‘bottom-end’,n    RIGHT     :
‘right-start’,n    RIGHTEND  : ‘right-end’,n    LEFT      : ‘left-start’,n 
LEFTEND   : ‘left-end’n  }nn  const Default = {n    offset      : 0,n   
flip        : true,n    boundary    : ‘scrollParent’,n    reference   :
‘toggle’,n    display     : ‘dynamic’n  }nn  const DefaultType = {n   
offset      : ‘(number|string|function)’,n    flip        : ‘boolean’,n   
boundary    : ‘(string|element)’,n    reference   : ‘(string|element)’,n   
display     : ‘string’n  }nn  /**n   * ————————————————————————n   * Class
Definitionn   * ————————————————————————n   */nn  class Dropdown {n   
constructor(element, config) {n      this._element  = elementn     
this._popper   = nulln      this._config   = this._getConfig(config)n     
this._menu     = this._getMenuElement()n      this._inNavbar =
this._detectNavbar()nn      this._addEventListeners()n    }nn    //
Gettersnn    static get VERSION() {n      return VERSIONn    }nn    static
get Default() {n      return Defaultn    }nn    static get DefaultType() {n
return DefaultTypen    }nn    // Publicnn    toggle() {n      if
(this._element.disabled || $(this._element).hasClass(ClassName.DISABLED))
{n        returnn      }nn      const parent   =
Dropdown._getParentFromElement(this._element)n      const isActive =
$(this._menu).hasClass(ClassName.SHOW)nn      Dropdown._clearMenus()nn     
if (isActive) {n        returnn      }nn      const relatedTarget = {n     
relatedTarget: this._elementn      }n      const showEvent =
$.Event(Event.SHOW, relatedTarget)nn      $(parent).trigger(showEvent)nn   
if (showEvent.isDefaultPrevented()) {n        returnn      }nn      //
Disable totally Popper.js for Dropdown in Navbarn      if (!this._inNavbar)
{n        /**n         * Check for Popper dependencyn         * Popper -
https://popper.js.orgn         */n        if (typeof Popper ===
‘undefined’) {n          throw new TypeError(‘Bootstrap dropdown require
Popper.js (https://popper.js.org)’)n        }nn        let referenceElement
= this._elementnn        if (this._config.reference === ‘parent’) {n       
referenceElement = parentn        } else if
(Util.isElement(this._config.reference)) {n          referenceElement =
this._config.referencenn          // Check if it’s jQuery elementn         
if (typeof this._config.reference.jquery !== ‘undefined’) {n           
referenceElement = this._config.reference[0]n          }n        }nn       
// If boundary is not `scrollParent`, then set position to `static`n       
// to allow the menu to "escape" the scroll parent’s boundariesn        //
https://github.com/twbs/bootstrap/issues/24251n        if
(this._config.boundary !== ‘scrollParent’) {n         
$(parent).addClass(ClassName.POSITION_STATIC)n        }n       
this._popper = new Popper(referenceElement, this._menu,
this._getPopperConfig())n      }nn      // If this is a touch-enabled
device we add extran      // empty mouseover listeners to the body’s
immediate children;n      // only needed because of broken event delegation
on iOSn      //
https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.htmln     
if (‘ontouchstart’ in document.documentElement &&n        
$(parent).closest(Selector.NAVBAR_NAV).length === 0) {n       
$(document.body).children().on(‘mouseover’, null, $.noop)n      }nn     
this._element.focus()n      this._element.setAttribute(‘aria-expanded’,
true)nn      $(this._menu).toggleClass(ClassName.SHOW)n      $(parent)n    
.toggleClass(ClassName.SHOW)n        .trigger($.Event(Event.SHOWN,
relatedTarget))n    }nn    dispose() {n      $.removeData(this._element,
DATA_KEY)n      $(this._element).off(EVENT_KEY)n      this._element = nulln
this._menu = nulln      if (this._popper !== null) {n       
this._popper.destroy()n        this._popper = nulln      }n    }nn   
update() {n      this._inNavbar = this._detectNavbar()n      if
(this._popper !== null) {n        this._popper.scheduleUpdate()n      }n   
}nn    // Privatenn    _addEventListeners() {n     
$(this._element).on(Event.CLICK, (event) => {n       
event.preventDefault()n        event.stopPropagation()n       
this.toggle()n      })n    }nn    _getConfig(config) {n      config = {n   
…this.constructor.Default,n        …$(this._element).data(),n       
…confign      }nn      Util.typeCheckConfig(n        NAME,n        config,n
this.constructor.DefaultTypen      )nn      return confign    }nn   
_getMenuElement() {n      if (!this._menu) {n        const parent =
Dropdown._getParentFromElement(this._element)n        this._menu =
$(parent).find(Selector.MENU)[0]n      }n      return this._menun    }nn   
_getPlacement() {n      const $parentDropdown = $(this._element).parent()n 
let placement = AttachmentMap.BOTTOMnn      // Handle dropupn      if
($parentDropdown.hasClass(ClassName.DROPUP)) {n        placement =
AttachmentMap.TOPn        if ($(this._menu).hasClass(ClassName.MENURIGHT))
{n          placement = AttachmentMap.TOPENDn        }n      } else if
($parentDropdown.hasClass(ClassName.DROPRIGHT)) {n        placement =
AttachmentMap.RIGHTn      } else if
($parentDropdown.hasClass(ClassName.DROPLEFT)) {n        placement =
AttachmentMap.LEFTn      } else if
($(this._menu).hasClass(ClassName.MENURIGHT)) {n        placement =
AttachmentMap.BOTTOMENDn      }n      return placementn    }nn   
_detectNavbar() {n      return $(this._element).closest(‘.navbar’).length >
0n    }nn    _getPopperConfig() {n      const offsetConf = {}n      if
(typeof this._config.offset === ‘function’) {n        offsetConf.fn =
(data) => {n          data.offsets = {n            …data.offsets,n         
…this._config.offset(data.offsets) || {}n          }n          return datan
}n      } else {n        offsetConf.offset = this._config.offsetn      }n  
const popperConfig = {n        placement: this._getPlacement(),n       
modifiers: {n          offset: offsetConf,n          flip: {n           
enabled: this._config.flipn          },n          preventOverflow: {n      
boundariesElement: this._config.boundaryn          }n        }n      }nn   
// Disable Popper.js if we have a static displayn      if
(this._config.display === ‘static’) {n       
popperConfig.modifiers.applyStyle = {n          enabled: falsen        }n  
}n      return popperConfign    }nn    // Staticnn    static
_jQueryInterface(config) {n      return this.each(function () {n        let
data = $(this).data(DATA_KEY)n        const _config = typeof config ===
‘object’ ? config : nullnn        if (!data) {n          data = new
Dropdown(this, _config)n          $(this).data(DATA_KEY, data)n        }nn 
if (typeof config === ‘string’) {n          if (typeof data[config] ===
‘undefined’) {n            throw new TypeError(`No method named
"${config}"`)n          }n          data[config]()n        }n      })n   
}nn    static _clearMenus(event) {n      if (event && (event.which ===
RIGHT_MOUSE_BUTTON_WHICH ||n        event.type === ‘keyup’ && event.which
!== TAB_KEYCODE)) {n        returnn      }nn      const toggles =
$.makeArray($(Selector.DATA_TOGGLE))n      for (let i = 0; i <
toggles.length; i++) {\n        const parent =
Dropdown._getParentFromElement(toggles[i])\n        const context =
$(toggles[i]).data(DATA_KEY)\n        const relatedTarget = {\n         
relatedTarget: toggles[i]\n        }\n\n        if (!context) {\n         
continue\n        }\n\n        const dropdownMenu = context._menu\n       
if (!$(parent).hasClass(ClassName.SHOW)) {\n          continue\n       
}\n\n        if (event && (event.type === 'click' &&\n           
/input|textarea/i.test(event.target.tagName) || event.type === 'keyup' &&
event.which === TAB_KEYCODE) &&\n            $.contains(parent,
event.target)) {\n          continue\n        }\n\n        const hideEvent
= $.Event(Event.HIDE, relatedTarget)\n       
$(parent).trigger(hideEvent)\n        if (hideEvent.isDefaultPrevented())
{\n          continue\n        }\n\n        // If this is a touch-enabled
device we remove the extra\n        // empty mouseover listeners we added
for iOS support\n        if ('ontouchstart' in document.documentElement)
{\n          $(document.body).children().off('mouseover', null, $.noop)\n  
}\n\n        toggles[i].setAttribute('aria-expanded', 'false')\n\n       
$(dropdownMenu).removeClass(ClassName.SHOW)\n        $(parent)\n         
.removeClass(ClassName.SHOW)\n          .trigger($.Event(Event.HIDDEN,
relatedTarget))\n      }\n    }\n\n    static
...
//  - And not a key in REGEXP_KEYDOWN => not a dropdown commandn      // If
input/textarea:n      //  - If space key => not a dropdown commandn      //
- If key is other than escapen      //    - If key is not up or down => not
a dropdown commandn      //    - If trigger inside the menu => not a
dropdown commandn      if (/input|textarea/i.test(event.target.tagName)n   
? event.which === SPACE_KEYCODE || event.which !== ESCAPE_KEYCODE &&n      
(event.which !== ARROW_DOWN_KEYCODE && event.which !== ARROW_UP_KEYCODE ||n
$(event.target).closest(Selector.MENU).length) :
!REGEXP_KEYDOWN.test(event.which)) {n        returnn      }nn     
event.preventDefault()n      event.stopPropagation()nn      if
(this.disabled || $(this).hasClass(ClassName.DISABLED)) {n        returnn  
}nn      const parent   = Dropdown._getParentFromElement(this)n      const
isActive = $(parent).hasClass(ClassName.SHOW)nn      if (!isActive &&
(event.which !== ESCAPE_KEYCODE || event.which !== SPACE_KEYCODE) ||n      
isActive && (event.which === ESCAPE_KEYCODE || event.which ===
SPACE_KEYCODE)) {n        if (event.which === ESCAPE_KEYCODE) {n         
const toggle = $(parent).find(Selector.DATA_TOGGLE)[0]n         
$(toggle).trigger(‘focus’)n        }nn        $(this).trigger(‘click’)n    
returnn      }nn      const items =
$(parent).find(Selector.VISIBLE_ITEMS).get()nn      if (items.length === 0)
{n        returnn      }nn      let index = items.indexOf(event.target)nn  
if (event.which === ARROW_UP_KEYCODE && index > 0) { // Upn        index–n 
}nn      if (event.which === ARROW_DOWN_KEYCODE && index < items.length -
1) { // Down\n        index++\n      }\n\n      if (index < 0) {\n       
index = 0\n      }\n\n      items[index].focus()\n    }\n  }\n\n  /**\n   *
------------------------------------------------------------------------\n 
...
Dropdown._dataApiKeydownHandler)\n    .on(Event.KEYDOWN_DATA_API,
Selector.MENU, Dropdown._dataApiKeydownHandler)\n   
.on(`${Event.CLICK_DATA_API} ${Event.KEYUP_DATA_API}`,
Dropdown._clearMenus)\n    .on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE,
function (event) {\n      event.preventDefault()\n     
event.stopPropagation()\n      Dropdown._jQueryInterface.call($(this),
'toggle')\n    })\n    .on(Event.CLICK_DATA_API, Selector.FORM_CHILD, (e)
=> {n      e.stopPropagation()n    })nn  /**n   * ————————————————————————n
* jQueryn   * ————————————————————————n   */nn  $.fn[NAME] =
Dropdown._jQueryInterfacen  $.fn[NAME].Constructor = Dropdownn 
$.fn[NAME].noConflict = function () {n    $.fn[NAME] = JQUERY_NO_CONFLICTn 
return Dropdown._jQueryInterfacen  }nn  return Dropdownn})($,
Popper)nnexport default Dropdownn”,“import $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————–n * Bootstrap (v4.1.0): modal.jsn
* Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnconst Modal = (($) => {n  /**n   *
————————————————————————n   * Constantsn   * ————————————————————————n  
*/nn  const NAME               = ‘modal’n  const VERSION            =
‘4.1.0’n  const DATA_KEY           = ‘bs.modal’n  const EVENT_KEY         
= `.${DATA_KEY}`n  const DATA_API_KEY       = ‘.data-api’n  const
JQUERY_NO_CONFLICT = $.fn[NAME]n  const ESCAPE_KEYCODE     = 27 //
KeyboardEvent.which value for Escape (Esc) keynn  const Default = {n   
backdrop : true,n    keyboard : true,n    focus    : true,n    show     :
truen  }nn  const DefaultType = {n    backdrop : ‘(boolean|string)’,n   
keyboard : ‘boolean’,n    focus    : ‘boolean’,n    show     : ‘boolean’n 
}nn  const Event = {n    HIDE              : `hide${EVENT_KEY}`,n    HIDDEN
: `hidden${EVENT_KEY}`,n    SHOW              : `show${EVENT_KEY}`,n   
SHOWN             : `shown${EVENT_KEY}`,n    FOCUSIN           :
`focusin${EVENT_KEY}`,n    RESIZE            : `resize${EVENT_KEY}`,n   
CLICK_DISMISS     : `click.dismiss${EVENT_KEY}`,n    KEYDOWN_DISMISS   :
`keydown.dismiss${EVENT_KEY}`,n    MOUSEUP_DISMISS   :
`mouseup.dismiss${EVENT_KEY}`,n    MOUSEDOWN_DISMISS :
`mousedown.dismiss${EVENT_KEY}`,n    CLICK_DATA_API    :
`click${EVENT_KEY}${DATA_API_KEY}`n  }nn  const ClassName = {n   
SCROLLBAR_MEASURER : ‘modal-scrollbar-measure’,n    BACKDROP           :
‘modal-backdrop’,n    OPEN               : ‘modal-open’,n    FADE          
: ‘fade’,n    SHOW               : ‘show’n  }nn  const Selector = {n   
DIALOG             : ‘.modal-dialog’,n    DATA_TOGGLE        :
‘[data-toggle="modal"]’,n    DATA_DISMISS       :
‘[data-dismiss="modal"]’,n    FIXED_CONTENT      : ‘.fixed-top,
.fixed-bottom, .is-fixed, .sticky-top’,n    STICKY_CONTENT     :
‘.sticky-top’,n    NAVBAR_TOGGLER     : ‘.navbar-toggler’n  }nn  /**n   *
————————————————————————n   * Class Definitionn   *
————————————————————————n   */nn  class Modal {n    constructor(element,
config) {n      this._config              = this._getConfig(config)n     
this._element             = elementn      this._dialog              =
$(element).find(Selector.DIALOG)[0]n      this._backdrop            = nulln
this._isShown             = falsen      this._isBodyOverflowing   = falsen 
this._ignoreBackdropClick = falsen      this._scrollbarWidth      = 0n   
}nn    // Gettersnn    static get VERSION() {n      return VERSIONn    }nn 
static get Default() {n      return Defaultn    }nn    // Publicnn   
toggle(relatedTarget) {n      return this._isShown ? this.hide() :
this.show(relatedTarget)n    }nn    show(relatedTarget) {n      if
(this._isTransitioning || this._isShown) {n        returnn      }nn      if
($(this._element).hasClass(ClassName.FADE)) {n        this._isTransitioning
= truen      }nn      const showEvent = $.Event(Event.SHOW, {n       
relatedTargetn      })nn      $(this._element).trigger(showEvent)nn      if
(this._isShown || showEvent.isDefaultPrevented()) {n        returnn     
}nn      this._isShown = truenn      this._checkScrollbar()n     
this._setScrollbar()nn      this._adjustDialog()nn     
$(document.body).addClass(ClassName.OPEN)nn      this._setEscapeEvent()n   
this._setResizeEvent()nn      $(this._element).on(n       
Event.CLICK_DISMISS,n        Selector.DATA_DISMISS,n        (event) =>
this.hide(event)n      )nn      $(this._dialog).on(Event.MOUSEDOWN_DISMISS,
() => {n        $(this._element).one(Event.MOUSEUP_DISMISS, (event) => {n  
if ($(event.target).is(this._element)) {n           
this._ignoreBackdropClick = truen          }n        })n      })nn     
this._showBackdrop(() => this._showElement(relatedTarget))n    }nn   
hide(event) {n      if (event) {n        event.preventDefault()n      }nn  
if (this._isTransitioning || !this._isShown) {n        returnn      }nn    
const hideEvent = $.Event(Event.HIDE)nn     
$(this._element).trigger(hideEvent)nn      if (!this._isShown ||
hideEvent.isDefaultPrevented()) {n        returnn      }nn     
this._isShown = falsen      const transition =
$(this._element).hasClass(ClassName.FADE)nn      if (transition) {n       
this._isTransitioning = truen      }nn      this._setEscapeEvent()n     
this._setResizeEvent()nn      $(document).off(Event.FOCUSIN)nn     
$(this._element).removeClass(ClassName.SHOW)nn     
$(this._element).off(Event.CLICK_DISMISS)n     
$(this._dialog).off(Event.MOUSEDOWN_DISMISS)nnn      if (transition) {n    
const transitionDuration  =
Util.getTransitionDurationFromElement(this._element)nn       
$(this._element)n          .one(Util.TRANSITION_END, (event) =>
this._hideModal(event))n         
.emulateTransitionEnd(transitionDuration)n      } else {n       
this._hideModal()n      }n    }nn    dispose() {n     
$.removeData(this._element, DATA_KEY)nn      $(window, document,
this._element, this._backdrop).off(EVENT_KEY)nn      this._config          
= nulln      this._element             = nulln      this._dialog           
= nulln      this._backdrop            = nulln      this._isShown          
= nulln      this._isBodyOverflowing   = nulln     
this._ignoreBackdropClick = nulln      this._scrollbarWidth      = nulln   
}nn    handleUpdate() {n      this._adjustDialog()n    }nn    // Privatenn 
_getConfig(config) {n      config = {n        …Default,n        …confign   
}n      Util.typeCheckConfig(NAME, config, DefaultType)n      return
confign    }nn    _showElement(relatedTarget) {n      const transition =
$(this._element).hasClass(ClassName.FADE)nn      if
(!this._element.parentNode ||n         this._element.parentNode.nodeType
!== Node.ELEMENT_NODE) {n        // Don’t move modal’s DOM positionn       
document.body.appendChild(this._element)n      }nn     
this._element.style.display = ‘block’n     
this._element.removeAttribute(‘aria-hidden’)n      this._element.scrollTop
= 0nn      if (transition) {n        Util.reflow(this._element)n      }nn  
$(this._element).addClass(ClassName.SHOW)nn      if (this._config.focus) {n
this._enforceFocus()n      }nn      const shownEvent = $.Event(Event.SHOWN,
{n        relatedTargetn      })nn      const transitionComplete = () => {n
if (this._config.focus) {n          this._element.focus()n        }n       
this._isTransitioning = falsen        $(this._element).trigger(shownEvent)n
}nn      if (transition) {n        const transitionDuration  =
Util.getTransitionDurationFromElement(this._element)nn       
$(this._dialog)n          .one(Util.TRANSITION_END, transitionComplete)n   
.emulateTransitionEnd(transitionDuration)n      } else {n       
transitionComplete()n      }n    }nn    _enforceFocus() {n     
$(document)n        .off(Event.FOCUSIN) // Guard against infinite focus
loopn        .on(Event.FOCUSIN, (event) => {n          if (document !==
event.target &&n              this._element !== event.target &&n           
$(this._element).has(event.target).length === 0) {n           
this._element.focus()n          }n        })n    }nn    _setEscapeEvent()
{n      if (this._isShown && this._config.keyboard) {n       
$(this._element).on(Event.KEYDOWN_DISMISS, (event) => {n          if
(event.which === ESCAPE_KEYCODE) {n            event.preventDefault()n     
this.hide()n          }n        })n      } else if (!this._isShown) {n     
$(this._element).off(Event.KEYDOWN_DISMISS)n      }n    }nn   
_setResizeEvent() {n      if (this._isShown) {n       
$(window).on(Event.RESIZE, (event) => this.handleUpdate(event))n      }
else {n        $(window).off(Event.RESIZE)n      }n    }nn    _hideModal()
{n      this._element.style.display = ‘none’n     
this._element.setAttribute(‘aria-hidden’, true)n      this._isTransitioning
= falsen      this._showBackdrop(() => {n       
$(document.body).removeClass(ClassName.OPEN)n       
this._resetAdjustments()n        this._resetScrollbar()n       
$(this._element).trigger(Event.HIDDEN)n      })n    }nn   
_removeBackdrop() {n      if (this._backdrop) {n       
$(this._backdrop).remove()n        this._backdrop = nulln      }n    }nn   
_showBackdrop(callback) {n      const animate =
$(this._element).hasClass(ClassName.FADE)n        ? ClassName.FADE : ”nn   
if (this._isShown && this._config.backdrop) {n        this._backdrop =
document.createElement(‘div’)n        this._backdrop.className =
ClassName.BACKDROPnn        if (animate) {n         
$(this._backdrop).addClass(animate)n        }nn       
$(this._backdrop).appendTo(document.body)nn       
$(this._element).on(Event.CLICK_DISMISS, (event) => {n          if
(this._ignoreBackdropClick) {n            this._ignoreBackdropClick =
falsen            returnn          }n          if (event.target !==
event.currentTarget) {n            returnn          }n          if
(this._config.backdrop === ‘static’) {n            this._element.focus()n  
} else {n            this.hide()n          }n        })nn        if
(animate) {n          Util.reflow(this._backdrop)n        }nn       
$(this._backdrop).addClass(ClassName.SHOW)nn        if (!callback) {n      
returnn        }nn        if (!animate) {n          callback()n         
returnn        }nn        const backdropTransitionDuration =
Util.getTransitionDurationFromElement(this._backdrop)nn       
$(this._backdrop)n          .one(Util.TRANSITION_END, callback)n         
.emulateTransitionEnd(backdropTransitionDuration)n      } else if
(!this._isShown && this._backdrop) {n       
$(this._backdrop).removeClass(ClassName.SHOW)nn        const callbackRemove
= () => {n          this._removeBackdrop()n          if (callback) {n      
callback()n          }n        }nn        if
($(this._element).hasClass(ClassName.FADE)) {n          const
backdropTransitionDuration =
Util.getTransitionDurationFromElement(this._backdrop)nn         
$(this._backdrop)n            .one(Util.TRANSITION_END, callbackRemove)n   
.emulateTransitionEnd(backdropTransitionDuration)n        } else {n        
callbackRemove()n        }n      } else if (callback) {n        callback()n
}n    }nn    // ———————————————————————-n    // the following methods are
used to handle overflowing modalsn    // todo (fat): these should probably
be refactored out of modal.jsn    // ———————————————————————-nn   
_adjustDialog() {n      const isModalOverflowing =n       
this._element.scrollHeight > document.documentElement.clientHeightnn     
if (!this._isBodyOverflowing && isModalOverflowing) {n       
this._element.style.paddingLeft = `${this._scrollbarWidth}px`n      }nn    
if (this._isBodyOverflowing && !isModalOverflowing) {n       
this._element.style.paddingRight = `${this._scrollbarWidth}px`n      }n   
}nn    _resetAdjustments() {n      this._element.style.paddingLeft = ”n    
this._element.style.paddingRight = ”n    }nn    _checkScrollbar() {n     
const rect = document.body.getBoundingClientRect()n     
