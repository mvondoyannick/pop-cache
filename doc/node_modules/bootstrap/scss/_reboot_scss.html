...
<title>_reboot.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_reboot.scss">

<p>// stylelint-disable at-rule-no-vendor-prefix, declaration-no-important,
selector-no-qualifying-type, property-no-vendor-prefix</p>

<p>// Reboot // // Normalization of HTML elements, manually forked from
Normalize.css to remove // styles targeting irrelevant browsers while
applying new styles. // // Normalize is licensed MIT. <a
href="https://github.com/necolas/normalize.css">github.com/necolas/normalize.css</a></p>

<p>// Document // // 1. Change from `box-sizing: content-box` so that `width`
is not affected by `padding` or `border`. // 2. Change the default font
family in all browsers. // 3. Correct the line height in all browsers. //
4. Prevent adjustments of font size after orientation changes in IE on
Windows Phone and in iOS. // 5. Change the default tap highlight to be
completely transparent in iOS.</p>

<p>*, *::before, *::after {</p>

<pre>box-sizing: border-box; // 1</pre>

<p>}</p>

<p>html {</p>

<pre>font-family: sans-serif; // 2
line-height: 1.15; // 3
-webkit-text-size-adjust: 100%; // 4
-webkit-tap-highlight-color: rgba($black, 0); // 5</pre>

<p>}</p>

<p>// Shim for “new” HTML5 structural elements to display correctly (IE10,
older browsers) // TODO: remove in v5 // stylelint-disable-next-line
selector-list-comma-newline-after article, aside, figcaption, figure,
footer, header, hgroup, main, nav, section {</p>

<pre>display: block;</pre>

<p>}</p>

<p>// Body // // 1. Remove the margin in all browsers. // 2. As a best
practice, apply a default `background-color`. // 3. Set an explicit initial
text-align value so that we can later use //    the `inherit` value on
things like `&lt;th&gt;` elements.</p>
...
color: $body-color;
text-align: left; // 3
background-color: $body-bg; // 2</pre>

<p>}</p>

<p>// Suppress the focus outline on elements that cannot be accessed via
keyboard. // This prevents an unwanted focus outline from appearing around
elements that // might still respond to pointer events. // // Credit: <a
href="https://github.com/suitcss/base">github.com/suitcss/base</a>
[tabindex=“-1”]:focus {</p>
...
<p>// Content grouping // // 1. Add the correct box sizing in Firefox. // 2.
Show the overflow in Edge and IE.</p>

<p>hr {</p>

<pre>box-sizing: content-box; // 1
height: 0; // 1
overflow: visible; // 2</pre>

<p>}</p>

<p>// // Typography //</p>

<p>// Remove top margins from headings // // By default,
`&lt;h1&gt;`-`&lt;h6&gt;` all receive top and bottom margins. We nuke the
top // margin for easier control within type scales as it avoids margin
collapsing. // stylelint-disable-next-line
selector-list-comma-newline-after h1, h2, h3, h4, h5, h6 {</p>

<pre>margin-top: 0;
margin-bottom: $headings-margin-bottom;</pre>

<p>}</p>

<p>// Reset margins on paragraphs // // Similarly, the top margin on
`&lt;p&gt;`s get reset. However, we also reset the // bottom margin to use
`rem` units instead of `em`. p {</p>

<pre>margin-top: 0;
margin-bottom: $paragraph-margin-bottom;</pre>

<p>}</p>

<p>// Abbreviations // // 1. Duplicate behavior to the data-* attribute for
our tooltip plugin // 2. Add the correct text decoration in Chrome, Edge,
IE, Opera, and Safari. // 3. Add explicit cursor to indicate changed
behavior. // 4. Remove the bottom border in Firefox 39-. // 5. Prevent the
text-decoration to be skipped.</p>

<p><a href="title">abbr</a>, <a href="data-original-title">abbr</a> { // 1</p>

<pre>text-decoration: underline; // 2
text-decoration: underline dotted; // 2
cursor: help; // 3
border-bottom: 0; // 4
text-decoration-skip-ink: none; // 5</pre>
...
<pre>font-weight: $dt-font-weight;</pre>
...
margin-left: 0; // Undo browser default</pre>
...
<pre>font-weight: $font-weight-bolder; // Add the correct font weight in Chrome, Edge, and Safari</pre>

<p>}</p>

<p>small {</p>

<pre>@include font-size(80%); // Add the correct font size in all browsers</pre>

<p>}</p>

<p>// // Prevent `sub` and `sup` elements from affecting the line height in //
all browsers. //</p>
...
<p>sub { bottom: -.25em; } sup { top: -.5em; }</p>
...
<p>// And undo these styles for placeholder links/named anchors (without href)
// which have not been made explicitly keyboard-focusable (without
tabindex). // It would be more straightforward to just use <a
href="href">a</a> in previous block, but that // causes specificity issues
in many other styles that are too complex to fix. // See <a
href="https://github.com/twbs/bootstrap/issues/19402">github.com/twbs/bootstrap/issues/19402</a></p>
...
<pre>font-family: $font-family-monospace;
@include font-size(1em); // Correct the odd `em` font sizing in all browsers.</pre>
...
<pre>// Apply a consistent margin strategy (matches our type styles).
margin: 0 0 1rem;</pre>

<p>}</p>

<p>// // Images and content //</p>

<p>img {</p>

<pre>vertical-align: middle;
border-style: none; // Remove the border on images inside links in IE 10-.</pre>

<p>}</p>

<p>svg {</p>

<pre>// Workaround for the SVG overflow bug in IE10/11 is still required.
...
<pre>border-collapse: collapse; // Prevent double borders</pre>

<p>}</p>

<p>caption {</p>

<pre>padding-top: $table-cell-padding;
...
<pre>// Matches default `&lt;td&gt;` alignment by inheriting from the `&lt;body&gt;`, or the
...
<pre>// Allow labels to use `margin` for spacing.
display: inline-block;
margin-bottom: $label-margin-bottom;</pre>

<p>}</p>

<p>// Remove the default `border-radius` that macOS Chrome adds. // // Details
at <a
href="https://github.com/twbs/bootstrap/issues/24093">github.com/twbs/bootstrap/issues/24093</a>
button {</p>

<pre>// stylelint-disable-next-line property-blacklist
border-radius: 0;</pre>

<p>}</p>

<p>// Work around a Firefox/IE bug where the transparent `button` background
// results in a loss of the default `button` focus styles. // // Credit: <a
href="https://github.com/suitcss/base">github.com/suitcss/base</a>/
button:focus {</p>
...
<pre>margin: 0; // Remove the margin in Firefox and Safari
font-family: inherit;
...
<pre>overflow: visible; // Show the overflow in Edge</pre>

<p>}</p>

<p>button, select {</p>

<pre>text-transform: none; // Remove the inheritance of text transform in Firefox</pre>

<p>}</p>

<p>// Remove the inheritance of word-wrap in Safari. // // Details at <a
href="https://github.com/twbs/bootstrap/issues/24990">github.com/twbs/bootstrap/issues/24990</a>
select {</p>
...
<p>// 1. Prevent a WebKit bug where (2) destroys native `audio` and `video` //
controls in Android 4. // 2. Correct the inability to style clickable types
in iOS and Safari. button, [type=“button”], // 1 [type=“reset”],</p>
<dl class="rdoc-list label-list"><dt>type=“submit”
<dd>
<p>{</p>
</dd></dl>

<pre>-webkit-appearance: button; // 2</pre>

<p>}</p>

<p>// Opinionated: add “hand” cursor to non-disabled button elements. @if
$enable-pointer-cursor-for-buttons {</p>
...
<p>// Remove inner border and padding from Firefox, but don&#39;t restore the
outline like Normalize. button::-moz-focus-inner,
[type=“button”]::-moz-focus-inner, [type=“reset”]::-moz-focus-inner,
[type=“submit”]::-moz-focus-inner {</p>
...
<pre>box-sizing: border-box; // 1. Add the correct box sizing in IE 10-
padding: 0; // 2. Remove the padding in IE 10-</pre>
...
<pre>// Remove the default appearance of temporal inputs to avoid a Mobile Safari
...
<pre>overflow: auto; // Remove the default vertical scrollbar in IE.
...
<pre>// Browsers set a default `min-width: min-content;` on fieldsets,
...
<p>// 1. Correct the text wrapping in Edge and IE. // 2. Correct the color
inheritance from `fieldset` elements in IE. legend {</p>
...
line-height: inherit;
color: inherit; // 2
white-space: normal; // 1</pre>

<p>}</p>

<p>progress {</p>

<pre>vertical-align: baseline; // Add the correct vertical alignment in Chrome, Firefox, and Opera.</pre>

<p>}</p>

<p>// Correct the cursor style of increment and decrement buttons in Chrome.
...
<pre>// This overrides the extra rounded corners on search inputs in iOS so that our
...
<p>// // Remove the inner padding in Chrome and Safari on macOS. //</p>
...
<p>// // 1. Correct the inability to style clickable types in iOS and Safari.
// 2. Change font properties to `inherit` in Safari. //</p>

<p>::-webkit-file-upload-button {</p>

<pre>font: inherit; // 2
-webkit-appearance: button; // 1</pre>

<p>}</p>

<p>// // Correct element displays //</p>
...
<pre>display: list-item; // Add the correct display in all browsers
cursor: pointer;</pre>
...
<pre>display: none; // Add the correct display in IE</pre>

<p>}</p>

<p>// Always hide an element with the `hidden` HTML attribute (from PureCSS).
// Needed for proper display in IE 10-.</p>
