...
<title>_carousel.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_carousel.scss">

<p>// Notes on the classes: // // 1. .carousel.pointer-event should ideally be
pan-y (to allow for users to scroll vertically) //    even when their
scroll action started on a carousel, but for compatibility (with Firefox)
//    we&#39;re preventing all actions instead // 2. The
.carousel-item-left and .carousel-item-right is used to indicate where //  
the active slide is heading. // 3. .active.carousel-item is the current
slide. // 4. .active.carousel-item-left and .active.carousel-item-right is
the current //    slide in its in-transition state. Only one of these
occurs at a time. // 5. .carousel-item-next.carousel-item-left and
.carousel-item-prev.carousel-item-right //    is the upcoming slide in
transition.</p>
...
@include transition($carousel-transition);</pre>
...
<p>// // Left/right controls for nav //</p>
...
<p>// Icons for within .carousel-control-prev-icon,
.carousel-control-next-icon {</p>
...
<pre>background-image: $carousel-control-prev-icon-bg;</pre>

<p>} .carousel-control-next-icon {</p>

<pre>background-image: $carousel-control-next-icon-bg;</pre>

<p>}</p>

<p>// Optional indicator pips // // Add an ordered list with the following
class and add a list item for each // slide your carousel holds.</p>
