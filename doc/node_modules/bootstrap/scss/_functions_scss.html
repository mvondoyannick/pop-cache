...
<title>_functions.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_functions.scss">

<p>// Bootstrap functions // // Utility mixins and functions for evaluating
source code across our variables, maps, and mixins.</p>

<p>// Ascending // Used to evaluate Sass maps like our grid breakpoints.
@mixin _assert-ascending($map, $map-name) {</p>
...
<p>// Starts at zero // Used to ensure the min-width of the lowest breakpoint
starts at 0. @mixin _assert-starts-at-zero($map, $map-name:
“$grid-breakpoints”) {</p>

<pre>$values: map-values($map);
...
<p>// Replace `$search` with `$replace` in `$string` // Used on our SVG icon
backgrounds for custom forms. // // @author Hugo Giraudel // @param
{String} $string - Initial string // @param {String} $search - Substring to
replace // @param {String} $replace (&#39;&#39;) - New value // @return
{String} - Updated string @function str-replace($string, $search, $replace:
“”) {</p>

<pre>$index: str-index($string, $search);
...
<p>// Color contrast @function color-yiq($color, $dark: $yiq-text-dark,
$light: $yiq-text-light) {</p>
...
<p>// Retrieve color Sass maps @function color($key: “blue”) {</p>

<pre>@return map-get($colors, $key);</pre>

<p>}</p>

<p>@function theme-color($key: “primary”) {</p>

<pre>@return map-get($theme-colors, $key);</pre>

<p>}</p>

<p>@function gray($key: “100”) {</p>

<pre>@return map-get($grays, $key);</pre>

<p>}</p>

<p>// Request a theme color level @function theme-color-level($color-name:
“primary”, $level: 0) {</p>

<pre>$color: theme-color($color-name);
$color-base: if($level &gt; 0, $black, $white);
$level: abs($level);

@return mix($color-base, $color, $level * $theme-color-interval);</pre>
