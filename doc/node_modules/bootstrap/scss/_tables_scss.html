...
<title>_tables.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_tables.scss">

<p>// // Basic Bootstrap table //</p>
...
<p>// // Condensed table w/ half padding //</p>
...
<p>// Border versions // // Add or remove borders all around the table and
between all the columns.</p>

<p>.table-bordered {</p>

<pre>border: $table-border-width solid $table-border-color;
...
<p>// Zebra-striping // // Default zebra-stripe styles (alternating gray and
transparent backgrounds)</p>

<p>.table-striped {</p>

<pre>tbody tr:nth-of-type(#{$table-striped-order}) {
  background-color: $table-accent-bg;
}</pre>

<p>}</p>

<p>// Hover effect // // Placed here since it has to come after the potential
zebra striping</p>
...
<p>// Table backgrounds // // Exact selectors below required to override
`.table-striped` and prevent // inheritance to nested tables.</p>

<p>@each $color, $value in $theme-colors {</p>

<pre>@include table-row-variant($color, theme-color-level($color, $table-bg-level), theme-color-level($color, $table-border-level));</pre>

<p>}</p>

<p>@include table-row-variant(active, $table-active-bg);</p>

<p>// Dark styles // // Same table markup, but inverted color scheme: dark
background and light text.</p>

<p>// stylelint-disable-next-line no-duplicate-selectors .table {</p>
...
<p>// Responsive tables // // Generate series of `.table-responsive-*` classes
for configuring the screen // size of where your table will overflow.</p>
