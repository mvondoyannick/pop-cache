...
<title>_rfs.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/vendor/_rfs.scss">

<p>// stylelint-disable property-blacklist, scss/dollar-variable-default</p>

<p>// SCSS RFS mixin // // Automated font-resizing // // See <a
href="https://github.com/twbs/rfs">github.com/twbs/rfs</a></p>

<p>// Configuration</p>

<p>// Base font size $rfs-base-font-size: 1.25rem !default;
$rfs-font-size-unit: rem !default;</p>

<p>// Breakpoint at where font-size starts decreasing if screen width is
smaller $rfs-breakpoint: 1200px !default; $rfs-breakpoint-unit: px
!default;</p>

<p>// Resize font-size based on screen height and width $rfs-two-dimensional:
false !default;</p>

<p>// Factor of decrease $rfs-factor: 10 !default;</p>

<p>@if type-of($rfs-factor) != “number” or $rfs-factor &lt;= 1 {</p>

<pre>@error &quot;`#{$rfs-factor}` is not a valid  $rfs-factor, it must be greater than 1.&quot;;</pre>

<p>}</p>

<p>// Generate enable or disable classes. Possibilities: false, “enable” or
“disable” $rfs-class: false !default;</p>

<p>// 1 rem = $rfs-rem-value px $rfs-rem-value: 16 !default;</p>

<p>// Safari iframe resize bug: <a
href="https://github.com/twbs/rfs/issues/14">github.com/twbs/rfs/issues/14</a>
$rfs-safari-iframe-resize-bug-fix: false !default;</p>

<p>// Disable RFS by setting $enable-responsive-font-sizes to false
$enable-responsive-font-sizes: true !default;</p>

<p>// Cache $rfs-base-font-size unit $rfs-base-font-size-unit:
unit($rfs-base-font-size);</p>

<p>// Remove px-unit from $rfs-base-font-size for calculations @if
$rfs-base-font-size-unit == “px” {</p>

<pre>$rfs-base-font-size: $rfs-base-font-size / ($rfs-base-font-size * 0 + 1);</pre>

<p>} @else if $rfs-base-font-size-unit == “rem” {</p>

<pre>$rfs-base-font-size: $rfs-base-font-size / ($rfs-base-font-size * 0 + 1 / $rfs-rem-value);</pre>

<p>}</p>

<p>// Cache $rfs-breakpoint unit to prevent multiple calls
$rfs-breakpoint-unit-cache: unit($rfs-breakpoint);</p>

<p>// Remove unit from $rfs-breakpoint for calculations @if
$rfs-breakpoint-unit-cache == “px” {</p>

<pre>$rfs-breakpoint: $rfs-breakpoint / ($rfs-breakpoint * 0 + 1);</pre>

<p>} @else if $rfs-breakpoint-unit-cache == “rem” or
$rfs-breakpoint-unit-cache == “em” {</p>

<pre>$rfs-breakpoint: $rfs-breakpoint / ($rfs-breakpoint * 0 + 1 / $rfs-rem-value);</pre>

<p>}</p>

<p>// Responsive font-size mixin @mixin rfs($fs, $important: false) {</p>
...
<p>// The font-size &amp; responsive-font-size mixin uses RFS to rescale font
sizes @mixin font-size($fs, $important: false) {</p>

<pre>@include rfs($fs, $important);</pre>

<p>}</p>

<p>@mixin responsive-font-size($fs, $important: false) {</p>
