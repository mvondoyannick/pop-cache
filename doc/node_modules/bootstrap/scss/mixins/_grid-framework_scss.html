...
<title>_grid-framework.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/mixins/_grid-framework.scss">

<p>// Framework grid generation // // Used only by Bootstrap to generate the
correct number of grid classes given // any value of `$grid-columns`.</p>

<p>@mixin make-grid-columns($columns: $grid-columns, $gutter:
$grid-gutter-width, $breakpoints: $grid-breakpoints) {</p>

<pre>// Common properties for all breakpoints
%grid-column {
  position: relative;
  width: 100%;
