...
<title>_breakpoints.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/mixins/_breakpoints.scss">

<p>// Breakpoint viewport sizes and media queries. // // Breakpoints are
defined as a map of (name: minimum width), order from small to large: // //
(xs: 0, sm: 576px, md: 768px, lg: 992px, xl: 1200px) // // The map defined
in the `$grid-breakpoints` global variable is used as the `$breakpoints`
argument by default.</p>

<p>// Name of the next breakpoint, or null for the last breakpoint. // //   
&gt;&gt; breakpoint-next(sm) //    md //    &gt;&gt; breakpoint-next(sm,
(xs: 0, sm: 576px, md: 768px, lg: 992px, xl: 1200px)) //    md //   
&gt;&gt; breakpoint-next(sm, $breakpoint-names: (xs sm md lg xl)) //    md
@function breakpoint-next($name, $breakpoints: $grid-breakpoints,
$breakpoint-names: map-keys($breakpoints)) {</p>

<pre>$n: index($breakpoint-names, $name);
@return if($n != null and $n &lt; length($breakpoint-names), nth($breakpoint-names, $n + 1), null);</pre>

<p>}</p>

<p>// Minimum breakpoint width. Null for the smallest (first) breakpoint. //
//    &gt;&gt; breakpoint-min(sm, (xs: 0, sm: 576px, md: 768px, lg: 992px,
xl: 1200px)) //    576px @function breakpoint-min($name, $breakpoints:
$grid-breakpoints) {</p>

<pre>$min: map-get($breakpoints, $name);
@return if($min != 0, $min, null);</pre>

<p>}</p>

<p>// Maximum breakpoint width. Null for the largest (last) breakpoint. // The
maximum value is calculated as the minimum of the next one less 0.02px //
to work around the limitations of `min-` and `max-` prefixes and viewports
with fractional widths. // See <a
href="https://www.w3.org/TR/mediaqueries-4/#mq-min-max">www.w3.org/TR/mediaqueries-4/#mq-min-max</a>
// Uses 0.02px rather than 0.01px to work around a current rounding bug in
Safari. // See <a
href="https://bugs.webkit.org/show_bug.cgi?id=178261">bugs.webkit.org/show_bug.cgi?id=178261</a>
// //    &gt;&gt; breakpoint-max(sm, (xs: 0, sm: 576px, md: 768px, lg:
992px, xl: 1200px)) //    767.98px @function breakpoint-max($name,
$breakpoints: $grid-breakpoints) {</p>

<pre>$next: breakpoint-next($name, $breakpoints);
@return if($next, breakpoint-min($next, $breakpoints) - .02, null);</pre>

<p>}</p>

<p>// Returns a blank string if smallest breakpoint, otherwise returns the
name with a dash in front. // Useful for making responsive utilities. // //
&gt;&gt; breakpoint-infix(xs, (xs: 0, sm: 576px, md: 768px, lg: 992px, xl:
1200px)) //    “”  (Returns a blank string) //    &gt;&gt;
breakpoint-infix(sm, (xs: 0, sm: 576px, md: 768px, lg: 992px, xl: 1200px))
//    “-sm” @function breakpoint-infix($name, $breakpoints:
$grid-breakpoints) {</p>

<pre>@return if(breakpoint-min($name, $breakpoints) == null, &quot;&quot;, &quot;-#{$name}&quot;);</pre>

<p>}</p>

<p>// Media of at least the minimum breakpoint width. No query for the
smallest breakpoint. // Makes the @content apply to the given breakpoint
and wider. @mixin media-breakpoint-up($name, $breakpoints:
$grid-breakpoints) {</p>

<pre>$min: breakpoint-min($name, $breakpoints);
@if $min {
  @media (min-width: $min) {
    @content;
  }
...
<p>// Media of at most the maximum breakpoint width. No query for the largest
breakpoint. // Makes the @content apply to the given breakpoint and
narrower. @mixin media-breakpoint-down($name, $breakpoints:
$grid-breakpoints) {</p>

<pre>$max: breakpoint-max($name, $breakpoints);
@if $max {
  @media (max-width: $max) {
    @content;
  }
...
<p>// Media that spans multiple breakpoint widths. // Makes the @content apply
between the min and max breakpoints @mixin media-breakpoint-between($lower,
$upper, $breakpoints: $grid-breakpoints) {</p>

<pre>$min: breakpoint-min($lower, $breakpoints);
...
<p>// Media between the breakpoint&#39;s minimum and maximum widths. // No
...
wider or narrower. @mixin media-breakpoint-only($name, $breakpoints:
$grid-breakpoints) {</p>
