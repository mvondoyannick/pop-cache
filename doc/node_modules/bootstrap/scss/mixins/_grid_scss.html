...
<main role="main" aria-label="Page node_modules/bootstrap/scss/mixins/_grid.scss">

<p>/// Grid system // // Generate semantic grid columns with these mixins.</p>

<p>@mixin make-container($gutter: $grid-gutter-width) {</p>
...
<p>// For each breakpoint, define the maximum width of the container in a
media query @mixin make-container-max-widths($max-widths:
$container-max-widths, $breakpoints: $grid-breakpoints) {</p>

<pre>@each $breakpoint, $container-max-width in $max-widths {
...
<p>@mixin make-row($gutter: $grid-gutter-width) {</p>

<pre>display: flex;
flex-wrap: wrap;
margin-right: -$gutter / 2;
margin-left: -$gutter / 2;</pre>

<p>}</p>

<p>@mixin make-col-ready($gutter: $grid-gutter-width) {</p>
...
width: 100%;
padding-right: $gutter / 2;
padding-left: $gutter / 2;</pre>

<p>}</p>

<p>@mixin make-col($size, $columns: $grid-columns) {</p>

<pre>flex: 0 0 percentage($size / $columns);
// Add a `max-width` to ensure content within each column does not blow out
// the width of the column. Applies to IE10+ and Firefox. Chrome and Safari
// do not appear to require this.
max-width: percentage($size / $columns);</pre>

<p>}</p>

<p>@mixin make-col-offset($size, $columns: $grid-columns) {</p>

<pre>$num: $size / $columns;
margin-left: if($num == 0, 0, percentage($num));</pre>
