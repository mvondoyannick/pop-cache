...
<title>_list-group.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_list-group.scss">

<p>// Base class // // Easily usable on &lt;ul&gt;, &lt;ol&gt;, or
&lt;div&gt;.</p>
...
<p>// Interactive list items // // Use anchor or button elements instead of
`li`s or `div`s to create interactive // list items. Includes an extra
`.active` modifier class for selected items.</p>

<p>.list-group-item-action {</p>

<pre>width: 100%; // For `&lt;button&gt;`s (anchors become 100% by default though)
...
<p>// Individual list items // // Use on `li`s or `div`s within the
`.list-group` parent.</p>
...
<p>// Horizontal // // Change the layout of list group items from vertical
(default) to horizontal.</p>
...
<p>// Flush list items // // Remove borders and border-radius to keep list
group items edge-to-edge. Most // useful within other components (e.g.,
cards).</p>

<p>.list-group-flush {</p>
...
<p>// Contextual variants // // Add modifier classes to change text and
background color on individual items. // Organizationally, this must come
after the `:hover` states.</p>

<p>@each $color, $value in $theme-colors {</p>

<pre>@include list-group-item-variant($color, theme-color-level($color, -9), theme-color-level($color, 6));</pre>
