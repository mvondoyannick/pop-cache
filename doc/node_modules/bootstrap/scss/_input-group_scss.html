...
<title>_input-group.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_input-group.scss">
...
<p>// Prepend and append // // While it requires one extra layer of HTML for
each, dedicated prepend and // append elements allow us to 1) be less
clever, 2) simplify our selectors, and // 3) support HTML5 form validation.</p>
...
<p>.input-group-prepend { margin-right: -$input-border-width; }
.input-group-append { margin-left: -$input-border-width; }</p>

<p>// Textual addons // // Serves as a catch-all element for any text or
radio/checkbox input you wish // to prepend or append to an input.</p>
...
<p>// Sizing // // Remix the default form control sizing classes into new ones
for easier // manipulation.</p>

<p>.input-group-lg &gt; .form-control:not(textarea), .input-group-lg &gt;
.custom-select {</p>

<pre>height: $input-height-lg;</pre>
...
<pre>padding: $input-padding-y-lg $input-padding-x-lg;
...
<pre>height: $input-height-sm;</pre>
...
<pre>padding: $input-padding-y-sm $input-padding-x-sm;
...
<pre>padding-right: $custom-select-padding-x + $custom-select-indicator-padding;</pre>

<p>}</p>

<p>// Prepend and append rounded corners // // These rulesets must come after
the sizing ones to properly override sm and lg // border-radius values when
extending. They&#39;re more specific than we&#39;d like // with the
`.input-group &gt;` part, but without it, we cannot override the sizing.</p>
...
<pre>@include border-right-radius(0);</pre>
...
<pre>@include border-left-radius(0);</pre>
