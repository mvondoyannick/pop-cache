...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_button-group.scss">

<p>// stylelint-disable selector-no-qualifying-type</p>

<p>// Make the div behave like a button .btn-group, .btn-group-vertical {</p>
...
<p>// Optional: Group multiple button groups together for a toolbar
.btn-toolbar {</p>
...
<pre>// Prevent double borders when buttons are next to each other
&gt; .btn:not(:first-child),
...
<p>// Sizing // // Remix the default button sizing classes into new ones for
easier manipulation.</p>

<p>.btn-group-sm &gt; .btn { @extend .btn-sm; } .btn-group-lg &gt; .btn {
@extend .btn-lg; }</p>
...
<pre>padding-right: $btn-padding-x * .75;
...
<p>.btn-sm + .dropdown-toggle-split {</p>

<pre>padding-right: $btn-padding-x-sm * .75;
padding-left: $btn-padding-x-sm * .75;</pre>

<p>}</p>

<p>.btn-lg + .dropdown-toggle-split {</p>

<pre>padding-right: $btn-padding-x-lg * .75;
padding-left: $btn-padding-x-lg * .75;</pre>

<p>}</p>

<p>// The clickable button for toggling the menu // Set the same inset shadow
as the :active state .btn-group.show .dropdown-toggle {</p>

<pre>@include box-shadow($btn-active-box-shadow);
...
<p>// Checkbox and radio options // // In order to support the browser&#39;s
form validation feedback, powered by the // `required` attribute, we have
to “hide” the inputs via `clip`. We cannot use // `display: none;` or
`visibility: hidden;` as that also hides the popover. // Simply visually
hiding the inputs via `opacity` would leave them clickable in // certain
cases which is prevented by using `clip` and `pointer-events`. // This way,
we ensure a DOM element is visible to position the popover from. // // See
<a
