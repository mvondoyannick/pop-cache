...
<title>_forms.scss - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/scss/_forms.scss">

<p>// stylelint-disable selector-no-qualifying-type</p>

<p>// // Textual form controls //</p>
...
<p>// Make file inputs better match text inputs by forcing them to new lines.
.form-control-file, .form-control-range {</p>
...
<p>// For use with horizontal and inline forms, when you need the label (or
legend) // text to align with the form controls. .col-form-label {</p>

<pre>padding-top: calc(#{$input-padding-y} + #{$input-border-width});
padding-bottom: calc(#{$input-padding-y} + #{$input-border-width});
margin-bottom: 0; // Override the `&lt;label&gt;/&lt;legend&gt;` default
@include font-size(inherit); // Override the `&lt;legend&gt;` default
line-height: $input-line-height;</pre>

<p>}</p>

<p>.col-form-label-lg {</p>

<pre>padding-top: calc(#{$input-padding-y-lg} + #{$input-border-width});
padding-bottom: calc(#{$input-padding-y-lg} + #{$input-border-width});
@include font-size($input-font-size-lg);
line-height: $input-line-height-lg;</pre>

<p>}</p>

<p>.col-form-label-sm {</p>

<pre>padding-top: calc(#{$input-padding-y-sm} + #{$input-border-width});
padding-bottom: calc(#{$input-padding-y-sm} + #{$input-border-width});
@include font-size($input-font-size-sm);
line-height: $input-line-height-sm;</pre>

<p>}</p>

<p>// Readonly controls as plain text // // Apply class to a readonly input to
make it appear like regular plain // text (without any border, background
color, focus indicator)</p>
...
<p>// Form control sizing // // Build on `.form-control` with modifier classes
to decrease or increase the // height and font-size of form controls. // //
Repeated in `_input_group.scss` to avoid Sass extend issues.</p>

<p>.form-control-sm {</p>

<pre>height: $input-height-sm;
padding: $input-padding-y-sm $input-padding-x-sm;
@include font-size($input-font-size-sm);
line-height: $input-line-height-sm;
@include border-radius($input-border-radius-sm);</pre>

<p>}</p>

<p>.form-control-lg {</p>

<pre>height: $input-height-lg;
padding: $input-padding-y-lg $input-padding-x-lg;
@include font-size($input-font-size-lg);
line-height: $input-line-height-lg;
@include border-radius($input-border-radius-lg);</pre>

<p>}</p>

<p>// stylelint-disable-next-line no-duplicate-selectors select.form-control {</p>
...
<p>// Form groups // // Designed to help with the organization and spacing of
vertical forms. For // horizontal forms, use the predefined grid classes.</p>

<p>.form-group {</p>

<pre>margin-bottom: $form-group-margin-bottom;</pre>
...
<pre>display: block;
margin-top: $form-text-margin-top;</pre>

<p>}</p>

<p>// Form grid // // Special replacement for our grid system&#39;s `.row` for
tighter form layouts.</p>
...
<p>// Checkboxes and radios // // Indent the labels to position
radios/checkboxes as hanging controls.</p>

<p>.form-check {</p>

<pre>position: relative;
display: block;
padding-left: $form-check-input-gutter;</pre>
...
<pre>margin-bottom: 0; // Override default `&lt;label&gt;` bottom margin</pre>
...
<p>// Form validation // // Provide feedback to users when form field values
are valid or invalid. Works // primarily for client-side validation via
scoped `:invalid` and `:valid` // pseudo-classes but also includes
`.is-invalid` and `.is-valid` classes for // server side validation.</p>

<p>@each $state, $data in $form-validation-states {</p>

<pre>@include form-validation-state($state, map-get($data, color), map-get($data, icon));</pre>

<p>}</p>

<p>// Inline forms // // Make forms appear inline(-block) by adding the
`.form-inline` class. Inline // forms begin stacked on extra small (mobile)
devices and then go inline when // viewports reach &lt;768px. // //
Requires wrapping inputs and labels with `.form-group` for proper display
of // default HTML form controls and our custom form controls (e.g., input
groups).</p>
