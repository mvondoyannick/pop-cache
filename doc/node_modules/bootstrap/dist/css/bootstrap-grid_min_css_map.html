...
<title>bootstrap-grid.min.css.map - RDoc Documentation</title>
...
nnhtml {n  box-sizing: border-box;n  -ms-overflow-style:
scrollbar;n}nn*,n*::before,n*::after {n  box-sizing: inherit;n}nn@import
"functions&quot;;n@import "variables&quot;;nn@import
"mixins/breakpoints&quot;;n@import "mixins/grid-framework&quot;;n@import
"mixins/grid&quot;;nn@import "grid&quot;;n@import
"utilities/display&quot;;n@import "utilities/flex&quot;;n@import
"utilities/spacing&quot;;n”,“  !n * Bootstrap Grid v4.3.1 (<a
href="https://getbootstrap.com/)\n">getbootstrap.com/)n</a> * Copyright
2011-2019 The Bootstrap Authorsn * Copyright 2011-2019 Twitter, Inc.n *
Licensed under MIT (<a
href="https://github.com/twbs/bootstrap/blob/master/LICENSE)\n">github.com/twbs/bootstrap/blob/master/LICENSE)n</a>
nhtml {n  box-sizing: border-box;n  -ms-overflow-style:
scrollbar;n}nn*,n*::before,n*::after {n  box-sizing: inherit;n}nn.container
{n  width: 100%;n  padding-right: 15px;n  padding-left: 15px;n 
margin-right: auto;n  margin-left: auto;n}nn@media (min-width: 576px) {n 
.container {n    max-width: 540px;n  }n}nn@media (min-width: 768px) {n 
.container {n    max-width: 720px;n  }n}nn@media (min-width: 992px) {n 
.container {n    max-width: 960px;n  }n}nn@media (min-width: 1200px) {n 
.container {n    max-width: 1140px;n  }n}nn.container-fluid {n  width:
100%;n  padding-right: 15px;n  padding-left: 15px;n  margin-right: auto;n 
...
margin-left: 91.666667%;n  }n}nn.d-none {n  display: none
!important;n}nn.d-inline {n  display: inline !important;n}nn.d-inline-block
{n  display: inline-block !important;n}nn.d-block {n  display: block
!important;n}nn.d-table {n  display: table !important;n}nn.d-table-row {n 
display: table-row !important;n}nn.d-table-cell {n  display: table-cell
!important;n}nn.d-flex {n  display: -ms-flexbox !important;n  display: flex
...
!important;n  }n}nn.flex-row {n  -ms-flex-direction: row !important;n 
flex-direction: row !important;n}nn.flex-column {n  -ms-flex-direction:
column !important;n  flex-direction: column
!important;n}nn.flex-row-reverse {n  -ms-flex-direction: row-reverse
!important;n  flex-direction: row-reverse
...
!important;n    align-self: stretch !important;n  }n}nn.m-0 {n  margin: 0
!important;n}nn.mt-0,n.my-0 {n  margin-top: 0 !important;n}nn.mr-0,n.mx-0
{n  margin-right: 0 !important;n}nn.mb-0,n.my-0 {n  margin-bottom: 0
...
margin-left: auto !important;n  }n}n/*#
sourceMappingURL=bootstrap-grid.css.map */”,“// Container widthsn//n// Set
the container width, and override it for fixed navbars in media
queries.nn@if $enable-grid-classes {n  .container {n    @include
make-container();n    @include make-container-max-widths();n  }n}nn// Fluid
containern//n// Utilizes the mixin meant for fixed width containers, but
with 100% width forn// fluid, full width layouts.nn@if $enable-grid-classes
{n  .container-fluid {n    @include make-container();n  }n}nn// Rown//n//
Rows contain and clear the floats of your columns.nn@if
...
}n}n”,“// stylelint-disable declaration-no-importantnn//n// Utilities for
common `display` valuesn//nn@each $breakpoint in
...
$value in $displays {n      .d#{$infix}-#{$value} { display: $value
!important; }n    }n  }n}nnn//n// Utilities for toggling `display` in
printn//nn@media print {n  @each $value in $displays {n   
.d-print-#{$value} { display: $value !important; }n  }n}n”,“//
stylelint-disable declaration-no-importantnn// Flex variationn//n// Custom
styles for additional flex alignment options.nn@each $breakpoint in
map-keys($grid-breakpoints) {n  @include media-breakpoint-up($breakpoint)
...
}n}n”,“// stylelint-disable declaration-no-importantnn// Margin and
Paddingnn@each $breakpoint in map-keys($grid-breakpoints) {n  @include
media-breakpoint-up($breakpoint) {n    $infix:
breakpoint-infix($breakpoint, $grid-breakpoints);nn    @each $prop, $abbrev
in (margin: m, padding: p) {n      @each $size, $length in $spacers {n     
.#{$abbrev}#{$infix}-#{$size} { #{$prop}: $length !important; }n       
.#{$abbrev}t#{$infix}-#{$size},n        .#{$abbrev}y#{$infix}-#{$size} {n  
#{$prop}-top: $length !important;n        }n       
.#{$abbrev}r#{$infix}-#{$size},n        .#{$abbrev}x#{$infix}-#{$size} {n  
#{$prop}-right: $length !important;n        }n       
.#{$abbrev}b#{$infix}-#{$size},n        .#{$abbrev}y#{$infix}-#{$size} {n  
#{$prop}-bottom: $length !important;n        }n       
.#{$abbrev}l#{$infix}-#{$size},n        .#{$abbrev}x#{$infix}-#{$size} {n  
#{$prop}-left: $length !important;n        }n      }n    }nn    // Negative
margins (e.g., where `.mb-n1` is negative version of `.mb-1`)n    @each
$size, $length in $spacers {n      @if $size != 0 {n       
.m#{$infix}-n#{$size} { margin: -$length !important; }n       
.mt#{$infix}-n#{$size},n        .my#{$infix}-n#{$size} {n         
margin-top: -$length !important;n        }n        .mr#{$infix}-n#{$size},n
.mx#{$infix}-n#{$size} {n          margin-right: -$length !important;n     
}n        .mb#{$infix}-n#{$size},n        .my#{$infix}-n#{$size} {n        
