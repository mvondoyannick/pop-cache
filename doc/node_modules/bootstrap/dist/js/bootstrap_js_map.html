...
<title>bootstrap.js.map - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/bootstrap/dist/js/bootstrap.js.map">

<p><a
href="data-target=\"${target}\"">“version”:3,“file”:“bootstrap.js”,“sources”:[“../../js/src/util.js”,“../../js/src/alert.js”,“../../js/src/button.js”,“../../js/src/carousel.js”,“../../js/src/collapse.js”,“../../js/src/dropdown.js”,“../../js/src/modal.js”,“../../js/src/tools/sanitizer.js”,“../../js/src/tooltip.js”,“../../js/src/popover.js”,“../../js/src/scrollspy.js”,“../../js/src/tab.js”,“../../js/src/toast.js”,“../../js/src/index.js”],“sourcesContent”:[“
n * ————————————————————————–n * Bootstrap (v4.3.1): util.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n   nnimport $ from ‘jquery’nn   n *
————————————————————————n * Private TransitionEnd Helpersn *
————————————————————————n   nnconst TRANSITION_END = ‘transitionend’nconst
MAX_UID = 1000000nconst MILLISECONDS_MULTIPLIER = 1000nn// Shoutout
AngusCroll (https://goo.gl/pxwQGp)nfunction toType(obj) {n  return
{}.toString.call(obj).match(/\s([a-z]+)/i)[1].toLowerCase()n}nnfunction
getSpecialTransitionEndEvent() {n  return {n    bindType: TRANSITION_END,n 
delegateType: TRANSITION_END,n    handle(event) {n      if
($(event.target).is(this)) {n        return
event.handleObj.handler.apply(this, arguments) // eslint-disable-line
prefer-rest-paramsn      }n      return undefined // eslint-disable-line
no-undefinedn    }n  }n}nnfunction transitionEndEmulator(duration) {n  let
called = falsenn  $(this).one(Util.TRANSITION_END, () => {n    called =
truen  })nn  setTimeout(() => {n    if (!called) {n     
Util.triggerTransitionEnd(this)n    }n  }, duration)nn  return
thisn}nnfunction setTransitionEndSupport() {n  $.fn.emulateTransitionEnd =
transitionEndEmulatorn  $.event.special[Util.TRANSITION_END] =
getSpecialTransitionEndEvent()n}nn/**n * ————————————————————————–n *
Public Util Apin * ————————————————————————–n */nnconst Util = {nn 
...
element.getAttribute(‘data-target’)nn    if (!selector || selector === ‘#’)
{n      const hrefAttr = element.getAttribute(‘href’)n      selector =
hrefAttr && hrefAttr !== ‘#’ ? hrefAttr.trim() : ”n    }nn    try {n     
return document.querySelector(selector) ? selector : nulln    } catch (err)
{n      return nulln    }n  },nn  getTransitionDurationFromElement(element)
{n    if (!element) {n      return 0n    }nn    // Get transition-duration
of the elementn    let transitionDuration =
$(element).css(‘transition-duration’)n    let transitionDelay =
$(element).css(‘transition-delay’)nn    const floatTransitionDuration =
parseFloat(transitionDuration)n    const floatTransitionDelay =
parseFloat(transitionDelay)nn    // Return 0 if element or transition
duration is not foundn    if (!floatTransitionDuration &&
!floatTransitionDelay) {n      return 0n    }nn    // If multiple durations
are defined, take the firstn    transitionDuration =
transitionDuration.split(‘,’)[0]n    transitionDelay =
transitionDelay.split(‘,’)[0]nn    return (parseFloat(transitionDuration) +
parseFloat(transitionDelay)) * MILLISECONDS_MULTIPLIERn  },nn 
...
obj).nodeTypen  },nn  typeCheckConfig(componentName, config, configTypes)
{n    for (const property in configTypes) {n      if
...
(!new RegExp(expectedTypes).test(valueType)) {n          throw new Error(n 
`${componentName.toUpperCase()}: ` +n            `Option "${property}"
provided type "${valueType}" ` +n            `but expected type
"${expectedTypes}".`)n        }n      }n    }n  },nn 
findShadowRoot(element) {n    if (!document.documentElement.attachShadow)
{n      return nulln    }nn    // Can find the shadow root otherwise it’ll
return the documentn    if (typeof element.getRootNode === ‘function’) {n  
const root = element.getRootNode()n      return root instanceof ShadowRoot
? root : nulln    }nn    if (element instanceof ShadowRoot) {n      return
elementn    }nn    // when we don’t find a shadow rootn    if
(!element.parentNode) {n      return nulln    }nn    return
Util.findShadowRoot(element.parentNode)n 
}n}nnsetTransitionEndSupport()nnexport default Utiln”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): alert.jsn * Licensed under
MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME                = ‘alert’nconst
VERSION             = ‘4.3.1’nconst DATA_KEY            = ‘bs.alert’nconst
EVENT_KEY           = `.${DATA_KEY}`nconst DATA_API_KEY        =
‘.data-api’nconst JQUERY_NO_CONFLICT  = $.fn[NAME]nnconst Selector = {n 
DISMISS : ‘[data-dismiss="alert"]’n}nnconst Event = {n  CLOSE          :
`close${EVENT_KEY}`,n  CLOSED         : `closed${EVENT_KEY}`,n 
CLICK_DATA_API : `click${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n
ALERT : ‘alert’,n  FADE  : ‘fade’,n  SHOW  : ‘show’n}nn/**n *
————————————————————————n * Class Definitionn * ————————————————————————n
*/nnclass Alert {n  constructor(element) {n    this._element = elementn 
}nn  // Gettersnn  static get VERSION() {n    return VERSIONn  }nn  //
Publicnn  close(element) {n    let rootElement = this._elementn    if
(element) {n      rootElement = this._getRootElement(element)n    }nn   
const customEvent = this._triggerCloseEvent(rootElement)nn    if
...
(selector) {n      parent = document.querySelector(selector)n    }nn    if
(!parent) {n      parent = $(element).closest(`.${ClassName.ALERT}`)[0]n   
}nn    return parentn  }nn  _triggerCloseEvent(element) {n    const
closeEvent = $.Event(Event.CLOSE)nn    $(element).trigger(closeEvent)n   
return closeEventn  }nn  _removeElement(element) {n   
...
.emulateTransitionEnd(transitionDuration)n  }nn  _destroyElement(element)
{n    $(element)n      .detach()n      .trigger(Event.CLOSED)n     
.remove()n  }nn  // Staticnn  static _jQueryInterface(config) {n    return
this.each(function () {n      const $element = $(this)n      let data      
= $element.data(DATA_KEY)nn      if (!data) {n        data = new
Alert(this)n        $element.data(DATA_KEY, data)n      }nn      if (config
=== ‘close’) {n        data[config](this)n      }n    })n  }nn  static
_handleDismiss(alertInstance) {n    return function (event) {n      if
(event) {n        event.preventDefault()n      }nn     
alertInstance.close(this)n    }n  }n}nn/**n * ————————————————————————n *
Data Api implementationn * ————————————————————————n */nn$(document).on(n 
Event.CLICK_DATA_API,n  Selector.DISMISS,n  Alert._handleDismiss(new
Alert())n)nn/**n * ————————————————————————n * jQueryn *
————————————————————————n */nn$.fn[NAME]             =
Alert._jQueryInterfacen$.fn[NAME].Constructor = Alertn$.fn[NAME].noConflict
= () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn  return
Alert._jQueryInterfacen}nnexport default Alertn”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): button.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nn/**n *
————————————————————————n * Constantsn * ————————————————————————n
*/nnconst NAME                = ‘button’nconst VERSION             =
‘4.3.1’nconst DATA_KEY            = ‘bs.button’nconst EVENT_KEY           =
`.${DATA_KEY}`nconst DATA_API_KEY        = ‘.data-api’nconst
JQUERY_NO_CONFLICT  = $.fn[NAME]nnconst ClassName = {n  ACTIVE : ‘active’,n
BUTTON : ‘btn’,n  FOCUS  : ‘focus’n}nnconst Selector = {n 
DATA_TOGGLE_CARROT : ‘[data-toggle^="button"]’,n  DATA_TOGGLE        :
‘[data-toggle="buttons"]’,n  INPUT              :
‘input:not([type="hidden"])’,n  ACTIVE             : ‘.active’,n  BUTTON   
: ‘.btn’n}nnconst Event = {n  CLICK_DATA_API      :
`click${EVENT_KEY}${DATA_API_KEY}`,n  FOCUS_BLUR_DATA_API :
`focus${EVENT_KEY}${DATA_API_KEY} ` +n                         
`blur${EVENT_KEY}${DATA_API_KEY}`n}nn/**n * ————————————————————————n *
Class Definitionn * ————————————————————————n */nnclass Button {n 
constructor(element) {n    this._element = elementn  }nn  // Gettersnn 
static get VERSION() {n    return VERSIONn  }nn  // Publicnn  toggle() {n  
let triggerChangeEvent = truen    let addAriaPressed = truen    const
rootElement = $(this._element).closest(n      Selector.DATA_TOGGLEn   
)[0]nn    if (rootElement) {n      const input =
this._element.querySelector(Selector.INPUT)nn      if (input) {n        if
(input.type === ‘radio’) {n          if (input.checked &&n           
this._element.classList.contains(ClassName.ACTIVE)) {n           
triggerChangeEvent = falsen          } else {n            const
activeElement = rootElement.querySelector(Selector.ACTIVE)nn            if
(activeElement) {n             
$(activeElement).removeClass(ClassName.ACTIVE)n            }n          }n  
}nn        if (triggerChangeEvent) {n          if
...
!this._element.classList.contains(ClassName.ACTIVE))n    }nn    if
(triggerChangeEvent) {n     
$(this._element).toggleClass(ClassName.ACTIVE)n    }n  }nn  dispose() {n   
$.removeData(this._element, DATA_KEY)n    this._element = nulln  }nn  //
Staticnn  static _jQueryInterface(config) {n    return this.each(function
() {n      let data = $(this).data(DATA_KEY)nn      if (!data) {n       
data = new Button(this)n        $(this).data(DATA_KEY, data)n      }nn     
if (config === ‘toggle’) {n        data[config]()n      }n    })n 
}n}nn/**n * ————————————————————————n * Data Api implementationn *
...
*/nn$.fn[NAME] = Button._jQueryInterfacen$.fn[NAME].Constructor =
Buttonn$.fn[NAME].noConflict = () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn 
return Button._jQueryInterfacen}nnexport default Buttonn”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): carousel.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME                   =
‘carousel’nconst VERSION                = ‘4.3.1’nconst DATA_KEY           
= ‘bs.carousel’nconst EVENT_KEY              = `.${DATA_KEY}`nconst
DATA_API_KEY           = ‘.data-api’nconst JQUERY_NO_CONFLICT     =
$.fn[NAME]nconst ARROW_LEFT_KEYCODE     = 37 // KeyboardEvent.which value
for left arrow keynconst ARROW_RIGHT_KEYCODE    = 39 // KeyboardEvent.which
value for right arrow keynconst TOUCHEVENT_COMPAT_WAIT = 500 // Time for
mouse compat events to fire after touchnconst SWIPE_THRESHOLD        =
40nnconst Default = {n  interval : 5000,n  keyboard : true,n  slide    :
false,n  pause    : ‘hover’,n  wrap     : true,n  touch    : truen}nnconst
DefaultType = {n  interval : ‘(number|boolean)’,n  keyboard : ‘boolean’,n 
slide    : ‘(boolean|string)’,n  pause    : ‘(string|boolean)’,n  wrap    
: ‘boolean’,n  touch    : ‘boolean’n}nnconst Direction = {n  NEXT     :
‘next’,n  PREV     : ‘prev’,n  LEFT     : ‘left’,n  RIGHT    :
‘right’n}nnconst Event = {n  SLIDE          : `slide${EVENT_KEY}`,n  SLID  
: `slid${EVENT_KEY}`,n  KEYDOWN        : `keydown${EVENT_KEY}`,n 
MOUSEENTER     : `mouseenter${EVENT_KEY}`,n  MOUSELEAVE     :
`mouseleave${EVENT_KEY}`,n  TOUCHSTART     : `touchstart${EVENT_KEY}`,n 
TOUCHMOVE      : `touchmove${EVENT_KEY}`,n  TOUCHEND       :
`touchend${EVENT_KEY}`,n  POINTERDOWN    : `pointerdown${EVENT_KEY}`,n 
POINTERUP      : `pointerup${EVENT_KEY}`,n  DRAG_START     :
`dragstart${EVENT_KEY}`,n  LOAD_DATA_API  :
`load${EVENT_KEY}${DATA_API_KEY}`,n  CLICK_DATA_API :
`click${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n  CAROUSEL      :
...
: ‘carousel-item’,n  POINTER_EVENT : ‘pointer-event’n}nnconst Selector = {n
ACTIVE      : ‘.active’,n  ACTIVE_ITEM : ‘.active.carousel-item’,n  ITEM   
: ‘.carousel-item’,n  ITEM_IMG    : ‘.carousel-item img’,n  NEXT_PREV   :
‘.carousel-item-next, .carousel-item-prev’,n  INDICATORS  :
‘.carousel-indicators’,n  DATA_SLIDE  : ‘[data-slide], [data-slide-to]’,n 
DATA_RIDE   : ‘[data-ride="carousel"]’n}nnconst PointerType = {n  TOUCH :
‘touch’,n  PEN   : ‘pen’n}nn/**n * ————————————————————————n * Class
Definitionn * ————————————————————————n */nclass Carousel {n 
constructor(element, config) {n    this._items         = nulln   
this._interval      = nulln    this._activeElement = nulln   
this._isPaused      = falsen    this._isSliding     = falsen   
this.touchTimeout   = nulln    this.touchStartX    = 0n    this.touchDeltaX
= 0nn    this._config            = this._getConfig(config)n   
this._element           = elementn    this._indicatorsElement =
this._element.querySelector(Selector.INDICATORS)n    this._touchSupported  
= ‘ontouchstart’ in document.documentElement || navigator.maxTouchPoints >
0n    this._pointerEvent      = Boolean(window.PointerEvent ||
window.MSPointerEvent)nn    this._addEventListeners()n  }nn  // Gettersnn 
static get VERSION() {n    return VERSIONn  }nn  static get Default() {n   
return Defaultn  }nn  // Publicnn  next() {n    if (!this._isSliding) {n   
this._slide(Direction.NEXT)n    }n  }nn  nextWhenVisible() {n    // Don’t
call next when the page isn’t visiblen    // or the carousel or its parent
isn’t visiblen    if (!document.hidden &&n     
...
pause(event) {n    if (!event) {n      this._isPaused = truen    }nn    if
(this._element.querySelector(Selector.NEXT_PREV)) {n     
Util.triggerTransitionEnd(this._element)n      this.cycle(true)n    }nn   
clearInterval(this._interval)n    this._interval = nulln  }nn  cycle(event)
{n    if (!event) {n      this._isPaused = falsen    }nn    if
(this._interval) {n      clearInterval(this._interval)n      this._interval
= nulln    }nn    if (this._config.interval && !this._isPaused) {n     
this._interval = setInterval(n        (document.visibilityState ?
this.nextWhenVisible : this.next).bind(this),n       
this._config.intervaln      )n    }n  }nn  to(index) {n   
this._activeElement = this._element.querySelector(Selector.ACTIVE_ITEM)nn  
const activeIndex = this._getItemIndex(this._activeElement)nn    if (index
> this._items.length - 1 || index < 0) {\n      return\n    }\n\n    if
(this._isSliding) {\n      $(this._element).one(Event.SLID, () =>
this.to(index))n      returnn    }nn    if (activeIndex === index) {n     
this.pause()n      this.cycle()n      returnn    }nn    const direction =
index > activeIndexn      ? Direction.NEXTn      : Direction.PREVnn   
this._slide(direction, this._items[index])n  }nn  dispose() {n   
$(this._element).off(EVENT_KEY)n    $.removeData(this._element, DATA_KEY)nn
this._items             = nulln    this._config            = nulln   
this._element           = nulln    this._interval          = nulln   
this._isPaused          = nulln    this._isSliding         = nulln   
this._activeElement     = nulln    this._indicatorsElement = nulln  }nn  //
Privatenn  _getConfig(config) {n    config = {n      …Default,n     
…confign    }n    Util.typeCheckConfig(NAME, config, DefaultType)n   
return confign  }nn  _handleSwipe() {n    const absDeltax =
Math.abs(this.touchDeltaX)nn    if (absDeltax <= SWIPE_THRESHOLD) {\n     
return\n    }\n\n    const direction = absDeltax / this.touchDeltaX\n\n   
// swipe left\n    if (direction > 0) {n      this.prev()n    }nn    //
swipe rightn    if (direction < 0) {\n      this.next()\n    }\n  }\n\n 
...
this.cycle(event))n    }nn    if (this._config.touch) {n     
this._addTouchEventListeners()n    }n  }nn  _addTouchEventListeners() {n   
if (!this._touchSupported) {n      returnn    }nn    const start = (event)
=> {n      if (this._pointerEvent &&
PointerType[event.originalEvent.pointerType.toUpperCase()]) {n       
this.touchStartX = event.originalEvent.clientXn      } else if
(!this._pointerEvent) {n        this.touchStartX =
event.originalEvent.touches[0].clientXn      }n    }nn    const move =
(event) => {n      // ensure swiping with one touch and not pinchingn     
if (event.originalEvent.touches && event.originalEvent.touches.length > 1)
{n        this.touchDeltaX = 0n      } else {n        this.touchDeltaX =
event.originalEvent.touches[0].clientX - this.touchStartXn      }n    }nn  
const end = (event) => {n      if (this._pointerEvent &&
PointerType[event.originalEvent.pointerType.toUpperCase()]) {n       
this.touchDeltaX = event.originalEvent.clientX - this.touchStartXn      }nn
this._handleSwipe()n      if (this._config.pause === ‘hover’) {n        //
If it’s a touch-enabled device, mouseenter/leave are fired asn        //
part of the mouse compatibility events on first tap - the carouseln       
// would stop cycling until user tapped out of it;n        // here, we
listen for touchend, explicitly pause the carouseln        // (as if it’s
the second time we tap on it, mouseenter compat eventn        // is NOT
fired) and after a timeout (to allow for mouse compatibilityn        //
events to fire) we explicitly restart cyclingnn        this.pause()n       
if (this.touchTimeout) {n          clearTimeout(this.touchTimeout)n       
}n        this.touchTimeout = setTimeout((event) => this.cycle(event),
TOUCHEVENT_COMPAT_WAIT + this._config.interval)n      }n    }nn   
$(this._element.querySelectorAll(Selector.ITEM_IMG)).on(Event.DRAG_START,
(e) => e.preventDefault())n    if (this._pointerEvent) {n     
$(this._element).on(Event.POINTERDOWN, (event) => start(event))n     
$(this._element).on(Event.POINTERUP, (event) => end(event))nn     
this._element.classList.add(ClassName.POINTER_EVENT)n    } else {n     
$(this._element).on(Event.TOUCHSTART, (event) => start(event))n     
$(this._element).on(Event.TOUCHMOVE, (event) => move(event))n     
$(this._element).on(Event.TOUCHEND, (event) => end(event))n    }n  }nn 
_keydown(event) {n    if (/input|textarea/i.test(event.target.tagName)) {n 
returnn    }nn    switch (event.which) {n      case ARROW_LEFT_KEYCODE:n   
event.preventDefault()n        this.prev()n        breakn      case
ARROW_RIGHT_KEYCODE:n        event.preventDefault()n        this.next()n   
breakn      default:n    }n  }nn  _getItemIndex(element) {n    this._items
= element && element.parentNoden      ?
[].slice.call(element.parentNode.querySelectorAll(Selector.ITEM))n      :
[]n    return this._items.indexOf(element)n  }nn 
_getItemByDirection(direction, activeElement) {n    const isNextDirection =
direction === Direction.NEXTn    const isPrevDirection = direction ===
Direction.PREVn    const activeIndex     =
this._getItemIndex(activeElement)n    const lastItemIndex   =
this._items.length - 1n    const isGoingToWrap   = isPrevDirection &&
activeIndex === 0 ||n                            isNextDirection &&
activeIndex === lastItemIndexnn    if (isGoingToWrap && !this._config.wrap)
{n      return activeElementn    }nn    const delta     = direction ===
Direction.PREV ? -1 : 1n    const itemIndex = (activeIndex + delta) %
this._items.lengthnn    return itemIndex === -1n      ?
...
this._getItemIndex(this._element.querySelector(Selector.ACTIVE_ITEM))n   
const slideEvent = $.Event(Event.SLIDE, {n      relatedTarget,n     
direction: eventDirectionName,n      from: fromIndex,n      to:
targetIndexn    })nn    $(this._element).trigger(slideEvent)nn    return
slideEventn  }nn  _setActiveIndicatorElement(element) {n    if
(this._indicatorsElement) {n      const indicators =
[].slice.call(this._indicatorsElement.querySelectorAll(Selector.ACTIVE))n  
$(indicators)n        .removeClass(ClassName.ACTIVE)nn      const
nextIndicator = this._indicatorsElement.children[n       
...
this._element.querySelector(Selector.ACTIVE_ITEM)n    const
...
orderClassName = ClassName.NEXTn      eventDirectionName = Direction.LEFTn 
} else {n      directionalClassName = ClassName.RIGHTn      orderClassName
= ClassName.PREVn      eventDirectionName = Direction.RIGHTn    }nn    if
(nextElement && $(nextElement).hasClass(ClassName.ACTIVE)) {n     
this._isSliding = falsen      returnn    }nn    const slideEvent =
this._triggerSlideEvent(nextElement, eventDirectionName)n    if
(slideEvent.isDefaultPrevented()) {n      returnn    }nn    if
(!activeElement || !nextElement) {n      // Some weirdness is happening, so
we bailn      returnn    }nn    this._isSliding = truenn    if (isCycling)
{n      this.pause()n    }nn   
this._setActiveIndicatorElement(nextElement)nn    const slidEvent =
$.Event(Event.SLID, {n      relatedTarget: nextElement,n      direction:
eventDirectionName,n      from: activeElementIndex,n      to:
nextElementIndexn    })nn    if
...
nextElementInterval = parseInt(nextElement.getAttribute(‘data-interval’),
10)n      if (nextElementInterval) {n        this._config.defaultInterval =
this._config.defaultInterval || this._config.intervaln       
this._config.interval = nextElementIntervaln      } else {n       
this._config.interval = this._config.defaultInterval ||
this._config.intervaln      }nn      const transitionDuration =
Util.getTransitionDurationFromElement(activeElement)nn     
...
this.cycle()n    }n  }nn  // Staticnn  static _jQueryInterface(config) {n  
return this.each(function () {n      let data = $(this).data(DATA_KEY)n    
let _config = {n        …Default,n        …$(this).data()n      }nn      if
(typeof config === ‘object’) {n        _config = {n          …_config,n    
…confign        }n      }nn      const action = typeof config === ‘string’
? config : _config.slidenn      if (!data) {n        data = new
Carousel(this, _config)n        $(this).data(DATA_KEY, data)n      }nn     
if (typeof config === ‘number’) {n        data.to(config)n      } else if
...
(_config.interval && _config.ride) {n        data.pause()n       
data.cycle()n      }n    })n  }nn  static _dataApiClickHandler(event) {n   
const selector = Util.getSelectorFromElement(this)nn    if (!selector) {n  
returnn    }nn    const target = $(selector)[0]nn    if (!target ||
!$(target).hasClass(ClassName.CAROUSEL)) {n      returnn    }nn    const
config = {n      …$(target).data(),n      …$(this).data()n    }n    const
slideIndex = this.getAttribute(‘data-slide-to’)nn    if (slideIndex) {n    
config.interval = falsen    }nn   
Carousel._jQueryInterface.call($(target), config)nn    if (slideIndex) {n  
$(target).data(DATA_KEY).to(slideIndex)n    }nn    event.preventDefault()n 
}n}nn/**n * ————————————————————————n * Data Api implementationn *
————————————————————————n */nn$(document)n  .on(Event.CLICK_DATA_API,
Selector.DATA_SLIDE,
Carousel._dataApiClickHandler)nn$(window).on(Event.LOAD_DATA_API, () => {n 
const carousels =
[].slice.call(document.querySelectorAll(Selector.DATA_RIDE))n  for (let i =
0, len = carousels.length; i < len; i++) {\n    const $carousel =
$(carousels[i])\n    Carousel._jQueryInterface.call($carousel,
$carousel.data())\n  }\n})\n\n/**\n *
------------------------------------------------------------------------\n
* jQuery\n *
------------------------------------------------------------------------\n
*/\n\n$.fn[NAME] = Carousel._jQueryInterface\n$.fn[NAME].Constructor =
Carousel\n$.fn[NAME].noConflict = () => {n  $.fn[NAME] =
JQUERY_NO_CONFLICTn  return Carousel._jQueryInterfacen}nnexport default
Carouseln”,“/**n * ————————————————————————–n * Bootstrap (v4.3.1):
collapse.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME                = ‘collapse’nconst
VERSION             = ‘4.3.1’nconst DATA_KEY            =
‘bs.collapse’nconst EVENT_KEY           = `.${DATA_KEY}`nconst DATA_API_KEY
= ‘.data-api’nconst JQUERY_NO_CONFLICT  = $.fn[NAME]nnconst Default = {n 
toggle : true,n  parent : ”n}nnconst DefaultType = {n  toggle : ‘boolean’,n
parent : ‘(string|element)’n}nnconst Event = {n  SHOW           :
`show${EVENT_KEY}`,n  SHOWN          : `shown${EVENT_KEY}`,n  HIDE         
: `hide${EVENT_KEY}`,n  HIDDEN         : `hidden${EVENT_KEY}`,n 
CLICK_DATA_API : `click${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n
SHOW       : ‘show’,n  COLLAPSE   : ‘collapse’,n  COLLAPSING :
‘collapsing’,n  COLLAPSED  : ‘collapsed’n}nnconst Dimension = {n  WIDTH  :
‘width’,n  HEIGHT : ‘height’n}nnconst Selector = {n  ACTIVES     : ‘.show,
.collapsing’,n  DATA_TOGGLE : ‘[data-toggle="collapse"]’n}nn/**n *
————————————————————————n * Class Definitionn * ————————————————————————n
*/nnclass Collapse {n  constructor(element, config) {n   
this._isTransitioning = falsen    this._element         = elementn   
this._config          = this._getConfig(config)n    this._triggerArray    =
[].slice.call(document.querySelectorAll(n     
`[data-toggle="collapse"][href="#${element.id}"],` +n     
`[data-toggle="collapse"][data-target="#${element.id}"]`n    ))nn    const
toggleList =
[].slice.call(document.querySelectorAll(Selector.DATA_TOGGLE))n    for (let
i = 0, len = toggleList.length; i < len; i++) {\n      const elem =
toggleList[i]\n      const selector = Util.getSelectorFromElement(elem)\n  
const filterElement = [].slice.call(document.querySelectorAll(selector))\n 
.filter((foundElem) => foundElem === element)nn      if (selector !== null
&& filterElement.length > 0) {n        this._selector = selectorn       
this._triggerArray.push(elem)n      }n    }nn    this._parent =
this._config.parent ? this._getParent() : nullnn    if
(!this._config.parent) {n     
this._addAriaAndCollapsedClass(this._element, this._triggerArray)n    }nn  
if (this._config.toggle) {n      this.toggle()n    }n  }nn  // Gettersnn 
static get VERSION() {n    return VERSIONn  }nn  static get Default() {n   
return Defaultn  }nn  // Publicnn  toggle() {n    if
($(this._element).hasClass(ClassName.SHOW)) {n      this.hide()n    } else
{n      this.show()n    }n  }nn  show() {n    if (this._isTransitioning ||n
$(this._element).hasClass(ClassName.SHOW)) {n      returnn    }nn    let
activesn    let activesDatann    if (this._parent) {n      actives =
[].slice.call(this._parent.querySelectorAll(Selector.ACTIVES))n       
.filter((elem) => {n          if (typeof this._config.parent === ‘string’)
{n            return elem.getAttribute(‘data-parent’) ===
this._config.parentn          }nn          return
elem.classList.contains(ClassName.COLLAPSE)n        })nn      if
(actives.length === 0) {n        actives = nulln      }n    }nn    if
(actives) {n      activesData =
$(actives).not(this._selector).data(DATA_KEY)n      if (activesData &&
activesData._isTransitioning) {n        returnn      }n    }nn    const
startEvent = $.Event(Event.SHOW)n    $(this._element).trigger(startEvent)n 
if (startEvent.isDefaultPrevented()) {n      returnn    }nn    if (actives)
{n      Collapse._jQueryInterface.call($(actives).not(this._selector),
‘hide’)n      if (!activesData) {n        $(actives).data(DATA_KEY, null)n 
}n    }nn    const dimension = this._getDimension()nn    $(this._element)n 
.removeClass(ClassName.COLLAPSE)n      .addClass(ClassName.COLLAPSING)nn   
this._element.style[dimension] = 0nn    if (this._triggerArray.length) {n  
$(this._triggerArray)n        .removeClass(ClassName.COLLAPSED)n       
.attr(‘aria-expanded’, true)n    }nn    this.setTransitioning(true)nn   
const complete = () => {n      $(this._element)n       
.removeClass(ClassName.COLLAPSING)n        .addClass(ClassName.COLLAPSE)n  
.addClass(ClassName.SHOW)nn      this._element.style[dimension] = ”nn     
this.setTransitioning(false)nn      $(this._element).trigger(Event.SHOWN)n 
}nn    const capitalizedDimension = dimension[0].toUpperCase() +
dimension.slice(1)n    const scrollSize = `scroll${capitalizedDimension}`n 
const transitionDuration =
Util.getTransitionDurationFromElement(this._element)nn    $(this._element)n
...
!$(this._element).hasClass(ClassName.SHOW)) {n      returnn    }nn    const
startEvent = $.Event(Event.HIDE)n    $(this._element).trigger(startEvent)n 
if (startEvent.isDefaultPrevented()) {n      returnn    }nn    const
...
.removeClass(ClassName.SHOW)nn    const triggerArrayLength =
this._triggerArray.lengthn    if (triggerArrayLength > 0) {n      for (let
i = 0; i < triggerArrayLength; i++) {\n        const trigger =
...
this._isTransitioning = nulln  }nn  // Privatenn  _getConfig(config) {n   
config = {n      …Default,n      …confign    }n    config.toggle =
Boolean(config.toggle) // Coerce string valuesn   
...
Dimension.WIDTH : Dimension.HEIGHTn  }nn  _getParent() {n    let parentnn  
if (Util.isElement(this._config.parent)) {n      parent =
this._config.parentnn      // It’s a jQuery objectn      if (typeof
this._config.parent.jquery !== ‘undefined’) {n        parent =
this._config.parent[0]n      }n    } else {n      parent =
document.querySelector(this._config.parent)n    }nn    const selector =n   
`[data-toggle="collapse"][data-parent="${this._config.parent}"]`nn    const
children = [].slice.call(parent.querySelectorAll(selector))n   
$(children).each((i, element) => {n      this._addAriaAndCollapsedClass(n  
Collapse._getTargetFromElement(element),n        [element]n      )n    })nn
return parentn  }nn  _addAriaAndCollapsedClass(element, triggerArray) {n   
const isOpen = $(element).hasClass(ClassName.SHOW)nn    if
(triggerArray.length) {n      $(triggerArray)n       
.toggleClass(ClassName.COLLAPSED, !isOpen)n        .attr(‘aria-expanded’,
isOpen)n    }n  }nn  // Staticnn  static _getTargetFromElement(element) {n 
const selector = Util.getSelectorFromElement(element)n    return selector ?
document.querySelector(selector) : nulln  }nn  static
_jQueryInterface(config) {n    return this.each(function () {n      const
$this   = $(this)n      let data      = $this.data(DATA_KEY)n      const
_config = {n        …Default,n        …$this.data(),n        …typeof config
=== ‘object’ && config ? config : {}n      }nn      if (!data &&
_config.toggle && /show|hide/.test(config)) {n        _config.toggle =
falsen      }nn      if (!data) {n        data = new Collapse(this,
_config)n        $this.data(DATA_KEY, data)n      }nn      if (typeof
config === ‘string’) {n        if (typeof data[config] === ‘undefined’) {n 
throw new TypeError(`No method named "${config}"`)n        }n       
data[config]()n      }n    })n  }n}nn/**n * ————————————————————————n *
Data Api implementationn * ————————————————————————n
*/nn$(document).on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE, function
(event) {n  // preventDefault only for <a> elements (which change the URL)
not inside the collapsible elementn  if (event.currentTarget.tagName ===
‘A’) {n    event.preventDefault()n  }nn  const $trigger = $(this)n  const
selector = Util.getSelectorFromElement(this)n  const selectors =
[].slice.call(document.querySelectorAll(selector))nn 
$(selectors).each(function () {n    const $target = $(this)n    const data 
= $target.data(DATA_KEY)n    const config  = data ? ‘toggle’ :
$trigger.data()n    Collapse._jQueryInterface.call($target, config)n 
})n})nn/**n * ————————————————————————n * jQueryn *
————————————————————————n */nn$.fn[NAME] =
Collapse._jQueryInterfacen$.fn[NAME].Constructor =
Collapsen$.fn[NAME].noConflict = () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn
return Collapse._jQueryInterfacen}nnexport default Collapsen”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): dropdown.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Popper from
‘popper.js’nimport Util from ‘./util’nn/**n * ————————————————————————n *
Constantsn * ————————————————————————n */nnconst NAME                     =
‘dropdown’nconst VERSION                  = ‘4.3.1’nconst DATA_KEY         
= ‘bs.dropdown’nconst EVENT_KEY                = `.${DATA_KEY}`nconst
DATA_API_KEY             = ‘.data-api’nconst JQUERY_NO_CONFLICT       =
$.fn[NAME]nconst ESCAPE_KEYCODE           = 27 // KeyboardEvent.which value
for Escape (Esc) keynconst SPACE_KEYCODE            = 32 //
KeyboardEvent.which value for space keynconst TAB_KEYCODE              = 9
// KeyboardEvent.which value for tab keynconst ARROW_UP_KEYCODE         =
38 // KeyboardEvent.which value for up arrow keynconst ARROW_DOWN_KEYCODE  
= 40 // KeyboardEvent.which value for down arrow keynconst
RIGHT_MOUSE_BUTTON_WHICH = 3 // MouseEvent.which value for the right button
(assuming a right-handed mouse)nconst REGEXP_KEYDOWN           = new
RegExp(`${ARROW_UP_KEYCODE}|${ARROW_DOWN_KEYCODE}|${ESCAPE_KEYCODE}`)nnconst
Event = {n  HIDE             : `hide${EVENT_KEY}`,n  HIDDEN           :
`hidden${EVENT_KEY}`,n  SHOW             : `show${EVENT_KEY}`,n  SHOWN     
: `shown${EVENT_KEY}`,n  CLICK            : `click${EVENT_KEY}`,n 
CLICK_DATA_API   : `click${EVENT_KEY}${DATA_API_KEY}`,n  KEYDOWN_DATA_API :
`keydown${EVENT_KEY}${DATA_API_KEY}`,n  KEYUP_DATA_API   :
`keyup${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n  DISABLED       
...
POSITION_STATIC : ‘position-static’n}nnconst Selector = {n  DATA_TOGGLE   :
‘[data-toggle="dropdown"]’,n  FORM_CHILD    : ‘.dropdown form’,n  MENU     
: ‘.dropdown-menu’,n  NAVBAR_NAV    : ‘.navbar-nav’,n  VISIBLE_ITEMS :
‘.dropdown-menu .dropdown-item:not(.disabled):not(:disabled)’n}nnconst
AttachmentMap = {n  TOP       : ‘top-start’,n  TOPEND    : ‘top-end’,n 
BOTTOM    : ‘bottom-start’,n  BOTTOMEND : ‘bottom-end’,n  RIGHT     :
‘right-start’,n  RIGHTEND  : ‘right-end’,n  LEFT      : ‘left-start’,n 
LEFTEND   : ‘left-end’n}nnconst Default = {n  offset    : 0,n  flip      :
true,n  boundary  : ‘scrollParent’,n  reference : ‘toggle’,n  display   :
‘dynamic’n}nnconst DefaultType = {n  offset    :
‘(number|string|function)’,n  flip      : ‘boolean’,n  boundary  :
‘(string|element)’,n  reference : ‘(string|element)’,n  display   :
‘string’n}nn/**n * ————————————————————————n * Class Definitionn *
————————————————————————n */nnclass Dropdown {n  constructor(element,
config) {n    this._element  = elementn    this._popper   = nulln   
this._config   = this._getConfig(config)n    this._menu     =
this._getMenuElement()n    this._inNavbar = this._detectNavbar()nn   
this._addEventListeners()n  }nn  // Gettersnn  static get VERSION() {n   
return VERSIONn  }nn  static get Default() {n    return Defaultn  }nn 
static get DefaultType() {n    return DefaultTypen  }nn  // Publicnn 
toggle() {n    if (this._element.disabled ||
$(this._element).hasClass(ClassName.DISABLED)) {n      returnn    }nn   
const parent   = Dropdown._getParentFromElement(this._element)n    const
isActive = $(this._menu).hasClass(ClassName.SHOW)nn   
Dropdown._clearMenus()nn    if (isActive) {n      returnn    }nn    const
relatedTarget = {n      relatedTarget: this._elementn    }n    const
showEvent = $.Event(Event.SHOW, relatedTarget)nn   
$(parent).trigger(showEvent)nn    if (showEvent.isDefaultPrevented()) {n   
returnn    }nn    // Disable totally Popper.js for Dropdown in Navbarn   
if (!this._inNavbar) {n      /**n       * Check for Popper dependencyn     
* Popper - https://popper.js.orgn       */n      if (typeof Popper ===
‘undefined’) {n        throw new TypeError(‘Bootstrap\’s dropdowns require
...
this._config.referencenn        // Check if it’s jQuery elementn        if
(typeof this._config.reference.jquery !== ‘undefined’) {n         
referenceElement = this._config.reference[0]n        }n      }nn      // If
boundary is not `scrollParent`, then set position to `static`n      // to
allow the menu to "escape" the scroll parent’s boundariesn      //
https://github.com/twbs/bootstrap/issues/24251n      if
(this._config.boundary !== ‘scrollParent’) {n       
$(parent).addClass(ClassName.POSITION_STATIC)n      }n      this._popper =
new Popper(referenceElement, this._menu, this._getPopperConfig())n    }nn  
// If this is a touch-enabled device we add extran    // empty mouseover
listeners to the body’s immediate children;n    // only needed because of
broken event delegation on iOSn    //
...
relatedTarget))n  }nn  show() {n    if (this._element.disabled ||
$(this._element).hasClass(ClassName.DISABLED) ||
$(this._menu).hasClass(ClassName.SHOW)) {n      returnn    }nn    const
relatedTarget = {n      relatedTarget: this._elementn    }n    const
showEvent = $.Event(Event.SHOW, relatedTarget)n    const parent =
Dropdown._getParentFromElement(this._element)nn   
$(parent).trigger(showEvent)nn    if (showEvent.isDefaultPrevented()) {n   
returnn    }nn    $(this._menu).toggleClass(ClassName.SHOW)n    $(parent)n 
.toggleClass(ClassName.SHOW)n      .trigger($.Event(Event.SHOWN,
relatedTarget))n  }nn  hide() {n    if (this._element.disabled ||
$(this._element).hasClass(ClassName.DISABLED) ||
!$(this._menu).hasClass(ClassName.SHOW)) {n      returnn    }nn    const
relatedTarget = {n      relatedTarget: this._elementn    }n    const
hideEvent = $.Event(Event.HIDE, relatedTarget)n    const parent =
Dropdown._getParentFromElement(this._element)nn   
$(parent).trigger(hideEvent)nn    if (hideEvent.isDefaultPrevented()) {n   
returnn    }nn    $(this._menu).toggleClass(ClassName.SHOW)n    $(parent)n 
.toggleClass(ClassName.SHOW)n      .trigger($.Event(Event.HIDDEN,
...
this._popper.destroy()n      this._popper = nulln    }n  }nn  update() {n  
this._inNavbar = this._detectNavbar()n    if (this._popper !== null) {n    
this._popper.scheduleUpdate()n    }n  }nn  // Privatenn 
_addEventListeners() {n    $(this._element).on(Event.CLICK, (event) => {n  
event.preventDefault()n      event.stopPropagation()n      this.toggle()n  
})n  }nn  _getConfig(config) {n    config = {n     
…this.constructor.Default,n      …$(this._element).data(),n      …confign  
}nn    Util.typeCheckConfig(n      NAME,n      config,n     
this.constructor.DefaultTypen    )nn    return confign  }nn 
_getMenuElement() {n    if (!this._menu) {n      const parent =
Dropdown._getParentFromElement(this._element)nn      if (parent) {n       
this._menu = parent.querySelector(Selector.MENU)n      }n    }n    return
this._menun  }nn  _getPlacement() {n    const $parentDropdown =
$(this._element.parentNode)n    let placement = AttachmentMap.BOTTOMnn   
// Handle dropupn    if ($parentDropdown.hasClass(ClassName.DROPUP)) {n    
placement = AttachmentMap.TOPn      if
($(this._menu).hasClass(ClassName.MENURIGHT)) {n        placement =
AttachmentMap.TOPENDn      }n    } else if
...
($(this._menu).hasClass(ClassName.MENURIGHT)) {n      placement =
AttachmentMap.BOTTOMENDn    }n    return placementn  }nn  _detectNavbar()
{n    return $(this._element).closest(‘.navbar’).length > 0n  }nn 
_getOffset() {n    const offset = {}nn    if (typeof this._config.offset
=== ‘function’) {n      offset.fn = (data) => {n        data.offsets = {n  
…data.offsets,n          …this._config.offset(data.offsets, this._element)
|| {}n        }nn        return datan      }n    } else {n     
offset.offset = this._config.offsetn    }nn    return offsetn  }nn 
_getPopperConfig() {n    const popperConfig = {n      placement:
this._getPlacement(),n      modifiers: {n        offset:
this._getOffset(),n        flip: {n          enabled: this._config.flipn   
},n        preventOverflow: {n          boundariesElement:
this._config.boundaryn        }n      }n    }nn    // Disable Popper.js if
we have a static displayn    if (this._config.display === ‘static’) {n     
popperConfig.modifiers.applyStyle = {n        enabled: falsen      }n   
}nn    return popperConfign  }nn  // Staticnn  static
...
‘undefined’) {n          throw new TypeError(`No method named
"${config}"`)n        }n        data[config]()n      }n    })n  }nn  static
_clearMenus(event) {n    if (event && (event.which ===
RIGHT_MOUSE_BUTTON_WHICH ||n      event.type === ‘keyup’ && event.which !==
TAB_KEYCODE)) {n      returnn    }nn    const toggles =
[].slice.call(document.querySelectorAll(Selector.DATA_TOGGLE))nn    for
(let i = 0, len = toggles.length; i < len; i++) {\n      const parent =
...
mouseover listeners we added for iOS support\n      if ('ontouchstart' in
...
input/textarea:\n    //  - And not a key in REGEXP_KEYDOWN => not a
dropdown commandn    // If input/textarea:n    //  - If space key => not a
dropdown commandn    //  - If key is other than escapen    //    - If key
is not up or down => not a dropdown commandn    //    - If trigger inside
the menu => not a dropdown commandn    if
(/input|textarea/i.test(event.target.tagName)n      ? event.which ===
SPACE_KEYCODE || event.which !== ESCAPE_KEYCODE &&n      (event.which !==
ARROW_DOWN_KEYCODE && event.which !== ARROW_UP_KEYCODE ||n       
$(event.target).closest(Selector.MENU).length) :
!REGEXP_KEYDOWN.test(event.which)) {n      returnn    }nn   
event.preventDefault()n    event.stopPropagation()nn    if (this.disabled
|| $(this).hasClass(ClassName.DISABLED)) {n      returnn    }nn    const
parent   = Dropdown._getParentFromElement(this)n    const isActive =
$(parent).hasClass(ClassName.SHOW)nn    if (!isActive || isActive &&
(event.which === ESCAPE_KEYCODE || event.which === SPACE_KEYCODE)) {n     
if (event.which === ESCAPE_KEYCODE) {n        const toggle =
parent.querySelector(Selector.DATA_TOGGLE)n       
$(toggle).trigger(‘focus’)n      }nn      $(this).trigger(‘click’)n     
returnn    }nn    const items =
[].slice.call(parent.querySelectorAll(Selector.VISIBLE_ITEMS))nn    if
(items.length === 0) {n      returnn    }nn    let index =
items.indexOf(event.target)nn    if (event.which === ARROW_UP_KEYCODE &&
index > 0) { // Upn      index–n    }nn    if (event.which ===
ARROW_DOWN_KEYCODE && index < items.length - 1) { // Down\n      index++\n 
}\n\n    if (index < 0) {\n      index = 0\n    }\n\n   
items[index].focus()\n  }\n}\n\n/**\n *
------------------------------------------------------------------------\n
...
'toggle')\n  })\n  .on(Event.CLICK_DATA_API, Selector.FORM_CHILD, (e) => {n
e.stopPropagation()n  })nn/**n * ————————————————————————n * jQueryn *
————————————————————————n */nn$.fn[NAME] =
Dropdown._jQueryInterfacen$.fn[NAME].Constructor =
Dropdownn$.fn[NAME].noConflict = () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn
return Dropdown._jQueryInterfacen}nnnexport default Dropdownn”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): modal.jsn * Licensed under
MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME               = ‘modal’nconst
VERSION            = ‘4.3.1’nconst DATA_KEY           = ‘bs.modal’nconst
EVENT_KEY          = `.${DATA_KEY}`nconst DATA_API_KEY       =
‘.data-api’nconst JQUERY_NO_CONFLICT = $.fn[NAME]nconst ESCAPE_KEYCODE    
= 27 // KeyboardEvent.which value for Escape (Esc) keynnconst Default = {n 
backdrop : true,n  keyboard : true,n  focus    : true,n  show     :
truen}nnconst DefaultType = {n  backdrop : ‘(boolean|string)’,n  keyboard :
‘boolean’,n  focus    : ‘boolean’,n  show     : ‘boolean’n}nnconst Event =
{n  HIDE              : `hide${EVENT_KEY}`,n  HIDDEN            :
`hidden${EVENT_KEY}`,n  SHOW              : `show${EVENT_KEY}`,n  SHOWN    
: `shown${EVENT_KEY}`,n  FOCUSIN           : `focusin${EVENT_KEY}`,n 
RESIZE            : `resize${EVENT_KEY}`,n  CLICK_DISMISS     :
`click.dismiss${EVENT_KEY}`,n  KEYDOWN_DISMISS   :
...
`click${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n  SCROLLABLE     
: ‘modal-dialog-scrollable’,n  SCROLLBAR_MEASURER :
‘modal-scrollbar-measure’,n  BACKDROP           : ‘modal-backdrop’,n  OPEN 
: ‘modal-open’,n  FADE               : ‘fade’,n  SHOW               :
‘show’n}nnconst Selector = {n  DIALOG         : ‘.modal-dialog’,n 
MODAL_BODY     : ‘.modal-body’,n  DATA_TOGGLE    :
‘[data-toggle="modal"]’,n  DATA_DISMISS   : ‘[data-dismiss="modal"]’,n 
FIXED_CONTENT  : ‘.fixed-top, .fixed-bottom, .is-fixed, .sticky-top’,n 
STICKY_CONTENT : ‘.sticky-top’n}nn/**n * ————————————————————————n * Class
Definitionn * ————————————————————————n */nnclass Modal {n 
constructor(element, config) {n    this._config              =
this._getConfig(config)n    this._element             = elementn   
this._dialog              = element.querySelector(Selector.DIALOG)n   
this._backdrop            = nulln    this._isShown             = falsen   
this._isBodyOverflowing   = falsen    this._ignoreBackdropClick = falsen   
this._isTransitioning     = falsen    this._scrollbarWidth      = 0n  }nn 
// Gettersnn  static get VERSION() {n    return VERSIONn  }nn  static get
Default() {n    return Defaultn  }nn  // Publicnn  toggle(relatedTarget) {n
return this._isShown ? this.hide() : this.show(relatedTarget)n  }nn 
show(relatedTarget) {n    if (this._isShown || this._isTransitioning) {n   
returnn    }nn    if ($(this._element).hasClass(ClassName.FADE)) {n     
this._isTransitioning = truen    }nn    const showEvent =
$.Event(Event.SHOW, {n      relatedTargetn    })nn   
$(this._element).trigger(showEvent)nn    if (this._isShown ||
showEvent.isDefaultPrevented()) {n      returnn    }nn    this._isShown =
truenn    this._checkScrollbar()n    this._setScrollbar()nn   
this._adjustDialog()nn    this._setEscapeEvent()n   
...
hide(event) {n    if (event) {n      event.preventDefault()n    }nn    if
(!this._isShown || this._isTransitioning) {n      returnn    }nn    const
hideEvent = $.Event(Event.HIDE)nn    $(this._element).trigger(hideEvent)nn 
if (!this._isShown || hideEvent.isDefaultPrevented()) {n      returnn   
}nn    this._isShown = falsen    const transition =
...
this._hideModal(event))n        .emulateTransitionEnd(transitionDuration)n 
} else {n      this._hideModal()n    }n  }nn  dispose() {n    [window,
this._element, this._dialog]n      .forEach((htmlElement) =>
$(htmlElement).off(EVENT_KEY))nn    /**n     * `document` has 2 events
`Event.FOCUSIN` and `Event.CLICK_DATA_API`n     * Do not move `document` in
`htmlElements` arrayn     * It will remove `Event.CLICK_DATA_API` event
that should remainn     */n    $(document).off(Event.FOCUSIN)nn   
$.removeData(this._element, DATA_KEY)nn    this._config              =
nulln    this._element             = nulln    this._dialog              =
nulln    this._backdrop            = nulln    this._isShown             =
nulln    this._isBodyOverflowing   = nulln    this._ignoreBackdropClick =
nulln    this._isTransitioning     = nulln    this._scrollbarWidth      =
nulln  }nn  handleUpdate() {n    this._adjustDialog()n  }nn  // Privatenn 
_getConfig(config) {n    config = {n      …Default,n      …confign    }n   
Util.typeCheckConfig(NAME, config, DefaultType)n    return confign  }nn 
_showElement(relatedTarget) {n    const transition =
...
this._element.removeAttribute(‘aria-hidden’)n   
this._element.setAttribute(‘aria-modal’, true)nn    if
($(this._dialog).hasClass(ClassName.SCROLLABLE)) {n     
this._dialog.querySelector(Selector.MODAL_BODY).scrollTop = 0n    } else {n
this._element.scrollTop = 0n    }nn    if (transition) {n     
Util.reflow(this._element)n    }nn   
$(this._element).addClass(ClassName.SHOW)nn    if (this._config.focus) {n  
this._enforceFocus()n    }nn    const shownEvent = $.Event(Event.SHOWN, {n 
relatedTargetn    })nn    const transitionComplete = () => {n      if
(this._config.focus) {n        this._element.focus()n      }n     
this._isTransitioning = falsen      $(this._element).trigger(shownEvent)n  
}nn    if (transition) {n      const transitionDuration  =
Util.getTransitionDurationFromElement(this._dialog)nn      $(this._dialog)n
.one(Util.TRANSITION_END, transitionComplete)n       
.emulateTransitionEnd(transitionDuration)n    } else {n     
transitionComplete()n    }n  }nn  _enforceFocus() {n    $(document)n     
.off(Event.FOCUSIN) // Guard against infinite focus loopn     
.on(Event.FOCUSIN, (event) => {n        if (document !== event.target &&n  
this._element !== event.target &&n           
$(this._element).has(event.target).length === 0) {n         
this._element.focus()n        }n      })n  }nn  _setEscapeEvent() {n    if
(this._isShown && this._config.keyboard) {n     
...
$(this._element).off(Event.KEYDOWN_DISMISS)n    }n  }nn  _setResizeEvent()
{n    if (this._isShown) {n      $(window).on(Event.RESIZE, (event) =>
this.handleUpdate(event))n    } else {n      $(window).off(Event.RESIZE)n  
}n  }nn  _hideModal() {n    this._element.style.display = ‘none’n   
this._element.setAttribute(‘aria-hidden’, true)n   
this._element.removeAttribute(‘aria-modal’)n    this._isTransitioning =
falsen    this._showBackdrop(() => {n     
$(document.body).removeClass(ClassName.OPEN)n     
this._resetAdjustments()n      this._resetScrollbar()n     
$(this._element).trigger(Event.HIDDEN)n    })n  }nn  _removeBackdrop() {n  
if (this._backdrop) {n      $(this._backdrop).remove()n      this._backdrop
= nulln    }n  }nn  _showBackdrop(callback) {n    const animate =
...
this._backdrop.classList.add(animate)n      }nn     
$(this._backdrop).appendTo(document.body)nn     
$(this._element).on(Event.CLICK_DISMISS, (event) => {n        if
(this._ignoreBackdropClick) {n          this._ignoreBackdropClick = falsen 
returnn        }n        if (event.target !== event.currentTarget) {n      
returnn        }n        if (this._config.backdrop === ‘static’) {n        
this._element.focus()n        } else {n          this.hide()n        }n    
})nn      if (animate) {n        Util.reflow(this._backdrop)n      }nn     
$(this._backdrop).addClass(ClassName.SHOW)nn      if (!callback) {n       
returnn      }nn      if (!animate) {n        callback()n        returnn   
}nn      const backdropTransitionDuration =
...
callbackRemove()n      }n    } else if (callback) {n      callback()n    }n
}nn  // ———————————————————————-n  // the following methods are used to
handle overflowing modalsn  // todo (fat): these should probably be
refactored out of modal.jsn  // ———————————————————————-nn  _adjustDialog()
{n    const isModalOverflowing =n      this._element.scrollHeight >
document.documentElement.clientHeightnn    if (!this._isBodyOverflowing &&
isModalOverflowing) {n      this._element.style.paddingLeft =
`${this._scrollbarWidth}px`n    }nn    if (this._isBodyOverflowing &&
!isModalOverflowing) {n      this._element.style.paddingRight =
`${this._scrollbarWidth}px`n    }n  }nn  _resetAdjustments() {n   
this._element.style.paddingLeft = ”n    this._element.style.paddingRight =
”n  }nn  _checkScrollbar() {n    const rect =
document.body.getBoundingClientRect()n    this._isBodyOverflowing =
rect.left + rect.right < window.innerWidth\n    this._scrollbarWidth =
this._getScrollbarWidth()\n  }\n\n  _setScrollbar() {\n    if
(this._isBodyOverflowing) {\n      // Note: DOMNode.style.paddingRight
returns the actual value or '' if not set\n      //   while
$(DOMNode).css('padding-right') returns the calculated value or 0 if not
set\n      const fixedContent =
[].slice.call(document.querySelectorAll(Selector.FIXED_CONTENT))\n     
const stickyContent =
[].slice.call(document.querySelectorAll(Selector.STICKY_CONTENT))\n\n     
// Adjust fixed content padding\n      $(fixedContent).each((index,
element) => {n        const actualPadding = element.style.paddingRightn    
const calculatedPadding = $(element).css(‘padding-right’)n       
$(element)n          .data(‘padding-right’, actualPadding)n         
.css(‘padding-right’, `${parseFloat(calculatedPadding) +
this._scrollbarWidth}px`)n      })nn      // Adjust sticky content marginn 
$(stickyContent).each((index, element) => {n        const actualMargin =
element.style.marginRightn        const calculatedMargin =
$(element).css(‘margin-right’)n        $(element)n         
.data(‘margin-right’, actualMargin)n          .css(‘margin-right’,
...
$(document.body).css(‘padding-right’)n      $(document.body)n       
.data(‘padding-right’, actualPadding)n        .css(‘padding-right’,
`${parseFloat(calculatedPadding) + this._scrollbarWidth}px`)n    }nn   
$(document.body).addClass(ClassName.OPEN)n  }nn  _resetScrollbar() {n    //
Restore fixed content paddingn    const fixedContent =
[].slice.call(document.querySelectorAll(Selector.FIXED_CONTENT))n   
$(fixedContent).each((index, element) => {n      const padding =
$(element).data(‘padding-right’)n     
$(element).removeData(‘padding-right’)n      element.style.paddingRight =
padding ? padding : ”n    })nn    // Restore sticky contentn    const
elements =
[].slice.call(document.querySelectorAll(`${Selector.STICKY_CONTENT}`))n   
$(elements).each((index, element) => {n      const margin =
$(element).data(‘margin-right’)n      if (typeof margin !== ‘undefined’) {n
$(element).css(‘margin-right’, margin).removeData(‘margin-right’)n      }n 
})nn    // Restore body paddingn    const padding =
$(document.body).data(‘padding-right’)n   
$(document.body).removeData(‘padding-right’)n   
document.body.style.paddingRight = padding ? padding : ”n  }nn 
...
scrollDiv.clientWidthn    document.body.removeChild(scrollDiv)n    return
scrollbarWidthn  }nn  // Staticnn  static _jQueryInterface(config,
relatedTarget) {n    return this.each(function () {n      let data =
$(this).data(DATA_KEY)n      const _config = {n        …Default,n       
…$(this).data(),n        …typeof config === ‘object’ && config ? config :
{}n      }nn      if (!data) {n        data = new Modal(this, _config)n    
$(this).data(DATA_KEY, data)n      }nn      if (typeof config === ‘string’)
{n        if (typeof data[config] === ‘undefined’) {n          throw new
TypeError(`No method named "${config}"`)n        }n       
data[config](relatedTarget)n      } else if (_config.show) {n       
data.show(relatedTarget)n      }n    })n  }n}nn/**n *
————————————————————————n * Data Api implementationn *
————————————————————————n */nn$(document).on(Event.CLICK_DATA_API,
Selector.DATA_TOGGLE, function (event) {n  let targetn  const selector =
Util.getSelectorFromElement(this)nn  if (selector) {n    target =
document.querySelector(selector)n  }nn  const config =
...
(showEvent.isDefaultPrevented()) {n      // Only register focus restorer if
modal will actually get shownn      returnn    }nn   
$target.one(Event.HIDDEN, () => {n      if ($(this).is(‘:visible’)) {n     
this.focus()n      }n    })n  })nn  Modal._jQueryInterface.call($(target),
config, this)n})nn/**n * ————————————————————————n * jQueryn *
————————————————————————n */nn$.fn[NAME] =
Modal._jQueryInterfacen$.fn[NAME].Constructor = Modaln$.fn[NAME].noConflict
= () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn  return
Modal._jQueryInterfacen}nnexport default Modaln”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): tools/sanitizer.jsn *
Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n
* ————————————————————————–n */nnconst uriAttrs = [n  ‘background’,n 
‘cite’,n  ‘href’,n  ‘itemtype’,n  ‘longdesc’,n  ‘poster’,n  ‘src’,n 
‘xlink:href’n]nnconst ARIA_ATTRIBUTE_PATTERN = /^aria-[\w-]*$/innexport
const DefaultWhitelist = {n  // Global attributes allowed on any supplied
element below.n  ‘*’: [‘class’, ‘dir’, ‘id’, ‘lang’, ‘role’,
ARIA_ATTRIBUTE_PATTERN],n  a: [‘target’, ‘href’, ‘title’, ‘rel’],n  area:
[],n  b: [],n  br: [],n  col: [],n  code: [],n  div: [],n  em: [],n  hr:
[],n  h1: [],n  h2: [],n  h3: [],n  h4: [],n  h5: [],n  h6: [],n  i: [],n 
img: [‘src’, ‘alt’, ‘title’, ‘width’, ‘height’],n  li: [],n  ol: [],n  p:
[],n  pre: [],n  s: [],n  small: [],n  span: [],n  sub: [],n  sup: [],n 
strong: [],n  u: [],n  ul: []n}nn/**n * A pattern that recognizes a
commonly useful subset of URLs that are safe.n *n * Shoutout to Angular 7
https://github.com/angular/angular/blob/7.2.4/packages/core/src/sanitization/url_sanitizer.tsn
*/nconst SAFE_URL_PATTERN =
/^(?:(?:https?|mailto|ftp|tel|file):|[^&:/?#]*(?:[/?#]|$))/ginn/**n * A
pattern that matches safe data URLs. Only matches image, video and audio
types.n *n * Shoutout to Angular 7
https://github.com/angular/angular/blob/7.2.4/packages/core/src/sanitization/url_sanitizer.tsn
*/nconst DATA_URL_PATTERN =
/^data:(?:image\/(?:bmp|gif|jpeg|jpg|png|tiff|webp)|video\/(?:mpeg|mp4|ogg|webm)|audio\/(?:mp3|oga|ogg|opus));base64,[a-z0-9+/]+=*$/innfunction
allowedAttribute(attr, allowedAttributeList) {n  const attrName =
attr.nodeName.toLowerCase()nn  if (allowedAttributeList.indexOf(attrName)
!== -1) {n    if (uriAttrs.indexOf(attrName) !== -1) {n      return
Boolean(attr.nodeValue.match(SAFE_URL_PATTERN) ||
attr.nodeValue.match(DATA_URL_PATTERN))n    }nn    return truen  }nn  const
regExp = allowedAttributeList.filter((attrRegex) => attrRegex instanceof
RegExp)nn  // Check if a regular expression validates the attribute.n  for
(let i = 0, l = regExp.length; i < l; i++) {\n    if
(attrName.match(regExp[i])) {\n      return true\n    }\n  }\n\n  return
false\n}\n\nexport function sanitizeHtml(unsafeHtml, whiteList, sanitizeFn)
{\n  if (unsafeHtml.length === 0) {\n    return unsafeHtml\n  }\n\n  if
(sanitizeFn && typeof sanitizeFn === 'function') {\n    return
sanitizeFn(unsafeHtml)\n  }\n\n  const domParser = new window.DOMParser()\n
const createdDocument = domParser.parseFromString(unsafeHtml,
'text/html')\n  const whitelistKeys = Object.keys(whiteList)\n  const
elements = [].slice.call(createdDocument.body.querySelectorAll('*'))\n\n 
for (let i = 0, len = elements.length; i < len; i++) {\n    const el =
elements[i]\n    const elName = el.nodeName.toLowerCase()\n\n    if
(whitelistKeys.indexOf(el.nodeName.toLowerCase()) === -1) {\n     
el.parentNode.removeChild(el)\n\n      continue\n    }\n\n    const
attributeList = [].slice.call(el.attributes)\n    const
whitelistedAttributes = [].concat(whiteList['*'] || [], whiteList[elName]
|| [])\n\n    attributeList.forEach((attr) => {n      if
(!allowedAttribute(attr, whitelistedAttributes)) {n       
el.removeAttribute(attr.nodeName)n      }n    })n  }nn  return
createdDocument.body.innerHTMLn}n”,“/**n * ————————————————————————–n *
Bootstrap (v4.3.1): tooltip.jsn * Licensed under MIT
(https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport {n  DefaultWhitelist,n 
sanitizeHtmln} from ‘./tools/sanitizer’nimport $ from ‘jquery’nimport
Popper from ‘popper.js’nimport Util from ‘./util’nn/**n *
————————————————————————n * Constantsn * ————————————————————————n
*/nnconst NAME                  = ‘tooltip’nconst VERSION               =
‘4.3.1’nconst DATA_KEY              = ‘bs.tooltip’nconst EVENT_KEY         
= `.${DATA_KEY}`nconst JQUERY_NO_CONFLICT    = $.fn[NAME]nconst
CLASS_PREFIX          = ‘bs-tooltip’nconst BSCLS_PREFIX_REGEX    = new
RegExp(`(^|\\s)${CLASS_PREFIX}\\S+`, ‘g’)nconst DISALLOWED_ATTRIBUTES =
[‘sanitize’, ‘whiteList’, ‘sanitizeFn’]nnconst DefaultType = {n  animation 
: ‘boolean’,n  template          : ‘string’,n  title             :
‘(string|element|function)’,n  trigger           : ‘string’,n  delay       
: ‘(number|object)’,n  html              : ‘boolean’,n  selector          :
‘(string|boolean)’,n  placement         : ‘(string|function)’,n  offset    
: ‘(number|string|function)’,n  container         :
‘(string|element|boolean)’,n  fallbackPlacement : ‘(string|array)’,n 
boundary          : ‘(string|element)’,n  sanitize          : ‘boolean’,n 
sanitizeFn        : ‘(null|function)’,n  whiteList         :
‘object’n}nnconst AttachmentMap = {n  AUTO   : ‘auto’,n  TOP    : ‘top’,n 
RIGHT  : ‘right’,n  BOTTOM : ‘bottom’,n  LEFT   : ‘left’n}nnconst Default =
{n  animation         : true,n  template          : ‘<div class=\"tooltip\"
role=\"tooltip\">’ +n                    ‘<div class=\"arrow\"></div>’ +n  
‘<div class=\"tooltip-inner\"></div></div>’,n  trigger           : ‘hover
focus’,n  title             : ”,n  delay             : 0,n  html           
: false,n  selector          : false,n  placement         : ‘top’,n  offset
: 0,n  container         : false,n  fallbackPlacement : ‘flip’,n  boundary 
: ‘scrollParent’,n  sanitize          : true,n  sanitizeFn        : null,n 
whiteList         : DefaultWhitelistn}nnconst HoverState = {n  SHOW :
‘show’,n  OUT  : ‘out’n}nnconst Event = {n  HIDE       :
...
`focusin${EVENT_KEY}`,n  FOCUSOUT   : `focusout${EVENT_KEY}`,n  MOUSEENTER
: `mouseenter${EVENT_KEY}`,n  MOUSELEAVE :
`mouseleave${EVENT_KEY}`n}nnconst ClassName = {n  FADE : ‘fade’,n  SHOW :
‘show’n}nnconst Selector = {n  TOOLTIP       : ‘.tooltip’,n  TOOLTIP_INNER
: ‘.tooltip-inner’,n  ARROW         : ‘.arrow’n}nnconst Trigger = {n  HOVER
: ‘hover’,n  FOCUS  : ‘focus’,n  CLICK  : ‘click’,n  MANUAL :
‘manual’n}nnn/**n * ————————————————————————n * Class Definitionn *
————————————————————————n */nnclass Tooltip {n  constructor(element,
...
this._activeTrigger = {}n    this._popper        = nullnn    // Protectedn 
this.element = elementn    this.config  = this._getConfig(config)n   
this.tip     = nullnn    this._setListeners()n  }nn  // Gettersnn  static
get VERSION() {n    return VERSIONn  }nn  static get Default() {n    return
Defaultn  }nn  static get NAME() {n    return NAMEn  }nn  static get
DATA_KEY() {n    return DATA_KEYn  }nn  static get Event() {n    return
Eventn  }nn  static get EVENT_KEY() {n    return EVENT_KEYn  }nn  static
get DefaultType() {n    return DefaultTypen  }nn  // Publicnn  enable() {n 
this._isEnabled = truen  }nn  disable() {n    this._isEnabled = falsen  }nn
toggleEnabled() {n    this._isEnabled = !this._isEnabledn  }nn 
toggle(event) {n    if (!this._isEnabled) {n      returnn    }nn    if
(event) {n      const dataKey = this.constructor.DATA_KEYn      let context
...
this._leave(null, this)n        returnn      }nn      this._enter(null,
this)n    }n  }nn  dispose() {n    clearTimeout(this._timeout)nn   
$.removeData(this.element, this.constructor.DATA_KEY)nn   
...
this._popper.destroy()n    }nn    this._popper = nulln    this.element =
nulln    this.config  = nulln    this.tip     = nulln  }nn  show() {n    if
($(this.element).css(‘display’) === ‘none’) {n      throw new Error(‘Please
use show on visible elements’)n    }nn    const showEvent =
$.Event(this.constructor.Event.SHOW)n    if (this.isWithContent() &&
this._isEnabled) {n      $(this.element).trigger(showEvent)nn      const
shadowRoot = Util.findShadowRoot(this.element)n      const isInTheDom =
$.contains(n        shadowRoot !== null ? shadowRoot :
this.element.ownerDocument.documentElement,n        this.elementn      )nn 
if (showEvent.isDefaultPrevented() || !isInTheDom) {n        returnn     
}nn      const tip   = this.getTipElement()n      const tipId =
Util.getUID(this.constructor.NAME)nn      tip.setAttribute(‘id’, tipId)n   
this.element.setAttribute(‘aria-describedby’, tipId)nn     
this.setContent()nn      if (this.config.animation) {n       
$(tip).addClass(ClassName.FADE)n      }nn      const placement  = typeof
this.config.placement === ‘function’n        ?
this.config.placement.call(this, tip, this.element)n        :
this.config.placementnn      const attachment =
this._getAttachment(placement)n      this.addAttachmentClass(attachment)nn 
const container = this._getContainer()n     
$(tip).data(this.constructor.DATA_KEY, this)nn      if
(!$.contains(this.element.ownerDocument.documentElement, this.tip)) {n     
$(tip).appendTo(container)n      }nn     
$(this.element).trigger(this.constructor.Event.INSERTED)nn     
this._popper = new Popper(this.element, tip, {n        placement:
attachment,n        modifiers: {n          offset: this._getOffset(),n     
flip: {n            behavior: this.config.fallbackPlacementn          },n  
arrow: {n            element: Selector.ARROWn          },n         
...
onUpdate: (data) => this._handlePopperPlacementChange(data)n      })nn     
$(tip).addClass(ClassName.SHOW)nn      // If this is a touch-enabled device
we add extran      // empty mouseover listeners to the body’s immediate
children;n      // only needed because of broken event delegation on iOSn  
// https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.htmln  
if (‘ontouchstart’ in document.documentElement) {n       
$(document.body).children().on(‘mouseover’, null, $.noop)n      }nn     
...
}n      }nn      if ($(this.tip).hasClass(ClassName.FADE)) {n        const
transitionDuration = Util.getTransitionDurationFromElement(this.tip)nn     
$(this.tip)n          .one(Util.TRANSITION_END, complete)n         
.emulateTransitionEnd(transitionDuration)n      } else {n       
complete()n      }n    }n  }nn  hide(callback) {n    const tip       =
this.getTipElement()n    const hideEvent =
$.Event(this.constructor.Event.HIDE)n    const complete = () => {n      if
(this._hoverState !== HoverState.SHOW && tip.parentNode) {n       
...
(this._popper !== null) {n        this._popper.destroy()n      }nn      if
(callback) {n        callback()n      }n    }nn   
$(this.element).trigger(hideEvent)nn    if (hideEvent.isDefaultPrevented())
{n      returnn    }nn    $(tip).removeClass(ClassName.SHOW)nn    // If
this is a touch-enabled device we remove the extran    // empty mouseover
listeners we added for iOS supportn    if (‘ontouchstart’ in
...
.one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)n    } else {n      complete()n   
}nn    this._hoverState = ”n  }nn  update() {n    if (this._popper !==
null) {n      this._popper.scheduleUpdate()n    }n  }nn  // Protectednn 
isWithContent() {n    return Boolean(this.getTitle())n  }nn 
addAttachmentClass(attachment) {n   
$(this.getTipElement()).addClass(`${CLASS_PREFIX}-${attachment}`)n  }nn 
getTipElement() {n    this.tip = this.tip || $(this.config.template)[0]n   
return this.tipn  }nn  setContent() {n    const tip = this.getTipElement()n
this.setElementContent($(tip.querySelectorAll(Selector.TOOLTIP_INNER)),
this.getTitle())n    $(tip).removeClass(`${ClassName.FADE}
${ClassName.SHOW}`)n  }nn  setElementContent($element, content) {n    if
(typeof content === ‘object’ && (content.nodeType || content.jquery)) {n   
// Content is a DOM node or a jQueryn      if (this.config.html) {n       
if (!$(content).parent().is($element)) {n         
$element.empty().append(content)n        }n      } else {n       
$element.text($(content).text())n      }nn      returnn    }nn    if
(this.config.html) {n      if (this.config.sanitize) {n        content =
sanitizeHtml(content, this.config.whiteList, this.config.sanitizeFn)n     
}nn      $element.html(content)n    } else {n      $element.text(content)n 
}n  }nn  getTitle() {n    let title =
this.element.getAttribute(‘data-original-title’)nn    if (!title) {n     
title = typeof this.config.title === ‘function’n        ?
this.config.title.call(this.element)n        : this.config.titlen    }nn   
return titlen  }nn  // Privatenn  _getOffset() {n    const offset = {}nn   
if (typeof this.config.offset === ‘function’) {n      offset.fn = (data) =>
{n        data.offsets = {n          …data.offsets,n         
…this.config.offset(data.offsets, this.element) || {}n        }nn       
return datan      }n    } else {n      offset.offset = this.config.offsetn 
}nn    return offsetn  }nn  _getContainer() {n    if (this.config.container
=== false) {n      return document.bodyn    }nn    if
(Util.isElement(this.config.container)) {n      return
$(this.config.container)n    }nn    return
$(document).find(this.config.container)n  }nn  _getAttachment(placement) {n
return AttachmentMap[placement.toUpperCase()]n  }nn  _setListeners() {n   
const triggers = this.config.trigger.split(‘ ’)nn   
triggers.forEach((trigger) => {n      if (trigger === ‘click’) {n       
$(this.element).on(n          this.constructor.Event.CLICK,n         
this.config.selector,n          (event) => this.toggle(event)n        )n   
} else if (trigger !== Trigger.MANUAL) {n        const eventIn = trigger
=== Trigger.HOVERn          ? this.constructor.Event.MOUSEENTERn          :
this.constructor.Event.FOCUSINn        const eventOut = trigger ===
Trigger.HOVERn          ? this.constructor.Event.MOUSELEAVEn          :
this.constructor.Event.FOCUSOUTnn        $(this.element)n          .on(n   
eventIn,n            this.config.selector,n            (event) =>
this._enter(event)n          )n          .on(n            eventOut,n       
this.config.selector,n            (event) => this._leave(event)n         
)n      }n    })nn    $(this.element).closest(‘.modal’).on(n     
‘hide.bs.modal’,n      () => {n        if (this.element) {n         
this.hide()n        }n      }n    )nn    if (this.config.selector) {n     
this.config = {n        …this.config,n        trigger: ‘manual’,n       
selector: ”n      }n    } else {n      this._fixTitle()n    }n  }nn 
_fixTitle() {n    const titleType = typeof
this.element.getAttribute(‘data-original-title’)nn    if
(this.element.getAttribute(‘title’) || titleType !== ‘string’) {n     
this.element.setAttribute(n        ‘data-original-title’,n       
this.element.getAttribute(‘title’) || ”n      )nn     
this.element.setAttribute(‘title’, ”)n    }n  }nn  _enter(event, context)
{n    const dataKey = this.constructor.DATA_KEYn    context = context ||
...
($(context.getTipElement()).hasClass(ClassName.SHOW) || context._hoverState
=== HoverState.SHOW) {n      context._hoverState = HoverState.SHOWn     
returnn    }nn    clearTimeout(context._timeout)nn    context._hoverState =
HoverState.SHOWnn    if (!context.config.delay ||
!context.config.delay.show) {n      context.show()n      returnn    }nn   
context._timeout = setTimeout(() => {n      if (context._hoverState ===
HoverState.SHOW) {n        context.show()n      }n    },
context.config.delay.show)n  }nn  _leave(event, context) {n    const
dataKey = this.constructor.DATA_KEYn    context = context ||
$(event.currentTarget).data(dataKey)nn    if (!context) {n      context =
new this.constructor(n        event.currentTarget,n       
this._getDelegateConfig()n      )n     
$(event.currentTarget).data(dataKey, context)n    }nn    if (event) {n     
...
clearTimeout(context._timeout)nn    context._hoverState = HoverState.OUTnn 
if (!context.config.delay || !context.config.delay.hide) {n     
context.hide()n      returnn    }nn    context._timeout = setTimeout(() =>
{n      if (context._hoverState === HoverState.OUT) {n       
context.hide()n      }n    }, context.config.delay.hide)n  }nn 
_isWithActiveTrigger() {n    for (const trigger in this._activeTrigger) {n 
if (this._activeTrigger[trigger]) {n        return truen      }n    }nn   
return falsen  }nn  _getConfig(config) {n    const dataAttributes =
$(this.element).data()nn    Object.keys(dataAttributes)n     
.forEach((dataAttr) => {n        if
(DISALLOWED_ATTRIBUTES.indexOf(dataAttr) !== -1) {n          delete
dataAttributes[dataAttr]n        }n      })nn    config = {n     
…this.constructor.Default,n      …dataAttributes,n      …typeof config ===
‘object’ && config ? config : {}n    }nn    if (typeof config.delay ===
‘number’) {n      config.delay = {n        show: config.delay,n       
hide: config.delayn      }n    }nn    if (typeof config.title === ‘number’)
{n      config.title = config.title.toString()n    }nn    if (typeof
config.content === ‘number’) {n      config.content =
config.content.toString()n    }nn    Util.typeCheckConfig(n      NAME,n    
config,n      this.constructor.DefaultTypen    )nn    if (config.sanitize)
{n      config.template = sanitizeHtml(config.template, config.whiteList,
config.sanitizeFn)n    }nn    return confign  }nn  _getDelegateConfig() {n 
const config = {}nn    if (this.config) {n      for (const key in
this.config) {n        if (this.constructor.Default[key] !==
this.config[key]) {n          config[key] = this.config[key]n        }n    
}n    }nn    return confign  }nn  _cleanTipClass() {n    const $tip =
$(this.getTipElement())n    const tabClass =
$tip.attr(‘class’).match(BSCLS_PREFIX_REGEX)n    if (tabClass !== null &&
tabClass.length) {n      $tip.removeClass(tabClass.join(”))n    }n  }nn 
_handlePopperPlacementChange(popperData) {n    const popperInstance =
popperData.instancen    this.tip = popperInstance.poppern   
this._cleanTipClass()n   
this.addAttachmentClass(this._getAttachment(popperData.placement))n  }nn 
_fixTransition() {n    const tip = this.getTipElement()n    const
initConfigAnimation = this.config.animationnn    if
(tip.getAttribute(‘x-placement’) !== null) {n      returnn    }nn   
$(tip).removeClass(ClassName.FADE)n    this.config.animation = falsen   
this.hide()n    this.show()n    this.config.animation =
initConfigAnimationn  }nn  // Staticnn  static _jQueryInterface(config) {n 
return this.each(function () {n      let data = $(this).data(DATA_KEY)n    
const _config = typeof config === ‘object’ && confignn      if (!data &&
/dispose|hide/.test(config)) {n        returnn      }nn      if (!data) {n 
data = new Tooltip(this, _config)n        $(this).data(DATA_KEY, data)n    
}nn      if (typeof config === ‘string’) {n        if (typeof data[config]
=== ‘undefined’) {n          throw new TypeError(`No method named
"${config}"`)n        }n        data[config]()n      }n    })n  }n}nn/**n *
————————————————————————n * jQueryn * ————————————————————————n
*/nn$.fn[NAME] = Tooltip._jQueryInterfacen$.fn[NAME].Constructor =
Tooltipn$.fn[NAME].noConflict = () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn 
return Tooltip._jQueryInterfacen}nnexport default Tooltipn”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): popover.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Tooltip from
‘./tooltip’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME                = ‘popover’nconst
VERSION             = ‘4.3.1’nconst DATA_KEY            =
‘bs.popover’nconst EVENT_KEY           = `.${DATA_KEY}`nconst
JQUERY_NO_CONFLICT  = $.fn[NAME]nconst CLASS_PREFIX        =
‘bs-popover’nconst BSCLS_PREFIX_REGEX  = new
RegExp(`(^|\\s)${CLASS_PREFIX}\\S+`, ‘g’)nnconst Default = {n 
…Tooltip.Default,n  placement : ‘right’,n  trigger   : ‘click’,n  content  
: ”,n  template  : ‘<div class=\"popover\" role=\"tooltip\">’ +n           
‘<div class=\"arrow\"></div>’ +n              ‘<h3
class=\"popover-header\"></h3>’ +n              ‘<div
class=\"popover-body\"></div></div>’n}nnconst DefaultType = {n 
…Tooltip.DefaultType,n  content : ‘(string|element|function)’n}nnconst
ClassName = {n  FADE : ‘fade’,n  SHOW : ‘show’n}nnconst Selector = {n 
TITLE   : ‘.popover-header’,n  CONTENT : ‘.popover-body’n}nnconst Event =
{n  HIDE       : `hide${EVENT_KEY}`,n  HIDDEN     : `hidden${EVENT_KEY}`,n 
SHOW       : `show${EVENT_KEY}`,n  SHOWN      : `shown${EVENT_KEY}`,n 
INSERTED   : `inserted${EVENT_KEY}`,n  CLICK      : `click${EVENT_KEY}`,n 
FOCUSIN    : `focusin${EVENT_KEY}`,n  FOCUSOUT   : `focusout${EVENT_KEY}`,n
MOUSEENTER : `mouseenter${EVENT_KEY}`,n  MOUSELEAVE :
`mouseleave${EVENT_KEY}`n}nn/**n * ————————————————————————n * Class
Definitionn * ————————————————————————n */nnclass Popover extends Tooltip
{n  // Gettersnn  static get VERSION() {n    return VERSIONn  }nn  static
get Default() {n    return Defaultn  }nn  static get NAME() {n    return
NAMEn  }nn  static get DATA_KEY() {n    return DATA_KEYn  }nn  static get
Event() {n    return Eventn  }nn  static get EVENT_KEY() {n    return
EVENT_KEYn  }nn  static get DefaultType() {n    return DefaultTypen  }nn 
// Overridesnn  isWithContent() {n    return this.getTitle() ||
this._getContent()n  }nn  addAttachmentClass(attachment) {n   
$(this.getTipElement()).addClass(`${CLASS_PREFIX}-${attachment}`)n  }nn 
getTipElement() {n    this.tip = this.tip || $(this.config.template)[0]n   
return this.tipn  }nn  setContent() {n    const $tip =
$(this.getTipElement())nn    // We use append for html objects to maintain
js eventsn    this.setElementContent($tip.find(Selector.TITLE),
this.getTitle())n    let content = this._getContent()n    if (typeof
content === ‘function’) {n      content = content.call(this.element)n    }n
this.setElementContent($tip.find(Selector.CONTENT), content)nn   
$tip.removeClass(`${ClassName.FADE} ${ClassName.SHOW}`)n  }nn  // Privatenn
_getContent() {n    return this.element.getAttribute(‘data-content’) ||n   
this.config.contentn  }nn  _cleanTipClass() {n    const $tip =
$(this.getTipElement())n    const tabClass =
$tip.attr(‘class’).match(BSCLS_PREFIX_REGEX)n    if (tabClass !== null &&
tabClass.length > 0) {n      $tip.removeClass(tabClass.join(”))n    }n  }nn
// Staticnn  static _jQueryInterface(config) {n    return
this.each(function () {n      let data = $(this).data(DATA_KEY)n      const
_config = typeof config === ‘object’ ? config : nullnn      if (!data &&
/dispose|hide/.test(config)) {n        returnn      }nn      if (!data) {n 
data = new Popover(this, _config)n        $(this).data(DATA_KEY, data)n    
}nn      if (typeof config === ‘string’) {n        if (typeof data[config]
=== ‘undefined’) {n          throw new TypeError(`No method named
"${config}"`)n        }n        data[config]()n      }n    })n  }n}nn/**n *
————————————————————————n * jQueryn * ————————————————————————n
*/nn$.fn[NAME] = Popover._jQueryInterfacen$.fn[NAME].Constructor =
Popovern$.fn[NAME].noConflict = () => {n  $.fn[NAME] = JQUERY_NO_CONFLICTn 
return Popover._jQueryInterfacen}nnexport default Popovern”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): scrollspy.jsn * Licensed
under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)n *
————————————————————————–n */nnimport $ from ‘jquery’nimport Util from
‘./util’nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME               = ‘scrollspy’nconst
VERSION            = ‘4.3.1’nconst DATA_KEY           =
‘bs.scrollspy’nconst EVENT_KEY          = `.${DATA_KEY}`nconst DATA_API_KEY
= ‘.data-api’nconst JQUERY_NO_CONFLICT = $.fn[NAME]nnconst Default = {n 
offset : 10,n  method : ‘auto’,n  target : ”n}nnconst DefaultType = {n 
offset : ‘number’,n  method : ‘string’,n  target :
‘(string|element)’n}nnconst Event = {n  ACTIVATE      :
`activate${EVENT_KEY}`,n  SCROLL        : `scroll${EVENT_KEY}`,n 
LOAD_DATA_API : `load${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n 
DROPDOWN_ITEM : ‘dropdown-item’,n  DROPDOWN_MENU : ‘dropdown-menu’,n 
ACTIVE        : ‘active’n}nnconst Selector = {n  DATA_SPY        :
...
‘.dropdown-toggle’n}nnconst OffsetMethod = {n  OFFSET   : ‘offset’,n 
POSITION : ‘position’n}nn/**n * ————————————————————————n * Class
Definitionn * ————————————————————————n */nnclass ScrollSpy {n 
...
(event) => this._process(event))nn    this.refresh()n    this._process()n 
}nn  // Gettersnn  static get VERSION() {n    return VERSIONn  }nn  static
get Default() {n    return Defaultn  }nn  // Publicnn  refresh() {n   
const autoMethod = this._scrollElement === this._scrollElement.windown     
...
this._getScrollTop() : 0nn    this._offsets = []n    this._targets = []nn  
this._scrollHeight = this._getScrollHeight()nn    const targets =
[].slice.call(document.querySelectorAll(this._selector))nn    targetsn     
.map((element) => {n        let targetn        const targetSelector =
Util.getSelectorFromElement(element)nn        if (targetSelector) {n       
target = document.querySelector(targetSelector)n        }nn        if
(target) {n          const targetBCR = target.getBoundingClientRect()n     
if (targetBCR.width || targetBCR.height) {n            // TODO (fat):
remove sketch reliance on jQuery position/offsetn            return [n     
$(target)[offsetMethod]().top + offsetBase,n              targetSelectorn  
]n          }n        }n        return nulln      })n      .filter((item)
=> item)n      .sort((a, b) => a[0] - b[0])n      .forEach((item) => {n    
this._offsets.push(item[0])n        this._targets.push(item[1])n      })n 
}nn  dispose() {n    $.removeData(this._element, DATA_KEY)n   
$(this._scrollElement).off(EVENT_KEY)nn    this._element       = nulln   
this._scrollElement = nulln    this._config        = nulln   
this._selector      = nulln    this._offsets       = nulln    this._targets
= nulln    this._activeTarget  = nulln    this._scrollHeight  = nulln  }nn 
// Privatenn  _getConfig(config) {n    config = {n      …Default,n     
…typeof config === ‘object’ && config ? config : {}n    }nn    if (typeof
...
}nn    Util.typeCheckConfig(NAME, config, DefaultType)nn    return confign 
}nn  _getScrollTop() {n    return this._scrollElement === windown      ?
this._scrollElement.pageYOffset : this._scrollElement.scrollTopn  }nn 
_getScrollHeight() {n    return this._scrollElement.scrollHeight ||
Math.max(n      document.body.scrollHeight,n     
document.documentElement.scrollHeightn    )n  }nn  _getOffsetHeight() {n   
return this._scrollElement === windown      ? window.innerHeight :
...
this._config.offset +n      scrollHeight -n      this._getOffsetHeight()nn 
if (this._scrollHeight !== scrollHeight) {n      this.refresh()n    }nn   
if (scrollTop >= maxScroll) {n      const target =
this._targets[this._targets.length - 1]nn      if (this._activeTarget !==
target) {n        this._activate(target)n      }n      returnn    }nn    if
(this._activeTarget && scrollTop < this._offsets[0] && this._offsets[0] >
0) {n      this._activeTarget = nulln      this._clear()n      returnn   
}nn    const offsetLength = this._offsets.lengthn    for (let i =
offsetLength; i–;) {n      const isActiveTarget = this._activeTarget !==
this._targets[i] &&n          scrollTop >= this._offsets[i] &&n         
(typeof this._offsets[i + 1] === ‘undefined’ ||n              scrollTop <
this._offsets[i + 1])\n\n      if (isActiveTarget) {\n       
this._activate(this._targets[i])\n      }\n    }\n  }\n\n 
_activate(target) {\n    this._activeTarget = target\n\n   
this._clear()\n\n    const queries = this._selector\n      .split(',')\n   
.map((selector) => `${selector</a>,$<a
href="href=\"${target}\"">selector</a>`)nn    const $link =
$([].slice.call(document.querySelectorAll(queries.join(&#39;,&#39;))))nn   
if ($link.hasClass(ClassName.DROPDOWN_ITEM)) {n     
$link.closest(Selector.DROPDOWN).find(Selector.DROPDOWN_TOGGLE).addClass(ClassName.ACTIVE)n
$link.addClass(ClassName.ACTIVE)n    } else {n      // Set triggered link
as activen      $link.addClass(ClassName.ACTIVE)n      // Set triggered
links parents as activen      // With both &lt;ul&gt; and &lt;nav&gt;
markup a parent is the previous sibling of any nav ancestorn     
$link.parents(Selector.NAV_LIST_GROUP).prev(`${Selector.NAV_LINKS},
${Selector.LIST_ITEMS}`).addClass(ClassName.ACTIVE)n      // Handle special
case when .nav-link is inside .nav-itemn     
...
[].slice.call(document.querySelectorAll(this._selector))n     
.filter((node) =&gt; node.classList.contains(ClassName.ACTIVE))n     
.forEach((node) =&gt; node.classList.remove(ClassName.ACTIVE))n  }nn  //
Staticnn  static _jQueryInterface(config) {n    return this.each(function
() {n      let data = $(this).data(DATA_KEY)n      const _config = typeof
config === &#39;object&#39; &amp;&amp; confignn      if (!data) {n       
data = new ScrollSpy(this, _config)n        $(this).data(DATA_KEY, data)n  
}nn      if (typeof config === &#39;string&#39;) {n        if (typeof <a
href="config">data</a> === &#39;undefined&#39;) {n          throw new
TypeError(`No method named "${config}&quot;`)n        }n        <a
href="config">data</a>()n      }n    })n  }n}nn/**n *
————————————————————————n * Data <a href="../../../../Api.html">Api</a>
implementationn * ————————————————————————n
*/nn$(window).on(Event.LOAD_DATA_API, () =&gt; {n  const scrollSpys =
[].slice.call(document.querySelectorAll(Selector.DATA_SPY))n  const
scrollSpysLength = scrollSpys.lengthnn  for (let i = scrollSpysLength; i–;)
{n    const $spy = $(<a href="i">scrollSpys</a>)n   
ScrollSpy._jQueryInterface.call($spy, $spy.data())n  }n})nn/**n *
————————————————————————n * jQueryn * ————————————————————————n */<a
href=”NAME“>nn$.fn</a> = <a
href="NAME">ScrollSpy._jQueryInterfacen$.fn</a>.Constructor = <a
href="NAME">ScrollSpyn$.fn</a>.noConflict = () =&gt; {n  $.<a
href="NAME">fn</a> = JQUERY_NO_CONFLICTn  return
ScrollSpy._jQueryInterfacen}nnexport default ScrollSpyn”,“/**n *
————————————————————————–n * Bootstrap (v4.3.1): tab.jsn * Licensed under
MIT (<a
href="https://github.com/twbs/bootstrap/blob/master/LICENSE)\n">github.com/twbs/bootstrap/blob/master/LICENSE)n</a>
* ————————————————————————–n */nnimport $ from &#39;jquery&#39;nimport Util
from &#39;./util&#39;nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME               =
&#39;tab&#39;nconst VERSION            = &#39;4.3.1&#39;nconst DATA_KEY    
= &#39;bs.tab&#39;nconst EVENT_KEY          = `.${DATA_KEY}`nconst
DATA_API_KEY       = &#39;.data-api&#39;nconst JQUERY_NO_CONFLICT = $.<a
href="NAME">fn</a>nnconst Event = {n  HIDE           : `hide${EVENT_KEY}`,n
HIDDEN         : `hidden${EVENT_KEY}`,n  SHOW           :
`show${EVENT_KEY}`,n  SHOWN          : `shown${EVENT_KEY}`,n 
CLICK_DATA_API : `click${EVENT_KEY}${DATA_API_KEY}`n}nnconst ClassName = {n
DROPDOWN_MENU : &#39;dropdown-menu&#39;,n  ACTIVE        :
&#39;active&#39;,n  DISABLED      : &#39;disabled&#39;,n  FADE          :
&#39;fade&#39;,n  SHOW          : &#39;show&#39;n}nnconst Selector = {n 
DROPDOWN              : &#39;.dropdown&#39;,n  NAV_LIST_GROUP        :
&#39;.nav, .list-group&#39;,n  ACTIVE                : &#39;.active&#39;,n 
ACTIVE_UL             : &#39;&gt; li &gt; .active&#39;,n  DATA_TOGGLE      
: &#39;[data-toggle=&quot;tab&quot;], [data-toggle=&quot;pill&quot;],
[data-toggle=&quot;list&quot;]&#39;,n  DROPDOWN_TOGGLE       :
&#39;.dropdown-toggle&#39;,n  DROPDOWN_ACTIVE_CHILD : &#39;&gt;
.dropdown-menu .active&#39;n}nn/**n * ————————————————————————n * Class
Definitionn * ————————————————————————n */nnclass Tab {n 
constructor(element) {n    this._element = elementn  }nn  // Gettersnn 
static get VERSION() {n    return VERSIONn  }nn  // Publicnn  show() {n   
if (this._element.parentNode &amp;&amp;n       
...
selector = Util.getSelectorFromElement(this._element)nn    if (listElement)
{n      const itemSelector = listElement.nodeName === &#39;UL&#39; ||
listElement.nodeName === &#39;OL&#39; ? Selector.ACTIVE_UL :
Selector.ACTIVEn      previous =
$.makeArray($(listElement).find(itemSelector))n      previous =
previous[previous.length - 1]n    }nn    const hideEvent =
$.Event(Event.HIDE, {n      relatedTarget: this._elementn    })nn    const
showEvent = $.Event(Event.SHOW, {n      relatedTarget: previousn    })nn   
if (previous) {n      $(previous).trigger(hideEvent)n    }nn   
$(this._element).trigger(showEvent)nn    if (showEvent.isDefaultPrevented()
||n        hideEvent.isDefaultPrevented()) {n      returnn    }nn    if
(selector) {n      target = document.querySelector(selector)n    }nn   
this._activate(n      this._element,n      listElementn    )nn    const
complete = () =&gt; {n      const hiddenEvent = $.Event(Event.HIDDEN, {n   
...
DATA_KEY)n    this._element = nulln  }nn  // Privatenn  _activate(element,
container, callback) {n    const activeElements = container &amp;&amp;
(container.nodeName === &#39;UL&#39; || container.nodeName ===
&#39;OL&#39;)n      ? $(container).find(Selector.ACTIVE_UL)n      :
$(container).children(Selector.ACTIVE)nn    const active = <a
href="0">activeElements</a>n    const isTransitioning = callback &amp;&amp;
(active &amp;&amp; $(active).hasClass(ClassName.FADE))n    const complete =
() =&gt; this._transitionComplete(n      element,n      active,n     
callbackn    )nn    if (active &amp;&amp; isTransitioning) {n      const
transitionDuration = Util.getTransitionDurationFromElement(active)nn     
$(active)n        .removeClass(ClassName.SHOW)n       
.one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)n    } else {n      complete()n   
}n  }nn  _transitionComplete(element, active, callback) {n    if (active)
{n      $(active).removeClass(ClassName.ACTIVE)nn      const dropdownChild
= $(active.parentNode).find(n        Selector.DROPDOWN_ACTIVE_CHILDn     
)[0]nn      if (dropdownChild) {n       
...
Util.reflow(element)nn    if (element.classList.contains(ClassName.FADE))
{n      element.classList.add(ClassName.SHOW)n    }nn    if
(element.parentNode &amp;&amp;
$(element.parentNode).hasClass(ClassName.DROPDOWN_MENU)) {n      const
dropdownElement = $(<a href="0">element).closest(Selector.DROPDOWN)</a>nn  
if (dropdownElement) {n        const dropdownToggleList =
[].slice.call(dropdownElement.querySelectorAll(Selector.DROPDOWN_TOGGLE))nn
$(dropdownToggleList).addClass(ClassName.ACTIVE)n      }nn     
element.setAttribute(&#39;aria-expanded&#39;, true)n    }nn    if
(callback) {n      callback()n    }n  }nn  // Staticnn  static
_jQueryInterface(config) {n    return this.each(function () {n      const
$this = $(this)n      let data = $this.data(DATA_KEY)nn      if (!data) {n 
data = new Tab(this)n        $this.data(DATA_KEY, data)n      }nn      if
(typeof config === &#39;string&#39;) {n        if (typeof <a
href="config">data</a> === &#39;undefined&#39;) {n          throw new
TypeError(`No method named "${config}&quot;`)n        }n        <a
href="config">data</a>()n      }n    })n  }n}nn/**n *
————————————————————————n * Data <a href="../../../../Api.html">Api</a>
implementationn * ————————————————————————n */nn$(document)n 
.on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE, function (event) {n   
event.preventDefault()n    Tab._jQueryInterface.call($(this),
&#39;show&#39;)n  })nn/**n * ————————————————————————n * jQueryn *
————————————————————————n */<a href=”NAME“>nn$.fn</a> = <a
href="NAME">Tab._jQueryInterfacen$.fn</a>.Constructor = <a
href="NAME">Tabn$.fn</a>.noConflict = () =&gt; {n  $.<a href="NAME">fn</a>
= JQUERY_NO_CONFLICTn  return Tab._jQueryInterfacen}nnexport default
Tabn”,“/**n * ————————————————————————–n * Bootstrap (v4.3.1): toast.jsn *
Licensed under MIT (<a
href="https://github.com/twbs/bootstrap/blob/master/LICENSE)\n">github.com/twbs/bootstrap/blob/master/LICENSE)n</a>
* ————————————————————————–n */nnimport $ from &#39;jquery&#39;nimport Util
from &#39;./util&#39;nn/**n * ————————————————————————n * Constantsn *
————————————————————————n */nnconst NAME               =
&#39;toast&#39;nconst VERSION            = &#39;4.3.1&#39;nconst DATA_KEY  
= &#39;bs.toast&#39;nconst EVENT_KEY          = `.${DATA_KEY}`nconst
JQUERY_NO_CONFLICT = $.<a href="NAME">fn</a>nnconst Event = {n 
CLICK_DISMISS : `click.dismiss${EVENT_KEY}`,n  HIDE          :
`hide${EVENT_KEY}`,n  HIDDEN        : `hidden${EVENT_KEY}`,n  SHOW         
: `show${EVENT_KEY}`,n  SHOWN         : `shown${EVENT_KEY}`n}nnconst
ClassName = {n  FADE    : &#39;fade&#39;,n  HIDE    : &#39;hide&#39;,n 
SHOW    : &#39;show&#39;,n  SHOWING : &#39;showing&#39;n}nnconst
DefaultType = {n  animation : &#39;boolean&#39;,n  autohide  :
&#39;boolean&#39;,n  delay     : &#39;number&#39;n}nnconst Default = {n 
animation : true,n  autohide  : true,n  delay     : 500n}nnconst Selector =
{n  DATA_DISMISS : &#39;[data-dismiss=&quot;toast&quot;]&#39;n}nn/**n *
————————————————————————n * Class Definitionn * ————————————————————————n
*/nnclass Toast {n  constructor(element, config) {n    this._element =
elementn    this._config  = this._getConfig(config)n    this._timeout =
nulln    this._setListeners()n  }nn  // Gettersnn  static get VERSION() {n 
return VERSIONn  }nn  static get DefaultType() {n    return DefaultTypen 
}nn  static get Default() {n    return Defaultn  }nn  // Publicnn  show()
{n    $(this._element).trigger(Event.SHOW)nn    if (this._config.animation)
{n      this._element.classList.add(ClassName.FADE)n    }nn    const
complete = () =&gt; {n     
this._element.classList.remove(ClassName.SHOWING)n     
this._element.classList.add(ClassName.SHOW)nn     
$(this._element).trigger(Event.SHOWN)nn      if (this._config.autohide) {n 
this.hide()n      }n    }nn   
this._element.classList.remove(ClassName.HIDE)n   
this._element.classList.add(ClassName.SHOWING)n    if
(this._config.animation) {n      const transitionDuration =
Util.getTransitionDurationFromElement(this._element)nn     
$(this._element)n        .one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)n    } else {n      complete()n   
}n  }nn  hide(withoutTimeout) {n    if
(!this._element.classList.contains(ClassName.SHOW)) {n      returnn    }nn 
$(this._element).trigger(Event.HIDE)nn    if (withoutTimeout) {n     
this._close()n    } else {n      this._timeout = setTimeout(() =&gt; {n    
this._close()n      }, this._config.delay)n    }n  }nn  dispose() {n   
clearTimeout(this._timeout)n    this._timeout = nullnn    if
(this._element.classList.contains(ClassName.SHOW)) {n     
this._element.classList.remove(ClassName.SHOW)n    }nn   
$(this._element).off(Event.CLICK_DISMISS)nn    $.removeData(this._element,
DATA_KEY)n    this._element = nulln    this._config  = nulln  }nn  //
Privatenn  _getConfig(config) {n    config = {n      …Default,n     
…$(this._element).data(),n      …typeof config === &#39;object&#39;
&amp;&amp; config ? config : {}n    }nn    Util.typeCheckConfig(n     
NAME,n      config,n      this.constructor.DefaultTypen    )nn    return
confign  }nn  _setListeners() {n    $(this._element).on(n     
Event.CLICK_DISMISS,n      Selector.DATA_DISMISS,n      () =&gt;
this.hide(true)n    )n  }nn  _close() {n    const complete = () =&gt; {n   
this._element.classList.add(ClassName.HIDE)n     
$(this._element).trigger(Event.HIDDEN)n    }nn   
this._element.classList.remove(ClassName.SHOW)n    if
(this._config.animation) {n      const transitionDuration =
Util.getTransitionDurationFromElement(this._element)nn     
$(this._element)n        .one(Util.TRANSITION_END, complete)n       
.emulateTransitionEnd(transitionDuration)n    } else {n      complete()n   
}n  }nn  // Staticnn  static _jQueryInterface(config) {n    return
this.each(function () {n      const $element = $(this)n      let data      
= $element.data(DATA_KEY)n      const _config  = typeof config ===
&#39;object&#39; &amp;&amp; confignn      if (!data) {n        data = new
Toast(this, _config)n        $element.data(DATA_KEY, data)n      }nn     
if (typeof config === &#39;string&#39;) {n        if (typeof <a
href="config">data</a> === &#39;undefined&#39;) {n          throw new
TypeError(`No method named "${config}&quot;`)n        }nn        <a
href="config">data</a>(this)n      }n    })n  }n}nn/**n *
————————————————————————n * jQueryn * ————————————————————————n */<a
href=”NAME“>nn$.fn</a>             = <a
href="NAME">Toast._jQueryInterfacen$.fn</a>.Constructor = <a
href="NAME">Toastn$.fn</a>.noConflict  = () =&gt; {n  $.<a
href="NAME">fn</a> = JQUERY_NO_CONFLICTn  return
Toast._jQueryInterfacen}nnexport default Toastn”,“import $ from
&#39;jquery&#39;nimport Alert from &#39;./alert&#39;nimport Button from
&#39;./button&#39;nimport Carousel from &#39;./carousel&#39;nimport
...
Tab from &#39;./tab&#39;nimport Toast from &#39;./toast&#39;nimport Tooltip
from &#39;./tooltip&#39;nimport Util from &#39;./util&#39;nn/**n *
————————————————————————–n * Bootstrap (v4.3.1): index.jsn * Licensed under
MIT (<a
href="https://github.com/twbs/bootstrap/blob/master/LICENSE)\n">github.com/twbs/bootstrap/blob/master/LICENSE)n</a>
* ————————————————————————–n */nn(() =&gt; {n  if (typeof $ ===
