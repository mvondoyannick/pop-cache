...
<title>deploy.sh - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/chart.js/scripts/deploy.sh">

<p>#!/bin/bash</p>

<p>set -e</p>

<p>TARGET_DIR=&#39;gh-pages&#39; TARGET_BRANCH=&#39;master&#39;
TARGET_REPO_URL=“<a
href="https://$GITHUB_AUTH_TOKEN@github.com/chartjs/chartjs.github.io.git">$GITHUB_AUTH_TOKEN@github.com/chartjs/chartjs.github.io.git</a>”
<a href=":digit:">VERSION_REGEX=‘[</a>]+.[[:<a
href=":digit:">]]+.[</a>]+(-.*)?&#39;</p>

<p># Make sure that this script is executed only for the release and master
branches if [ “$TRAVIS_BRANCH” == “release” ]; then</p>

<pre># Travis executes this script from the repository root, so at the same level than package.json
VERSION=$(node -p -e &quot;require(&#39;./package.json&#39;).version&quot;)</pre>

<p>elif [ “$TRAVIS_BRANCH” == “master” ]; then</p>

<pre class="ruby"><span class="ruby-constant">VERSION</span>=<span class="ruby-string">&quot;master&quot;</span>
</pre>

<p>else</p>

<pre class="ruby"><span class="ruby-identifier">echo</span> <span class="ruby-string">&quot;Skipping deploy because this is not the master or release branch&quot;</span>
<span class="ruby-identifier">exit</span> <span class="ruby-value">0</span>
</pre>

<p>fi</p>

<p>function update_latest {</p>

<pre>local out_path=$1
local latest=($(ls -v $out_path | egrep &#39;^(&#39;$VERSION_REGEX&#39;)$&#39; | tail -1))
if [ &quot;$latest&quot; == &quot;&quot; ]; then latest=&#39;master&#39;; fi
rm -f $out_path/latest
ln -s $latest $out_path/latest</pre>

<p>}</p>

<p>function deploy_files {</p>

<pre>local in_files=$1
local out_path=$2
rm -rf $out_path/$VERSION
mkdir -p $out_path/$VERSION
cp -r $in_files $out_path/$VERSION
update_latest $out_path</pre>

<p>}</p>

<p># Clone the repository and checkout the gh-pages branch git clone
$TARGET_REPO_URL $TARGET_DIR cd $TARGET_DIR git checkout $TARGET_BRANCH</p>

<p># Copy dist files deploy_files &#39;../dist/*.js&#39; &#39;./dist&#39;</p>

<p># Copy generated documentation deploy_files &#39;../dist/docs/*&#39;
&#39;./docs&#39;</p>

<p># Copy samples … deploy_files &#39;../samples/*&#39; &#39;./samples&#39;</p>

<p># … and relocate samples Chart/js scripts for f in $(find
./samples/$VERSION -name &#39;*.html&#39;); do</p>

<pre>sed -i -E &quot;s/((\.\.\/)+dist\/)/..\/\1$VERSION\//&quot; $f</pre>

<p>done</p>

<p>git add -A</p>

<p>git remote add auth-origin $TARGET_REPO_URL git config –global user.email
“$GITHUB_AUTH_EMAIL” git config –global user.name “Chart.js” git commit -m
“Deploy $VERSION from $TRAVIS_REPO_SLUG” -m “Commit: $TRAVIS_COMMIT” git
push -q auth-origin $TARGET_BRANCH git remote rm auth-origin</p>

<p># Cleanup cd .. rm -rf $TARGET_DIR</p>
</main>
