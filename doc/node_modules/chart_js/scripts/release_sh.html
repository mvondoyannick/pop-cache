...
<title>release.sh - RDoc Documentation</title>
...
<main role="main" aria-label="Page node_modules/chart.js/scripts/release.sh">

<p>#!/bin/bash</p>

<p>set -e</p>

<p>if [ “$TRAVIS_BRANCH” != “release” ]; then</p>

<pre class="ruby"><span class="ruby-identifier">echo</span> <span class="ruby-string">&quot;Skipping release because this is not the &#39;release&#39; branch&quot;</span>
<span class="ruby-identifier">exit</span> <span class="ruby-value">0</span>
</pre>

<p>fi</p>

<p># Travis executes this script from the repository root, so at the same
level than <a href="../../../package_json.html">package.json</a>
VERSION=$(node -p -e “require(&#39;./package.json&#39;).version”)</p>

<p># Make sure that the associated tag doesn&#39;t already exist GITTAG=$(git
ls-remote origin refs/tags/v$VERSION) if [ “$GITTAG” != “” ]; then</p>

<pre class="ruby"><span class="ruby-identifier">echo</span> <span class="ruby-string">&quot;Tag for package.json version already exists, aborting release&quot;</span>
<span class="ruby-identifier">exit</span> <span class="ruby-value">1</span>
</pre>

<p>fi</p>

<p>git remote add auth-origin <a
href="https://$GITHUB_AUTH_TOKEN@github.com/$TRAVIS_REPO_SLUG.git">$GITHUB_AUTH_TOKEN@github.com/$TRAVIS_REPO_SLUG.git</a>
git config –global user.email “$GITHUB_AUTH_EMAIL” git config –global
user.name “Chart.js” git checkout –detach –quiet git add -f dist/*.js
bower.json git commit -m “Release $VERSION” git tag -a “v$VERSION” -m
“Version $VERSION” git push -q auth-origin refs/tags/v$VERSION
2&gt;/dev/null git remote rm auth-origin git checkout -f @{-1}</p>
</main>
