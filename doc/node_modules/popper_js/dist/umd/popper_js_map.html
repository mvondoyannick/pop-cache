...
<main role="main" aria-label="Page node_modules/popper.js/dist/umd/popper.js.map">
...
DOM access heren  const window = element.ownerDocument.defaultView;n  const
css = window.getComputedStyle(element, null);n  return property ? <a
...
element.offsetParent || null;n  // Skip hidden elements which don&#39;t
have an offsetParentn  while (offsetParent === noOffsetParent &amp;&amp;
...
.offsetParent will return the closest TH, TD or TABLE in casen  // no
offsetParent is present, I hate this job…n  if (n    [&#39;TH&#39;,
&#39;TD&#39;, &#39;TABLE&#39;].indexOf(offsetParent.nodeName) !== -1
&amp;&amp;n    getStyleComputedProperty(offsetParent, &#39;position&#39;)
...
isIE(10)n      ? (<a href="`offset${axis}`">parseInt(html</a>) + n     
parseInt(computedStyle[`margin${axis === &#39;Height&#39; ? &#39;Top&#39; :
&#39;Left&#39;}`]) + n      parseInt(computedStyle[`margin${axis ===
&#39;Height&#39; ? &#39;Bottom&#39; : &#39;Right&#39;}`]))n    : 0 n 
);n}nnexport default function getWindowSizes(document) {n  const body =
...
: {};n  const width =n    sizes.width || element.clientWidth ||
result.right - result.left;n  const height =n    sizes.height ||
element.clientHeight || result.bottom - result.top;nn  let horizScrollbar =
element.offsetWidth - width;n  let vertScrollbar = element.offsetHeight -
height;nn  // if an hypothetical scrollbar is detected, we must be sure
it&#39;s not a `border`n  // we make this check conditional for performance
reasonsn  if (horizScrollbar || vertScrollbar) {n    const styles =
getStyleComputedProperty(element);n    horizScrollbar -=
getBordersSize(styles, &#39;x&#39;);n    vertScrollbar -=
...
&amp;&amp; isHTML) {n    parentRect.top = Math.max(parentRect.top, 0);n   
parentRect.left = Math.max(parentRect.left, 0);n  }n  let offsets =
...
childrenRect.height,n  });n  offsets.marginTop = 0;n  offsets.marginLeft =
...
if (!isIE10 &amp;&amp; isHTML) {n    const marginTop =
parseFloat(styles.marginTop, 10);n    const marginLeft =
parseFloat(styles.marginLeft, 10);nn    offsets.top -= borderTopWidth -
marginTop;n    offsets.bottom -= borderTopWidth - marginTop;n   
offsets.left -= borderLeftWidth - marginLeft;n    offsets.right -=
borderLeftWidth - marginLeft;nn    // Attach marginTop and marginLeft
because in some circumstances we may need themn    offsets.marginTop =
marginTop;n    offsets.marginLeft = marginLeft;n  }nn  if (n    isIE10
&amp;&amp; !fixedPositionn      ? parent.contains(scrollParent)n      :
parent === scrollParent &amp;&amp; scrollParent.nodeName !==
&#39;BODY&#39;n  ) {n    offsets = includeScroll(offsets, parent);n  }nn 
return offsets;n}n”,“import getOffsetRectRelativeToArbitraryNode from
...
&#39;position&#39;) === &#39;fixed&#39;) {n    return true;n  }n  const
parentNode = getParentNode(element);n  if (!parentNode) {n    return
false;n  }n  return isFixed(parentNode);n}n”,“import
getStyleComputedProperty from &#39;./getStyleComputedProperty&#39;;nimport
isIE from &#39;./isIE&#39;;n/**n * Finds the first parent of an element
that has a transformed property definedn * @methodn * @memberof
Popper.Utilsn * @argument {Element} elementn * @returns {Element} first
transformed parent or documentElementn */nnexport default function
...
getWindowSizes(popper.ownerDocument);n      boundaries.top += offsets.top -
offsets.marginTop;n      boundaries.bottom = height + offsets.top;n     
boundaries.left += offsets.left - offsets.marginLeft;n     
boundaries.right = width + offsets.left;n    } else {n      // for all the
other DOM elements, this one is goodn      boundaries = offsets;n    }n 
}nn  // Add paddingsn  padding = padding || 0;n  const isPaddingNumber =
typeof padding === &#39;number&#39;;n  boundaries.left += isPaddingNumber ?
padding : padding.left || 0; n  boundaries.top += isPaddingNumber ? padding
: padding.top || 0; n  boundaries.right -= isPaddingNumber ? padding :
padding.right || 0; n  boundaries.bottom -= isPaddingNumber ? padding :
padding.bottom || 0; nn  return boundaries;n}n”,“import getBoundaries from
...
window = element.ownerDocument.defaultView;n  const styles =
window.getComputedStyle(element);n  const x = parseFloat(styles.marginTop
|| 0) + parseFloat(styles.marginBottom || 0);n  const y =
parseFloat(styles.marginLeft || 0) + parseFloat(styles.marginRight || 0);n 
const result = {n    width: element.offsetWidth + y,n    height:
...
&#39;../utils/getSupportedPropertyName&#39;;nn/**n * Destroys the popper.n
...
});nn  return options;n}n”,“/**n * @functionn * @memberof Popper.Utilsn *
@argument {Object} data - The data object generated by `update` methodn *
@argument {Boolean} shouldRound - If the offsets should be rounded at alln
* @returns {Object} The popper&#39;s position offsets roundedn *n * The
tale of pixel-perfect positioning. It&#39;s still not 100% perfect, but asn
* good as it can be within reason.n * Discussion here: <a
href="https://github.com/FezVrasta/popper.js/pull/715\n">github.com/FezVrasta/popper.js/pull/715n</a>
*n * Low DPI screens cause a popper to be blurry if not using full pixels
(Safarin * as well on High DPI screens).n *n * Firefox prefers no rounding
for positioning and does not have blurriness onn * high DPI screens.n *n *
Only horizontal placement and left/right values need to be considered.n
*/nexport default function getRoundedOffsets(data, shouldRound) {n  const {
popper, reference } = data.offsets;n  const { round, floor } = Math;n 
const noRound = v =&gt; v;n  n  const referenceWidth =
round(reference.width);n  const popperWidth = round(popper.width);n  n 
const isVertical = [&#39;left&#39;,
&#39;right&#39;].indexOf(data.placement) !== -1;n  const isVariation =
data.placement.indexOf(&#39;-&#39;) !== -1;n  const sameWidthParity =
referenceWidth % 2 === popperWidth % 2;n  const bothOddWidth =
referenceWidth % 2 === 1 &amp;&amp; popperWidth % 2 === 1;nn  const
horizontalToInteger = !shouldRoundn    ? noRoundn    : isVertical ||
isVariation || sameWidthParityn    ? roundn    : floor;n  const
verticalToInteger = !shouldRound ? noRound : round;nn  return {n    left:
horizontalToInteger(n      bothOddWidth &amp;&amp; !isVariation &amp;&amp;
shouldRoundn        ? popper.left - 1n        : popper.leftn    ),n    top:
verticalToInteger(popper.top),n    bottom:
verticalToInteger(popper.bottom),n    right:
horizontalToInteger(popper.right),n  };n}n”,“import
getSupportedPropertyName from
...
&#39;../utils/getBoundingClientRect&#39;;nimport getRoundedOffsets from
&#39;../utils/getRoundedOffsets&#39;;nimport isBrowser from
&#39;../utils/isBrowser&#39;;nnconst isFirefox = isBrowser &amp;&amp;
/Firefox/i.test(navigator.userAgent);nn/**n * @functionn * @memberof
Modifiersn * @argument {Object} data - The data object generated by
...
const styles = {n    position: popper.position,n  };nn  const offsets =
getRoundedOffsets(n    data,n    window.devicePixelRatio &lt; 2 ||
!isFirefoxn  );nn  const sideA = x === &#39;bottom&#39; ? &#39;top&#39; :
&#39;bottom&#39;;n  const sideB = y === &#39;right&#39; ? &#39;left&#39; :
&#39;right&#39;;nn  // if gpuAcceleration is set to `true` and transform is
supported,n  //  we use `translate3d` to apply the position to the popper
wen  // automatically use the supported prefixed version if neededn  const
prefixedProperty = getSupportedPropertyName(&#39;transform&#39;);nn  //
now, let&#39;s make a step back and look at this code closely (wtf?)n  //
If the content of the popper grows once it&#39;s been positioned, itn  //
may happen that the popper gets misplaced because of the new contentn  //
overflowing its reference elementn  // To avoid this problem, we provide
two options (x and y), which allown  // the consumer to define the offset
origin.n  // If we position a popper on top of a reference element, we can
setn  // `x` to `top` to make the popper grow towards its top instead ofn 
// its bottom.n  let left, top;n  if (sideA === &#39;bottom&#39;) {n    //
when offsetParent is &lt;html&gt; the positioning is relative to the bottom
of the screen (excluding the scrollbar)n    // and not the bottom of the
html elementn    if (offsetParent.nodeName === &#39;HTML&#39;) {n      top
= -offsetParent.clientHeight + offsets.bottom;n    } else {n      top =
-offsetParentRect.height + offsets.bottom;n    }n  } else {n    top =
offsets.top;n  }n  if (sideB === &#39;right&#39;) {n    if
(offsetParent.nodeName === &#39;HTML&#39;) {n      left =
-offsetParent.clientWidth + offsets.right;n    } else {n      left =
-offsetParentRect.width + offsets.right;n    }n  } else {n    left =
...
in conjunctionn  //nn  // top/left siden  if (<a
href="opSide">reference</a> - arrowElementSize &lt; <a
href="side">popper</a>) {n    <a href="side">data.offsets.popper</a> -=n   
<a href="side">popper</a> - (<a href="opSide">reference</a> -
arrowElementSize);n  }n  // bottom/right siden  if (<a
href="side">reference</a> + arrowElementSize &gt; <a
...
aligned)n * - `bottom` (on bottom, centered)n * - `auto-end` (on the side
...
*n   * It accepts the following units:n   * - `px` or unit-less,
interpreted as pixelsn   * - `%` or `%r`, percentage relative to the length
of the reference elementn   * - `%p`, percentage relative to the length of
the popper elementn   * - `vw`, CSS viewport width unitn   * - `vh`, CSS
...
modifier.n   * &gt; You can read more on this at this [issue](<a
...
boundary.n   *n   * A scenario exists where the reference itself is not
...
the popper. This makes sure the popper always has a little paddingn     *
...
reference and its popper stay near each othern   * without leaving any gap
between the two. Especially useful when the arrow isn   * enabled and you
want to ensure that it points to its reference element.n   * It cares only
...
detect how manyn   * pixels of conjunction are needed.n   *n   * It has no
...
the `top` and `left` propertiesn     */n    gpuAcceleration: true,n    /**n
* @prop {string} [x=&#39;bottom&#39;]n     * Where to anchor the X axis
...
disable this modifier and define your own to achieve the desired effect.n  
...
Otherwise, it will use the `top` and `left` propertiesn     */n   
gpuAcceleration: undefined,n  },n};nn/**n * The `dataObject` is an object
containing all the information used by Popper.js.n * This object is passed
...
out of boundaries, useful to know when to hide the poppern * @property
...
measurements of popper, reference and arrow elementsn * @property {Object}
...
constructor.&lt;br /&gt;n * These can be overridden using the `options`
argument of Popper.js.&lt;br /&gt;n * To override an option, simply pass an
object with the samen * structure of the `options` object, as the 3rd
argument. For example:n * “`n * new Popper(ref, pop, {n *   modifiers: {n *
preventOverflow: { enabled: false }n *   }n * })n * “`n * @type {Object}n *
@staticn * @memberof Poppern */nexport default {n  /**n   * Popper&#39;s
placement.n   * @prop {Popper.placements} placement=&#39;bottom&#39;n   */n
placement: &#39;bottom&#39;,nn  /**n   * Set this to true if you want
popper to position it self in &#39;fixed&#39; moden   * @prop {Boolean}
positionFixed=falsen   */n  positionFixed: false,nn  /**n   * Whether
events (resize, scroll) are initially enabled.n   * @prop {Boolean}
...
created.&lt;br /&gt;n   * By default, it is set to no-op.&lt;br /&gt;n   *
...
default, it is set to no-op.&lt;br /&gt;n   * Access Popper.js instance
with `data.instance`.n   * @prop {onUpdate}n   */n  onUpdate: () =&gt;
{},nn  /**n   * List of modifiers used to modify the offsets before they
are applied to the popper.n   * They provide most of the functionalities of
Popper.js.n   * @prop {modifiers}n   */n  modifiers,n};nn/**n * @callback
...
Creates a new Popper.js instance.n   * @class Poppern   * @param
...
used as the poppern   * @param {Object} options - Your custom options to
...
}nn  /**n   * Schedules an update. It will run on the next UI update
available.n   * @method scheduleUpdaten   * @memberof Poppern   */n 
...
Explorer 10.n * @name referenceObjectn * @property {Function}
