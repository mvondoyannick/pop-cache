...
namespace :bank do
  ...
  desc "Count all new customer authenticated on the plateform"
  task :count_customer => :environment do
    puts "Debut de denombrement des utilisateurs authentifiés"
    if DateTime.now.hour == 20 #and DateTime.now.minute == 00
      puts "generaton en cours ..."
      customer = Customer.all.where(two_fa: 'authenticate', authentication_token: !nil?).count
      puts "#{customer} utilisateur(s) trouvés dans la plateforme"
    else
      puts "Impossible de generer la télécompensation, heure invalides"
    end
  end

  desc "Generate telecompensation and send it to bank"
  task :telecompensation => :environment do
    if DateTime.now.hour == 20
      commission = Commission.all.where(created_at: Date.today.beginning_of_day..Date.today.end_of_day).sum(:amount_commission)

      #starting send email to PAYMEQUICK admin and AFB admin operators
      #
    else
      puts "Heure de generation non conforme"
    end
  end

end

namespace :newyear do
  desc "Souhaiter un joyeux anniverssaire aux utilisateurs de PAYMEQUICK"
  task :newyear => :environment do
    if Date.today.year == 2019 && Date.today.month == 12 && Date.today.day == 31
      Customer.all.each do |customer|
        if customer.two_fa == "authenticate"
          Sms.nexah(customer.phone, "Chèr(e) #{customer.complete_name.upcase}, tout en vous remerciant de votre fidelité, PAYMEQUICK vous souhaite une excellente fin d'année et une heureuse année 2020.")
        else
          Sms.nexah(customer.phone, "Chèr(e) UTILISATEUR PAYMEQUICK, tout en vous remerciant de votre fidelité, PAYMEQUICK vous souhaite une excellente fin d'année et une heureuse année 2020")
        end
      end
      Sms.nexah(691451189, "Tous les SMS de fin d'année envoyé à #{Customer.all.count}. Terminé!")
    else
      puts "Temps de notifications incorrecte"
    end
  end
end